...
| 13 | ✅ | Phase 13 commit: `25d493e8` (content(mecfs): integrate long-COVID brain microstructure and neurochemical MRI; 15 files: 3 chapter files + changelog + bib + appendix + registry + scrape-registry + hypotheses-trees + 5 ops artifacts + moved PDF). Shared-file ownership re-check: my registry block (3 entries), bib keys (13 in long-covid.bib), appendix entries (13), scrape-registry section, hypotheses-trees row all present in HEAD. **Parallel (leptin) cycle's files left unstaged/untracked** (endocrine/sleep/treatments bibs, ch10 circadian file, Fleury PDF, their brainstorm/plan/research) — committed in separate commit by their cycle. **Glossary-en/fr/de.json left unstaged** — my 5 terms + their `cell_type` entry intermingled in shared file; left for shared-file reconciliation to avoid committing their incomplete `cell_type` category (which lacks a CATEGORY_LABEL_SETS label and breaks the glossary-tooltip test). Post-commit build PASS (exit 0). `tmp/` audit scratch NOT committed. |
