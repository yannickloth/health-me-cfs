...
A cross-viral family comparison would:
1. Strengthen the convergence argument by showing exactly which mechanisms are shared vs. virus-specific
2. Highlight that fundamentally different viral strategies (DNA latency vs. RNA tissue persistence vs. spike persistence) produce clinically indistinguishable disease
...
|---|---|---|---|
| Members implicated | EBV, HHV-6, CMV, VZV | Coxsackie B, echovirus, enterovirus D68 | SARS-CoV-2 |
| Genome | Large dsDNA (120-230 kb) | Small ssRNA+ (7-8 kb) | Large ssRNA+ (30 kb) |
| Persistence strategy | True latency programs (episomal/integrated), abortive lytic replication | No latency; tissue reservoir persistence (muscle, GI mucosa) | Spike protein/RNA persistence in tissues; no classical latency |
| Primary tropism | B cells (EBV), T cells (HHV-6), monocytes (CMV), neurons (VZV) | Muscle, GI epithelium, CNS (rare) | Respiratory epithelium, endothelium, CNS (via multiple routes) |
| Immune evasion | MHC-I downregulation, viral IL-10 homologs, complement evasion proteins | Protease cleavage of host signaling (eIF4G, MAVS), IRES-mediated cap-independent translation | NSP interference with IFN signaling, ORF3b/ORF6 antagonism of STAT1 |
| ME/CFS progression rate | EBV: ~10-12% of mono cases | Less well quantified; historical epidemic clusters | ~3-8% of COVID infections → Long COVID (subset meets ME/CFS criteria) |
...
3. **Persistence mechanism**: true latency vs. tissue reservoir vs. antigen persistence — fundamentally different strategies with similar downstream consequences
4. **Proposed ME/CFS pathogenic mechanism**: which "entry door" (ch16 framework) each family preferentially opens
5. **Epidemiological evidence**: progression rates, OR for ME/CFS association, quality of evidence
...
- Enteroviruses: protease-mediated cleavage of RIG-I/MDA5 signaling components
- SARS-CoV-2: NSP1/NSP13/ORF6 interference with STAT1 nuclear translocation
...
- Enteroviruses: low-level replication in immunoprivileged tissue reservoirs
- SARS-CoV-2: spike protein/RNA fragments persist in tissues after replication-competent virus is cleared
...
- Immune evasion gene count (herpesviruses encode dozens; RNA viruses have fewer, more multifunctional)
- Co-evolutionary adaptation (herpesviruses are highly adapted; SARS-CoV-2 is a recent spillover)
- Tissue tropism breadth (herpesviruses are narrow/specific; SARS-CoV-2 is broad via ACE2)
...
- Enterovirus-triggered → metabolic support (CoQ10, D-ribose, mitochondrial cofactors)
- COVID-triggered → anticoagulation/antiplatelet (microclot hypothesis), neuroinflammation targets
- Mixed/reactivation → combination approaches
...
**Rationale**:
- Ch07 already contains the most viral content and is the natural home for mechanistic comparison
- Sits between herpesvirus-specific detail and the broader "other viruses" section, creating a natural bridge
- Ch16 references back to ch07 for mechanistic detail; adding the comparison here maintains that referencing structure
...
Brief intro: why comparing across families matters (2-3 paragraphs)

\subsubsection{Comparison of Primary ME/CFS-Associated Virus Families}
  - Structured comparison table (LaTeX table, not just prose)
  - Dimensions: genome type, persistence mechanism, tropism, immune evasion,
    proposed entry pathway, epidemiological evidence strength

\subsubsection{Convergent Immune Evasion}
  - IFN suppression across families
  - MHC-I interference across families
  - NF-κB modulation across families
  - Testable prediction re: IFN-competent viruses

\subsubsection{Three Solutions to Persistence}
  - ALR (herpes) vs. tissue reservoirs (entero) vs. antigen persistence (corona)
  - Shared downstream: immune activation without clearable target
  - Cross-reference to ALR section (sec:abortive-lytic)

\subsubsection{Cross-Family Interactions and Compounding}
  - SARS-CoV-2 → herpesvirus reactivation cascade
  - Implications for Long COVID as compound viral insult
  - Poly-pathogen burden hypothesis

\begin{hypothesis}[Pathogen-Specific Entry Door Selection]
  - Formalize the ch16 speculation with supporting evidence from this comparison
  - Certainty: 0.35-0.40
  - Testable predictions
  - Treatment implications
\end{hypothesis}

\begin{open_question}[Does Genome Type Predict Disease Mechanism?]
  - DNA vs RNA distinction and what it does/doesn't predict
\end{open_question}
```
...
1. **Reviews comparing viral persistence strategies in post-infectious syndromes** — any existing comparative frameworks
2. **IFN signaling interference** across virus families — comparative immunology literature
3. **ME/CFS progression rates by triggering pathogen** — epidemiological data, especially the Hwang 2023 meta-analysis already cited in ch05-onset-patterns
4. **SARS-CoV-2 triggering herpesvirus reactivation** — quantitative data on rates and clinical significance
...
|---|---|---|
| Comparison table (factual viral properties) | Regular table | N/A (established virology) |
| Convergent IFN suppression | Prose + citations | High (well-established in virology) |
| Three persistence strategies | Prose + citations | High (established) |
| COVID → herpesvirus reactivation | Prose + citations | Moderate-high (multiple studies) |
| Pathogen-specific entry door hypothesis | `hypothesis` | 0.35-0.40 |
