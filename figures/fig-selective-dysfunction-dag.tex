...
\textbf{Key Structural Feature:} The CNS energy crisis node has \emph{no clinically significant causal path} to autonomous processes (hair, nails, wound healing). These processes have sub-threshold CNS-dependency ($\delta_{CNS} \leq 0.2$, $S \leq 0.24$), well below the lowest impaired process ($S = 0.79$). Their preservation despite severe ME/CFS symptoms supports the selective dysfunction hypothesis: the disease targets CNS-dependent, demand-responsive processes while sparing CNS-independent autonomous functions.
};

\end{tikzpicture}
\caption{Causal diagram of selective energy dysfunction in ME/CFS\@. Edge weights indicate certainty (0--1 scale) based on evidence quality. The diagram includes a feedback edge (exercise intolerance $\to$ CNS crisis, certainty 0.35), making it a causal diagram rather than a strict DAG\@. Preserved processes (green) have sub-threshold CNS-dependency ($\delta_{CNS} \leq 0.2$, $S \leq 0.24$) and no clinically significant causal path from the CNS crisis node.}
