...
\end{tikzpicture}
\caption{Causal diagram of selective energy dysfunction in ME/CFS. Edge weights indicate certainty (0--1 scale) based on evidence quality. The diagram includes a feedback edge (exercise intolerance $\to$ CNS crisis, certainty 0.35), making it a causal diagram rather than a strict DAG. Preserved processes (green) have sub-threshold CNS-dependency ($\delta_{CNS} \leq 0.2$, $S < 0.12$) and no clinically significant causal path from the CNS crisis node.}
