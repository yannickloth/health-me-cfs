...
\begin{scope}[shift={(0,0)}]
  ...
  % =====================================================================
  % Ceiling steps increase in size (accelerating ratchet, alpha > 0)
  \draw[red!60!black, thick, densely dashed]
    (0, 7.0) -- (1.5, 7.0)    % pre-event 1
    (1.5, 6.4) -- (3.8, 6.4)  % post-event 1: delta=0.6
    (3.8, 5.4) -- (6.0, 5.4)  % post-event 2: delta=1.0
    (6.0, 4.0) -- (8.2, 4.0)  % post-event 3: delta=1.4
    (8.2, 2.2) -- (11.0, 2.2);% post-event 4: delta=1.8

  % --- Ceiling drop arrows (vertical, at each event) ---
  \foreach \x/\ytop/\ybot in {1.5/7.0/6.4, 3.8/6.4/5.4, 6.0/5.4/4.0, 8.2/4.0/2.2} {
  ...
  \draw[blue!70!black, very thick]
    ...
    (1.5, 7.0) -- (1.5, 4.8)
    % Recovery toward ceiling 6.4 (doesn't quite reach it)
    .. controls (2.0, 5.1) and (2.8, 5.9) .. (3.8, 6.2)
    % Event 2 (PEM crash): sharp drop
    -- (3.8, 3.6)
    % Recovery toward ceiling 5.4
    .. controls (4.3, 3.9) and (5.2, 4.9) .. (6.0, 5.2)
    % Event 3 (surgery): sharp drop
    -- (6.0, 2.2)
    % Recovery toward ceiling 4.0
    .. controls (6.5, 2.6) and (7.4, 3.5) .. (8.2, 3.8)
    % Event 4 (infection): sharp drop
    -- (8.2, 1.2)
    % Recovery toward ceiling 2.2 (ongoing)
    .. controls (8.8, 1.5) and (9.8, 1.9) .. (11.0, 2.1);
  ...
  \node[font=\sffamily\tiny, orange!80!black, anchor=south, rotate=45]
    at (3.8, 6.7) {PEM crash};
  \node[font=\sffamily\tiny, orange!80!black, anchor=south, rotate=45]
    at (6.0, 5.7) {Surgery};
  \node[font=\sffamily\tiny, orange!80!black, anchor=south, rotate=45]
    at (8.2, 4.3) {Infection};

  % =====================================================================
  % Annotations: delta_k and Delta_k for event 1
  % =====================================================================
  % Irreversible loss (delta_k): gap between old and new ceiling
  \draw[<->, red!60!black, thin] (1.8, 7.0) -- (1.8, 6.4);
  \node[font=\sffamily\tiny, red!60!black, anchor=west] at (1.9, 6.7)
    {$\delta_1$};

  % Total acute drop (Delta_k): from pre-event B to post-event B
  \draw[<->, blue!50!black, thin] (1.2, 7.0) -- (1.2, 4.8);
  \node[font=\sffamily\tiny, blue!50!black, anchor=east] at (1.1, 5.9)
    {$\Delta_1$};
...
\end{tikzpicture}
\caption{The energy ratchet model. (a)~A single patient trajectory
(accelerating ratchet, $\alpha > 0$) showing baseline functional capacity
$B(t)$ (solid blue) and the irreversible ceiling $B_{\max}$ (dashed red)
declining through four damaging events (infections, PEM crashes, surgery).
Each event produces a total acute drop $\Delta_k$ (blue arrow), of which
only $\delta_k$ (red arrow) is irreversible; note the increasing step sizes
reflecting damage sensitisation. Recovery between events is partial and
ATP-dependent (Equation~\ref{eq:ratchet-interevent}). Horizontal bands
indicate clinical severity levels (Equation~\ref{eq:ratchet-severity}).
(b)~Two of four trajectory classes: \emph{stable} ratchet with equal ceiling
steps ($\alpha \approx 0$) and \emph{accelerating} ratchet ($\alpha > 0$).
...
See Equations~\ref{eq:ratchet-jump-ceiling}--\ref{eq:ratchet-jump-acute}
and~\ref{eq:ratchet-irreversible-loss} for the formal jump map.}
