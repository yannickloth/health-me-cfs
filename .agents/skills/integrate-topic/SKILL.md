...
description: Use this skill to evaluate and integrate a new topic into the ME/CFS paper — end-to-end pipeline from literature research and evidence synthesis through content integration, hypothesis compatibility checking, review convergence, and commit. Use when adding new research findings, treatments, mechanisms, or biomarkers. Supports explicit rejection when evidence doesn't warrant integration.
---
...
- Search PubMed, Google Scholar, preprint servers — include null results, contradicting evidence, and competing mechanisms:
  ...
  - **Indirect biochemical/systemic links (MANDATORY):** Not just direct "[topic] AND ME/CFS" — also map what the topic *does* biochemically (its mechanism of action, its downstream effects, the pathways it touches) and search for those mechanisms in ME/CFS. A biomarker/drug/antibody/etc. may lack direct ME/CFS studies but its known biochemistry may intersect with documented ME/CFS pathophysiology. Search pattern: "[downstream effect] ME/CFS", "[pathway touched by topic] chronic fatigue", "[biochemical consequence] myalgic encephalomyelitis". Examples: RF → search "immune complex complement ME/CFS" and "cryoglobulin fatigue" and "Fc receptor chronic fatigue" because RF forms IgG ICs that activate complement, engage FcγRs, and can precipitate as cryoglobulins — all biochemically downstream of the topic even if RF itself isn't named. Document the logic: "Topic X does Y (literature: @ref); searched for 'Y AND ME/CFS' → N results."
...
1. **Evidence landscape summary:**
   ...
   - **What indirect biochemical/systemic links exist?** — Papers that don't study the topic directly but document downstream mechanisms of the topic in ME/CFS or related conditions. These are scored under Phase 1's indirect-link search. Summarize: "N indirect papers found linking [topic's biochemistry] to [ME/CFS-relevant mechanism/pathway]. Key connections: [list]." This column is critical for topics with zero direct studies — the integration decision must weigh indirect biochemical plausibility against the direct evidence vacuum.
...
- **Scope-boundary rule (not an absolute ban).** *Rationale:* a retroactive synthesis (a) edits another completed cycle's committed narrative (outside the current cycle's scope) and (b) makes an editorial *emphasis* choice — which themes to elevate — that the topic's author may want to own. So the default is **flag, don't silently write**. This is a scope/ownership guard, NOT a claim that the work is unsafe.
  - **User explicitly requested it** (e.g. "run Phase 10a", "check for missing synthesis") → the flag-and-ask gate is already satisfied; proceed to write the eligible single-topic syntheses directly. Do not ask again — re-confirming an explicit request is pure friction.
  - **Not user-requested** (triggered by a reinforcing cycle or a review finding) → flag the opportunity and ask before writing.
  - **Either way:** a single-*topic* synthesis (condensing one completed integration's own environments) is in-bounds once the gate is satisfied. A cross-*topic* synthesis or a decision to *merge two topics' syntheses* is a distinct scientific-structuring judgment with more than one defensible answer → always flag that specific choice to the user even under an explicit "run Phase 10a" (see step 5).
