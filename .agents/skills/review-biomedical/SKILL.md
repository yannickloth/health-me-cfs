...
description: Use this skill to run all 20 biomedical review categories on specified files — structural, terminology, scientific rigor, domain-specific biology, safety, and publication readiness. Use when reviewing biomedical content, chapters, or the full document for scientific quality. Reports aggregate findings at the end.
---
...
- Phase 4 keyword grep uses domain keywords from the table — grep is **case-insensitive** (`-i`). Agent is skipped entirely if zero keyword matches in scope files.
- Phase 5 always runs regardless of scope — safety and patient impact checks are non-optional.
- `consequence-field-scanner` (step 5.4) checks for `*Consequence:*` fields in environments — their absence is a finding, not an error.
- Report and checkpoint paths: `$XDG_RUNTIME_DIR` → fall back to `tmp/` if env var not set.
...
4. Write report → `$XDG_RUNTIME_DIR/review-biomedical-[timestamp].md` (fallback: `tmp/review-biomedical-[timestamp].md` if `$XDG_RUNTIME_DIR` not set)
5. Report location to user.

## Checkpoint

After each phase, write checkpoint to `$XDG_RUNTIME_DIR/review-biomedical-checkpoint.md` (fallback: `tmp/review-biomedical-checkpoint.md` if `$XDG_RUNTIME_DIR` not set):
