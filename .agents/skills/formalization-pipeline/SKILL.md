...
description: Use this skill to transform ME/CFS pathophysiology from qualitative descriptions into formal models — causal DAG, EPC, and ODE — with explicit uncertainty quantification. Use when formalizing biological mechanisms, building causal models, or quantifying pathway dynamics.
---
...
<!-- Long skill (>250 lines). If context pressure increases, consider extracting Stages 1-3 into references/stages-1-3-formalization.md. -->
...
- **Stage 5 writes into the paper:** Run `nix build` after Stage 5 — the document integration stage modifies `.typ` files directly.
...
| After stage | Gate | If gate fails |
|---|---|---|
| Stage 1 | ≥50 causal claims with citations | Return to Stage 1 — expand literature survey, lower certainty threshold, or reduce scope |
| Stage 2 | No unresolved DAG contradictions | Return to Stage 2 — document all contradictions; if unresolvable, flag in dag-report.md |
| Stage 3 | ≥2–3 processes for quantitative modeling; models validate R²>0.6 | Return to Stage 3 — select different processes, or downgrade depth level |
| Stage 4 | No critical validation failures | Return to Stage 4 — fix specific validation; if DAG↔EPC ↔ODE mismatch, re-run the upstream model |
| Stage 5 | `/review-typst` converged (2 consecutive zero-finding rounds) | Return to Stage 5 — fix review findings, re-run review until convergence |
| Stage 7 | Build clean, all math passes | Return to Stage 7 — fix build errors; if math fails, re-run math-verifier on specific models |
