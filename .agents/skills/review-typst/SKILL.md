---
name: review-typst
description: Use this skill to perform iterative adversarial review-fix-build rounds on Typst files until convergence to zero findings or max rounds. Use when reviewing Typst chapters, sections, or any .typ file for scientific rigor, citation accuracy, notation consistency, and writing quality.
---
...
- The available environments list and star variants may drift from `template.typ` — verify against the template's current exports before reviewing, not from memory or this SKILL.md.
- Never introduce citation keys without verifying existence in `references.bib`. Ambiguous key → flag; do not guess.
- Never introduce Typst function calls without verifying the function exists in `template.typ`.
- `typst compile typst/ms.typ` must pass after fixes before proceeding to next round. Build failure → stop and report; do not accumulate breakage.
- Convergence = 2 consecutive zero-finding rounds. Any correction resets the counter.
