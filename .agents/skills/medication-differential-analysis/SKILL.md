...
description: Use this skill to generate differential diagnostic analysis for medications in the ME/CFS paper — what clinical response and non-response reveal about broken mechanisms, root causes, and the next targeted treatment step. Use when building the differential diagnostic framework, analyzing medication responses, or after integrating a new treatment topic.
---
...
- **Per-medication invocations are expensive:** The `medication-differential-analyst` agent (opus) is invoked once per medication — batch by mechanistic class for efficiency. Individual invocations per med are wasteful.
- **Phase 2 reads only mechanism sections:** Do not re-read entire chapters — extract only the Part II sections relevant to the medication's target mechanism.
- **Phase 5 cross-reference verification:** `grep` both `tmp/differential-*.md` outputs AND the source tree (`src/main/typst/`) — labels may exist in the paper but be mistyped in the differential entries.
- **Retroactive mode is costly:** "all" or "retroactive" scope triggers 8-12 agent invocations (one per medication class batch + synthesis passes). Confirm with user before proceeding.
