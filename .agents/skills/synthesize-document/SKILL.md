...
description: Use this skill for cross-document synthesis — scan all environments across chapters, identify convergent clusters of evidence, and write synthesis environments to surface global patterns no single chapter shows. Use after multiple topic integrations or when reviewers note fragmentation.
---
...
- **Shared term vocabulary is mandatory:** Agents must use canonical forms exactly as listed in the Phase 1 term table. Non-canonical variants create false negatives during merge — the same mechanism identified under different names in different chapters will not cluster.
