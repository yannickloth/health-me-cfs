---
name: review-chapter
description: Use this skill for comprehensive chapter review — run all review agents sequentially on a single chapter to produce a consolidated report. Use when reviewing a book chapter, checking chapter quality, or performing a full chapter audit.
---
...
- Phase 1 >20 critical findings → **STOP immediately.** Deeper review phases won't help until structural issues are fixed. Report and abort.
- Agent mapping is format-dependent (LaTeX vs Typst) — use the correct agent variant for the detected format. Verify each agent exists before launching.
- Not all agents apply to every chapter — check the `Condition` column before launching (e.g., skip `proof-validator` if chapter has no proofs, skip `exercise-validator` if no exercises).
- Checkpoint path uses `tmp/` (project-relative), not `$XDG_RUNTIME_DIR`.
...
- Before each phase: verify the agents required for that phase exist. If agent not found → report, skip that step, continue.
