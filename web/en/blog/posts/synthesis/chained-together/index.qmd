...
The clinical translation is the structured trial architecture used throughout this series: one drug at a time, named target, named dose, named reassessment point, named stop-rule. Not because single drugs are expected to work — but because you cannot know what each drug contributes if you start them all at once. The sequence is: trial A → if partial response, maintain A and trial B → if partial response, maintain A+B and trial C → until the symptom burden is acceptable or the drug burden becomes unacceptable.
