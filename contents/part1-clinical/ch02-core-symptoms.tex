...
Post-exertional malaise represents the failure to clear normal post-exertion byproducts --- metabolic waste, inflammatory mediators, oxidative damage --- that healthy physiology resolves within hours. In ME/CFS, these byproducts persist and accumulate, producing a characteristic symptom syndrome. Unlike normal fatigue, PEM is characterized by:

\begin{itemize}
    \item \textbf{Delayed onset}: Post-exertion byproducts accumulate to symptomatic levels 12--72 hours after the triggering activity
    \item \textbf{Disproportionate severity}: Minimal exertion produces severe and prolonged symptom burden far beyond normal post-exertion fatigue
...
\paragraph{Dietary and Environmental Factors}
\begin{itemize}
    \item Certain foods, including wheat in patients with non-celiac wheat sensitivity (approximately 15\% of ME/CFS patients)
    \item Food sensitivities may interact synergistically with physical exertion, amplifying PEM severity
    \item Environmental exposures (chemical sensitivities, infections)
    \item Sleep disruption
\end{itemize}

See \cref{sec:food-sensitivities-wheat} for detailed discussion of wheat sensitivity mechanisms and clinical assessment approaches.
...
\begin{limitation}[PEM Vicious Cycle Model: Illustrative Framework, Not Quantified Biology]
...
\begin{itemize}
    \item Numerical values for ``cycle gain'' (5--10\% vs.\ 30--50\% ATP reduction per iteration), ``repair capacity'' (15--20\% biogenesis rate), and ``critical thresholds'' are hypothetical estimates chosen to illustrate the model's logic, not measurements from ME/CFS patients.
    \item The distinction between ``escapable'' and ``inescapable'' cycles has not been validated by longitudinal studies tracking cycle parameters over time in individual patients.
    \item Whether ME/CFS progression truly follows a sequential cycle-recruitment pattern or reflects a single initial insult with variable downstream consequences is undetermined.
    \item The model's predictions (earlier intervention = better outcomes, cycle number predicts severity) are consistent with clinical observation but have not been prospectively tested.
\end{itemize}
\end{limitation}
...
Studies have documented~\cite{Jackson2012sleepAbnormalities,%
Reeves2006sleepCharacteristics}:
\begin{itemize}
    \item \textbf{Reduced slow-wave sleep (Stage N3)}: The deepest, most restor\-ative sleep stage is diminished
...
\begin{itemize}
    ...
    \item \textbf{Treatment}: Iron supple\-mentation (if ferritin <75 ng/mL~\cite{Allen2018RLSironThreshold}), dopamine agonists, gabapentin
\end{itemize}
...
Autonomic dysfunction is present in 70--90\% of ME/CFS patients~\cite{Newton2007autonomicDysfunction}, manifesting as ortho\-static in\-tolerance, temperature dys\-regulation, and cardio\-vascular symptoms. The autonomic nervous system controls in\-voluntary functions including heart rate, blood pressure, digestion, temperature regulation, and bladder control. Dys\-autono\-mia in ME/CFS creates a cascade of disabling symptoms often mis\-attributed to anxiety or de\-conditioning.
