% FILE: Definition, history, epidemiology, terminology — disease overview, historical context, prevalence
\chapter{Introduction to ME/CFS}
\label{ch:introduction}
\changedIn{5}{Myalgic encephalomyelitis/chronic fatigue syndrome (ME/CFS) is a complex, chronic, multi-system disease characterized by severe and disabling fatigue, post-exertional malaise, unrefreshing sleep, cognitive dysfunction, and autonomic dysregulation. This document provides a comprehensive overview of current understanding, research, and clinical approaches to ME/CFS.}
...
\subsection{Toward a More Accurate Name: Dysergopoiesis}
\label{subsec:proposed-name-dysergopoiesis}

Given the accumulated evidence that ME/CFS is a systemic biological disease---not a syndrome, not mere fatigue, and certainly not a psychological condition---this document proposes a new designation that more accurately reflects the core pathology: \textbf{dysergopoiesis}.

The term is constructed from three Greek roots:

\begin{description}
    \item[\emph{dys-} (δυσ-)] impaired, disordered---the standard medical prefix indicating dysfunction, as in dys\-lexia, dys\-pnea, dys\-tonia.
    \item[\emph{ergo-} (ἔργον)] work, energy---the root found in \emph{erg} (the unit of energy), \emph{ergonomics}, and \emph{synergy}.
    \item[\emph{-poiesis} (ποίησις)] making, production---the suffix used throughout hematology and cell biology: \emph{hematopoiesis} (blood cell production), \emph{erythropoiesis} (red blood cell production), \emph{leukopoiesis} (white blood cell production).
\end{description}

\noindent Dysergopoiesis therefore means, literally, \textbf{impaired energy production}. As a Greek scientific coinage, the term is essentially identical across languages---just as \emph{dyslexia}, \emph{anemia}, and \emph{hematopoiesis} require no translation:
...
\subsubsection{Why ``Dysergopoiesis''?}

\paragraph{It names the core pathology, not a symptom.}
The defining biological feature of ME/CFS is not ``fatigue'' but a measurable failure of cellular energy production. The 2024 NIH deep phenotyping study demonstrated objective abnormalities at every level of the energy production chain: depleted catecholamines in cerebrospinal fluid, exhausted T-cell populations unable to switch metabolic modes, dysfunctional B-cell maturation, and decreased temporoparietal junction activity~\cite{walitt2024deep}. Mitochondrial oxidative phosphorylation is compromised (see Chapter~\ref{ch:energy-metabolism}), immune cells cannot perform the glycolysis-to-OXPHOS transition required for sustained function (see Chapter~\ref{ch:immune-dysfunction}), and autonomic regulation of energy distribution fails (see Chapter~\ref{ch:cardiovascular}). The word \emph{dysergopoiesis} names this multi-system energy production failure directly.

\paragraph{It explains post-exertional malaise.}
...
\paragraph{It follows established medical word formation.}
The \emph{dys- + [process] + -poiesis} pattern is standard in medicine. Just as \emph{dyserythropoiesis} means impaired red blood cell production and \emph{dyshematopoiesis} means impaired blood cell production, \emph{dysergopoiesis} means impaired energy production. Any physician who encounters the term can parse it immediately.

\subsubsection{Advantages Over Existing Nomenclature}

\begin{itemize}
    \item \textbf{No trivialization}: ``Dysergopoiesis'' cannot be confused with ordinary tiredness. No one says ``I have a bit of dysergopoiesis'' after a long day at work.
    \item \textbf{Mechanism-centered}: The name points to the core pathology (impaired energy production) rather than to a single symptom (fatigue) or to an outdated neurological hypothesis (encephalomyelitis).
    \item \textbf{Clinically actionable}: A name centered on impaired energy production naturally orients treatment toward mitochondrial support, metabolic correction, and immune metabolic reprogramming---rather than toward ``managing fatigue.''
    \item \textbf{Neutral with respect to etiology}: The name does not presuppose a specific trigger (viral, autoimmune, genetic). It describes \emph{what is broken} regardless of \emph{why}, making it applicable across all patient subgroups.
    \item \textbf{No psychogenic connotation}: The Greek medical construction carries inherent biological authority. No component of the word can be misread as implying volition, preference, or psychological causation.
    \item \textbf{Universally intelligible}: Being a Greek scientific coinage, the term requires no translation---it is the same word in every language, just as \emph{dyslexia} is. This eliminates the problem of divergent national names (ME in the UK, CFS in the US, SFC in France) that has fragmented the patient community and confused the research literature.
    \item \textbf{Shorter and more pronounceable than ``myalgic encephalomyelitis''}: At six syllables versus ten, \emph{dysergopoiesis} is more compact and follows familiar phonetic patterns (\emph{dys-er-go-poi-e-sis}). Patients can learn, spell, and pronounce it---unlike ``encephalomyelitis,'' which most patients cannot spell and most physicians cannot pronounce correctly on first encounter.
\end{itemize}

\subsubsection{Comparison with Previous Renaming Attempts}

The 2015 Institute of Medicine proposal ``Systemic Exertion Intolerance Disease'' (SEID) failed to gain adoption. SEID was criticized for still centering the name on a \emph{symptom} (intolerance to exertion) rather than on the underlying pathology. It also retained the word ``disease,'' which, while technically correct, did not address the \emph{specificity} problem---many diseases involve exertion intolerance.

Dysergopoiesis differs fundamentally: it names the \emph{process that fails} (energy production), not the \emph{symptom that results} (exertion intolerance or fatigue). This distinction is analogous to naming diabetes mellitus after the metabolic defect (insulin insufficiency) rather than after its most visible symptom (excessive urination), or to naming \emph{dyserythropoiesis} after the failed process (red blood cell production) rather than after its clinical consequence (anemia).
...
A systematic review and meta\-analysis of 45 studies found a pooled prevalence of 0.89\% (95\% CI: 0.60--1.33) using CDC-1994 (Fukuda) criteria~\cite{Lim2020prevalence}. Applied globally, this suggests approximately 71 million people are affected. However, prevalence estimates vary sub\-stantially based on diagnostic criteria used, ranging from 0.39\% to 1.40\%.
