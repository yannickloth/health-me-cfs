...
\begin{itemize}
    ...
    \item Has objective correlates (2-day CPET showing Day 2 deterioration~\cite{Lim2020,keller2024cpet})
...
\begin{itemize}
    \item \textbf{Objective biomarkers}: GPCR autoantibodies~\cite{Loebel2016,Bynke2020}, Heng 7-marker panel~\cite{heng2025mecfs}, 2-day CPET~\cite{Lim2020,keller2024cpet} provide biological stratification
...
Two-day cardiopulmonary exercise testing showing Day 2 deterioration: workload at ventilatory threshold decreases $\geq$20\% on Day 2 compared to Day 1~\cite{Lim2020,keller2024cpet}.
\end{requirement}
...
\subsubsection{Hypothetical Phenotype Under Investigation: Viral-Immune-Metabolic Type}
\label{sec:vim-phenotype}

\begin{warning}[Speculative Phenotype Cluster]
The ``Viral-Immune-Metabolic'' phenotype described below is \textbf{hypothetical and unvalidated}. It is based on:
\begin{itemize}
    \item Single-case clinical observation extrapolation
    \item Mechanistic reasoning without controlled validation
    \item Limited anecdotal reports of cimetidine response
\end{itemize}
...
\paragraph{Proposed Phenotype Definition.}

The ``Viral-Immune-Metabolic'' cluster (see Section~\ref{sec:cimetidine-responder} for detailed discussion) is characterized by:
\begin{itemize}
    \item Post-infectious onset (EBV, HHV-6, or other herpesvirus)
    \item MCAS or histamine intolerance (HIT) comorbidity
    \item POTS/dysautonomia
    \item Strong response to amino acid supplementation
    \item Dramatic response to cimetidine (rare but distinctive)
\end{itemize}

\paragraph{Targeted Test Panel for Suspected Viral-Immune-Metabolic Cluster.}

\begin{table}[htbp]
\centering
\caption{Tier 2 test panels for Viral-Immune-Metabolic phenotype}
\label{tab:vim-testpanels}
\begin{tabular}{p{2.8cm}p{4cm}p{4cm}p{2.5cm}}
\toprule
\textbf{Domain} & \textbf{Priority 1 Tests} & \textbf{Priority 2 Tests} & \textbf{Phenotype Indicator} \\
\midrule
\textbf{Viral-Immune} & EBV serology (VCA IgG/IgM, EBNA, EA-D), HHV-6 serology & HHV-6/EBV PCR, T cell exhaustion panel (PD-1, LAG-3) & Cimetidine response \\
\addlinespace
\textbf{Metabolic} & Amino acid panel (serum), D-Ribose therapeutic trial & Lactate/pyruvate ratio, organic acids (urine) & Amino acid response \\
\addlinespace
\textbf{Mast Cell} & Serum tryptase, plasma histamine, DAO activity & 24h urine N-methylhistamine, prostaglandin D$_2$ & HIT/MCAS symptoms \\
\addlinespace
\textbf{Dysautonomia} & Tilt test, lying/standing catecholamines & HRV analysis, QSART & POTS + MCAS coupling \\
\addlinespace
\textbf{Endothelial} & Flow-mediated dilation, ADMA & L-Arginine/L-Citrulline therapeutic response & NO dysfunction \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Clinical Decision Tree.}

If this phenotype is suspected:
\begin{enumerate}
    \item \textbf{Cimetidine shows benefit} $\rightarrow$ Prioritize viral serology (EBV/HHV-6) + amino acid panel
    \item \textbf{Strong amino acid response} $\rightarrow$ Prioritize intestinal permeability markers (Zonulin, LPS) + mitochondrial assessment
    \item \textbf{POTS + MCAS both present} $\rightarrow$ Prioritize dysautonomia subtyping (hyperadrenergic vs.\ neuropathic)
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Drug interaction monitoring}: Cimetidine is a CYP450 inhibitor affecting metabolism of many medications (warfarin, phenytoin, theophylline, benzodiazepines, beta-blockers, calcium channel blockers)
    \item \textbf{Renal function assessment}: Valacyclovir requires dose adjustment in renal impairment; accumulation can cause neurotoxicity
    \item \textbf{Adverse effect monitoring}: CNS effects (cimetidine), nephrotoxicity (antivirals), hypotension (histamine blockade)
    \item \textbf{Laboratory monitoring}: Renal function (creatinine, eGFR) before and during antiviral therapy; liver function tests; complete blood count
\end{itemize}

\textbf{Do not attempt this protocol without medical consultation and ongoing physician oversight.}
\end{warning}

For confirmed Viral-Immune-Metabolic cluster:
\begin{enumerate}
    \item \textbf{Foundation}: Pacing + H1/H2 dual blockade (mast cell stabilization)
    \item \textbf{Metabolic support}: NAC 1800~mg/day, L-citrulline-malate 6--8~g/day, D-ribose 5~g TID
    \item \textbf{If viral titers elevated}: Consider antiviral trial (valacyclovir for EBV, valganciclovir for CMV/HHV-6)
    \item \textbf{If cimetidine response}: Continue cimetidine 200--400~mg BID (immunomodulation benefit)
    \item \textbf{Reassess}: 3-month follow-up with repeat amino acid panel and symptom tracking
\end{enumerate}

\begin{warning}[Evidence Limitations]
This phenotype cluster is based on clinical observation and mechanistic reasoning, not validated clinical trials. The specific test panel and treatment sequence are proposed for systematic evaluation, not established standard of care. See Section~\ref{sec:cimetidine-responder} for detailed certainty assessment.
\end{warning}
...
\begin{enumerate}
    ...
    \item \textbf{Neuroinflammation} (quick win, high accessibility): Low-Dose Naltrexone (LDN) 3 mg at bedtime, light/sound environmental control
...
\begin{limitation}[Novel Diagnostic Framework: Entirely Unvalidated]
...
\begin{itemize}
    \item No prospective study has tested whether the five biological domains (neuroinflammation, autoimmune, mitochondrial, dysautonomia, vascular-endothelial) are clinically separable, whether their presence predicts treatment response, or whether multi-domain assessment improves outcomes over existing diagnostic criteria.
    \item The ``rate-limiting domain'' concept---that treatment response depends on which domain is the primary bottleneck---is a theoretical construct not yet supported by clinical data. The daratumumab responder/non-responder distinction is cited as supporting evidence, but the study was not designed to test this framework.
    \item Biomarker thresholds for domain positivity have not been established, validated, or replicated. Existing studies use different assays, populations, and cut-points, making standardisation premature.
    \item The framework's reliance on the Heng 2025 7-biomarker panel~\cite{heng2025mecfs} as validation of multi-system coordination rests on a single study that has not been independently replicated.
\end{itemize}
\end{limitation}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textbf{Test}: Randomized controlled trial comparing CoQ10 monotherapy vs. CoQ10 + Low-Dose Naltrexone (LDN) + fludrocortisone (in patients with mitochondrial + neuroinflammatory + dysautonomia domains positive)
    \end{itemize}
