...
\subsubsection{Eating Disorders and ME/CFS}
\label{subsubsec:eating-disorders-differential}
...
\paragraph{Eating Difficulties Are Common in ME/CFS.}
Approximately 10\% of children and adolescents with ME/CFS experience significant eating difficulties, and over half experience nausea or abdominal pain~\cite{Harris2017eating}. These difficulties are driven by organic illness mechanisms:

\begin{itemize}
    \item \textbf{Nausea and abdominal pain}: Upper abdominal pain and nausea significantly worsen at 1, 10, and 20 minutes after eating, consistent with visceral hypersensitivity and impaired gastric accommodation~\cite{Gijsbers2023gastric}
    \item \textbf{Profound fatigue}: Eating requires energy; in severe ME/CFS the effort of preparing and consuming food may exceed available energy reserves
    \item \textbf{Eating as a PEM trigger}: Large meals, heavy or hard-to-digest foods, and even the upright posture required for eating can trigger post-exertional worsening via autonomic mechanisms
    \item \textbf{Sensory changes}: Altered taste and smell perception, documented in ME/CFS and long COVID, reduce appetite and food palatability
    \item \textbf{Sickness behavior anorexia}: Chronic elevation of pro-inflammatory cytokines (IL-1$\beta$, IL-6, TNF-$\alpha$, GDF-15) suppresses hypothalamic appetite circuits through well-characterized immune-to-brain signaling pathways (Chapter~\ref{ch:endocrine}), producing involuntary appetite suppression without psychological control-seeking
\end{itemize}
...
The key distinction is between \emph{organic food avoidance} (driven by physical symptoms) and \emph{psychologically motivated food restriction} (driven by distorted body image or control seeking). Table~\ref{tab:ed-mecfs-differential} summarizes the primary distinguishing features.

\begin{table}[ht]
\centering
\small
\caption{Differential diagnosis: ME/CFS eating difficulties vs.\ primary eating disorders}
\label{tab:ed-mecfs-differential}
\begin{tabular}{p{3.5cm} p{4.5cm} p{4.5cm}}
\toprule
\textbf{Feature} & \textbf{ME/CFS eating difficulties} & \textbf{Primary eating disorder (AN/BN)} \\
\midrule
Motivation for restriction & Physical inability: nausea, fatigue, pain, PEM & Deliberate restriction; fear of weight gain; body image disturbance \\[4pt]
Attitude toward food & Distress at inability to eat; wishes to eat normally & Ambivalence, fear, or satisfaction in restriction \\[4pt]
Symptom sequence & Eating difficulties follow ME/CFS onset and worsen with disease severity & Eating disorder often antedates or precedes somatic complaints \\[4pt]
Activity level & Profoundly reduced due to debility & May be normal or elevated (compulsive exercise in AN) \\[4pt]
Weight loss attitude & Patient alarmed and distressed; does not desire weight loss & Weight loss may be goal-directed or regarded with ambivalence \\[4pt]
Response to nutrition & Nutritional support generally accepted and welcomed & May resist supplemental nutrition \\[4pt]
Body image & Usually intact & Typically distorted (perceives self as overweight despite low weight) \\[4pt]
PEM on eating & Present: specific foods or large meals trigger delayed worsening & Absent: eating may cause anxiety but not physiological crashes \\[4pt]
...
\paragraph{ARFID and ME/CFS.}
Avoidant/Restrictive Food Intake Disorder (ARFID) is a DSM-5 diagnosis covering restricted food intake due to sensory aversion, fear of adverse consequences (choking, vomiting, allergic reaction), or low appetite---without the body image disturbance of anorexia nervosa~\cite{Fisher2014arfid}. ME/CFS-driven food avoidance most closely resembles ARFID's ``fear of adverse consequences'' subtype, where eating is avoided because it reliably triggers nausea, pain, or PEM. Whether to assign an ARFID diagnosis to an ME/CFS patient with organically driven food restriction is a clinical judgment: the label may be useful for accessing nutritional support services, but risks overpathologizing a rational adaptive response to genuine physical consequences.

\paragraph{Bulimia Nervosa and ME/CFS.}
Bulimia nervosa (BN) presents a distinct set of differential diagnostic challenges from anorexia nervosa. Unlike AN, BN patients are typically of normal or near-normal weight, making weight loss an unreliable distinguishing marker. The hallmark of BN is a cycle of binge eating followed by compensatory purging behaviors (self-induced vomiting, laxative misuse, or excessive exercise)~\cite{Mehler2015bulimia}.

Several BN complications can produce a clinical picture superficially resembling ME/CFS autonomic features. Repeated purging causes dehydration and electrolyte depletion---particularly hypokalemia and metabolic alkalosis (pseudo-Bartter's syndrome)---which drive orthostatic tachycardia, fatigue, and exercise intolerance~\cite{Mehler2015bulimia}. A clinician unfamiliar with both conditions may mistake purging-induced orthostatic tachycardia for POTS, or purging-induced fatigue and muscle weakness for ME/CFS. The crucial differentiating features are:

\begin{itemize}
    \item \textbf{Purging behavior}: In BN, vomiting is intentional and compensatory (following a binge); in ME/CFS, vomiting or nausea is involuntary, driven by autonomic dysfunction, postprandial hypoperfusion, or visceral hypersensitivity
    \item \textbf{Electrolyte pattern}: BN produces a characteristic hypokalemia with metabolic alkalosis~\cite{Mehler2015bulimia}; ME/CFS has no characteristic serum electrolyte disturbance
    \item \textbf{Reversibility}: BN-induced orthostatic and autonomic symptoms resolve with cessation of purging and nutritional restoration; ME/CFS orthostatic symptoms are primary and persistent
    \item \textbf{Body image disturbance}: Present in BN; absent in ME/CFS. Patients with ME/CFS typically wish to eat normally and are distressed by weight loss, not motivated by it
    \item \textbf{PEM}: Absent in BN; the defining feature of ME/CFS. Exertional worsening is not a feature of purging-induced fatigue
\end{itemize}
...
\paragraph{Comorbidity: When Both Diagnoses Co-Exist.}
Fisher et al.\ (2002) documented four adolescents with both CFS and an eating disorder (subtype unspecified); in all four cases the eating disorder preceded the CFS diagnosis~\cite{Fisher2002cfs-ed}. This sequencing suggests that eating disorders may predispose to or unmask ME/CFS in susceptible individuals---or that it reflects diagnostic delay in ME/CFS recognition rather than causal precedence. Comorbid cases require management of both conditions simultaneously, which is particularly challenging because standard eating disorder treatments may be contraindicated in ME/CFS.

\paragraph{Misdiagnosis and Its Consequences.}
Misdiagnosis of ME/CFS as an eating disorder is well-documented, particularly in adolescent girls with severe disease causing significant weight loss~\cite{Baxter2021malnutrition}. The clinical consequences are serious:

\begin{itemize}
    \item Nutritional support is withheld or delayed under the assumption that the patient is refusing food voluntarily
    \item Psychiatric interventions (behavioral activation, exposure therapy, refeeding under psychological pressure) are applied to what is an organic medical condition
    \item In some cases, severely ill children have been misdiagnosed with \emph{Pervasive Refusal Syndrome} (PRS)---a rare condition involving total refusal of food, speech, and movement---because the medical team did not recognize severe ME/CFS. The key distinction: ME/CFS patients are \emph{frustrated} by their inability to function and strongly desire recovery; PRS is characterized by apathy and indifference to illness~\cite{Baxter2021malnutrition}
    \item Tube feeding---sometimes life-saving in very severe ME/CFS---has been withheld in cases where the treating team incorrectly attributed the patient's inability to eat to an eating disorder
\end{itemize}

\begin{warning}[Do Not Withhold Nutritional Support Pending Psychiatric Diagnosis]
\label{warn:ed-nutritional-delay}
In a patient with established or suspected ME/CFS who is losing weight or unable to maintain adequate intake, nutritional assessment and support should proceed on medical grounds regardless of whether an eating disorder is simultaneously suspected. The two assessments are not mutually exclusive, and delaying nutritional intervention while pursuing psychiatric workup risks life-threatening malnutrition~\cite{Baxter2021malnutrition}. See Chapter~\ref{ch:urgent-action-severe} for tube feeding indications.
\end{warning}

\paragraph{Eating Disorder Treatments Contraindicated in ME/CFS.}
Standard evidence-based eating disorder treatments require significant modification when ME/CFS is present or suspected:

\begin{itemize}
    \item \textbf{Graded exposure to feared foods}: If food avoidance is driven by genuine physical consequences (nausea, PEM), exposure-based approaches are inappropriate and may worsen physical status
    \item \textbf{Behavioral activation}: Increasing activity to stimulate appetite is contraindicated due to PEM risk
    \item \textbf{Motivational enhancement}: Assuming restriction is volitional and targeting motivation to change is inappropriate if restriction is physically driven
    \item \textbf{Appropriate modifications}: Psychological support for anxiety and mood \emph{secondary} to eating difficulties is appropriate; pacing of meal preparation; small frequent meals; addressing nausea through medical means (antiemetics, orthostatic management) before addressing psychological components
\end{itemize}
