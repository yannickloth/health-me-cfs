...
Research and patient reports suggest that \textbf{repeated episodes of post-exertional malaise cause cumulative physiological damage}~\cite{Chu2019,Maksoud2020natural}. While individual crashes may appear to resolve, each episode may contribute to:

\begin{itemize}
    \item \textbf{Mitochondrial dysfunction accumulation} (Section~\ref{sec:mitochondrial-dysfunction}): Repeated ATP depletion and oxidative stress damage mitochondrial membranes and DNA
    \item \textbf{Endothelial dysfunction} (Section~\ref{sec:endothelial}): Each PEM episode involves vascular dysfunction; repeated insults impair vessel reactivity permanently
    \item \textbf{Neuroinflammation} (Section~\ref{sec:neuroinflammation}): Repeated microglial activation leads to chronic neuroinflammatory state
    \item \textbf{Immune exhaustion} (Section~\ref{sec:chronic-activation}): Chronic activation depletes immune cell populations and function
    \item \textbf{Metabolic state transition} (Chapter~\ref{ch:energy-metabolism}): Progression from hypermetabolic (early, potentially reversible) to hypometabolic (established, potentially irreversible) state
\end{itemize}
