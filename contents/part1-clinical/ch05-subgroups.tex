...
\begin{itemize}
    \item No subgroup-specific treatments have been validated in randomized controlled trials
    ...
    \item Clinical management largely remains the same regardless of potential subgroup
\end{itemize}
\revend

However, the absence of formal validation does not mean phenotyping lacks clinical utility. Two independent lines of evidence suggest that unguided empirical treatment carries disproportionate risk: (1) two-day CPET studies demonstrate that ME/CFS patients have severely impaired energy production that worsens after exertion~\cite{Keller2024MetabolicCPET,VanCampen2020SeverityCPET}, and (2) pharmacological processing of any exogenous substance imposes metabolic demands on this already-depleted system. The \textit{cost of not phenotyping}---irreversible deterioration from empirical treatment trials---may therefore exceed the cost of imperfect phenotype-guided treatment selection (see Chapter~\ref{ch:integrative-treatment} for a treatment safety framework built on this reasoning).
...
\begin{itemize}
    ...
    \item \textbf{Treatment safety}: Enabling phenotype-guided treatment selection to reduce treatment-induced crashes in severe patients (see Section~\ref{sec:subtype-approaches} for subtype-specific treatment pathways)
\end{itemize}
\revend
...
\subsection{From Subgroups to Stratified Medicine}
...
\subsubsection*{Convergence Despite Heterogeneity}
...
\rev{5}
\textbf{miRNA convergence.} Cheema et al.~\cite{Cheema2023mirna} analyzed circulating miRNA profiles across ME/CFS patients and found striking heterogeneity in individual miRNA expression. However, when these heterogeneous miRNA signatures were analyzed at the pathway level, they converged on a highly specific set of target gene clusters ($p < 0.002$), governing exercise hyperemia, angiogenic adaptation, antioxidant defenses, and mitochondrial dynamics. This suggests that while patients arrive at dysfunction through different miRNA alterations, the functional consequences converge on a narrow set of pathways.

\rev{5}
\textbf{Metabolic convergence.} Naviaux et al.~\cite{Naviaux2016metabolic} described a ``dauer-like'' hypometabolic state observed across ME/CFS cohorts, characterized by downregulation of metabolic pathways, mitochondrial ATP production, and oxidative stress defenses. The metabolic signature showed diagnostic accuracy of 94--96\%, with 80\% of diagnostic metabolites decreased across the cohort. This suggests that regardless of how a patient develops ME/CFS, the resulting metabolic reorganization follows a stereotyped pattern.

\rev{5}
\textbf{Genetic convergence.} More recently, Birch \& Younger~\cite{Birch2025precision} examined genetic variation across ME/CFS patients and found gene-level heterogeneity in which specific genes were dysregulated. However, at the pathway level, patients consistently showed convergence on impaired energy production pathways, reduced stress resilience, and dysregulated post-exertional metabolic recovery. Thus genetic heterogeneity at the individual gene level masks functional convergence at the pathway level.
\revend

\subsubsection*{Cross-Disease Paradigm Lessons}
...
\rev{5}
\textbf{Asthma.} Severe asthma was long treated as a single entity with poor outcomes in trial-and-error approaches. Only with endotyping (Th2-high vs.\ Th2-low stratification) did molecular stratification become possible~\cite{Woodruff2009asthma}. Subsequent clinical trials demonstrated that Th2-high patients respond to IL-4/IL-13 inhibition and anti-IgE therapy, while Th2-low endotypes require different approaches. The heterogeneity was not the problem---the lack of stratification was.

\rev{5}
\textbf{Depression.} Major depressive disorder appears clinically homogeneous but encompasses distinct biological subtypes. Miller \& Raison~\cite{MillerRaison2023depression} demonstrated that a subset of depressed patients with elevated inflammatory markers (CRP $> 3$~mg/L) show poor response to conventional antidepressants but improved outcomes with anti-TNF biologics (infliximab). Patients without inflammatory elevation do not benefit from anti-inflammatory add-ons, showing that stratification is essential.

\rev{5}
\textbf{Breast cancer.} Perou et al.~\cite{Perou2000breast} established that morphologically similar breast cancers represent distinct molecular subtypes (luminal A/B, HER2-enriched, basal-like), each with different prognoses and treatment responses. This molecular stratification transformed oncology from cytotoxic chemotherapy for all to subtype-matched targeted therapies (ER/PR inhibitors, HER2 inhibitors, etc.), dramatically improving outcomes.
...
\subsubsection*{Dual Strategy Framework for ME/CFS Therapeutics}

\rev{5}
These principles suggest that ME/CFS therapeutic development should pursue \textit{both} upstream and downstream targets:

\rev{5}
\textbf{Upstream stratification.} Identify ME/CFS subgroups defined by biomarker endotypes and match treatments to these subgroups:
\begin{itemize}
    \item \textbf{Autoimmune-predominant} (elevated GPCR autoantibodies): Immunoadsorption and monoclonal antibodies (daratumumab) targeting autoreactive B cells~\cite{Fluge2025daratumumab, Stein2024immunoadsorption}
    \item \textbf{Immune-inflammatory-predominant} (elevated pro-inflammatory cytokines, T-cell exhaustion): Immunomodulation targeting specific cytokine elevations (see Chapter~\ref{ch:medications})
    \item \textbf{Autonomic-predominant} (sympathetic dysautonomia cluster): Targeted autonomic interventions including heart rate variability training, selective norepinephrine support, or sympatholytic approaches depending on dysautonomia subtype~\cite{Slomko2020autonomic}
\end{itemize}

\rev{5}
\textbf{Downstream convergent targets.} Pursue shared pathway interventions that may benefit across subgroups (see Chapters~\ref{ch:supplements} and~\ref{ch:medications} for evidence and dosing):
\begin{itemize}
    \item Mitochondrial support (CoQ10, L-carnitine, ubiquinol)
    \item PDH complex enhancement (thiamine, alpha-lipoic acid)
    \item Antioxidant restoration (NAC, vitamin E, selenium)
    \item Metabolic stabilization (low-dose naltrexone, metabolic cofactors)
\end{itemize}
...
\subsubsection*{Endotype-Specific Treatment Mapping}

\rev{5}
Based on emerging literature, the following endotype-to-treatment mappings are suggested. Table~\ref{tab:endotype-comprehensive} extends the basic mapping with reproducibility status, clinical accessibility of biomarkers, and evidence levels for proposed treatments.

\rev{5}
\begin{table}[htbp]
\centering
\caption{Comprehensive endotype-to-biomarker-to-treatment mapping in ME/CFS. Evidence: \textbf{A}~=~RCT data; \textbf{B}~=~pilot/open-label; \textbf{C}~=~mechanistic rationale only. Reproducibility: \textbf{R}~=~replicated in $\geq$2 independent cohorts; \textbf{P}~=~preliminary (single study or unreplicated).}
\label{tab:endotype-comprehensive}
\small
\begin{tabular}{@{}p{2.2cm}p{3.2cm}p{1.2cm}p{3.5cm}p{1cm}p{1.3cm}@{}}
\toprule
\textbf{Proposed Endotype} & \textbf{Minimum Biomarkers} & \textbf{Access} & \textbf{Targeted Treatments} & \textbf{Evid.} & \textbf{Reprod.} \\
\midrule
Autoimmune & Elevated GPCR autoantibodies ($\beta_1$, $\beta_2$, M3, M4); B-cell activation markers & Tier~3 &
  Immunoadsorption~\cite{Scheibenbogen2018immunoadsorption}; daratumumab~\cite{Fluge2025daratumumab}; BC007~\cite{Hohberger2021bc007} & B & R \\
\addlinespace
Metabolic (ME-M2) & Elevated TAG/insulin ratio; low FFAs; high pyruvate; BMI trend~\cite{Germain2020metabolic} & Tier~2--3 &
  Mitochondrial cofactors (CoQ10+NADH)~\cite{CastroMarrero2021fatigue}; metabolic support; insulin sensitization & A & P \\
\addlinespace
Autonomic-Predominant & Abnormal tilt-table; reduced HRV; abnormal CO/CBF~\cite{VanCampenEtAl2024} & Tier~2 &
  Beta-blockers (POTS); ivabradine; volume expansion; HRV biofeedback~\cite{Slomko2020autonomic} & B & R \\
\addlinespace
Post-Infectious (viral) & Documented viral onset; elevated HHV-6/EBV titers; disease duration 2--8~yr~\cite{Strayer2020rintatolimod} & Tier~2 &
  Valganciclovir (HHV-6/EBV subset)~\cite{Montoya2013valganciclovir}; rintatolimod~\cite{Strayer2020rintatolimod} & A/B & P \\
\addlinespace
Immune-Inflammatory & Elevated IL-6, TNF-$\alpha$; T-cell exhaustion (PD-1$^+$); reduced NK cytotoxicity~\cite{EatonFitch2019} & Tier~2--3 &
...
\rev{5}
\begin{limitation}[Endotype Classification: Provisional Framework]
This classification is provisional and based on emerging evidence. No endotype has been validated prospectively in a stratified clinical trial. Patients are not expected to fall neatly into single categories; many will show features of multiple endotypes, requiring combination approaches or sequential treatment. The framework serves as a research scaffold for biomarker-guided clinical decision-making, not as a validated diagnostic system. Biomarker accessibility tiers refer to Table~\ref{tab:phenotyping-tiers}.
\end{limitation}
\revend

\subsection{From ``Not Clinically Actionable'' to ``Increasingly Necessary''}
\label{sec:phenotyping-imperative}

\rev{5}
\paragraph{The Cost of Not Phenotyping.}
The traditional treatment approach in ME/CFS---empirical trial-and-error---assumes that failed trials carry manageable costs. In practice, this assumption fails catastrophically for severe patients. Each treatment trial imposes metabolic processing demands on an energy-depleted system (see Treatment Energy Categories, Section~\ref{subsec:energy-categories}). When a trial triggers post-exertional malaise, the resulting crash can cause lasting functional decline---the crash severity dose-response hypothesis estimates 5--15\% permanent function loss from severe crashes and potentially greater loss from catastrophic ones (Section~\ref{subsubsec:crash-dose-response}, Table~\ref{tab:crash-severity-tiers}).
...
\rev{5}
\begin{hypothesis}[Prospective Phenotyping as Harm Reduction]
\label{hyp:phenotyping-harm-reduction}
...
\begin{enumerate}
    \item \textbf{Reducing trial number}: Matching treatment to subtype reduces the expected number of failed trials before finding an effective intervention
    \item \textbf{Energy category matching}: Phenotyping enables prioritizing Category A (energy-providing) treatments for metabolic-predominant subtypes and reserving Category C (energy-demanding) treatments for subtypes where biomarkers predict benefit
    \item \textbf{Avoiding known risks}: Pharmacogenomic data (CYP450 metabolizer status) can prevent paradoxical reactions before they occur
    \item \textbf{Asymmetric cost}: The cost of phenotyping is primarily financial and one-time, while the cost of a treatment-induced crash is primarily biological and potentially permanent. Though these costs are measured on different scales, the irreversibility of severe crashes (loss of functional capacity that may never be recovered) makes the comparison asymmetric in favor of phenotyping for patients with limited remaining functional reserves
\end{enumerate}

\textbf{Certainty: 0.35} (strong mechanistic reasoning from crash dose-response data and pharmacogenomic principles; clinical observation of treatment-induced deterioration in severe patients; no RCTs comparing phenotype-guided vs. empirical treatment selection in ME/CFS)

\textbf{Testable prediction}: Severe ME/CFS patients treated with phenotype-guided protocols will experience fewer treatment-induced crashes and faster time-to-effective-treatment compared to patients treated empirically.
\end{hypothesis}

\rev{5}
\paragraph{A Tiered Prospective Phenotyping Protocol.}
...
\label{tab:phenotyping-tiers}
\small
\begin{tabular}{@{}p{2cm}p{4cm}p{4cm}p{3cm}@{}}
\toprule
\textbf{Tier} & \textbf{Assessments} & \textbf{What it identifies} & \textbf{Treatment guidance} \\
\midrule
\textbf{Tier 1} \newline Any GP &
CBC, CMP, TSH, ESR/CRP, ferritin, vitamin D, B12/folate; vital signs including orthostatic test; detailed symptom questionnaire; medication history &
Severity level; dominant symptom cluster (fatigue vs. pain vs. cognitive vs. autonomic); basic metabolic status; obvious comorbidities &
Severity-appropriate starting point; foundation-first sequencing (Section~\ref{subsec:prioritizing-interventions}); avoid Category C treatments until Tier 2 \\
\midrule
\textbf{Tier 2} \newline Specialist &
Tilt table test; NK cell function; lymphocyte subsets; viral serology (EBV, HHV-6, CMV); amino acid/organic acid panel; cortisol rhythm; tryptase &
Immune-predominant vs. autonomic-predominant vs. metabolic-predominant subtype; MCAS status; viral reactivation burden &
Subtype-specific treatment pathway (Section~\ref{sec:subtype-approaches}); targeted Category B--C trials where biomarkers predict benefit \\
\midrule
\textbf{Tier 3} \newline Research &
Metabolomics; autoantibody panels (GPCR antibodies); TRPM3 function; pharmacogenomics (CYP2D6, CYP2C19, COMT, MTHFR); cytokine panel &
Specific metabolic phenotype (ME-M1/M2/M3); autoimmune subtype; drug metabolism profile; paradoxical reactor risk &
Precision treatment matching (Section~\ref{spec:precision-medicine-framework}); pharmacogenomic-guided dosing (Section~\ref{sec:pharmacogenomics}) \\
\bottomrule
\end{tabular}
\end{table}

\rev{5}
The Septad diagnostic hierarchy (Section~\ref{sec:septad-diagnostic-hierarchy}) already demonstrates how presentation-based phenotyping can guide clinical decisions. The tiered protocol above extends this principle from comorbidity diagnosis to treatment selection.

\rev{5}
For treatment-selection implications of phenotyping, see the subtype-specific treatment pathways in Section~\ref{sec:subtype-approaches} and the treatment safety framework in Section~\ref{sec:treatment-planning}.
\revend
...
Rather than seven independent conditions with coincidental co-occurrence, the Septad may represent a pathophysiological cascade where upstream conditions drive downstream manifestations. \textbf{Primary initiators} (hEDS, chronic infection, MCAS) drive \textbf{secondary amplifiers} (dysautonomia, SFN, GI dysmotility, autoimmunity), which ultimately cause \textbf{tertiary consequences} (mitochondrial dysfunction, ME/CFS phenotype).
\end{hypothesis}

\textbf{Primary Initiators (Upstream Conditions):}
\begin{itemize}
...
\textbf{Secondary Amplifiers (Downstream Conditions):}
\begin{itemize}
...
\rev{5}
\begin{limitation}[Cascade Model Epistemic Status]
...
\textbf{If MCAS/HIT Features Dominate:}
\begin{enumerate}
...
\textbf{If Post-Infectious Pattern:}
\begin{enumerate}
...
\textbf{If Hypermobility/hEDS Features:}
\begin{enumerate}
...
\textbf{If Autonomic Features Dominate:}
\begin{enumerate}
...
\rev{5}
\begin{limitation}[Septad: Clinical Co-occurrence $\neq$ Shared Pathophysiology]
...
\textbf{What data do not establish:}
\begin{itemize}
...
The critical distinction: \emph{clinical co-occurrence is documented; shared pathophysiology is not}.
\end{limitation}
\revend
...
\rev{5}
\begin{limitation}[Autophagy--Septad Extrapolation Boundaries]
This hypothesis extrapolates from a single uncontrolled pilot study to multi-system effects not measured in that trial. The rapamycin study enrolled 86 patients; 70 completed day 36 and 40 completed the full 90-day protocol, representing 53\% attrition that may bias results. The study did not assess Septad component status, mast cell markers, nerve fiber density, or GI function. The mechanistic connections (autophagy $\rightarrow$ each Septad component) are individually plausible based on cellular biology but have not been demonstrated in ME/CFS cohorts.
\end{limitation}
\revend
...
\rev{5}
\begin{limitation}[Connective Tissue Hypothesis: Untested Causal Claims]
...
\begin{itemize}
    ...
    \item Strong response to amino acid supplementation (especially L-citrulline, N-Acetylcysteine (NAC))
...
\begin{enumerate}
    ...
    \item \textbf{If positive response}: Add comprehensive amino acid protocol (N-Acetylcysteine (NAC), L-citrulline-malate, consider antiviral if viral titers elevated)
