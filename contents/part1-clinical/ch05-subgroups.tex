...
Based on emerging literature, the following endotype-to-treatment mappings are suggested:

\begin{center}
\begin{tabular}{|p{3cm}|p{4.5cm}|p{3.5cm}|}
\hline
\textbf{Proposed Endotype} & \textbf{Biomarker Features} & \textbf{Potential Treatments} \\
\hline
Autoimmune & Elevated GPCR autoantibodies, B-cell activation & Immunoadsorption, daratumumab, rituximab \\
\hline
Metabolic (ME-M2) & Lipid accumulation, elevated TAG/insulin ratio, hypometabolism & Insulin sensitization, metabolic support, mitochondrial cofactors \\
\hline
Autonomic-Predominant & Sympathetic dysautonomia cluster, reduced HRV, BP dysregulation & HR variability training, targeted autonomic agents \\
\hline
Post-Infectious & Disease duration 2--8 years post-viral onset & Rintatolimod (TLR-3 agonist)~\cite{Strayer2020rintatolimod}, antiviral considerations \\
\hline
Immune-Inflammatory & Elevated IL-6, TNF-$\alpha$, T-cell exhaustion markers & Targeted immunomodulation, anti-cytokine therapy \\
\hline
\end{tabular}
\end{center}

The classification above is provisional and based on emerging evidence. Critically, patients are not expected to fall neatly into single categories; many will show features of multiple endotypes, requiring combination approaches or sequential treatment. The framework serves as a research scaffold for biomarker-guided clinical decision-making, not as a validated diagnostic system.
