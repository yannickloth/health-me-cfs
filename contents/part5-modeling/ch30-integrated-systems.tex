...
A crucial epistemological point: the attractor structure of this model was not designed in. The ODEs in Chapters~\ref{ch:energy-metabolism-models}--\ref{ch:integrated-systems} were built from biochemical literature---enzyme kinetics, immune cell dynamics, mitochondrial biophysics---without targeting any particular dynamical behaviour. A modeller working from the same literature without knowledge of dynamical systems theory would write structurally identical equations. The bistability is therefore a \emph{prediction} of the biochemistry, not a premise. This section demonstrates that claim analytically, by deriving attractor existence from first principles on the reduced two-variable subsystem that captures the core positive feedback loop.
...
This is a sigmoidal (Hill) function of $A$, increasing from 0 at $A = 0$ to a plateau $I_{\max} = k_{\text{act,0}} I_0 / (k_{\text{exh}} + d_a)$ as $A \to \infty$. The sigmoidal shape is solely a consequence of the Hill kinetics governing ATP-dependent immune activation---a standard biochemical form, not a modelling choice.
...
$P(A)$ reflects ATP production kinetics: it rises with decreasing ATP (as AMP accumulates and activates glycolysis via PFK-1) up to a maximum, then flattens as the ETC saturates. For the ME/CFS regime, where Complex~I is partially inhibited ($\alpha_{\text{CI}} < 1$), $P(A)$ is compressed. The $A$-nullcline is therefore a non-monotone curve in the $(A, I)$ plane: it rises at low $A$, peaks, and may descend again as production saturates.

\paragraph{Intersection multiplicity condition.}
Bistability exists when the $I$-nullcline and the $A$-nullcline intersect \emph{three} times. From Equation~\ref{eq:i-nullcline}, the $I$-nullcline is strictly monotone and sigmoidal. The $A$-nullcline (Equation~\ref{eq:a-nullcline}) has an inverse-S or bell shape depending on the production kinetics. Three intersections occur when the following condition holds:
...
With reduced Complex~I activity, $P(A)$ is compressed: $P(A) - \beta_0$ is positive over a narrower range of $A$ and the peak of $P'(A)$ is lower. Simultaneously, elevated $k_{\text{exh}}$ reduces the $(k_{\text{exh}} + d_a)$ denominator of $I^*(A)$, raising $I_{\max}$---the immune system can sustain higher activated-cell counts. These two changes together cause the $I$-nullcline and $A$-nullcline to intersect three times, satisfying condition~\ref{eq:bistability-inequality}. The three fixed points are:

\begin{enumerate}
    \item \textbf{Healthy-adjacent fixed point} $(A_H, I_H)$: high ATP, low immune activation. Jacobian analysis gives two negative real eigenvalues---a \textbf{stable node}. This is the healthy attractor. It persists even under ME/CFS parameters, but its basin of attraction is now small.
    \item \textbf{Saddle point} $(A_s, I_s)$: intermediate ATP, intermediate immune activation. One positive and one negative eigenvalue---an \textbf{unstable saddle}. This saddle lies on the separatrix: its stable manifold is the boundary between the two basins of attraction.
    \item \textbf{Disease fixed point} $(A_D, I_D)$: low ATP, high immune activation. Jacobian analysis gives two negative real eigenvalues---a \textbf{stable node}. This is the \textbf{disease attractor}.
\end{enumerate}
...
The reduced 2D system proves three results that carry over qualitatively to the full 64-variable model:

\begin{enumerate}
    \item \textbf{Both regimes have a healthy attractor}: even ME/CFS parameters support a healthy fixed point---but with a much smaller basin of attraction. Recovery is theoretically possible without parameter restoration, but requires a large directed perturbation.
    \item \textbf{The disease attractor is absent in the healthy regime}: it is created by the bifurcation that occurs as $\alpha_{\text{CI}}$ decreases and $k_{\text{exh}}$ increases. There is no disease state to fall into before these parameters shift.
    \item \textbf{The bistability is structurally robust}: it depends only on (a)~a Hill-type ATP-dependent immune activation term (standard cooperative kinetics), and (b)~a concave production function $P(A)$ (a consequence of ETC saturation, standard biophysics). Any model built from the same biochemistry will exhibit the same bifurcation structure.
\end{enumerate}

\begin{limitation}[Reduced Model Scope]
The 2D analysis abstracts away 62 state variables. The qualitative conclusions---bistability exists, healthy parameters support one attractor, ME/CFS parameters support two---are robust to this reduction because they depend on the topological structure of the nullclines, not on the precise values of omitted variables. Quantitative claims (exact bifurcation thresholds, basin sizes) require numerical analysis of the full system with continuation software (AUTO, MATCONT) and have not been computed.
\end{limitation}
