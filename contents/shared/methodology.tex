\chapter*{Methodology and Quality Assurance}
\addcontentsline{toc}{chapter}{Methodology and Quality Assurance}
\label{ch:methodology}

A document of this scope---over \RoundedPageCount{} pages of medical content spanning pathophysiology, treatment, research synthesis, and mathematical modeling---produced through human--AI collaboration and without formal medical training on the author's part, demands rigorous and transparent quality assurance. This chapter describes the methodology used to maintain scientific accuracy, epistemic honesty, and internal consistency throughout the manuscript.

\section*{Epistemic Classification}
\label{sec:meth-epistemic}
...
\paragraph{Claim environments.}
The following environments are used consistently throughout the manuscript:

\begin{description}
\item[Achievement] A research finding supported by strong evidence: replicated results from peer-reviewed studies with adequate sample sizes and sound methodology. These represent the most reliable claims in the document.

\item[Hypothesis] An unproven conjecture or working theory with moderate plausibility (certainty $\geq 0.45$). Hypotheses are clearly marked because they may be wrong. Each hypothesis is required to include at least one falsifiable prediction, identified limitations, and relevant citations.

\item[Speculation] A conjecture with lower plausibility (certainty $< 0.45$), often based on limited evidence, mechanistic reasoning, or extrapolation from related conditions. Speculations are held to the same structural requirements as hypotheses but carry explicit low-confidence warnings.

\item[Prediction] A testable claim about future observations or experimental outcomes. Predictions specify what research should find if a given hypothesis is correct, providing a concrete path toward validation or falsification.

\item[Warning] A critical caveat about limitations, risks, or potential misinterpretations. Warnings flag contraindications, preliminary evidence, or situations where claims should be interpreted with particular caution.

\item[Limitation] An explicit epistemic boundary marking what the available data cannot conclude. Distinct from warnings (which address practical caveats), limitations delineate the outer edges of justified inference.

\item[Recommendation] An evidence-based clinical or management suggestion. Recommendations are distinguished from hypotheses: a recommendation says ``consider doing this,'' grounded in clinical evidence, whereas a hypothesis says ``this might be true,'' grounded in mechanistic reasoning. This distinction is enforced through automated review (see \emph{Automated Multi-Layer Review} below).
\end{description}
...
\begin{description}
\item[High certainty] Large sample size ($n > 100$), peer-reviewed in a reputable journal, independently replicated, consistent across studies.

\item[Medium certainty] Moderate sample ($n = 20$--$100$), peer-reviewed but limited replication, sound methodology. Promising but requiring confirmation.

\item[Low certainty] Small sample ($n < 20$), preprint or conference abstract, methodological concerns, or contradicted by other studies.
\end{description}

\paragraph{Replication status.}
Key findings are annotated with their replication status: \emph{fully replicated} (confirmed by independent cohorts), \emph{partially replicated} (some confirmation with caveats), or \emph{not yet replicated} (single study or research group). This classification reflects the reality that much ME/CFS research remains at early replication stages due to historical underfunding.

\section*{Literature Integration}
\label{sec:meth-literature}
...
\paragraph{Integration protocol.}
Each integrated paper passes through a defined sequence:

\begin{enumerate}
\item \textbf{Bibliographic entry}: Full citation added to the bibliography with complete metadata (authors, title, journal, DOI, year).
\item \textbf{Certainty assessment}: Evidence strength classified as high, medium, or low based on sample size, peer-review status, replication, and methodology.
\item \textbf{Content placement}: Findings inserted into the appropriate chapter using the correct epistemic environment (achievement, hypothesis, or speculation) with explicit certainty annotation.
\item \textbf{Cross-referencing}: Links established to related content in other chapters, the hypothesis registry, and the annotated bibliography.
\end{enumerate}

\paragraph{Annotated bibliography.}
Appendix~\ref{app:annotated-bibliography} provides structured summaries of key papers organized by topic, enabling readers to evaluate primary sources directly.

\section*{Automated Multi-Layer Review}
\label{sec:meth-review}
...
\paragraph{Phase 3: Scientific rigor.}
The core audit layer. Verifies that substantive claims carry citations, that hypotheses and speculations use the correct environments, that falsifiable predictions accompany hypotheses, and that reasoning avoids circularity or completeness gaps. This phase also cross-validates the hypothesis registry (see \emph{Cross-Document Consistency} below) against chapter content.

\paragraph{Phase 4: Domain-specific biology.}
...
\paragraph{Phase 5: Safety and patient impact.}
...
\paragraph{Phase 6: Publication readiness.}
...
\paragraph{Adversarial convergence.}
...
\section*{Author Critical Oversight}
\label{sec:meth-author-oversight}
...
\paragraph{Directed questioning.}
The author systematically challenges AI outputs through targeted questions: \emph{What is the actual sample size of this study? Has this finding been replicated independently? Is this mechanism established or hypothetical? Does this treatment recommendation follow from the cited evidence, or from mechanistic reasoning alone?} These questions frequently reveal overstatements, missing qualifications, or conflation of related but distinct findings. AI systems tend to present information with uniform confidence; the author's role is to impose appropriate epistemic gradation.

\paragraph{Redirection and correction.}
...
\paragraph{Conceptual integration.}
...
\paragraph{Lived experience as epistemic resource.}
...
\section*{Formal Modeling Pipeline}
\label{sec:meth-modeling}
...
\paragraph{Three-layer formalization.}
Each biological process amenable to formal modeling is represented at three levels of abstraction:

\begin{enumerate}
\item \textbf{Causal directed acyclic graphs (DAGs)}: Represent causal relationships between biological variables with edge-level certainty annotations derived from the literature. DAGs make causal assumptions explicit and testable.
\item \textbf{Event-driven process chains (EPCs)}: Capture the temporal sequence of biological events, including branching points, feedback loops, and probabilistic transitions. EPCs bridge the gap between qualitative causal understanding and quantitative dynamics.
\item \textbf{Ordinary differential equations (ODEs)}: Quantitative models with parameters estimated from published data where available, and explicitly flagged as assumed where data are lacking. Parameter uncertainty is propagated through the model hierarchy.
\end{enumerate}
...
\label{sec:meth-consistency}

Maintaining consistency across over \RoundedPageCount{} pages requires systematic tracking mechanisms.

\paragraph{Hypothesis registry.}
...
\paragraph{Terminology harmonization.}
...
\section*{Limitations of This Methodology}
...
\paragraph{Evidence base limitations.}
...
\paragraph{Evolving methodology.}
...
\vspace{1em}
\noindent The methodology described in this chapter aims to make the document's epistemic commitments, evidence standards, and quality processes fully transparent. Readers are encouraged to use this information when evaluating the reliability of specific claims throughout the manuscript.
