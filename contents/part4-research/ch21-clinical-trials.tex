...
\begin{itemize}
    ...
    \item Severe and very severe patients are systematically excluded from trials due to inability to attend research sites; the ACHTSAM pilot study~\cite{Fricke2026achtsam} is the first protocol specifically designed to evaluate home-based diagnostic tolerability in patients with Bell scores $\leq$30
...
The failure of rituximab in an unselected ME/CFS population, contrasted with apparent benefit in patients with elevated autoantibodies, suggests that ME/CFS comprises mechanistically distinct subgroups. Future trials should stratify by: (1)~autoantibody profile (for immunomodulatory interventions), (2)~viral reactivation markers (for antiviral interventions), (3)~metabolomic profile (for mitochondrial support), (4)~autonomic subtype (for cardiovascular interventions), and (5)~antigen persistence status---whether an active antigenic source (viral reservoir, autoantibody production, or active reactivation) is demonstrable by validated assays (for source-reduction versus multi-lock strategies; see Chapter~\ref{ch:causal-hierarchy}, Section~\ref{sec:antigen-stratification}). The challenge is developing validated biomarker panels for stratification before launching large trials; antigen persistence assays for most ME/CFS-relevant pathogens are not yet standardised, and stratification by this variable is a research aspiration rather than a currently feasible requirement.
\end{open_question}
