...
\rev{5}
\begin{limitation}[Translational Findings: Shared Mechanisms Do Not Guarantee Shared Treatments]
...
\begin{itemize}
    \item Shared symptom profiles and overlapping biomarker abnormalities do not establish shared aetiology; convergent phenotypes can arise from divergent upstream causes.
    \item Most ME/CFS treatment evidence cited for translational application comes from open-label or pilot studies (e.g., daratumumab n=10~\cite{Fluge2025daratumumab}, immunoadsorption n=10~\cite{Scheibenbogen2018immunoadsorption}); extrapolating these to other conditions compounds the uncertainty.
    \item The ``certainty levels'' framework (High/Medium/Low) used in this chapter is the authors' judgement, not a validated classification; readers should not interpret ``High Certainty'' as equivalent to established clinical evidence.
    \item Condition-specific pathophysiology (e.g., spike protein persistence in Long COVID, central sensitisation in fibromyalgia) may make ME/CFS-derived treatments ineffective or harmful in related conditions.
\end{itemize}
\end{limitation}
\revend
...
Long COVID and ME/CFS share post-viral onset, exercise intolerance with delayed symptom exacerbation, autonomic dysfunction, and cognitive impairment, with a substantial proportion of Long COVID patients meeting activity-based case definitions also meeting ME/CFS diagnostic criteria~\cite{Jason2023LongCOVID}.
...
\midrule
GPCR autoantibodies & 29.5--91\% prevalence~\cite{Loebel2016,Bynke2020} & $\beta$2-AR, M3 autoantibodies detected \\
Plasma cell autoimmunity & Daratumumab 60\% response~\cite{Fluge2025daratumumab} & BC007 case reports \\
Endothelial dysfunction & Peripheral endothelial dysfunction~\cite{Scherbakov2020} & Microclotting, VWF elevation \\
NAD$^+$ depletion & Metabolomic studies~\cite{heng2025mecfs} & NR trial raised NAD$^+$ levels; clinical benefit variable~\cite{Wu2025NRLongCOVID} \\
Neuroinflammation & PET imaging~\cite{Nakatomi2014neuroinflammation} & MRI, CSF abnormalities \\
Small fiber neuropathy & SFN studies~\cite{Azcue2023sfn} & Documented in subset \\
\bottomrule
\end{tabular}
\end{table}
\revend
...
\begin{enumerate}
    ...
    \item \textbf{Immunoadsorption for GPCR Autoantibodies}: 70\% rapid improvement during treatment in a pilot study (n=10) of ME/CFS patients with elevated $\beta$2-adrenergic receptor autoantibodies, though only 30\% sustained improvement at 6--12 months~\cite{Scheibenbogen2018immunoadsorption}.

    \textbf{Implication for Long COVID}: Autoantibody screening could identify subset likely to respond to immunoadsorption.

    \item \textbf{NAD$^+$ Restoration with Nicotinamide Riboside}: A 2025 Long COVID RCT (n=58; 20-week intervention) showed NR 2000~mg/day increased NAD$^+$ levels 2.6--3.1$\times$~\cite{Wu2025NRLongCOVID}; cognitive benefits were variable, with some individuals improving after $\geq$10 weeks.

    \textbf{Implication for Long COVID}: NAD$^+$ depletion may be a shared mechanism; prolonged treatment ($>$10 weeks) may be needed for clinical benefit.
\end{enumerate}
\revend
...
\rev{5}
\begin{itemize}
    \item \textbf{Mitochondrial support}: CoQ10 ubiquinol (100--300~mg/day) + NADH (10--20~mg/day), D-ribose (5g TID), acetyl-L-carnitine (500--2000~mg/day), NAD$^+$ precursors (NR 300--1000~mg/day; the only RCT-tested dose is 2000~mg/day~\cite{Wu2025NRLongCOVID}; see Chapter~\ref{ch:supplements})
...
\begin{enumerate}
    ...
    \textbf{Implication for POTS}: Central (not just peripheral) catecholamine deficiency may drive compensatory tachycardia. This suggests catecholamine synthesis support (L-tyrosine, Tetrahydrobiopterin (BH4) cofactors) could be therapeutic.
...
\begin{enumerate}
    \item \textbf{Small Fiber Neuropathy}: Quantitative sensory testing and skin biopsy studies document small fiber dysfunction in a subset of ME/CFS patients, correlating with pain and dysautonomia~\cite{Azcue2023sfn}.

    \textbf{Implication for Fibromyalgia}: Small fiber neuropathy has been documented in fibromyalgia as well~\cite{Oaklander2013sfn}. This suggests shared peripheral nerve pathology beyond central sensitization.

    \item \textbf{Mitochondrial ATP Depletion}: Multiple ME/CFS studies show impaired ATP production, early lactate accumulation, and elevated acylcarnitines indicating impaired fatty acid oxidation~\cite{Naviaux2016metabolomics,Syed2025MitoCFS}.
    ...
    \item \textbf{Mast Cell Activation}: Ketotifen (mast cell stabilizer) was tested in a fibromyalgia Phase~1 RCT but did not show significant benefit at the dose tested (4~mg/day for 8 weeks; $p = 0.7$ for pain)~\cite{Moldofsky2015}. Nevertheless, ME/CFS research provides mechanistic understanding of mast cell--pain--fatigue connections that may warrant further investigation at higher doses.
...
\rev{5}
\begin{itemize}
    \item \textbf{D-ribose}: 5g TID showed improvements across energy, sleep, and well-being in two open-label studies without placebo control~\cite{Teitelbaum2006ribose,Teitelbaum2012ribose}; see Chapter~\ref{ch:supplements} for evidence limitations
    \item \textbf{CoQ10 ubiquinol + NADH}: CoQ10 100--300~mg/day (300~mg used in fibromyalgia trials~\cite{Cordero2013}) + NADH 10--20~mg/day (combination supported by ME/CFS RCTs~\cite{CastroMarrero2021fatigue}; see Chapter~\ref{ch:supplements})
    \item \textbf{Acetyl-L-carnitine}: 500--2000~mg/day for neuroprotection and brain fog reduction (see Chapter~\ref{ch:supplements})
    \item \textbf{Low-dose naltrexone}: 3--4.5 mg at bedtime for neuroinflammation and pain modulation
    \item \textbf{NAD$^+$ precursors}: NR 300--1000~mg/day (the only RCT-tested dose is 2000~mg/day~\cite{Wu2025NRLongCOVID}); see Chapter~\ref{ch:supplements} for full dosing
\end{itemize}
\revend
...
\begin{enumerate}
    ...
    \item \textbf{Tetrahydrobiopterin (BH4) measurement in EDS with orthostatic intolerance}: Is the ME/CFS finding translatable?
...
\textbf{Certainty}: \textbf{Medium} for vascular mechanisms; \textbf{Low-Medium} for $\beta$2-receptor pathway; \textbf{Low} for Tetrahydrobiopterin (BH4) dysregulation; \textbf{None} for plasma cell autoimmunity.
...
\begin{enumerate}
    ...
    \textbf{Implication for PTLDS}: If persistent neuroinflammation drives symptoms, Low-Dose Naltrexone (LDN) could provide benefit through microglial modulation.
...
\begin{enumerate}
    \item \textbf{NAD$^+$ Restoration Therapy}: NR 2000~mg/day increased NAD$^+$ levels 2.6--3.1 fold in Long COVID (n=58; 20-week intervention; measured at interim timepoints); cognitive benefits were variable, with some improvement after $\geq$10 weeks~\cite{Wu2025NRLongCOVID}. The mechanism of NAD$^+$ restoration is hypothesized to also address chemotherapy-induced NAD$^+$ depletion, by analogy with the Long COVID findings.

    \textbf{Implication for CRF}: NAD$^+$ precursors could restore depleted NAD$^+$ pools, improving mitochondrial function and reducing fatigue (\textbf{post-treatment only}; NAD$^+$ supplementation during active cancer is contraindicated due to theoretical tumor growth promotion---see Section~\ref{sec:mito-support-universal}, Safety considerations).

    \item \textbf{Comprehensive Mitochondrial Support Stack}: CoQ10 ubiquinol (100--300~mg/day) + NADH (10--20~mg/day), D-ribose (5g TID), acetyl-L-carnitine (500--2000~mg/day), alpha-lipoic acid (300--600~mg/day; classified under antioxidants in Chapter~\ref{ch:supplements}), B vitamins. See Chapter~\ref{ch:supplements} for full dosing and evidence levels.

    \textbf{Implication for CRF}: Addresses multiple points of mitochondrial dysfunction caused by chemotherapy. Note: both D-ribose and alpha-lipoic acid can lower blood glucose; monitor closely in patients on corticosteroids or with glucose dysregulation (common during cancer treatment).
...
\begin{enumerate}
    \item \textbf{WASF3/ER Stress Pathway}: ME/CFS research identified ER stress inducing WASF3, which disrupts mitochondrial supercomplexes and impairs Complex IV~\cite{Wang2023WASF3}.
    ...
    \item \textbf{MitoQ (Mitochondria-Targeted Ubiquinone)}: 10--20 mg/day; delivers CoQ10 directly to mitochondria via triphenylphosphonium targeting, achieving greater mitochondrial accumulation than standard CoQ10 in preclinical studies. While developed outside ME/CFS research, its mechanism is particularly relevant to the mitochondrial dysfunction documented in ME/CFS (not reviewed in Chapter~\ref{ch:supplements}; limited ME/CFS-specific data; clinical dosing and comparative human pharmacokinetics not yet established).

    \textbf{Implication for Mitochondrial Disorders}: More effective CoQ10 delivery to dysfunctional mitochondria.

    \item \textbf{D-Ribose for ATP Pool Regeneration}: 5g TID showed improvements in energy, sleep, and well-being in two open-label studies without placebo control~\cite{Teitelbaum2006ribose,Teitelbaum2012ribose}; see Chapter~\ref{ch:supplements} for evidence limitations. D-ribose is the pentose sugar component of ATP.

    \textbf{Implication for Mitochondrial Disorders}: Accelerates adenine nucleotide pool resynthesis, providing ATP precursors that may be depleted in mitochondrial disorders. Does not bypass oxidative phosphorylation---cells still require functional mitochondria for ATP production.

    \item \textbf{Comprehensive Support Stack}: Combined approach addresses multiple dysfunction points simultaneously.

    \textbf{Implication for Mitochondrial Disorders}: ME/CFS protocols provide a mechanistically informed combination therapy framework with partial RCT support (see Chapter~\ref{ch:supplements}, Section~\ref{subsec:mito-combination-guide}).
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Comprehensive Autonomic-Metabolic Protocol}: Combining catecholamine support (tyrosine, BH4 cofactors, iron optimization) with mitochondrial protection (MitoQ, N-Acetylcysteine (NAC), alpha-lipoic acid).
...
\begin{enumerate}
    ...
    \textbf{ME/CFS Finding}: PET imaging showed widespread microglial activation correlating with cognitive symptoms~\cite{Nakatomi2014neuroinflammation}.
    ...
    \textbf{Established in Neurodegenerative Disease}: NAD$^+$ declines with aging~\cite{Massudi2012nad}; depletion impairs mitochondrial function, DNA repair (PARP), and sirtuin-mediated protein homeostasis~\cite{Dehhaghi2022AD}.

    \textbf{Translational Opportunity}: The only RCT-tested dose is NR 2000~mg/day (20-week intervention; cognitive benefit variable)~\cite{Wu2025NRLongCOVID}. Lower doses (300--1000~mg/day) are more commonly used in clinical practice but lack controlled evidence. See Chapter~\ref{ch:supplements} for full dosing.
    ...
    \textbf{ME/CFS Finding}: Oxidative stress markers elevated; peroxynitrite formation damaging cellular components~\cite{Syed2025MitoCFS}.

    \textbf{Established in Neurodegenerative Disease}: Oxidative damage to proteins, lipids, DNA accelerates neurodegeneration.

    \textbf{Translational Opportunity}: Comprehensive antioxidant protocol (MitoQ, alpha-lipoic acid, N-Acetylcysteine (NAC), vitamin E, selenium).
...
\begin{enumerate}
    ...
    \textbf{Translational Opportunity}: Anti-inflammatory approaches (omega-3, curcumin, Low-Dose Naltrexone (LDN)) could reduce inflammatory burden.
...
\begin{itemize}
    \item Comprehensive mitochondrial stack---includes CoQ10, NADH, NAD$^+$ precursors (NR), D-ribose, ALCAR, and B vitamins (see Section~\ref{sec:mito-support-universal} for full protocol and Chapter~\ref{ch:supplements} for dosing)
\end{itemize}
\revend

\textbf{Rationale}: These three systems are hypothesized to interact; addressing multiple targets simultaneously may provide greater benefit than isolated interventions, though this has not been tested in controlled comparisons.
...
\begin{itemize}
    ...
    \item N-Acetylcysteine (NAC): Precursor to glutathione, reduces oxidative stress
\end{itemize}
...
\textbf{Metabolomic abnormalities}: Tryptophan-NAD$^+$ pathway dysregulation~\cite{heng2025mecfs}

\textbf{PARP activation}: Oxidative stress and DNA damage trigger PARP, consuming NAD$^+$~\cite{Dehhaghi2022AD}

\textbf{Chronic inflammation}: Inflammatory cytokines induce cellular stress $\rightarrow$ PARP activation $\rightarrow$ NAD$^+$ consumption~\cite{Dehhaghi2022AD} (proposed mechanism; direct ME/CFS data limited)

\textbf{Clinical trial}: 2025 Long COVID RCT (n=58) showed NR 2000~mg/day increased NAD$^+$ levels 2.6--3.1$\times$; cognitive benefits were variable~\cite{Wu2025NRLongCOVID}
...
\begin{itemize}
    \item \textbf{Aging-related decline}: NAD$^+$ in human tissue declines significantly with age (approximately 50\% by age 50 in skin tissue measurements)~\cite{Massudi2012nad}
    \item \textbf{Neurodegenerative diseases}: Impaired mitochondrial function, protein homeostasis~\cite{Dehhaghi2022AD}
    \item \textbf{Metabolic syndrome}: Insulin resistance proposed to be linked to NAD$^+$ depletion (mechanistic evidence from animal models~\cite{Stromsdorfer2016NADinsulin})
    \item \textbf{Cancer-related fatigue}: Chemotherapy and radiation are proposed to deplete NAD$^+$ (via PARP activation and direct mitochondrial toxicity)~\cite{Dehhaghi2022AD}
    \item \textbf{Chronic inflammatory conditions}: PARP activation consumes NAD$^+$~\cite{Dehhaghi2022AD}
...
Mitochondrial dysfunction is documented in ME/CFS~\cite{Wang2023WASF3,Syed2025MitoCFS}; analogous findings have been reported in Long COVID~\cite{Wu2025NRLongCOVID}, fibromyalgia~\cite{Cordero2013}, and other chronic fatigue conditions~\cite{Parikh2009}. The comprehensive mitochondrial support stack addresses multiple points of failure:

\rev{5}
\begin{enumerate}
    \item \textbf{Electron transport chain support}: CoQ10 (shuttles electrons from Complex~I and Complex~II to Complex~III), NADH (electron donor to Complex~I)
    \item \textbf{ATP synthesis cofactors}: D-ribose (substrate), magnesium (ATPase cofactor)
    \item \textbf{Oxidative stress protection}: Alpha-lipoic acid (mitochondrial antioxidant; see Chapter~\ref{ch:supplements}). Optional additions: vitamin~E (membrane antioxidant) and phosphatidylcholine (membrane repair)---these are not reviewed individually in this text and lack ME/CFS-specific evidence; inclusion is based on theoretical rationale only
    \item \textbf{Fatty acid transport}: Acetyl-L-carnitine (mitochondrial fatty acid oxidation)
...
\textbf{Evidence base}:
\begin{itemize}
    \item ME/CFS: CoQ10 + NADH demonstrated significant improvements in cognitive fatigue, overall fatigue, and sleep parameters in the largest RCT (Castro-Marrero 2021, n=207)~\cite{CastroMarrero2021fatigue}; an earlier smaller trial (Castro-Marrero 2015, n=73) showed improvements in fatigue and biochemical parameters~\cite{CastroMarrero2015}
    \item Long COVID: NR 2000~mg/day increased NAD$^+$ levels 2.6--3.1 fold (Wu 2025, n=58)~\cite{Wu2025NRLongCOVID}
    \item Fibromyalgia: CoQ10 300~mg/day reduced pain and fatigue (Cordero 2013, n=20)~\cite{Cordero2013}
...
\begin{itemize}
    \item \textbf{Coenzyme Q10}: 100--300~mg/day typical (ubiquinol form preferred; RCT dose 200~mg; up to 400--600~mg in studies)
    ...
    \item \textbf{Acetyl-L-carnitine}: 500--2000~mg/day, divided doses (start low; see Chapter~\ref{ch:supplements})
    \item \textbf{Alpha-lipoic acid}: 300--600~mg/day (R-lipoic acid form preferred; start at 100--150~mg in ME/CFS and titrate---sensitive patients may experience paradoxical worsening (increased fatigue, malaise, or gastrointestinal symptoms) at higher initial doses; clinical observation)
    \item \textbf{Magnesium glycinate}: 200--600~mg/day elemental (divided doses to avoid diarrhea; see Chapter~\ref{ch:supplements})
...
\begin{itemize}
    \item \textbf{Nicotinamide riboside (NR)}: 300--1000~mg/day (morning dosing; the only RCT-tested dose is 2000~mg/day~\cite{Wu2025NRLongCOVID}; see Chapter~\ref{ch:supplements})
    \item \textbf{Pyrroloquinoline quinone (PQQ)}: 10--20~mg/day (mitochondrial biogenesis)
    \item \textbf{Creatine monohydrate}: 3--5~g/day (ATP buffering, cognitive support)
\end{itemize}
\revend
...
\rev{5}
\begin{enumerate}
    \item \textbf{Titration}: Start with 25--50\% of target doses, increase weekly (clinical experience; no controlled titration protocol data). Introduce supplements one at a time per Chapter~\ref{ch:supplements}, Section~\ref{sec:protocols}; apply this titration to each supplement individually as it is added
    ...
    \item \textbf{Minimum trial duration}: 8--12 weeks per individual supplement (mitochondrial adaptations require time). With sequential introduction (1--2 weeks between additions per Chapter~\ref{ch:supplements}), the full stack requires substantially longer
    \item \textbf{Responder identification}: Responder rates have not been established in controlled trials; clinical experience suggests some patients improve, but no reliable proportion can be cited. Non-responders may have different rate-limiting pathology
\end{enumerate}

\rev{5}
\textbf{Safety considerations}:
\begin{itemize}
    \item CoQ10: May reduce warfarin effectiveness---the mechanism may involve enhanced CYP450-mediated warfarin hydroxylation~\cite{Zhou2005CoQ10Warfarin}; a structural similarity to vitamin~K has also been proposed but is less well-documented. Monitor INR closely with physician guidance; see Chapter~\ref{ch:supplements}, Section~\ref{subsec:mito-combination-guide}
    \item Alpha-lipoic acid: Monitor glucose in diabetics (insulin-sensitizing effect)
    \item Carnitine: May theoretically lower seizure threshold in pre-existing epilepsy; consult physician (no controlled data; clinical caution). Also inhibits thyroid hormone nuclear uptake---patients on thyroid replacement should monitor thyroid function~\cite{Benvenga2001CarnitineHyperthyroidism}; see Chapter~\ref{ch:supplements}, Section~\ref{subsec:mito-combination-guide} for additional drug interactions
    \item Magnesium: Dose-dependent diarrhea; reduce dose or switch to magnesium threonate
    \item NR/NMN: Theoretical concern about NAD$^+$ promoting tumor growth---rapidly proliferating cancer cells may exploit elevated NAD$^+$ for energy and DNA repair~\cite{Palmer2021NADcancer}. Avoid in active cancer; consult oncologist before use in patients with cancer history
\end{itemize}
\revend
...
\begin{itemize}
    ...
    \item Cancer-related fatigue (post-treatment, not during active treatment; see NR/NMN safety note above)
...
\textbf{Advanced additions}:
\begin{itemize}
    \item \textbf{Sapropterin (Tetrahydrobiopterin, BH4)}: 5--10mg/kg/day (prescription; for documented BH4 deficiency)
...
Neuroinflammation contributes to ME/CFS, Long COVID, fibromyalgia, neurodegenerative diseases, and potentially autoimmune conditions~\cite{Tate2022neuroinflammation}. Reduction strategies target:
...
\begin{enumerate}
    ...
    \item \textbf{Catecholamine synthesis support in POTS}: L-tyrosine + Tetrahydrobiopterin (BH4) cofactors vs. placebo
