...
\rev{6}
\begin{limitation}[Brainstorm Chapter: Pattern Recognition Without Empirical Validation]
...
\rev{6}
\begin{prediction}[Epigenetic Mast Cell Markers in ME/CFS]
\label{pred:mast-memory}
...
\rev{6}
\begin{prediction}[Vagal Tone Correlates With Multi-System Symptoms]
\label{pred:vagus-hub}
...
\rev{6}
\begin{prediction}[Mitochondrial Throttling Reversal Triggers PEM]
\label{pred:protective-throttle}
...
\rev{6}
\begin{prediction}[Circadian Restoration Precedes Immune Normalization]
\label{pred:circadian-core}
...
\rev{6}
\begin{prediction}[Microclot Burden Correlates With Exercise Intolerance]
\label{pred:microclot}
...
\rev{6}
\begin{speculation}[ME/CFS as Stable Dysfunctional State]
\label{spec:bistable}
...
\rev{6}
\begin{limitation}[Bistable Model: Metaphor, Not Mechanism]
...
\rev{6}
\begin{limitation}[Combination Hypotheses: No Drug Interaction or Safety Data]
