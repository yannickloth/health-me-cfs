...
\vspace{0.3cm}
\footnotesize
\textbf{Notes:} $S$ calculated with $\alpha = 0.6$, $\beta = 0.5$, $\gamma = 0.4$. Values $>1.0$ indicate severe predicted dysfunction (additive model; theoretical maximum $= \alpha + \beta + \gamma = 1.5$). Certainty reflects confidence in classification based on literature review.
...
\begin{enumerate}
    ...
    \item \textbf{Sub-threshold edge: CNS $\rightarrow$ Autonomous Processes}: Hair, nails, wound healing have minimal CNS-dependency ($\delta_{CNS} \leq 0.2$); under the model parameters the resulting predicted dysfunction $S < 0.12$, which we treat as functionally negligible and below the dysfunction threshold (see Table~\ref{tab:process-classification})
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Subtype-specific targeting}: Match intervention to primary dysfunction compartment (see subtype classification, Speculation~\ref{hyp:subtypes})
\end{enumerate}
\end{keypoint}
...
\begin{itemize}
    ...
    \item Three mechanistic hypotheses (Hypothesis~\ref{hyp:energy-triage}: CNS energy triage; Hypothesis~\ref{hyp:bbb-vulnerability}: BBB vulnerability; Hypothesis~\ref{hyp:sickness-behavior}: sickness behavior persistence) and two speculations (Speculation~\ref{hyp:astrocyte-gate}: astrocyte energy gate; Speculation~\ref{hyp:torpor}: partial torpor trap) explaining CNS selective vulnerability
