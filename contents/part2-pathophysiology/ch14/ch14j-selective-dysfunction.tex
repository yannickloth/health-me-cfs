...
\textbf{Note on range:} $S(P)$ is unbounded above; its theoretical maximum is $\alpha + \beta + \gamma$, achieved when $\delta_{CNS} = \tilde{\rho} = 1$. Values exceeding $1.0$ are possible and indicate severe predicted dysfunction (see Table~\ref{tab:process-classification}).
\end{definition}
...
\begin{enumerate}
    ...
    \item \textbf{No edge: CNS $\rightarrow$ Autonomous Processes}: Hair, nails, wound healing operate independently (no CNS coordination required; see Hypothesis~\ref{hyp:selective-dysfunction} and Table~\ref{tab:process-classification})
\end{enumerate}
