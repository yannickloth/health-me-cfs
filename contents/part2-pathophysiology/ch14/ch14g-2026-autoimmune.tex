...
\rev{5}
\begin{limitation}[2026 Autoimmune Hypotheses: Extrapolation from Adjacent Diseases]
...
\begin{itemize}
    \item The EBV-B cell CNS infiltration hypothesis is based on MS research~\cite{Pless2026ebv_demyelination}; whether LMP1-expressing B cells cross the blood-brain barrier in ME/CFS patients has not been investigated.
    \item The receptor internalisation mechanism derives from NMDA receptor cryo-EM studies~\cite{Kim2026nmdar_cryoem}; whether GPCR autoantibodies in ME/CFS cause receptor internalisation (rather than blocking or agonism) is unknown.
    \item The ``antigenic hotspot vulnerability'' and ``molecular mimicry'' hypotheses generate testable predictions but are currently untested computational predictions, not empirical findings.
    \item The unified EBV-autoimmune cascade model integrates seven individually uncertain hypotheses into a single narrative; the integrated model inherits the uncertainty of all components and adds additional uncertainty from the assumed causal connections between them.
\end{itemize}
\end{limitation}
\revend
