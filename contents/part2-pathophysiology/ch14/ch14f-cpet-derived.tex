...
\begin{enumerate}
    ...
    \item ROS damages catecholamine synthetic enzymes and depletes Tetrahydrobiopterin (BH4) cofactor
...
\begin{enumerate}
    ...
    \item Tetrahydrobiopterin (BH4) levels should show exercise-dependent depletion with slow recovery kinetics
    \item Interventions supporting both catecholamine synthesis (Tetrahydrobiopterin (BH4), tyrosine, cofactors) and mitochondrial protection (antioxidants) should show synergistic effects exceeding either alone
...
\begin{itemize}
    ...
    \item Sapropterin (Tetrahydrobiopterin, BH4) or methylfolate + B12 (BH4 recycling pathway support)
...
\begin{itemize}
    ...
    \item N-Acetylcysteine (NAC) 1200--1800~mg/day (glutathione precursor, oxidative stress buffer)
    \item Alpha-lipoic acid 600~mg/day (mitochondrial antioxidant, Tetrahydrobiopterin (BH4) regeneration support)
...
\rev{5}
\begin{limitation}[CPET-Derived Hypotheses: Untested Mechanistic Inferences from Functional Data]
The two-day CPET protocol~\cite{keller2024cpet} provides robust evidence of \emph{functional} deterioration (reduced VO$_2$max, workload, and ventilatory threshold on Day~2). However, the mechanistic hypotheses built upon this finding have not been directly tested:
\begin{itemize}
    \item The ``autonomic-mitochondrial feedback loop,'' ``mitochondrial turnover rate limitation,'' and ``circadian recovery gating'' are interpretive models imposed on the 13-day recovery observation---the recovery duration alone cannot distinguish between these and simpler explanations (e.g., delayed inflammatory resolution, psychological anticipation).
    \item All supplement protocols proposed (autonomic-mitochondrial co-support, accelerated mitochondrial turnover, chronotherapy) are speculative combinations that have not been tested even in pilot studies; individual components have varying evidence levels, but the specific combinations and rationales are hypothetical.
    \item The ``metabolic preconditioning'' hypothesis directly contradicts established pacing principles and carries risk of harm; it is included as a theoretical exercise, not a recommendation.
    \item Quantitative model parameters (13-day recovery, mitochondrial half-life estimates) are drawn from healthy-population physiology and may not apply to ME/CFS patients with altered mitochondrial dynamics.
\end{itemize}
\end{limitation}
\revend
...
\begin{tabularx}{\textwidth}{p{3.2cm}p{1.2cm}p{1.5cm}p{4.5cm}X}
\toprule
\textbf{Hypothesis} & \textbf{Evidence} & \textbf{Therapeutic Potential} & \textbf{Key Prediction} & \textbf{Nearest-Term Test} \\
\midrule
