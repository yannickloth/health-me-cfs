...
Vascular-Immune-Energy Triad & Preliminary & Heng 2025 7-bio\-marker panel; Hoel 2026 serum pro\-teomics cross-vali\-dation~\cite{Hoel2026proteome}; coordinated abnor\-malities across 3 systems from two independent cohorts & Multi-system nature; why single-target treatments fail & Triple-target protocol; simul\-taneous inter\-vention & Unknown (untested) \\
\addlinespace

Endo\-thelial dys\-function / micro\-clotting (ch14) & Preliminary & Elevated VWF, fibro\-nectin, thrombo\-spondin; Hoel 2026: reduced VIM, ANTXR2, CXADR + disrupted Ephrin signaling~\cite{Hoel2026proteome}; Long COVID micro\-clot findings & Exercise intol\-erance; brain fog; multi-system involve\-ment & Anti\-coagu\-lation; fibrin\-olytics; endo\-thelial support; Ephrin-targeted therapy? & Moderate (if confirmed) \\
\addlinespace
...
Trained endotheliopathy & Speculative & Endothelial markers elevated (Heng 2025); Hoel 2026 impaired repair signature (reduced ANTXR2, VIM, Ephrin disruption)~\cite{Hoel2026proteome}; innate immune training established; vascular symptoms & Multi-system involvement; persistent endothelial activation; impaired vascular repair; microvascular dysfunction & Vascular-focused protocol; angio\-genesis support; epigenetic reversal? & Unknown \\
...
The Heng 2025 study~\cite{heng2025mecfs} suggests these are not independent---the 7-biomarker panel spanning all three systems achieved 91\% diagnostic accuracy. This correlation is consistent with coordinated dysfunction. If these systems are functionally coupled, this would have profound implications:

\begin{limitation}[Diagnostic Correlation $\neq$ Causal Interdependence]
...
\begin{itemize}
    ...
    \item The ResetME RCT ($n=66$, placebo-controlled) is now recruiting to test these findings (Appendix~\ref{app:research-registry}, Section~\ref{sec:registry-resetme})
\end{itemize}
...
\paragraph{Inflammation-Barrier Bidirectional Cycle.}
...
\begin{enumerate}
    \item \textbf{Baseline systemic inflammation}: ME/CFS patients exhibit elevated pro-inflammatory cytokines (IL-1$\beta$, IL-6, TNF-$\alpha$, IFN-$\gamma$)~\cite{Montoya2017} that correlate with disease severity.
    \item \textbf{Tight junction disruption}: These cytokines directly suppress expression of tight junction proteins (claudin-1, occludin, zonula occludens-1), disrupting the intestinal epithelial seal~\cite{GutPermeability2023}.
    \item \textbf{LPS translocation}: Permeability increases, allowing bacterial lipopolysaccharide (LPS) to cross the intestinal epithelium and enter systemic circulation~\cite{GutPermeability2023}.
    \item \textbf{TLR4 activation and cytokine amplification}: Translocated LPS activates toll-like receptor 4 (TLR4) on innate immune cells, triggering production of additional pro-inflammatory cytokines (IL-1$\beta$, IL-6, TNF-$\alpha$)~\cite{Junker2012}.
    \item \textbf{Barrier repair impairment}: The elevated cytokines that trigger barrier disruption simultaneously suppress anti-inflammatory mechanisms and stress the repair capacity of already compromised enterocytes. Additionally, low cortisol in severe patients (discussed in Chapter~\ref{ch:endocrine}) reduces expression of tight junction proteins~\cite{Papadopoulos2012HPA}.
    \item \textbf{Cycle amplification}: With each cycle, baseline permeability increases, baseline cytokines increase, and the barrier's repair capacity becomes increasingly exhausted. This self-sustaining loop may explain why disease severity tends to increase over time in some severe patients without intervention.
\end{enumerate}

\begin{hypothesis}[Breaking the Inflammation-Barrier Cycle in Severe Patients]
\label{hyp:multi-target-intervention}
Ceasing the perpetuation of this cycle may require simultaneous multi-target interventions rather than single-pathway strategies. Possible approaches include: (1) \textbf{Barrier restoration} via glutamine supplementation, collagen peptides, or other enterocyte-supportive nutrients; (2) \textbf{Wheat elimination} to remove zonulin-mediated barrier stress; (3) \textbf{Anti-inflammatory support} via omega-3 fatty acids, curcumin, or other dietary polyphenols; (4) \textbf{Mast cell stabilization} with quercetin or ketotifen to reduce histamine-mediated inflammatory mediator release; and (5) \textbf{Cortisol optimization} to support endogenous barrier repair. Addressing only one mechanism (e.g., wheat elimination without anti-inflammatory support or barrier repair substrates) may prove insufficient if the cycle has become deeply entrenched, requiring comprehensive multi-pathway interventions for clinical efficacy in severe patients.

\textbf{Certainty: 0.75} (mechanistic logic strong; direct evidence for this integrated multi-target approach lacking in ME/CFS).
\end{hypothesis}
...
\begin{keypoint}[OI as Mechanistic Lynchpin in Early ME/CFS]
\label{key:oi-lynchpin}

\textbf{Certainty: 0.55.} Orthostatic intolerance may function as an upstream mechanistic driver in early ME/CFS, triggering a cascade of secondary dysfunction across immune, metabolic, and neuroimmune systems. This model is grounded in pediatric clinical observations and splanchnic perfusion physiology, though high-quality trials testing the causal direction remain limited.

\paragraph{Clinical Foundation: Pediatric Treatment Response}
Pediatric ME/CFS specialists consistently report that aggressive orthostatic intolerance (OI) treatment produces improvements extending far beyond cardiovascular symptoms---including fatigue duration and severity, cognitive function, post-exertional malaise patterns, and general wellbeing~\cite{Rowe2017pediatric}. Rowe and colleagues document recovery or substantial improvement in 40--70\% of pediatric ME/CFS patients with aggressive tilt table-guided fludrocortisone, midodrine, and compression therapy, even in patients with severe initial disability. Critically, symptom improvements precede increases in exercise tolerance by weeks to months, suggesting causality rather than simple symptom clustering.

\paragraph{Proposed Mechanism: Splanchnic Perfusion as Integrative Hub}
...
\textit{Gut barrier failure and endotoxemia.} Splanchnic ischemia impairs intestinal epithelial function, increasing zonulin-mediated permeability and allowing lipopolysaccharide (LPS) translocation. Chronic LPS exposure triggers mast cell activation, systemic inflammation, and adaptive immune responses (Section~\ref{sec:zonulin-permeability}).

\textit{Metabolic dysregulation.} The small intestine normally produces 40\% of the body's glutamine, a critical fuel for rapidly dividing cells. Splanchnic hypoperfusion reduces intestinal glutamine synthesis, depleting substrates for immune cell proliferation and mitochondrial recovery. This may explain the selective energy dysfunction pattern in ME/CFS.

\textit{Neuroimmune amplification.} Chronic endotoxemia and sleep disruption (consequent to autonomic instability) activate neuroinflammation through microglia and astrocytes. Over weeks to months, epigenetic changes (histone modifications, DNA methylation) lock cells into pro-inflammatory phenotypes (Section~\ref{sec:trained-immunity}), and autoantibodies against autonomous nerve receptors establish, perpetuating dysfunction even if OI is later corrected.

\paragraph{Integration with Septad Framework}
Within the vicious cycle septad (Section~\ref{sec:septad}), OI occupies a distinctive position: it is simultaneously a primary symptom (orthostatic intolerance) and a mechanistic hub connecting infection/immune activation → autonomic dysfunction → splanchnic ischemia → gut barrier failure, endotoxemia, and secondary metabolic and immune dysfunction. Early OI treatment interrupts the cycle before epigenetic lock-in and autoimmune establishment, explaining the higher efficacy in recently-onset disease.

\paragraph{Testable Predictions}
This framework generates specific, falsifiable predictions:

\begin{enumerate}
    \item \textbf{Age-stratified efficacy.} Aggressive early OI treatment should produce higher remission rates in patients with disease onset $<$6 months versus $>$2 years, due to prevention of epigenetic reprogramming and autoantibody establishment.

    \item \textbf{Barrier marker response.} Patients responding to OI treatment should show declining serum zonulin, I-FABP, and LPS-binding antibodies; non-responders should show persistent elevation despite OI correction.

    \item \textbf{Symp tom sequence.} Orthostatic symptoms should improve first (weeks), followed by fatigue (4--12 weeks), then cognition and immune symptoms (8--16 weeks). Reversal of this sequence (cognition improving before orthostasis) would argue against OI as primary driver.
\end{enumerate}

\paragraph{Adult Implications and Caveats}
In adults with long-standing ME/CFS (>5 years), aggressive OI treatment shows lower response rates~\cite{Newton2007}, consistent with the hypothesis that secondary dysfunctions have become epigenetically entrenched. However, recent-onset adult ME/CFS ($<$1 year, high OI severity) may show responses comparable to pediatric patients, suggesting disease duration rather than age per se is the critical variable. This predicts that OI-focused treatment in recently-diagnosed adults with prominent autonomic features should receive high priority.

\paragraph{Clinical Rationale: Front-Loading OI Treatment}
This model provides mechanistic justification for front-loading orthostatic interventions (Section~\ref{subsubsec:front-loading-strategy}) early in disease course, even when cardiovascular symptoms seem modest. Delaying OI treatment in hope of spontaneous resolution risks epigenetic reprogramming and autoimmunity establishment, narrowing the window for cascade interruption.
...
\begin{hypothesis}[Neutrophil Functional Exhaustion Hypothesis]
\label{hyp:neutrophil-exhaustion}

Hoel et al.\ (2026)~\cite{Hoel2026proteome} documented a paradox: ME/CFS patients have normal peripheral neutrophil counts but profoundly suppressed circulating neutrophil-derived proteins---hallmark granule components (BPI, PADI4, MMP9, ELANE, AZU1, LTF) are all significantly reduced. We propose that ME/CFS neutrophils exist in a state of \textbf{functional exhaustion}: they are present in normal numbers but hypoactivated, failing to degranulate or mount appropriate antimicrobial responses.
...
\paragraph{Testable Predictions.}
\begin{enumerate}
    \item Ex vivo stimulation of ME/CFS neutrophils with PMA or fMLP should produce attenuated degranulation and ROS production compared to healthy controls, despite normal cell counts
    \item Neutrophil granule protein content (measured by intracellular flow cytometry) should be reduced in ME/CFS
    \item Recovery from ME/CFS (e.g., daratumumab responders) should coincide with normalization of circulating neutrophil granule proteins
    \item Interventions restoring neutrophil function (G-CSF priming, if tolerable) should improve innate immune defense markers
\end{enumerate}
...
\paragraph{Testable Predictions.}
\begin{enumerate}
    \item FGF-21 and GDF-15 levels should correlate with disease severity (particularly functional capacity and PEM threshold)
    \item Successful treatment (by any mechanism) should normalize exerkine levels as downstream consequence
    \item ME/CFS patients should show blunted \textit{additional} exerkine response to actual exercise (ceiling effect), unlike healthy controls who show large fold-increases
    \item Metabotype 2 patients (highest FGF-21 in Hoel 2026) should have the most severe metabolic symptoms and lowest exercise tolerance
\end{enumerate}

\paragraph{Treatment Implications.}
Rather than targeting exerkines directly (they may be compensatory), treatment should address the upstream energy deficit: mitochondrial support (CoQ10, NAD$^+$ precursors), tissue oxygenation improvement (addressing endothelial dysfunction), or substrate utilization enhancement. FGF-21 could serve as a treatment response biomarker---declining levels would indicate metabolic recovery even before symptom improvement becomes apparent.

\textbf{Certainty: 0.45.} FGF-21 elevation robustly validated (n=278, ELISA). The exerkine framework is well-established in exercise physiology. However, the ``stuck cycle'' mechanism is inferred from the chronic elevation pattern, and alternative explanations (hepatic stress, mitochondrial myopathy) contribute to uncertainty. Longitudinal studies tracking exerkines before and after treatment are needed.
\end{hypothesis}

\begin{speculation}[Secretome-Driven Vascular Autoimmune Loop]
\label{spec:secretome-autoimmune}

Hoel et al.\ (2026)~\cite{Hoel2026proteome} documented two seemingly contradictory patterns: (1) reduced intracellular protein release (especially from skeletal muscle and bone marrow) and (2) increased secreted protein release (coagulation factors, complement, chemokines, hormones). We speculate that this secretome shift---elevated extracellular proteins with altered post-translational modifications from the disease environment---may generate \textbf{neo-antigens} that drive autoimmune targeting of the vasculature.
...
\paragraph{Why This Is Speculative.}
...
\paragraph{Testable Predictions.}
\begin{enumerate}
    \item ME/CFS serum should contain proteins with higher rates of oxidative and citrullination modifications than healthy controls
    \item Autoantibodies targeting modified vascular proteins (citrullinated vimentin, oxidized LIPG) should be detectable in a subset of patients
    \item The autoimmune ME/CFS subset (daratumumab responders) should show the strongest secretome shift
\end{enumerate}

\textbf{Certainty: 0.25.} Speculative hypothesis connecting established findings (secretome shift, autoimmunity, vascular dysfunction) through an unproven mechanism. Requires dedicated proteomic studies of post-translational modifications in ME/CFS secreted proteins.
\end{speculation}

\begin{speculation}[Bone Marrow Stress and Immune Lineage Reprogramming]
\label{spec:bone-marrow-reprogramming}

The Hoel et al.\ (2026) tissue panel analysis~\cite{Hoel2026proteome} revealed that bone marrow showed the most dramatic protein reductions of any tissue type (25.4\% overall net downregulation), with widespread decreases in both intracellular and secreted proteins. Simultaneously, the immune cell analysis showed a shift from innate (neutrophil/granulocyte suppression) toward adaptive predominance (elevated NK and T cell markers). We speculate that ME/CFS involves a \textbf{bone marrow stress state} that reprograms hematopoietic stem cell lineage commitment---favoring lymphoid over myeloid differentiation---resulting in the observed immune imbalance.
...
\paragraph{Testable Predictions.}
\begin{enumerate}
    \item Bone marrow biopsy in ME/CFS patients should show altered myeloid:lymphoid progenitor ratios
    \item Hematopoietic stem cells from ME/CFS patients should show epigenetic marks consistent with inflammatory reprogramming
    \item The bone marrow protein signature should improve with effective immunomodulatory treatment
\end{enumerate}

\textbf{Certainty: 0.20.} Highly speculative---bone marrow has not been directly studied in ME/CFS. The tissue panel analysis is indirect (circulating proteins annotated to bone marrow origin). However, the pattern is striking and consistent with known inflammatory reprogramming of hematopoiesis.
\end{speculation}
...
Neutrophil functional exhaustion (\S\ref{hyp:neutrophil-exhaustion}) & Address upstream drivers (autoantibodies, chronic antigens); resolution mediators (resolvins); cautious G-CSF priming & Theoretical & Variable & Neutrophil functional assays needed first \\
\addlinespace

Chronic exerkine signaling (\S\ref{hyp:chronic-exerkine}) & Mitochondrial support; tissue oxygenation; FGF-21 as treatment response biomarker & Theoretical & High (biomarker monitoring) & ELISA-based; FGF-21 decline may predict clinical improvement \\
\addlinespace
...
\begin{keypoint}[Three-Field Convergence Against the Deconditioning Narrative]
\label{key:deconditioning-convergence}
...
\begin{enumerate}
    \item \textbf{Exercise physiology:} Charlton et al.\ (2026), writing in the \textit{British Journal of Sports Medicine}---a mainstream sports medicine journal---argued that post-exertional malaise cannot be explained by cardiac deconditioning and called for rethinking the pathophysiology of exercise intolerance in post-viral illness~\cite{Charlton2026pem}. This marks the exercise science community challenging its own prior paradigm.

    \item \textbf{Clinical guideline revision:} The UK National Institute for Health and Care Excellence withdrew graded exercise therapy from its ME/CFS guidelines in 2021, declaring it potentially harmful based on systematic evidence review~\cite{NICE2021mecfs}. This was a policy-level repudiation of the deconditioning model.

    \item \textbf{Comparative virology:} The Kol et al.\ (2026) demonstration that coronavirus persists in lymphocytes after antiviral treatment and clinical recovery~\cite{Kol2026fipv} provides mechanistic plausibility against the deconditioning narrative: if post-viral symptoms are maintained by a viral reservoir in immune cells, exercise cannot address the underlying pathology. Note that this is a feline coronavirus model; cross-species extrapolation to human ME/CFS requires caution.
\end{enumerate}

This convergence from independent fields---each applying different methodologies and answering to different epistemic communities---constitutes stronger evidence than any single line of inquiry alone. The deconditioning narrative persists in some clinical settings despite this convergence, raising questions about knowledge translation and the sociology of medical belief change (see Section~\ref{sec:deconditioning-hypothesis}).
\end{keypoint}
