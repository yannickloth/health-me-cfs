...
A 2023 study by Hwang et al., using muscle biopsies from the NIH intramural ME/CFS cohort, identified a specific molecular pathway linking cellular stress to mitochondrial dysfunction~\cite{hwang2023wasf3}.

\begin{achievement}[ER Stress--WASF3--Mitochondrial Dysfunction Pathway: Druggable Mechanism]
\label{ach:wasf3-mechanism}
A 2023 study by Hwang et al.~\cite{hwang2023wasf3}, using muscle biopsies from the NIH intramural ME/CFS cohort, identified a specific molecular pathway linking cellular stress to mitochondrial dysfunction. WASF3 (Wiskott-Aldrich syndrome protein family member 3) was significantly elevated in ME/CFS patient muscle biopsies compared to controls. This protein, when overexpressed, localizes to mitochondria and disrupts respiratory chain function.
...
\rev{5}
Che, Hornig, and Lipkin~\cite{CheHornig2025multiomics} (published in Cell Reports Medicine) synthesized energy metabolism, immune composition, and plasma proteomics data. Their findings showed simultaneous dysregulation at multiple levels:

\rev{5}
\begin{itemize}
    \item \textbf{Energy deficit markers}: Elevated AMP/ADP ratio, consistent with low cellular energy charge
    \item \textbf{Immune dysregulation}: Altered proportions of immune cell subsets, reduced naive T cells, expanded exhausted/memory populations
    \item \textbf{Inflammatory environment}: Elevated proinflammatory cytokines (IL-6, TNF-alpha, IFN-gamma)
    \item \textbf{Vascular dysfunction markers}: Elevated vascular injury (VEGFRs, E-selectin)
\end{itemize}

\rev{5}
These are not independent findings but interconnected components of a single disease process: immune activation drives energy depletion, which limits adaptive immune function, which maintains innate hyperactivation, which perpetuates energy failure. Breaking this cycle may require simultaneous intervention at multiple levels.
\revend
...
\revend
    ...
    \item[WASF3--ER stress--Complex IV pathway] Viral ER stress elevates WASF3, which translocates to mitochondria and disrupts respiratory chain supercomplex assembly, specifically impairing Complex IV~\cite{hwang2023wasf3} (Section~\ref{sec:mito-damage}).
