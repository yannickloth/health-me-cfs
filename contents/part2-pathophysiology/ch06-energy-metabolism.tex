...
Figures~\ref{fig:energy-cascade-normal} and~\ref{fig:energy-cascade-mecfs} illustrate normal ATP production and the multiple impairment points in ME/CFS. ATP deficit cascades into multi-system dysfunction affecting muscle, brain, immune, cardiovascular, and autonomic systems.
...
\paragraph{The Heng et al.\ 2025 Multi-Omics Study}
A landmark 2025 study by Heng et al.~\cite{heng2025mecfs}, published in \textit{Cell Reports Medicine}, applied multi-omics analysis to 61 ME/CFS patients matched with 61 healthy controls, revealing coordinated dysfunction across energy metabolism, immune function, and vascular systems. Key energy metabolism findings included:

\begin{itemize}
    \item \textbf{Elevated AMP and ADP}: White blood cells showed significantly higher levels of adenosine monophosphate (AMP) and adenosine diphosphate (ADP), with median AMP levels of 312.2~nM in ME/CFS versus 147.2~nM in controls
    \item \textbf{Reduced ATP/ADP ratio}: Consistent with decreased ATP generation and cellular energy stress
    \item \textbf{NAD$^+$ metabolism alterations}: Abnormal nicotinamide adenine dinucleotide metabolism affecting cellular energy production
\end{itemize}

The study identified a predictive model comprising seven biological variables that distinguished ME/CFS patients with 85.2\% sensitivity, 96.7\% specificity, and 91\% accuracy. These seven biomarkers span adenosine metabolism (AMP), immune functions (cDC1, LYVE1, IGHG2), and vascular factors (FN1, VWF, THBS1)---demonstrating that energy dysfunction in ME/CFS is not isolated but integrated with immune and vascular abnormalities. This multi-system integration suggests that \textbf{future treatments may need to target energy metabolism, immune maturation, and vascular health simultaneously}.
...
\paragraph{Key Discovery: Elevated WASF3}
WASF3 (Wiskott-Aldrich syndrome protein family member 3) was significantly elevated in ME/CFS patient muscle biopsies compared to controls. This protein, when overexpressed, localizes to mitochondria and disrupts respiratory chain function.

\paragraph{The ER Stress--Mitochondria Connection}
The study revealed a causal chain:

\begin{enumerate}
    \item \textbf{ER stress activation}: The endoplasmic reticulum unfolded protein response was aberrantly elevated in ME/CFS muscle
    \item \textbf{WASF3 induction}: ER stress drives increased WASF3 expression
    \item \textbf{Mitochondrial localization}: WASF3 translocates to mitochondria
    \item \textbf{Supercomplex disruption}: WASF3 interferes with respiratory chain supercomplex assembly, particularly affecting Complex IV (cytochrome c oxidase)
    \item \textbf{Functional impairment}: Decreased oxygen consumption and reduced exercise endurance
\end{enumerate}

\paragraph{Therapeutic Implications}
Critically, pharmacologic inhibition of ER stress in patient-derived cells improved mitochondrial function, suggesting this pathway represents a potentially druggable target. ER stress modulators or WASF3 inhibitors could restore normal mitochondrial respiration.

\paragraph{Integration with Other Findings}
The WASF3 mechanism provides a molecular explanation for several ME/CFS features:
\begin{itemize}
    \item \textbf{Post-infectious onset}: Viral infection can trigger ER stress through viral protein accumulation
    \item \textbf{Chronic persistence}: Once established, ER stress can become self-perpetuating
    \item \textbf{Exercise intolerance}: Complex IV impairment directly limits oxidative capacity
    \item \textbf{Reduced VO$_2$peak}: Observed in CPET studies including the NIH deep phenotyping study
\end{itemize}

This finding bridges the gap between cellular stress responses and the clinical manifestation of exercise intolerance, providing mechanistic support for the energy deficit model of ME/CFS.
...
\paragraph{Key Findings from the Keller et al.\ 2024 Study}
...
\textbf{Day 2 Performance Decrements in ME/CFS:}

\begin{itemize}
    \item \textbf{Peak oxygen consumption (VO$_2$peak)}: $-5.3\%$ decline (p < 0.01)
    \item \textbf{Work output}: $-5.5\%$ decline (p < 0.01)
    \item \textbf{Ventilation (V$_{\text{E}}$)}: $-7.8\%$ decline (p < 0.01)
    \item \textbf{Heart rate}: $-2.6\%$ decline (p < 0.05)
    \item \textbf{Oxygen pulse (O$_2$ pulse)}: $-4.0\%$ decline (p < 0.05)
    \item \textbf{Anaerobic threshold VO$_2$}: $-6.7\%$ decline (p < 0.05)
\end{itemize}

Control participants showed \textbf{no significant changes} in any parameter between Day 1 and Day 2.

\paragraph{Independence from Deconditioning}

Critically, when ME/CFS participants were matched with controls having identical baseline VO$_2$peak (aerobic capacity), the abnormal Day 2 responses persisted. This demonstrates that:

\begin{itemize}
    \item Impaired recovery is not attributable to fitness level
    \item The phenomenon represents a disease-specific pathophysiological process
    \item Improving fitness through training would not resolve the underlying deficit
\end{itemize}
