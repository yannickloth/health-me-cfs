...
Scherbakov et al.~\cite{Scherbakov2020endothelial} found endothelial dysfunction (ED) in 51\% of ME/CFS patients using peripheral arterial tonometry (EndoPAT). This early indicator of vascular disease correlates with systemic atherosclerosis risk and predicts cardiovascular events. Haffke et al.~\cite{Haffke2023fmd} measured flow-mediated dilation (FMD), the gold-standard non-invasive endothelial function test, and found severely impaired responses in ME/CFS patients (5.1\% vs 8.2\% in healthy controls, $p < 0.0001$). This indicates reduced nitric oxide (NO) bioavailability and impaired vasodilatory capacity.
...
Red blood cell function is critical for oxygen delivery: RBCs must deform to traverse narrow capillaries and must maintain membrane fluidity to exchange gases efficiently. Saha et al.~\cite{Saha2019rbc} studied ME/CFS patient RBCs using ektacytometry and rheological analysis, finding:
...
When Na$^+$/K$^+$-ATPase fails to maintain the sodium gradient, the sodium-calcium exchanger (NCX), which normally exports calcium in exchange for importing sodium, reverses direction. Instead of exporting calcium, it imports calcium driven by the sodium gradient. Calcium accumulates in the cytoplasm and mitochondria, causing mitochondrial damage, further reducing ATP production, amplifying the original problem.
...
\begin{itemize}
    \item \textbf{Energy deficit markers}: Elevated AMP/ADP ratio (low energy charge)
...
\begin{enumerate}
    ...
    \item \textbf{Proton accumulation}: Intracellular acidosis activates sodium-hydrogen exchangers (NHE1), importing sodium to extrude protons.
