...
Cullen et al.~\cite{Cullen2026ATP} found that cells from young adults with major depression produce paradoxically elevated ATP at rest but show markedly reduced capacity to upscale energy output under metabolic stress---a pattern they term \textit{stress-responsive mitochondrial failure}. This phenotype is conceptually similar to the post-exertional energy collapse in ME/CFS: near-normal baseline function until an energy demand threshold is exceeded, whereupon output fails catastrophically. Whether ME/CFS mitochondria share this specific regulatory defect (rather than, or in addition to, absolute ATP deficiency) remains to be directly tested in ME/CFS populations.
...
\begin{speculation}[NETosis as Metabolic Sink Contributing to Energy Depletion]
\label{spec:netosis-metabolic-sink}
...
mitochondria already show stress-responsive failure~\cite{Cullen2026ATP}, ongoing
NETosis~\cite{Krinsky2023} may act as a metabolic drain that competes with normal
...
\subsubsection{Homocysteine and Methylation Cycle}
...
\label{sec:sodium-calcium-cascade}

Wirth and Scheibenbogen~\cite{WirthScheibenbogen2020} integrated emerging evidence into a model of ion homeostasis failure in ME/CFS. The cascade begins with hypoperfusion (Section~\ref{sec:microcirculation-oxygen}), which forces cells into anaerobic metabolism. Anaerobic metabolism generates protons as a byproduct. Accumulating protons increase the activity of the sodium-hydrogen exchanger NHE1 (SLC9A1), which extrudes protons at a 1:1 stoichiometry with sodium import. Under normal aerobic conditions, the Na$^+$/K$^+$-ATPase (an ATP-consuming pump) can clear this sodium, but under hypoperfusion and ATP limitation this pump becomes rate-limited and eventually substrate-starved.

When the Na$^+$/K$^+$-ATPase fails to maintain the inward sodium gradient, the plasma membrane sodium-calcium exchanger NCX1 (SLC8A1) --- which operates electrogenically in a 3~Na$^+$:1~Ca$^{2+}$ exchange --- reverses direction. In forward mode NCX1 imports three sodium ions to export one calcium ion; in reverse mode (driven by elevated intracellular Na$^+$) it imports calcium at the expense of three sodium ions per cycle. The thermodynamic reversal point is governed by the NCX reversal potential:

\begin{equation}
E_\text{NCX} = 3\,E_\text{Na} - 2\,E_\text{Ca}
\label{eq:ncx-reversal}
\end{equation}

where $E_\text{Na}$ and $E_\text{Ca}$ are the Nernst potentials for Na$^+$ and Ca$^{2+}$ respectively. As intracellular Na$^+$ rises, $E_\text{Na}$ becomes less negative (or positive), shifting $E_\text{NCX}$ and driving the exchanger into reverse-mode calcium import. Within mitochondria, the distinct mitochondrial sodium-calcium exchanger NCLX (SLC8B1) --- the primary route of mitochondrial calcium efflux --- is simultaneously impaired by elevated cytoplasmic sodium, since it operates by importing sodium to export calcium. Sodium overload therefore prevents NCLX from clearing mitochondrial calcium. The net result: cytoplasmic and mitochondrial calcium accumulate simultaneously via two independent mechanisms (NCX1 reverse-mode influx; NCLX efflux block), causing direct mitochondrial damage, further reducing ATP production and amplifying the original sodium overload.

Petter et al.~\cite{Petter2022sodium} provided the first direct \textit{in vivo} quantification of this cascade using $^{23}$Na-MRI in ME/CFS patients ($n=6$ per group). Baseline intracellular sodium was elevated across all five lower-leg muscle compartments (12.20~mM vs 9.38~mM in anterior extensors; $p=0.0034$). Post-exercise sodium accumulation was greater in ME/CFS patients, and intracellular sodium correlated inversely with hand-grip strength ($r$; $p=0.0319$) --- a quantitative link between the ionic signature and functional impairment. Wirth et al.~\cite{WirthScheibenbogen2021muscle} extended these findings with electron microscopy showing subsarcolemmal mitochondrial damage preferentially localised at sites of calcium influx.

Scheibenbogen and Wirth~\cite{Scheibenbogen2025muscle} synthesised accumulated histological, MRI, and proteomic data into the concept of \textbf{acquired ischemic mitochondrial myopathy} (AIMM): a disease phenotype in which post-infectious vascular dysfunction produces chronic muscle ischemia, driving the NHE1$\to$Na$^+$ overload$\to$NCX1 reversal$\to$NCLX block$\to$calcium overload$\to$mitochondrial damage cycle as a self-reinforcing acquired pathology rather than a primary genetic mitochondrial disease. Proteomics confirmed reduced expression of Na$^+$/K$^+$-ATPase subunits (weakened sodium clearance) and upregulated ENaC expression (sodium-sensing retention), indicating the dysfunction is not transient but involves sustained pathological remodelling at the protein level. The AIMM concept is directly relevant to pharmacological targeting: unlike primary mitochondrial diseases requiring genetic correction, an acquired ionic myopathy is potentially reversible if the sodium/calcium overload is pharmacologically corrected (see Section~\ref{sec:mdc002} in Chapter~\ref{ch:emerging-therapies}).
