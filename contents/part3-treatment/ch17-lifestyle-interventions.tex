...
However, the \textbf{synergistic amplification of PEM by wheat+exercise} has not been directly tested in ME/CFS. This mechanistic model is inferred from separate pathways and theoretical integration. Overall model certainty: \textbf{0.40 (medium-low)}.
...
\textbf{Implication}: Many patients attributed wheat sensitivity is actually fructan sensitivity or mixed FODMAP intolerance. This complicates claims about gluten-specific mechanisms and requires careful dietary control in elimination protocols (see \emph{Clinical Implementation} section below).
...
\begin{itemize}
    \item Wheat elimination (4--6 weeks) should reduce baseline PEM severity by 20--50\% in affected patients
    \item Post-elimination wheat reintroduction should trigger PEM exacerbation within 24--72 hours in responders
    \item Exercise tolerance (steps, activity duration before PEM) should improve 10--30\% in responders during elimination
...
\begin{itemize}
    ...
    \item High individual variability in wheat sensitivity (affects ~15\% of ME/CFS cohorts per Uhde et al.\ 2018~\cite{Uhde2018})
...
\begin{itemize}
    ...
    \item \textbf{Phase 3 (Week 9+): Decision} --- If improved during elimination + worsened on rechallenge, continue wheat-free diet. If no change, reintroduce wheat. If ambiguous, consider pure gluten challenge (fructan-free) to isolate mechanism.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Bread}: Rice bread, certified gluten-free bread (avoid high-fiber versions if GI intolerance), sourdough (reduced fermentation)
