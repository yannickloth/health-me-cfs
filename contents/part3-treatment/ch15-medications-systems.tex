...
\begin{itemize}
    \item Increases whole-body resting energy expenditure by approximately 7\%---the body burns \textit{more} energy at baseline while on the drug~\cite{Leddy2012methylphenidateREE}
...
Evidence is very limited. One small lisdexamfetamine trial (n=26, 2013) showed positive results~\cite{Young2013lisdexamfetamine}.
...
\begin{itemize}
    ...
    \item \textbf{Oxaloacetate}: 2,000~mg/day reduced fatigue 25\% versus placebo in the RESTORE~ME trial (n=76); mechanism involves AMPK activation and NAD$^+$/NADH ratio improvement~\cite{An2024oxaloacetate}. One of the few interventions that may genuinely improve energy production
...
\section{Antihistamine Up-Dosing in MCAS}
\label{sec:antihistamine-updosing}

Standard allergy doses of antihistamines are often insufficient for mast cell activation syndrome (MCAS). The EAACI/GA\textsuperscript{2}LEN/EuroGuiDerm/APAAACI international urticaria guideline recommends \textbf{up to fourfold doses} of second-generation H1 antihistamines as Step~2 treatment when standard doses fail after 2--4 weeks~\cite{Zuberbier2022urticaria}. MCAS specialists (Afrin, Molderings) have adopted and extended this principle for MCAS management~\cite{Afrin2013MCAS,Molderings2016pharmacological}.

\begin{warning}[Guideline Origin and Extrapolation]
The fourfold up-dosing recommendation originates from \textbf{chronic spontaneous urticaria (CSU)} guidelines, not MCAS-specific trials. A systematic review found only five small, mostly historical trials of H1 antihistamines in primary mast cell activation syndromes~\cite{Nurmatov2015H1systematic}. The application to MCAS is based on shared pathophysiology (histamine-mediated symptoms from mast cell degranulation) and expert clinical practice, not direct randomized evidence in MCAS populations.
\end{warning}

\subsection{Safety of Higher-Than-Standard Doses}

A study of 59 patients receiving \textbf{above-fourfold doses} (median 8$\times$, range 5--12$\times$ standard) found that only 10\% reported side effects, predominantly somnolence, with \textbf{no serious adverse events}~\cite{VandenElzen2017updosing}. A systematic review of up-dosing across multiple second-generation antihistamines confirmed that higher doses increase drowsiness (9.5--59\% of patients) but produce no dose-dependent systemic complications~\cite{Podder2023updosing}.

\subsection{H1 Antihistamine Dosing: Standard vs.\ MCAS}
\label{sec:h1-updosing}

\begin{table}[htbp]
\centering
\caption{H1 antihistamine dosing: standard allergy vs.\ MCAS therapeutic range}
\label{tab:h1-updosing}
\begin{tabular}{lccccc}
\toprule
\textbf{Drug} & \textbf{Standard} & \textbf{MCAS initial} & \textbf{MCAS max (4$\times$)} & \textbf{Evidence} & \textbf{Responder rate} \\
\midrule
Cetirizine & 10 mg/day & 10 mg BID & 40 mg/day & Grade A\textsuperscript{a} & 54\%\textsuperscript{b} \\
Fexofenadine & 180 mg/day & 180 mg BID & 720 mg/day & Grade A\textsuperscript{a} & 83\%\textsuperscript{b} \\
Loratadine & 10 mg/day & 10 mg 2--3$\times$/day & 40 mg/day & Limited\textsuperscript{c} & --- \\
Rupatadine & 10 mg/day & 20 mg/day & 40 mg/day & Cold urticaria\textsuperscript{d} & --- \\
\bottomrule
\end{tabular}

\vspace{0.5em}
{\footnotesize
\textsuperscript{a}~Grade A recommendation from CSU up-dosing review~\cite{Podder2023updosing}. \textsuperscript{b}~Responder rate among patients uncontrolled at standard dose~\cite{Podder2023updosing}. \textsuperscript{c}~Fourfold guideline applies class-wide, but direct loratadine trial data above 20~mg/day are lacking; desloratadine at 4$\times$ showed 30\% response (Grade~B)~\cite{Podder2023updosing}. \textsuperscript{d}~Studied in chronic cold urticaria at 20~mg and 40~mg; somnolence universal at 40~mg~\cite{Magerl2015rupatadine}.
}
\end{table}

\paragraph{Drug-specific notes.}

\textbf{Cetirizine}: MCAS dosing per Afrin: 10~mg every 12 hours, escalating to 20~mg BID if needed~\cite{Afrin2013MCAS}. Maximum studied in clinical trials: 20~mg/day (matching the 2$\times$ dose used in the severe protocol in Chapter~\ref{ch:urgent-action-severe}).

\textbf{Fexofenadine}: Highest responder rate among all second-generation antihistamines at up-dosing (83\%)~\cite{Podder2023updosing}. Maintains cardiac safety at 4$\times$ doses; does not cause tachyphylaxis. MCAS dosing per Afrin: 180~mg every 12 hours~\cite{Afrin2013MCAS}. Stepwise escalation: 240$\rightarrow$360$\rightarrow$540$\rightarrow$720~mg/day at 1--2 week intervals. In long~COVID, fexofenadine 180~mg + famotidine 40~mg daily for 20 days resolved fatigue in 43\% and brain fog in 43\% of patients~\cite{Salvucci2023antihistamineLongCOVID}.

\textbf{Loratadine}: The fourfold recommendation applies class-wide, but direct clinical trial data for loratadine above 20~mg/day are limited. Its active metabolite desloratadine at 4$\times$ showed only 30\% response in CSU~\cite{Podder2023updosing}. MCAS dosing per Afrin: 10~mg two to three times daily~\cite{Afrin2013MCAS}.

\textbf{Rupatadine}: Unique dual action (H1 antagonist + PAF antagonist) makes it particularly relevant for MCAS, where mast cells release both histamine and PAF (see Section~\ref{sec:h1-updosing-rupatadine} below). Studied at 20~mg and 40~mg in cold urticaria~\cite{Magerl2015rupatadine}; dose-dependent somnolence at 40~mg. In mastocytosis, 20~mg produced significant reductions in Darier's sign, flushing, tachycardia, and headache~\cite{Izquierdo2024rupatadine}. No formal CSU up-dosing recommendation exists specifically for rupatadine.

\subsubsection{Rupatadine: Special Considerations for MCAS}
\label{sec:h1-updosing-rupatadine}

Rupatadine occupies a unique position among H1 antihistamines due to its triple mechanism: H1 antagonism, PAF antagonism (31$\times$ more potent than loratadine), and direct mast cell stabilization (inhibits IL-8 release by 80\%, VEGF by 73\%, histamine by 88\%)~\cite{Pinero-Gonzalez2024}. This multi-target profile means that rupatadine at standard doses may already provide mast cell control that other H1 antihistamines only achieve at higher doses.

\begin{table}[htbp]
\centering
\caption{Rupatadine dose escalation protocol for MCAS}
\label{tab:rupatadine-escalation}
\begin{tabular}{lll}
\toprule
\textbf{Step} & \textbf{Dose} & \textbf{Context} \\
\midrule
...
\subsection{H2 Antihistamine Dosing: Standard vs.\ MCAS}
\label{sec:h2-updosing}

\begin{warning}[No Formal H2 Up-Dosing Guideline]
Unlike H1 antihistamines, there is \textbf{no equivalent formal guideline} recommending H2 up-dosing for MCAS. Higher H2 doses are based on MCAS specialist clinical practice~\cite{Afrin2013MCAS,Molderings2016pharmacological} and extrapolation from FDA-approved doses for pathological hypersecretory conditions (e.g., Zollinger-Ellison syndrome). The H1+H2 combination is standard MCAS practice, but H2 dose escalation is expert opinion, not guideline-driven.
\end{warning}

\begin{table}[htbp]
\centering
\caption{H2 antihistamine dosing: standard vs.\ MCAS therapeutic range}
\label{tab:h2-updosing}
\begin{tabular}{lcccc}
\toprule
\textbf{Drug} & \textbf{Standard (GI)} & \textbf{MCAS initial} & \textbf{MCAS max} & \textbf{Notes} \\
\midrule
Famotidine & 20 mg/day & 20 mg BID & 40 mg BID\textsuperscript{a} & Preferred: minimal CYP interaction \\
Cimetidine & 400 mg BID & 400 mg BID & 800 mg BID\textsuperscript{b} & CYP450 inhibitor; immunomodulatory \\
\bottomrule
\end{tabular}

\vspace{0.5em}
{\footnotesize
\textsuperscript{a}~Afrin protocol: 20--40~mg every 12 hours, up to 80~mg every 12 hours in severe cases~\cite{Afrin2013MCAS}. \textsuperscript{b}~FDA-approved for pathological hypersecretory conditions at 800~mg BID or 400~mg QID (1600~mg/day total). See Section~\ref{sec:h2-comparison} for cimetidine vs.\ famotidine trade-offs.
}
\end{table}

\paragraph{Famotidine dose escalation.} Start 20~mg once daily; increase to 20~mg BID after 1--2 weeks; if insufficient, increase to 40~mg BID~\cite{Afrin2013MCAS}. Drying effects may increase at higher doses; titrate slowly. In long~COVID, famotidine 40~mg daily combined with fexofenadine 180~mg reduced tachycardia in 57\% of patients~\cite{Salvucci2023antihistamineLongCOVID}.

\paragraph{Cimetidine dose escalation.} Standard MCAS dose is 400~mg BID (800~mg/day). Can escalate to 800~mg BID for refractory symptoms. Cimetidine retains unique immunomodulatory properties (T-cell enhancement, NK cell activation) not shared by famotidine~\cite{Goldstein1986CimetidineEBV}, making it the preferred H2 blocker when immunomodulation is desired (see Section~\ref{sec:cimetidine-antiviral-synergy}). However, strong CYP450 inhibition (1A2, 2D6, 3A4) requires careful drug interaction review before dose escalation.

\subsection{Practical Up-Dosing Protocol for ME/CFS Patients}
\label{sec:updosing-protocol}

\begin{protocol}[MCAS Antihistamine Up-Dosing in ME/CFS]
\label{prot:mcas-updosing}

\textbf{Prerequisite}: Clinical suspicion of MCAS (see Section~\ref{sec:mcas-mild-moderate}) and inadequate response to standard-dose H1+H2 combination after 4 weeks.

\textbf{Step 1 (Weeks 1--4)}: Standard doses
\begin{itemize}
    \item H1: One second-generation antihistamine at standard dose (e.g., rupatadine 10~mg, cetirizine 10~mg, or fexofenadine 180~mg once daily)
    \item H2: Famotidine 20~mg once daily
\end{itemize}

\textbf{Step 2 (Weeks 5--8)}: Double frequency
\begin{itemize}
    \item H1: Increase to BID dosing (e.g., cetirizine 10~mg BID, fexofenadine 180~mg BID, rupatadine 20~mg/day)
    \item H2: Increase to famotidine 20~mg BID
    \item Monitor: Drowsiness (main dose-limiting side effect), mood changes
\end{itemize}

\textbf{Step 3 (Weeks 9--12)}: Up-dose if needed
\begin{itemize}
    \item H1: Increase toward 4$\times$ standard (e.g., cetirizine 20~mg BID, fexofenadine 360~mg BID)
    \item H2: Increase to famotidine 40~mg BID
    \item Escalate one agent at a time to identify which provides additional benefit
\end{itemize}

\textbf{Step 4}: Add-on therapies if still insufficient
\begin{itemize}
    \item Mast cell stabilizer (ketotifen, cromolyn)
    \item Leukotriene inhibitor (montelukast)
    \item Omalizumab (per urticaria Step 3 guideline~\cite{Zuberbier2022urticaria})
\end{itemize}

\textbf{ME/CFS-specific caution}: Introduce dose changes more slowly than in general MCAS populations. ME/CFS patients with medication sensitivity phenotypes (Section~\ref{sec:med-sensitivity}) may not tolerate rapid escalation. Consider 2-week intervals between dose steps rather than 1-week intervals.
