...
\begin{enumerate}
    ...
    \item \textbf{Window of opportunity}: Earlier intervention may prevent immune exhaustion phase (Achievement~\ref{ach:cytokine-duration}).
    \item \textbf{Quality of life}: Even mild ME/CFS significantly impairs function and well-being; deserves treatment.
    \item \textbf{Biomarker evidence}: Cytokine dysregulation and immune cell abnormalities are detectable even in mild ME/CFS~\cite{Hornig2015,Giloteaux2023}.
\end{enumerate}
\revend
...
\rev{5}
\begin{limitation}[Severity-Stratified Pathways: Expert-Derived, Not Trial-Validated]
...
\begin{itemize}
    \item Functional capacity percentage thresholds (75\%, 50\%, 25\%) are pragmatic divisions, not empirically validated cut-points; individual patients may not fit neatly into these categories and may shift between them week to week.
    \item The intervention sequencing (pacing first, then OI treatment, then mitochondrial support, etc.) reflects clinical logic but has not been compared against alternative orderings in any study.
    \item The ``window of opportunity'' argument---that early aggressive intervention in the first 1--2 years may prevent progression---is inferred from observational data on illness duration and prognosis; no interventional study has tested whether early treatment changes long-term trajectory.
    \item Supplement protocols (CoQ10, L-carnitine, D-ribose) are recommended at standard doses for mild patients, but dose-response relationships and optimal durations have not been established in ME/CFS-specific trials.
\end{itemize}
\end{limitation}
\revend
...
\textbf{Rationale:} Not all mild-moderate ME/CFS patients need identical treatment sequences. The selective energy dysfunction hypothesis (Section~\ref{sec:selective-dysfunction}) proposes four subtypes with different treatment priorities.
...
\textbf{Evidence level}: Plausible (subtype framework from Section~\ref{sec:selective-dysfunction}); requires validation
...
The selective energy dysfunction hypothesis (Section~\ref{sec:selective-dysfunction}) proposes that the CNS implements a hardwired energy allocation hierarchy under scarcity, with complex cognition (Tier 6) sacrificed first, while sensory and motor functions (Tier 2--3) are preserved longer.
...
\textbf{Mechanism:} Schedule cognitively demanding tasks (Tier 5--6) during peak energy only; shift to simpler tasks (Tier 2--3) when fatigued. This preserves cognitive function for priorities while allowing continued engagement with less demanding activities. See Section~\ref{sec:selective-dysfunction} for the CNS energy triage hypothesis.
...
\textbf{Evidence level}: Plausible (formal triage model from Section~\ref{sec:selective-dysfunction}; clinical validation pending)
...
\rev{5}
\paragraph{Informal Patient Observation: ``Crash Limit Rule.''}
Patient communities have reported an informal pattern suggesting cumulative worsening with repeated severe crashes, sometimes called the ``crash limit rule.'' No systematic study has quantified crash thresholds, and the specific numbers circulating in patient communities (e.g., 5 severe crashes, 2-week to 6-month recovery progression) have no empirical basis---they should not be treated as clinical cutoffs. The \emph{qualitative} pattern of progressively impaired recovery with repeated severe crashes is, however, consistent with the mitochondrial and epigenetic mechanisms described below.
\revend
...
\begin{enumerate}
    ...
    \item \textbf{Emergency metabolic support}: D-ribose 15~g, MCT oil 30~mL, NAD$^+$ precursor 1000--2000~mg, high-dose antioxidants (see Emergency PEM Protocol, Chapter~\ref{ch:emerging-therapies}, \S\ref{subsubsec:emergency-pem-protocol}). \textit{Note: these supplements are mechanistically motivated but lack controlled trial evidence for acute PEM management; efficacy is based on patient reports and biological plausibility}
...
\rev{5}
\begin{open_question}[Sports-Adapted Pacing: Unresolved Evidence Gaps]
\label{q:sports-pacing-gaps}
...
The blood-brain barrier (BBB) may limit delivery of compounds needed for cognitive support in ME/CFS (Hypothesis~\ref{hyp:bbb-vulnerability}). Intranasal delivery bypasses the BBB via olfactory and trigeminal nerve pathways, achieving 2--10 fold higher CSF concentrations than oral routes.
\end{keypoint}
\revend
...
\begin{itemize}
    ...
    \item \textbf{Future compounds}: As understanding of the astrocyte energy gate hypothesis (Speculation~\ref{spec:astrocyte-gate}) improves, intranasal delivery of lactate, ketone bodies, or neuroprotective compounds may emerge as targeted interventions.
\end{itemize}
...
\rev{5}
\begin{speculation}[tDCS Energy Cost Reduction via DLPFC Modulation]
\label{spec:tdcs-dlpfc-energy}
\textbf{Certainty: 0.25.} Anodal tDCS targeting the DLPFC modulates cortical excitability and has demonstrated improvements in working memory, attention, and executive function in multiple studies~\cite{Li2022tDCS}. Applied to ME/CFS, this neural efficiency gain may reduce the energy cost of Tier 5 cognitive tasks (Section~\ref{sec:selective-dysfunction}), thereby improving sustainable cognitive performance within the patient's energy envelope. No ME/CFS-specific trials exist; the application to energy triage theory is speculative extrapolation.
\end{speculation}
\revend
...
The selective energy dysfunction hypothesis (Hypothesis~\ref{hyp:circadian-failure}) proposes that SCN dysfunction impairs circadian allocation of energy budgets, explaining why many patients experience energy crashes mid-afternoon but a late-evening ``second wind.''
\end{keypoint}
\revend
...
\textbf{Mechanism:} Sleep spindles (brief high-frequency brain activity during NREM sleep) are reduced in ME/CFS. Acoustic stimulation may enhance spindle production, potentially improving sleep restorativeness (Hypothesis~\ref{hyp:sleep-architecture}).
...
\textbf{Mechanism:} Compression garments reduce autonomic coordination load by maintaining peripheral venous pressure, reducing baroreceptor-mediated sympathetic activation required for orthostatic compensation (Speculation~\ref{spec:sfn-interface}: SFN interface failure).
...
\textbf{Mechanism:} Mast cell activation episodes amplify fatigue and cognitive crashes through inflammatory mediators (Hypothesis~\ref{hyp:mcas-amplifier}). Proactive medication intensification 1--2 days before high-demand activities can reduce crash severity.
...
\rev{5}
\begin{assumption}[Septad Treatment Sequencing: MCAS First]
\label{ass:septad-sequencing}
...
Mild-moderate patients have a critical advantage: potential to intervene before immune exhaustion phase (Achievement~\ref{ach:cytokine-duration}). This provides opportunity for disease modification rather than pure symptom management.

\subsubsection{The Front-Loading Strategy}
\label{subsubsec:front-loading-strategy}

The striking difference in recovery rates between pediatric and adult ME/CFS patients (54--94\%~\cite{Joyce1997prognosis} versus $\leq$22\%~\cite{Cairns2005prognosis}) suggests that biological plasticity plays a critical role in determining outcomes. While some of this advantage may be inherent to developing biology, pediatric care patterns offer a potentially actionable insight: children are typically diagnosed earlier and treated more aggressively from the outset.

\begin{hypothesis}[Front-Loading Treatment Intensity]
\label{hyp:front-loading-intensity}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Rationale: OI may be an upstream driver (see Hypothesis~\ref{hyp:front-loading-intensity}); early correction may prevent downstream system involvement, though this causal relationship has not been established
    \end{itemize}
...
\begin{enumerate}
    \item \textbf{Critical window phenomenon}: Diagnostic delays beyond 23 months correlate with substantially worse outcomes~\cite{CastroMarrero2022prognosis}, suggesting a therapeutic window in the first 2 years, with the first 6 months potentially most critical.
...
\rev{5}
\textit{Note: The following outcome estimates are author-generated projections extrapolated from general ME/CFS recovery literature and diagnostic delay studies. They have not been validated for this specific protocol.}
\revend
...
\rev{5}
\begin{speculation}[D-Ribose Accelerates ATP Recovery]
\label{spec:d-ribose-atp}
...
\rev{5}
\begin{speculation}[Citrulline-Malate Addresses TCA/Urea Cycle Dysfunction]
\label{spec:citrulline-malate-tca}
...
\subsubsection{Sequential Gut-Vagal Repair Protocol}
\label{subsubsec:gut-vagal-protocol}

\rev{5}
\begin{speculation}[Sequential Gut-Vagal Repair Protocol]
\label{spec:gut-vagal-protocol}
\textbf{Certainty: 0.25.} This protocol is derived from the mechanistic model of Wirth and Scheibenbogen~\cite{WirthScheibenbogen2025Neurotransmitter} and the documented butyrate-producer deficiency in ME/CFS~\cite{ButyrateDeficiency2023}. It addresses the dysbiosis $\to$ butyrate deficiency $\to$ impaired enterochromaffin serotonin $\to$ vagal dysfunction chain in logical sequence. The 5-phase structure is a clinical framework based on mechanistic plausibility, not a validated trial protocol. HRV (heart rate variability) serves as the primary objective monitoring endpoint throughout.
\revend

\textbf{Phase 1: Eradication (Weeks 1--4)}
\begin{itemize}
    \item Test for SIBO (hydrogen/methane breath test) and treat if present: rifaximin, elemental diet, or herbal antimicrobials
    \item Test for fungal overgrowth and treat if documented (see antifungal approach above)
    \item Goal: Clear pathogenic overgrowth before introducing fermentable substrates
    \item Prerequisite: RS2 resistant starch and probiotics are contraindicated until this phase is completed
\end{itemize}

\textbf{Phase 2: Motility Restoration (Weeks 4--8)}
\begin{itemize}
    \item Prokinetic therapy: low-dose erythromycin 50 mg at bedtime or prucalopride 1--2 mg daily (see Section~\ref{subsec:prokinetics} for mechanisms and prescribing details)
    \item Begin vagal activation exercises: 5 minutes of resonant-frequency breathing (typically 4.5--7 breaths/minute; individualise to the rate that maximally amplifies HRV~\cite{Lehrer2014hrv}) twice daily; gargling 30 seconds after brushing teeth
    \item Goal: Restore normal gut motility to prevent SIBO recurrence; begin vagal tone training
\end{itemize}

\textbf{Phase 3: Microbiome Recolonization (Weeks 8--16)}
\begin{itemize}
    \item Resistant starch RS2 (raw potato starch 1 tablespoon daily, increase slowly to 2 tablespoons)
    \item Multi-strain probiotic emphasizing \textit{Faecalibacterium prausnitzii} (if available), \textit{Lactobacillus rhamnosus}, \textit{Bifidobacterium infantis}
    \item Dietary fiber diversity (multiple fiber types to support microbial diversity)
    \item Goal: Restore butyrate-producing bacterial community
\end{itemize}

\textbf{Phase 4: Serotonergic Support (Weeks 12+)}
\begin{itemize}
    \item Sodium butyrate or tributyrin 1--2 g three times daily with meals (Chapter~\ref{ch:supplements}, Section~\ref{subsec:butyrate})
    \item 5-HTP 50--100 mg two to three times daily (bypasses IDO2 diversion; see Chapter~\ref{ch:supplements}, Section~\ref{sec:5-htp})
    \item P5P (active B6) 25--50 mg daily (decarboxylase cofactor)
    \item Zinc glycinate or zinc carnosine 15--30 mg elemental zinc daily
    \item \textbf{Safety note}: 5-HTP is absolutely contraindicated with SSRIs, SNRIs, MAOIs, tramadol, or other serotonergic drugs---physician review required before starting
    \item Goal: Optimize enterochromaffin serotonin synthesis and vagal afferent input
\end{itemize}

\textbf{Phase 5: Vagal Amplification (Ongoing)}
\begin{itemize}
    \item Continue butyrate supplementation
    \item Consider pyridostigmine 30--60 mg three times daily if orthostatic symptoms persist (amplifies vagal efferent output; standard POTS dosing)
    \item HRV biofeedback training 10--20 minutes twice daily (HeartMath or similar device)
    \item Goal: Maximize and sustain vagal tone and autonomic balance
\end{itemize}

\textbf{Monitoring:} Measure HRV (5-minute resting measurement) at each phase transition. Track gut symptom severity, fatigue scores, and orthostatic tolerance. HRV improvement is the primary indicator that the gut-vagal pathway is responding.

\textbf{Who may benefit:} ME/CFS patients with prominent GI symptoms (IBS, gastroparesis, SIBO), low HRV, and butyrate-producer deficiency on stool testing. Patients without GI involvement may have limited response.
\end{speculation}
