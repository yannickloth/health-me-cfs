...
While severe ME/CFS patients cannot typically participate in controlled exercise-tolerance studies, emerging evidence suggests wheat elimination may benefit bedbound patients through a different mechanism: by reducing baseline gut permeability independent of activity triggers~\cite{GutPermeability2023}. Unlike ambulatory patients trialing wheat elimination to prevent exercise-induced post-exertional malaise (PEM), severe patients may experience gradual baseline improvements in symptom ``noise floor'' and quality of life.

\textbf{Rationale:} Baseline gut barrier dysfunction (elevated zonulin, LPS, sCD14 in ME/CFS populations~\cite{GutPermeability2023}) combined with chronic low-grade splanchnic hypoperfusion from dysautonomia~\cite{vanWijck2011} creates sustained intestinal ischemic stress in bedbound patients. Wheat consumption (which upregulates zonulin via gliadin and $\alpha$-amylase trypsin inhibitors~\cite{Junker2012}) may perpetually sensitize tight junctions. For severe patients without exercise capacity to test, chronic wheat elimination becomes a reasonable trial: the only ``activity'' needed is dietary compliance.
...
\begin{enumerate}
    ...
    \item \textbf{Expected outcomes}: Reduced brain fog, pain, and symptom severity; improved orthostatic tolerance if dysautonomia-linked; gradual energy floor improvement within severe limitations; NOT expected: rapid breakthrough improvements or PEM prevention (since patient doesn't exercise).
...
\textbf{Evidence Strength (Certainty: 0.60):} Direct evidence for wheat elimination efficacy in severe/bedbound ME/CFS patients is lacking. This trial design extrapolates from ambulatory wheat-exercise intolerance data and baseline gut permeability findings in ME/CFS (see Section~\ref{sec:leaky-gut} for intestinal permeability evidence). Mechanistically grounded but untested specifically in bedbound populations.
