...
\midrule
Lim 2020~\cite{Lim2020} & 2-day CPET; repeated measures & n=51 ME/CFS, n=10 sedentary controls & VO$_2$max reduced 25\% on Day 2 in ME/CFS; controls unchanged; ventilatory threshold reduced & Objective PEM biomarker; 24-72h delayed impairment & HIGH (replicated) \\
\midrule
...
\midrule
\textbf{2-Day CPET} & VO$_2$max Day 1 vs Day 2 & 25\% reduction Day 2 in ME/CFS~\cite{Lim2020} & Objective PEM documentation; disability assessment & VALIDATED (replicated) \\
\midrule
...
\midrule
\textbf{Mitochondrial dysfunction} & ATP depletion, Complex IV deficits, delayed recovery & WASF3 elevation~\cite{wang2023wasf3}; 2-day CPET~\cite{Lim2020}; systematic review~\cite{Syed2025} & Causation vs consequence; specific complex deficits vary & HIGH \\
\midrule
\textbf{Post-exertional malaise (PEM)} & Objective VO$_2$max reduction Day 2; 24--72h delay & 2-day CPET 25\% reduction~\cite{Lim2020}; patient "<5 crash rule" & Molecular trigger; why delayed; recovery kinetics & HIGH \\
\midrule
...
\midrule
\textbf{24--72 hour PEM delay} & Symptom crash occurs 1--3 days post-exertion, not immediately & Activity tracking must account for delayed consequences; "you won't know until Day 2" & VALIDATED by 2-day CPET~\cite{Lim2020} \\
\midrule
...
\midrule
\textbf{Why PEM is delayed 24--72 hours} & Objective 2-day CPET shows delay~\cite{Lim2020}; molecular trigger unknown & Mitophagy markers; ATP kinetics; lactate clearance; serial muscle biopsies \\
\midrule
...
\section{Selective Energy Dysfunction Hypothesis Research Materials}
\label{sec:selective-dysfunction-materials}

The Selective Energy Dysfunction Hypothesis (see Chapter~\ref{ch:energy-metabolism} Section~\ref{sec:selective-energy-dysfunction} and Chapter~\ref{ch:proposed-studies} Section~\ref{sec:selective-energy-dysfunction-study}) has generated six research deliverables available in the project staging area (five completed, one planned). These materials are designed to facilitate hypothesis testing and clinical translation.

\subsection{Patient Assessment Tools}

\paragraph{Hair and Nail Health Survey.}
...
\begin{itemize}
    \item Demographics and disease characteristics stratified by Bell Scale severity
    \item Hair health assessment (growth rate, thickness, quality, loss patterns)
    \item Nail health assessment (growth rate, quality, appearance)
    \item Correlation tracking with disease activity changes
    \item Confounder screening (nutrition, medications, hormones)
\end{itemize}

\textbf{Location:} \texttt{.claude/content-staging/survey-hair-nail-health.md}

\textbf{Target population:} 200 ME/CFS patients (50 each severity level) + 100 healthy controls + 100 disease controls

\textbf{Expected outcomes:} Hair/nail growth rates should be statistically equivalent to controls despite severe functional impairment, providing objective evidence for selective (not global) dysfunction.

\paragraph{PEM Self-Tracking Protocol.}
...
\begin{itemize}
    \item \textbf{Simple mode}: 3 data points (activity type, PEM occurrence, severity) for patients with severe brain fog
    \item \textbf{Detailed mode}: Activity categorization by CNS involvement, PEM severity across 5 dimensions, timeline tracking
\end{itemize}

\textbf{Location:} \texttt{.claude/content-staging/protocol-pem-tracking.md}

\textbf{Key prediction:} Cognitive exertion (pure CNS) should cause more severe PEM than passive activities (minimal CNS demand).

\textbf{Data collection period:} 4--8 weeks for preliminary pattern identification

\subsection{Existing Dataset Analysis}

An inventory of 8 major ME/CFS datasets that could test hypothesis predictions without requiring new data collection:

\begin{itemize}
    \item \textbf{NIH Intramural Study (Walitt 2024)}: Deep phenotyping with brain imaging, autonomic testing, exercise physiology (n=17; highest priority for collaboration)
    \item \textbf{2-Day CPET Databases}: Multiple studies testing demand-response failure (100s of patients; immediate meta-analysis possible)
    \item \textbf{Van Campen Autonomic Studies}: CBF during orthostatic challenge (n=400+; replicates demand-response pattern)
    \item \textbf{UK Biobank ME/CFS Cohort}: Large-scale genetic and activity data (n=1,600; established access process)
\end{itemize}

\textbf{Location:} \texttt{.claude/content-staging/existing-datasets-analysis.md}

\textbf{Contains:} Dataset inventory table, prediction-to-dataset mapping, feasibility assessment, contact information for data access

\subsection{Proposed Research Studies}

\paragraph{Multi-Modal Hypothesis Testing Study.}
A comprehensive research proposal (integrated into Chapter~\ref{ch:proposed-studies}) testing 5 core predictions with n=72 participants (36 ME/CFS + 36 controls) over 3 years. See Section~\ref{sec:selective-energy-dysfunction-study} for full study protocol.

\textbf{Budget:} \$2.15M total

\textbf{Funding targets:} NIH R01, Solve M.E. Initiative, Open Medicine Foundation

\textbf{Full proposal location:} \texttt{.claude/content-staging/research-proposal-selective-dysfunction.md}

\paragraph{Review Article Outline.}
A 9-section outline for a comprehensive review article presenting the Selective Energy Dysfunction Hypothesis to the ME/CFS research community. Target journals: \emph{Frontiers in Neurology}, \emph{Journal of Translational Medicine}.

\textbf{Location:} \texttt{.claude/content-staging/review-article-outline.md}

\textbf{Length:} 8,000--10,000 words with 7 figures, 7 tables, 150--200 references

\textbf{Key sections:} Evidence review (preserved vs. impaired processes), mechanistic models, testable predictions, clinical implications, research agenda

\subsection{Treatment Translation}

\paragraph{Brain-Centric Treatment Protocol.}
\textbf{[PLANNED]} An evidence-based treatment protocol translating the Selective Energy Dysfunction Hypothesis into clinical practice. All recommendations are \textbf{PRELIMINARY} and require physician review before implementation.

\textbf{Location:} \texttt{.claude/content-staging/treatment-protocol-brain-centric.md} \textbf{(not yet created)}

\textbf{Core intervention categories:}
\begin{enumerate}
    \item \textbf{Brain energy support}: MCT oil (MODERATE evidence), CoQ10/ubiquinol (MODERATE), creatine (MODERATE-emerging), D-ribose (WEAK)
    \item \textbf{CNS demand reduction}: Heart rate-guided pacing, cognitive energy budgeting, sensory load reduction
    \item \textbf{Pharmacological CNS bypass}: Midodrine (MODERATE), fludrocortisone (MODERATE), LDN (MODERATE), LDA (WEAK-theoretical)
    \item \textbf{Clinical decision algorithm}: Foundation → first-line → second-line → third-line with clear response criteria
\end{enumerate}

\textbf{Evidence grading:} Every intervention labeled as Strong/Moderate/Weak/Theoretical with supporting citations

\textbf{Safety:} Comprehensive drug interaction table, red flags, contraindications included

\subsection{Research Community Access}

All materials in \texttt{.claude/content-staging/} are available for:
\begin{itemize}
    \item Patient advocacy organizations (for survey distribution and protocol testing)
    \item ME/CFS researchers (for collaboration on dataset analysis or study implementation)
    \item Clinicians (for evidence-based protocol adaptation)
    \item Medical students/trainees (for hypothesis-driven research education)
\end{itemize}

\paragraph{Contact for collaboration.} [To be determined based on institutional affiliation]
