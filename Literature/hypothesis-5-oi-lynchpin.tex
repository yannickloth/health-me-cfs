...
\begin{itemize}
    \item \textit{Neuroimmune}: Neuroinflammation secondary to recurrent hypoxic stress (cf. Section~\ref{spec:glial-maturation})
...
Third, pediatric treatment success with OI management might reflect pediatric-specific regenerative capacity (HSC reserves, immune tolerance, neuroplasticity; Sections~\ref{spec:hsc-exhaustion}, \ref{hyp:immune-memory-pruning}, \ref{spec:glial-maturation}) rather than OI being upstream. Treating OI simply removes one barrier to recovery that developmental processes enable. This predicts that aggressive OI treatment in adults would not replicate pediatric success.
