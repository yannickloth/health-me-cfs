...
Coenzyme Q10 (CoQ10) deficiency has been documented in ME/CFS: Maes et al.\ (2009) found that 44.8% of patients had plasma CoQ10 below the lowest control value, with lower CoQ10 correlating with fatigue, autonomic, and neurocognitive symptoms ($n = 58$) CoQ10 has been proposed as both a biomarker of mitochondrial electron transport chain dysfunction and a marker of treatment response
...
In ME/CFS, Tsilioni et al.\ found exosome-associated mtDNA elevated after exercise challenge---a different compartment and timing. Whether resting ccf-mtDNA differs between ME/CFS and Long COVID patients is unknown but represents a testable research question (see Section:cross-disease for the cross-disease hypothesis).
...
If ccf-mtDNA reflects mitochondrial quality control status, treatment-induced changes in mitophagy should be traceable. Specific predictions: (a) mitophagy-inducing agents (urolithin A) should produce a transient ccf-mtDNA rise as accumulated damaged mitochondria are cleared, followed by normalization; (b) T3 supplementation in the Low T3 subgroup should gradually raise ccf-mtDNA by restoring ERR$alpha$-mediated mitophagy coordination (Chapter:energy-metabolism, Section:thyroid-mito-regulation); (c) immunoadsorption or plasmapheresis removing pathogenic IgG should raise ccf-mtDNA if IgG-mediated mitochondrial fragmentation was driving the mitophagy block. These predictions are testable in small open-label trials using serial ccf-mtDNA measurement as a secondary endpoint. A well-designed definitive study would use a four-arm design (ME/CFS vs.\ Long COVID vs.\ anxiety disorder vs.\ healthy controls---the anxiety arm is essential because anxiety shows similarly low ccf-mtDNA, testing diagnostic specificity) and measure _both_ free ccf-mtDNA and exosome-associated mtDNA at rest and serially post-exercise to capture compartment-specific dynamics.
...
=== NET Degradation and DNase Activity as Candidate Post-Viral Biomarkers
<sec:net-dnase-biomarker>

NET degradation capacity—the balance between NET production and DNase-mediated NET clearance—has emerged from COVID-19 research as a potential post-viral biomarker with direct relevance to ME/CFS. Garcia et al. @GarciaLabroucheColomer2024 demonstrated that the ratio of NET markers (MPO-DNA, H3cit, H3cit-DNA) to functional DNase activity was markedly elevated in severe and critical COVID-19 patients and correlated with CRP and neutrophil/lymphocyte ratio—standard clinical severity markers. Crucially, H3cit and H3cit-DNA levels predicted hospitalisation among ambulatory patients, suggesting NET/DNase imbalance is an early rather than late feature of severe disease @GarciaLabroucheColomer2024.

*Assay components.* A clinically applicable NET degradation panel would measure: (1) NET remnants (MPO-DNA complexes, citrullinated histone H3) by ELISA; (2) functional DNase activity by residual dsDNA degradation assay; (3) NET/DNase ratio as an integrated imbalance metric; and optionally (4) DNase1 and DNase1L3 antigen levels to distinguish enzymatic deficiency from consumption. The Garcia functional DNase assay—measuring residual dsDNA after plasma incubation—provides a scalable, quantitative method suitable for large cohorts that avoids the technical demands of zymography or continuous fluorometric kinetic assays @GarciaLabroucheColomer2024. Complementary calprotectin (S100A8/S100A9) measurement offers a clinically accessible proxy for NET burden ($r >= 0.745$ with H3-NET levels) @Hetland2022.

*ME/CFS applicability.* No study has measured NET remnants, DNase activity, or NET/DNase ratios in ME/CFS patients. This is a notable gap given that: (a) ME/CFS shares thrombo-inflammatory features with long COVID; (b) herpesviruses implicated in ME/CFS triggering (EBV, CMV) are established NETosis inducers @Schonrich2016; (c) ME/CFS patients have documented neutrophil dysfunction that may include defective NET clearance; and (d) NET/DNase imbalance is genetically influenced via _DNASE1_ polymorphisms, providing a testable genetic susceptibility hypothesis (Chapter @ch:genetics-epigenetics). The NET/DNase ratio could serve as a post-viral stratification biomarker distinguishing patients with ongoing thrombo-inflammatory NET pathology from those with predominantly metabolic or neurological phenotypes.

*Technical precedent.* The Garcia et al. assay was applied to n=145 COVID-19 patients across three severity strata with robust statistical adjustment (age, sex, BMI) @GarciaLabroucheColomer2024. The key challenge for ME/CFS application is the lower expected NET burden in chronic post-viral compared to acute severe infection, requiring high assay sensitivity and potentially enrichment for patients with recent symptom exacerbation or known thrombo-inflammatory features.

#open-question(title: [Can NET/DNase Imbalance Stratify ME/CFS Patients for Thrombo-Inflammatory Phenotype?])[
Would ME/CFS patients with elevated NET/DNase ratios form a distinct clinical subgroup (thrombo-inflammatory ME/CFS) characterised by more severe orthostatic intolerance, higher D-dimer, microclot presence, and potentially differential response to NET-targeting therapies such as DNase I? The Garcia et al. @GarciaLabroucheColomer2024 finding that H3cit and H3cit-DNA levels predicted hospitalisation among ambulatory patients suggests the ratio has prognostic as well as diagnostic value. A validation study would measure NET markers, DNase activity, calprotectin, D-dimer, and microclot presence in n >= 100 ME/CFS patients versus healthy and long COVID controls, with clinical correlation to orthostatic intolerance severity and PEM duration.
...
A 2026 study by Chalder and Moreau introduces a multi-layer epigenetic biomarker framework for ME/CFS patient stratification Three measurement modalities span distinct regulatory levels:
...
Retrotransposon activation provides a comparative framework across disease categories. In cancer, HSAT2 serves as a validated biomarker with established detection methods. In post-viral syndromes, endogenous retrovirus (HERV) activation has been documented during and after SARS-CoV-2 infection @Charvet2023HERV, with HERV-W ENV protein expression correlating with disease severity @Charvet2023HERV. However, *direct HSAT2 measurements in Long COVID, fibrotic diseases, or other post-viral conditions represent a critical research gap* (certainty: 0.45 — established for HERV, absent for HSAT2).
...
- *Serum HSAT2 RNA*: TRAP-ddPCR quantification
- *Plasma cfDNA HSAT2*: Hybridization capture assay
- *Exosomal HSAT2/HERV-K*: EV isolation + ddPCR or RNA-seq
- *MDSC frequency*: CD33+HLA-DR−/lo immunophenotyping
...
The HSAT2/HERV-K exosomal hypothesis:hsat2-exosome-mdsc:hsat2-postviral-memory proposes a molecular mechanism for chronic ME/CFS immune suppression. If confirmed, it generates a specific set of measurable biomarkers that could stratify patients, guide treatment selection, and serve as pharmacodynamic endpoints in trials.
...
If state marker: HSAT2 could serve as an objective PEM biomarker, complementing exercise proteomics Serial sampling around CPET (0 h, 24 h, 48 h, 72 h) would test the PEM amplification hypothesis:hsat2-pem-amplification.
...
This study resolves the state-vs-trait question:hsat2-longitudinal and tests the PEM amplification hypothesis:hsat2-pem-amplification with the direct molecular readout. It also provides the temporal pattern needed to optimise the fold-change diagnostic:hsat2-cpet-fold-change (which timepoint is most discriminatory?).
...
The HSAT2 exosomal loop proposes two measurable outputs: (1) EV-packaged HSAT2 RNA (the causal signal, upstream) and (2) MDSC arginase-1 activity (the functional effector, downstream). Measuring both simultaneously in the same plasma sample provides: an RNA-cargo signal (detectable by TRAP-ddPCR, adapted to ME/CFS plasma) and an enzyme-activity signal (arginase-1 ELISA or colorimetric assay, readily available). The two markers are mechanistically non-redundant — HSAT2 identifies the loop driver; arginase-1 confirms its immunosuppressive output.
...
Baseline EV HSAT2 may be insufficient to discriminate ME/CFS from controls if the HSAT2 loop is only intermittently active. An exertional challenge (standardized CPET or step test) followed by serial EV HSAT2 measurement would test the PEM amplification hypothesis:hsat2-pem-amplification and simultaneously generate a fold-change biomarker. The fold-change (24 h post-CPET / pre-CPET) would capture the dynamic responsiveness of the HSAT2 loop to physiological stress — expected elevated in ME/CFS and normal in healthy controls.
...
ECM1 (extracellular matrix protein 1) autoantibodies are present in approximately 75% of vulvar lichen sclerosus cases and function as near-pathognomonic markers for LS ECM1 regulates basement membrane integrity, angiogenesis, and immune cell trafficking; its disruption sustains local Th1 inflammation through neoantigenic exposure. If lichen sclerosus co-occurs in ME/CFS patients at elevated frequency (see Section:lichen-sclerosus-immune-terrain), ECM1 autoantibody status could serve as a stratification biomarker for the autoimmune-terrain subgroup in ME/CFS research cohorts.
...
HHV-6 miR-aU14 drives mitochondrial fragmentation via the miR-30--p53--DRP1 axis and has been detected in ME/CFS post-mortem CNS tissue including dorsal root ganglia If measurable in plasma extracellular vesicles, miR-aU14 would stratify patients into HHV-6-reactivating versus non-reactivating subgroups, predicting response to ISR-modulating interventions (@hyp:isr-bidirectional) and antiviral treatment (Chapter:emerging-therapies).

*Technical approach:* EV isolation from plasma (size exclusion chromatography) followed by miRNA sequencing adapted from Key challenge: distinguishing virion-packaged from exosome-packaged miR-aU14 (latent vs active reactivation). (Certainty: 0.50 for technical feasibility; 0.35 for clinical utility.)
] <oq:mir-au14-exosome>
...
The Omdal null result establishes that baseline PBMC ISR markers are uninformative. However, the *dynamics* of ISR resolution may still differ. A functional assay would expose PBMCs to acute ER stressor (thapsigargin 200 nM, 30 min, then washout) and measure phospho-eIF2α kinetics over 0--6 hours.
...
Recent progress in exosome biology, including the demonstration that engineered exosomes can deliver functional mRNA across the BBB to reverse neuroinflammation @Kang2026ExosomeHSP70Sleep, has implications for biomarker development in ME/CFS, independent of therapeutic applications. Brain-derived exosomes (BDEs) — exosomes shed by neurons, astrocytes, microglia, and oligodendrocytes — cross the BBB into peripheral blood where they can be captured and analysed. Each subpopulation carries molecular cargo reflecting the state of its cell of origin.

#speculation(title: [Brain-Derived Exosome Biomarker Panel for ME/CFS Neuroinflammation and Proteostasis])[
*Certainty: 0.35.* Exosome biomarker concept established in neurodegeneration; no ME/CFS-specific validation; isolation and characterisation methods not yet standardised for clinical use.

*Rationale.* ME/CFS involves compartmentalised CNS pathology that blood-based assays systematically miss — neuroinflammation, microglial activation, glymphatic dysfunction, and impaired proteostasis are documented by PET and CSF studies but are invisible to standard peripheral blood tests @Nakatomi2014neuroinflammation. BDEs provide a window into this compartment: neuron-derived exosomes (captured via L1CAM immunoprecipitation) can report on synaptic health (neurogranin, synaptophysin), astrocyte-derived exosomes (GFAP capture) on glial activation and AQP4 status, and microglia-derived exosomes (TMEM119 capture) on inflammatory activation state.

*Candidate cargo analytes relevant to ME/CFS:*
    - *HSP70 and ubiquitin*: Chaperone capacity and proteostasis — reduced cargo suggests impaired protein quality control @Kang2026ExosomeHSP70Sleep
    - *Inflammatory cytokines*: IL-6, TNF-α, IL-1β within exosomes reflect CNS rather than peripheral production
    - *Neuron-specific enolase (NSE) and neurogranin*: Neuronal stress and synaptic integrity
    - *α-synuclein, amyloid-β, phospho-tau*: Neurodegenerative risk markers accumulating when glymphatic clearance fails
    - *miRNA cargo*: let-7b-5p already documented as downregulated in ME/CFS EVs and correlating with fatigue

*Subtyping potential.* A multi-analyte exosome panel could address two persistent problems in ME/CFS biomarker research: (1) heterogeneity — neuroinflammatory-dominant, metabolic-dominant, and autoimmune-dominant subgroups may show distinct exosome cargo signatures, and (2) temporal dynamics — serial exosome measurements after exercise challenge could reveal CNS stress responses invisible to peripheral blood.

*Testable predictions.*
    - ME/CFS neuron-derived exosomes (L1CAM+) should show lower HSP70 and higher IL-6 cargo compared to healthy controls, with HSP70 levels correlating with cognitive symptom severity
    - Exosome cargo profiles should stratify patients into subgroups that correspond to distinct clinical phenotypes (e.g., neuroinflammatory vs metabolic vs dysautonomic)
    - Post-exertion (24--48h) exosome cargo should change in ME/CFS patients but not controls, reflecting delayed CNS stress response

*Limitations.* Certainty 0.35. BDE isolation requires specialised immunoprecipitation protocols not standardised across laboratories. L1CAM capture enriches for neuron-derived exosomes but co-isolates non-exosomal L1CAM fragments. Exosome cargo reflects a snapshot that may not capture chronic steady-state pathology. Reference ranges for healthy CNS exosome cargo across age and sex are not established. No ME/CFS BDE study has been published. The diagnostic value is unproven; the concept is derived from Alzheimer's and TBI research where BDE biomarkers have shown promise but not yet achieved clinical deployment.
...
*Limitations.* Certainty 0.30. No data on neuronal exosomal HSP70 in any human population. HSP70 is ubiquitously expressed — distinguishing CNS-derived from peripheral exosomal HSP70 requires cell-type-specific capture. The causal direction is unclear: does low HSP70 cause poor sleep quality, or does poor sleep deplete HSP70? Longitudinal studies would be needed to disentangle this. An orthogonal salivary metabolomic approach (Section @sec:saliva-sleep-metabolomics) may complement exosomal biomarkers by providing a non-invasive, at-home metabolic fingerprint of sleep quality that does not require BBB crossing or L1CAM immunoprecipitation.
] <spec:exosomal-hsp70-biomarker>

=== Salivary Metabolomic Fingerprint of Sleep Deprivation as Candidate ME/CFS Biomarker
<sec:saliva-sleep-metabolomics>

The absence of a biochemical test for sleep deprivation has been a long-standing gap in both forensic and clinical medicine. Acute sleep loss produces cognitive and motor impairment comparable to alcohol intoxication, yet no objective roadside or clinical assay exists to quantify it. Scholz et al. (2026) demonstrated the first direct salivary metabolomic biomarkers of acute sleep deprivation, achieving 94% classification accuracy with a 12-feature machine learning model @Scholz2026SleepMetabolome.

#speculation(title: [Salivary Metabolomic Profiling Distinguishes ME/CFS Sleep Phenotypes])[
*Certainty: 0.35.* The primary evidence is Scholz et al.@Scholz2026SleepMetabolome (n=20 healthy males, 24h total sleep deprivation vs control, cross-over RCT, LC-HRMS, F0.5=0.90 with 12 molecular features). Oishi et al.@Oishi2025ChronicSleepSaliva provides chronic sleep disorder evidence (n=100, PSQI-defined poor vs good sleep, 13 metabolites altered, 86.6% classification accuracy). No ME/CFS data exist. Directly relevant because unrefreshing sleep is reported by 91--100% of ME/CFS patients @Maksoud2021MECFSSleepReview.

The central finding of Scholz et al. is a negative one with direct ME/CFS implications: acute 24h sleep deprivation produced a robust and identifiable metabolic fingerprint, but *four nights of chronic sleep restriction to 6h produced no exploitable metabolic changes*. This dissociation between acute and chronic sleep loss is a critical calibration point: ME/CFS unrefreshing sleep is a chronic state (months to decades), not acute deprivation, and may therefore not produce the Scholz et al. acute fingerprint. Conversely, Oishi et al.'s finding that chronic poor-quality sleep (PSQI-defined, n=100) does produce distinct salivary metabolomic changes (13 metabolites, 36% 2-hydroxybutyrate increase in poor sleepers) suggests that chronic sleep disruption can be detected in saliva if the measurement captures the *chronic* rather than *acute* metabolomic signature.

*Prediction.* If salivary metabolomic profiling is applied to ME/CFS patients versus healthy controls versus primary insomnia controls, the metabolic fingerprint of unrefreshing sleep will differ from both the healthy rested profile and the acute sleep deprivation profile, and will most closely resemble the chronic sleep disruption profile. If the ME/CFS pattern is indistinguishable from rested controls, the subjective experience of unrefreshing sleep may reflect central perception disconnect (consistent with the preserved PSG architecture finding in Maksoud et al.@Maksoud2021MECFSSleepReview) rather than a peripheral metabolic disturbance.

*Feasibility.* Saliva collection is non-invasive, requires no clinic visit, and can be performed at home by severely ill patients — a critical practical advantage over blood-based metabolomics for ME/CFS biomarker research. The LC-HRMS analytical workflow is established. The planned international validation cohort (1,000+ samples including women and shift workers) will address the current n=20 all-male limitation.

*Limitations.* Certainty 0.35. All evidence from healthy populations; no ME/CFS salivary metabolomics study exists. The acute vs chronic dissociation means the Scholz biomarker may not apply to chronic conditions. Oishi et al. used PSQI self-report, not objective sleep measures. The 12-metabolite panel is not yet identified at compound level (features, not named metabolites, in most cases). No replication in independent lab. The Jason et al.@Jason2021SalivaFBIMECFS salivary fatigue biomarker (FBI, n=98, distinguishes severe ME/CFS) demonstrates feasibility of salivary disease biomarkers in ME/CFS but measures fatigue-related peptides, not sleep-specific metabolites.
] <spec:saliva-sleep-metabolomic-biomarker>

#open-question(title: [Can Salivary Metabolomics Distinguish ME/CFS Unrefreshing Sleep from Primary Insomnia?])[
ME/CFS patients report unrefreshing sleep at 91--100% prevalence, yet objective PSG often shows near-normal sleep architecture @Maksoud2021MECFSSleepReview. This subjective-objective disconnect is one of the most puzzling features of the disease. If salivary metabolomics can detect sleep disruption metabolically, it may resolve this paradox: either (a) a chronic sleep disruption metabolic signature is present, supporting a biological basis for unrefreshing sleep not captured by PSG, or (b) no metabolic signature exists, supporting a central perception model where the *experience* of non-restorative sleep arises from CNS dysfunction (e.g., impaired glymphatic clearance, orexin deficiency) rather than peripheral sleep disruption.

A study would collect morning saliva samples from n >= 30 ME/CFS patients (stratified by unrefreshing sleep severity on PROMIS Sleep Disturbance), n >= 30 primary insomnia patients, and n >= 30 healthy controls matched for age, sex, and habitual sleep duration. LC-HRMS metabolomic profiling would compare metabolite signatures across groups. Prediction: ME/CFS unrefreshing sleep will show a distinct metabolic fingerprint that differs from both primary insomnia (where sleep is subjectively poor and objectively disrupted) and healthy controls.

*Key advantage:* the Oishi et al. chronic poor sleep model provides a reference panel against which ME/CFS can be compared. If ME/CFS unrefreshing sleep resembles chronic poor sleep metabolically, it suggests a genuine peripheral sleep disruption. If it does not, the subjective-objective disconnect may shift from "unexplained" to "explained by CNS-only mechanism."
] <oq:saliva-metabolomics-me-cfs-insomnia>

#speculation(title: [Circadian Metabolite Phase Locking Failure Drives Unrefreshing Sleep])[
*Certainty: 0.30.* Derived from Scholz et al.@Scholz2026SleepMetabolome (7 of 12 sleep-deprivation features exhibit 24h circadian rhythmicity, with metabolite identity depending on time of collection — strongest fingerprint at 08:00--12:00). The circadian dimension of the Scholz finding suggests that healthy sleep synchronises metabolite rhythms to the sleep-wake cycle. In ME/CFS, existing evidence documents circadian disruption (phase-shifted melatonin@Wyller2007thermoregCFS, altered cortisol rhythm@ch09:@sec:circadian, skin temperature rhythm disruption@Cambras2018CircadianCFS). No study has measured circadian-phase salivary metabolomics in ME/CFS.

The core mechanistic hypothesis: salivary metabolites that normally peak in phase with sleep (metabolites of cellular repair, glymphatic clearance, metabolic reset) become *phase-locked out of synchrony* with sleep timing. The brain detects this phase misalignment and interprets sleep as non-restorative — explaining the subjective-objective disconnect (normal PSG architecture + unrefreshing sleep) via a *circadian-metabolic* rather than a *sleep-architectural* mechanism.

*Prediction.* Dense time-series saliva collection (every 2--4h for 48h) in ME/CFS patients will reveal phase-shifted peaks of Scholz's 12 circadian-rhythmic features relative to sleep onset/wake times, with phase delays >= 2h compared to healthy controls. Phase offset magnitude will correlate with PROMIS Sleep Disturbance scores.

*Limitations.* Certainty 0.30. Entirely untested; all circadian inference from n=20 healthy males. No time-series salivary metabolomics data in ME/CFS. Phase-locking failure could equally be a consequence (poor sleep begets phase misalignment) rather than a cause. Requires LC-HRMS + controlled sleep schedule for 7 days pre-sampling — logistically demanding for severe ME/CFS patients.
] <spec:circadian-phase-locking-saliva>

#speculation(title: [Multi-Metabolite Sleep Refreshment Index for ME/CFS Subtyping])[
*Certainty: 0.30.* The Scholz@Scholz2026SleepMetabolome 12-feature panel (94% accuracy for acute deprivation) and the Oishi@Oishi2025SleepSaliva 6-metabolite panel (86.6% for chronic poor sleep) provide complementary metabolomic signatures of two qualitatively different sleep disruptions. An ME/CFS Sleep Refreshment Index (SRI) combining both panels — plus potential disease-specific features — could quantify the degree of sleep-related metabolic disturbance independent of PSG architecture.

The index value would serve three purposes: (a) *subtyping* — sleep-dominant vs fatigue-dominant vs cognitive-dominant ME/CFS endotypes should show different SRI values; (b) *treatment response* — interventions targeting sleep (DORAs, orexin agonists, clonidine, sleep hygiene) should shift SRI toward the healthy rested range; (c) *severity tracking* — SRI should worsen during PEM and correlate with cognitive and fatigue symptom burden.

*Prediction.* A composite SRI derived from the Scholz + Oishi panels will show (a) significant difference between ME/CFS and healthy controls, (b) significant difference between ME/CFS and primary insomnia, (c) correlation with unrefreshing sleep severity (r >= 0.5), and (d) responsiveness to effective sleep intervention.

*Limitations.* Certainty 0.30. No composite index has been constructed or validated. Both source panels derive from healthy males (Scholz n=20; Oishi n=100 males). Compound identity for most Scholz features is unknown (LC-HRMS features, not named metabolites). Cross-cohort stability of individual metabolite levels is unestablished. The planned international validation (1,000+ samples) may resolve some of these issues but will not include ME/CFS patients.
] <spec:multi-metabolite-sleep-refreshment-index>

#open-question(title: [Can Salivary Sleep Metabolomics Bridge Post-COVID and ME/CFS Sleep Phenotypes?])[
Long COVID patients report unrefreshing sleep at comparable prevalence to ME/CFS, and post-COVID sleep disorder shows glymphatic dysfunction correlates with sleep quality @Tang2025covidSleepDTIALPS. Both conditions share proposed mechanisms: glymphatic impairment, orexin deficiency, autonomic dysfunction. Comparing salivary metabolomic signatures of unrefreshing sleep across post-COVID condition (PCC), ME/CFS, and primary insomnia could determine whether these conditions share a common metabolic sleep disruption pathway or represent mechanistically distinct sleep pathologies.

A cross-condition study would collect morning saliva from n >= 40 per group (ME/CFS, PCC, primary insomnia, healthy controls) and apply LC-HRMS metabolomics with the Scholz and Oishi panels as pre-specified analysis targets. Prediction: ME/CFS and PCC will show overlapping salivary signatures that form a "post-infectious cluster" distinct from primary insomnia. Falsified if (a) ME/CFS and PCC signatures are statistically indistinguishable from primary insomnia (multivariate effect size d < 0.3) or (b) all three conditions show pairwise significant differences with no post-infectious clustering pattern (p < 0.05, FDR-corrected). Finding distinct profiles in all three groups would refute the shared post-infectious mechanism hypothesis.

*Feasibility.* Saliva collection is trivially non-invasive; this is one of the few biomarker modalities equally accessible to severe ME/CFS, PCC, and primary insomnia patients. The Scholz international validation cohort (shift workers, frequent drivers) could serve as a reference population if ME/CFS/PCC-specific data were added.
] <oq:saliva-sleep-metabolomics-cross-condition>
