...
As real-world contrast to the unevaluated-specialist-clinic question above, the Bateman Horne Center (BHC, Salt Lake City) — the leading U.S. ME/CFS Center of Excellence — operationalizes specialist care through a specific care-delivery architecture. Its components are *descriptive* (documented from the center's own published operational materials) and, as with all specialist-clinic models, carry **zero comparative-effectiveness evidence** (see @oq:specialist-clinic-evidence): the membership-based clinic model pairs a comprehensive initial assessment with a PCP-maintenance requirement, formalizing the specialist-vs-primary-care division of labor; the center's explicit opioid policy declines to manage chronic daily opioids; its EMR is customized with ME/CFS-specific fields (Good Day/Bad Day status, head-up-angle tracking); and it runs a Project ECHO-style provider-education effort to spread specialist knowledge to community clinicians. These are presented not as a validated template for reform but as a concrete, documented example of what a specialist ME/CFS care-delivery structure looks like — the descriptive detail the broader specialist-clinic question leaves unaddressed. (Certainty: N/A — descriptive care-model documentation, not a causal or comparative claim. Severity applicability: the model is designed for the full severity range a specialist center serves, but implementation detail for severe/very-severe home-bound care is not detailed in the source.)

*Consequence:* This example gives health-system planners and advocates a concrete picture of how a leading specialist center structures membership, PCP coordination, opioid policy, EMR fields, and provider education — useful as a reference model, but unevaluated: it provides no evidence that this structure improves outcomes over usual care. (Origin: /integrate-topic bhc-clinical-operations-model — B-gate exploratory; BHC website source.)
