...
= Diagnostic Tools and Assessment Scales
...
The DePaul Symptom Questionnaire is a validated self-report instrument developed specifically for ME/CFS research and clinical assessment @Jason2006.
...
/ *Scoring*: Composite domain scores are calculated for fatigue, post-exertional malaise, sleep dysfunction, pain, neurocognitive symptoms, autonomic dysfunction, neuroendocrine symptoms, and immune symptoms. A symptom is considered clinically significant when frequency $gt.eq 2$ and severity $gt.eq 2$.
/ *Interpretation*: The DSQ enables operationalized application of diagnostic criteria. For the ICC, PEM must score $gt.eq 2$ on both frequency and severity, plus at least one symptom in each required category (neurological, immune, energy metabolism).
...
The Bell Disability Scale provides a single-number rating of functional capacity @Roberts2016.
...
/ *Interpretation*: Scores correspond approximately to NICE 2021 severity grades @NICE2021mecfs: Mild ($gt.eq 60$), Moderate (40–50), Severe (20–30), Very Severe ($lt.eq 10$).
...
The SF-36 is a generic health-related quality of life instrument widely used across chronic diseases, providing a benchmark for comparing ME/CFS functional impact with other conditions @kingdon2018functional.
...
/ *Limitations*: Developed as a general fatigue measure, not specific to ME/CFS. Does not distinguish ME/CFS-type fatigue from fatigue due to depression, sleep disorders, or other causes. Used as a primary outcome in the controversial PACE trial @White2011pace, which has been criticized for using recovery thresholds that overlapped with entry criteria @geraghty2019cognitive. Not recommended as a sole outcome measure in ME/CFS trials.
...
/ *Protocol*: Two maximal graded exercise tests on consecutive days, typically on a cycle ergometer:
    ...
    - Day 1: Standard incremental exercise test to volitional exhaustion. Measures peak VO#sub[2], anaerobic threshold (AT/VT1), peak workload, and respiratory exchange ratio.
...
/ *Interpretation*: In healthy controls and most other chronic diseases, day-2 performance equals or exceeds day-1 performance. In ME/CFS, day-2 performance is significantly reduced:
    ...
    - Peak VO#sub[2] decreases by $gt.eq 8$% (clinically significant decline)
...
Standard clinic-based diagnostic procedures are poorly tolerated by severe and very severe ME/CFS patients (Bell score $lt.eq$30), with $>$60% unable to complete inpatient protocols due to PEM @Fricke2026achtsam. The ACHTSAM study @Fricke2026achtsam is the first protocol to systematically evaluate which assessments can be safely administered through home visits, using a staged approach that orders procedures from low to high burden with mandatory rest periods.

/ *Low-burden assessments*: ($lt.eq$5min, supine-tolerant): Resting ECG, pupillography, osteosonography (bone density), bioelectrical impedance body composition, near-infrared spectroscopy (NIRS) for muscle oxygenation, light activity sensor.
...
/ *Tolerability hypothesis*: Non-invasive supine-position assessments are expected to achieve $gt.eq$80% completion, while high-burden tests requiring sustained mental exertion or standing are expected to achieve $<$30%.
...
/ *Protocol*: The patient rests supine for 10–20 minutes, then is passively tilted to 60–70#super[compose] for up to 45 minutes. Continuous heart rate and blood pressure monitoring throughout.
/ *Diagnostic criteria*:

    - *POTS*: Sustained heart rate increase $gt.eq 30$bpm (or $gt.eq 40$bpm in ages 12–19) within 10 minutes of tilt, without orthostatic hypotension
    - *Neurally mediated hypotension*: Drop in systolic BP $gt.eq 20$mmHg or diastolic BP $gt.eq 10$mmHg, often with paradoxical bradycardia
    - *Orthostatic hypotension*: Sustained drop in systolic BP $gt.eq 20$mmHg or diastolic $gt.eq 10$mmHg within 3 minutes of tilt
...
The following tests are recommended to exclude conditions that can mimic ME/CFS @NICE2021mecfs @Carruthers2011ICC:
...
No validated diagnostic biomarker for ME/CFS exists, but several candidates are under investigation:
    ...
    - *Metabolomics panels*: Reduced amino acids, altered lipid profiles, elevated lactate-to-pyruvate ratios @Naviaux2016metabolomics @Germain2020metabolic
    - *Cytokine panels*: Disease-duration-dependent profiles @Hornig2015 @Montoya2017
    - *Epigenetic markers*: DNA methylation signatures at specific CpG sites @deVega2021dna_methylation
    - *Two-day CPET*: While not a blood test, the day-2 decrement serves as a functional biomarker (Section @sec:objective-tests)

#warning-env(title: [No Single Diagnostic Test Exists])[
