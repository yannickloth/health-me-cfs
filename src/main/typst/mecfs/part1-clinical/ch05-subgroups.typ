...
#speculation(title: [Skeletal Asymmetry as a Subgroup-Defining Feature])[
*Certainty: 0.10.* A subset of ME/CFS patients may have fixed skeletal asymmetries — rotoscoliosis, pelvic obliquity from leg length discrepancy (ILMI), or DAMI (Des Axes Morphostatiques Insuffisants, a French clinical concept describing insufficient morphostatic axes) — as the primary upstream driver of their illness. Unlike the hypermobility/hEDS subgroup (@sec:septad), whose pathology is in connective tissue quality, the mechanical/postural subgroup has normal collagen but abnormal skeletal geometry. The key clinical distinction is that symptoms are *positionally modulated* — worse with standing and walking, better with lying flat — not because of orthostatic intolerance from blood pooling (though that may co-occur) but because upright posture activates the compensatory muscle chains and mechanical nerve compression described in @spec:skeletal-asymmetry-cascade.
...
- Worsening with age as cervical arthrosis progresses (@spec:skeletal-asymmetry-cascade, Pathway 4)
- Pain lateralized to the overloaded (longer-limb/convexity) side
- Positive response to postural interventions (shoe lift, pelvic realignment) in clinical anecdotes — no controlled data
