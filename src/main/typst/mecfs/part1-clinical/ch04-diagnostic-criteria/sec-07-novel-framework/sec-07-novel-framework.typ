...
==== The Logic Chain: From Biology to Diagnosis
...
Post-exertional malaise (PEM) with delayed onset, disproportionate severity, and prolonged recovery distinguishes ME/CFS from other major fatiguing conditions, including depression and deconditioning. This symptom:
    ...
    - Has objective correlates (2-day CPET showing Day 2 deterioration @Lim2020 @keller2024cpet)
    - Reflects underlying cellular energy failure (ATP depletion @heng2025mecfs @Syed2025)
...
This explains:
    ...
    - Why daratumumab works in 60% not 100% of patients @Fluge2025daratumumab
...
*Logical consequence*: Tier 2 must enable multi-target treatment protocols. We cannot predict _a priori_ which domain is rate-limiting, so we must:
...
The natural history literature @Maksoud2020natural and patient reports converge on temporal patterns:
...
==== Why Three Tiers? Why Not Two or Four?
...
==== How Recent Advances Enable This Framework
...
#include "requirements/req-pem-criterion.typ"
#include "requirements/req-energy-insufficiency.typ"
#include "requirements/req-duration-exclusion.typ"
...
In addition to the three core features, patients must have at least three of the following five supporting features:
    ...
    - *Unrefreshing Sleep*
        ...
        - Sleep that fails to restore energy regardless of duration
        - Waking feeling as exhausted as when going to bed
        - Present in 95–100% of ME/CFS patients @Jason2010sleepMECFS @Unger2016sleepPrevalence
    

    - *Cognitive Impairment*
    
        - Processing speed deficits (most robust finding: Hedges' g = -0.82) @Cvejic2022cognitive
        - Attention and working memory impairment
        - Word-finding difficulties, linguistic reversals
        - Brain fog that is not attributable to fatigue or depression @MCAM2024cognitive
    

    - *Autonomic Dysfunction*
    
        - Orthostatic intolerance: symptoms worsened by upright posture
        - POTS (heart rate increase $gt.eq$30 bpm upon standing), orthostatic hypotension, or neurally mediated hypotension
        - Temperature dysregulation, inappropriate sweating or lack of sweating
        - Present in 70–90% of ME/CFS patients @Newton2007autonomicDysfunction
    

    - *Pain*
    
        - Myalgia (muscle pain), particularly with post-exertional exacerbation
        - Arthralgia (joint pain, characteristically migratory without inflammation)
        - Headaches (migraine or tension-type) @Ravindran2011headache
        - Pain present in $tilde$80% of patients @Unger2017pain
    ...
    - *Sensory Hypersensitivity*
        ...
        - Present in 70–90% of patients @Jason2013sensory
    

Tier 1 criteria establish ME/CFS as a clinical syndrome with mandatory post-exertional malaise, baseline energy insufficiency, and 6-month duration. Supporting features (sleep, cognition, autonomic, pain, sensory) must be present in sufficient number ($gt.eq$3 of 5) to confirm the characteristic multi-system presentation. These criteria are compatible with existing frameworks (Canadian Consensus, ICC, IOM) but add explicit recognition of the effort-performance disconnect and specify the 6-month threshold as marking transition to established disease.
=== Tier 2: Biological Phenotyping (Multi-Domain Assessment)
<subsec:tier2>
...
==== Rationale: Co-Occurrence Rather Than Predominance
...
#include "hypotheses/hyp-multi-domain-cooccurrence.typ"
...
*Treatment Implications:*
    ...
    - Candidate for immunoadsorption (IgG removal) @Stein2024immunoadsorption
    - Candidate for daratumumab (anti-CD38, depletes plasma cells) @Fluge2025daratumumab
    - Candidate for BC007 (GPCR autoantibody neutralizer) @Hohberger2021bc007
    - Monitor for worsening with immune-stimulating interventions

*Prevalence:* 30–60% of ME/CFS patients

==== Domain 2: Mitochondrial/Metabolic Dysfunction

*Assessment:*

    - Heng 7-marker panel (if available): Elevated AMP, ADP (energy depletion arm) @heng2025mecfs
    - Elevated lactate: Resting >2.0 mmol/L or abnormal accumulation during 2-day CPET @Lien2019lactate
    - ATP profile abnormalities (if specialized testing available)
    - WASF3 elevation on skeletal muscle biopsy (if indicated for severe cases) @wang2023wasf3
...
*Treatment Implications:*
    ...
    - NAD#super[+] precursors (nicotinamide riboside 1000–2000 mg/day, treatment duration $gt.eq$10 weeks)
...
==== Domain 3: Neuroinflammation/Central Sensitization

*Assessment:*

    - *Research settings*: PET evidence of microglial activation @Nakatomi2014neuroinflammation, fMRI showing altered temporoparietal junction or salience network connectivity @walitt2024deep @Shan2020neuroimaging
    - *Clinically accessible*:
    
        - Central sensitization confirmed by quantitative sensory testing: pressure pain thresholds \<5th percentile at $gt.eq$3 standardized sites @Nijs2021sensitization
        - Small fiber neuropathy: skin biopsy showing intraepidermal nerve fiber density \<5th percentile @Oaklander2022SFN
...
==== Domain 4: Dysautonomia

*Assessment:*

    - *Gold standard*: Tilt table testing showing POTS (heart rate increase $gt.eq$30 bpm within 10 minutes), orthostatic hypotension (blood pressure drop $gt.eq$20/10 mmHg), or neurally mediated hypotension
    - *Clinically accessible*: NASA Lean Test (10-minute standing test; positive if heart rate increases $gt.eq$30 bpm)
...
==== Domain 5: Endothelial Dysfunction

*Assessment:*

    - Heng 7-marker panel (if available): Elevated von Willebrand factor, fibronectin, thrombospondin-1 (endothelial activation arm) @heng2025mecfs
...
#include "warnings/warn-domain6-evidence-limitations.typ"
...
#include "warnings/warn-speculative-phenotype-cluster.typ"
...
The "Viral-Immune-Metabolic" cluster (see Section @sec:cimetidine-responder for detailed discussion) is characterized by:
...
#figure(
  ...
  [*Viral-Immune*], [EBV serology (VCA IgG/IgM, EBNA-1, EA-D), HHV-6 serology — note: VCA IgG/EBNA-1 IgG reflect remote infection, not current activity; VCA IgM may indicate recent primary or polyclonal activation; interpret separately (Section @sec:herpesvirus-antibody-persistence)], [HHV-6/EBV PCR, T cell exhaustion panel (PD-1, LAG-3)], [Cimetidine response],
  ...
  [*Mast Cell*], [Serum tryptase, plasma histamine, DAO activity], [24h urine N-methylhistamine, prostaglandin D#sub[2]], [HIT/MCAS symptoms],
...
If this phenotype is suspected:
    ...
    - *Cimetidine shows benefit* $arrow.r$ Prioritize viral serology (EBV/HHV-6) + amino acid panel
    - *Strong amino acid response* $arrow.r$ Prioritize intestinal permeability markers (Zonulin, LPS) + mitochondrial assessment
    - *POTS + MCAS both present* $arrow.r$ Prioritize dysautonomia subtyping (hyperadrenergic vs. neuropathic)

*Treatment Approach (Hypothetical).*

#include "warnings/warn-medical-supervision-required.typ"
...
*Reassessment*: 3-month follow-up to evaluate response in each domain; adjust treatment based on which domains improve vs. persist
=== Tier 3: Severity Classification and Progression Risk
<subsec:tier3>
...
#include "warnings/warn-progression-risk.typ"

==== Critical Temporal Windows

<obs:temporal-windows>
Two temporal thresholds mark critical transitions in ME/CFS natural history @Maksoud2020natural:

*6-Month Persistence Mark:*
...
*2-Year Establishment Threshold:*
Around 2 years post-onset, ME/CFS transitions from early disease (hypermetabolic, potentially modifiable) to established disease (hypometabolic, potentially entrenched). This transition involves:
    ...
    - Immune exhaustion (CD8#super[+] T cell exhaustion @iu2024tcell_exhaustion, NK cell dysfunction)
    - Normalization of inflammatory markers despite ongoing dysfunction
    - Brain structural changes visible on advanced imaging @Shan2020neuroimaging
...
*Clinical application*: Patients diagnosed within 6 months of onset should be counseled on the criticality of aggressive pacing to prevent establishment. Patients approaching the 2-year mark should undergo comprehensive Tier 2 phenotyping to guide maximal intervention before the window closes.
=== Implementation and Clinical Workflow
<subsec:implementation>

==== Minimum Diagnostic Workup (All Patients)
...
==== Advanced Phenotyping (Tier 2, If Resources Permit)

    - *Autoimmune Domain*:
    
        - GPCR autoantibody panel ($beta_2$-adrenergic, M3 muscarinic, M4 muscarinic)
        - NK cell count and function assay
        - Flow cytometry for plasma cell populations (CD38#super[+]CD138#super[+])
...
==== Treatment Prioritization Based on Phenotype
...
Table @tab:framework-comparison compares the novel biology-informed framework to established diagnostic criteria.



#figure(
  table(
    columns: (1fr, 1.2fr, 1.2fr, 1.2fr, 1.5fr),
...
The novel framework is _compatible_ with existing criteria rather than contradictory:
    ...
    - Tier 2 and Tier 3 represent _additions_ that do not invalidate previous diagnoses
...
=== Clinical Advantages of the Novel Framework
<subsec:advantages>
