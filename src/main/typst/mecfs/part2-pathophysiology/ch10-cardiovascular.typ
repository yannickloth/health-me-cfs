...
*Cross-Disease Corneal Confocal Validation.* Corneal confocal microscopy (CCM) is a non-invasive, repeatable technique that images the corneal subbasal nerve plexus and has been validated across multiple diseases for small fiber neuropathy detection. Multinational normative data (CNFD, CNBD, CNFL, tortuosity) are available from a multicenter collaboration of over 340 healthy subjects @tavakoli2015normaliveccm, with ethnic-specific reference values also established @cao2022cornealreference. A systematic review and meta-analysis by Ranathunga et al.\ (2026) found significant small fiber pathology across non-neuropathic chronic pain conditions @ranathunga2026nonneuropathic, and a scoping review by Oreskovic et al.\ (2026) mapped CCM evidence across multiple neurodegenerative diseases (PD, MS, AD, ALS) @oreskovic2026scoping. In fibromyalgia syndrome, approximately 50% of patients have reduced skin innervation, and CCM detects SFN in approximately 59% of FMS patients @sommer2025fibromyalgiasfn — comparable to the ~30% figure for ME/CFS. In multiple sclerosis, a meta-analysis confirmed significant corneal nerve loss and increased dendritic cell density, with CCM parameters correlating with disability scores (EDSS) @akowuah2025msccm. In Parkinson's disease, corneal nerve fiber loss independently predicted pain severity @yin2025pdpain, and CCM distinguished MSA from idiopathic PD @niu2024pdmsa. In systemic lupus erythematosus, CCM detected small fiber damage correlating with disease activity, neuropathic pain, and quality of life @gharib2025sleccm. In chemotherapy-induced peripheral neuropathy, CCM detects early nerve loss before clinical symptoms appear, with dose-dependent, time-resolved trajectories across multiple studies @baghdasaryan2026chemo @chiang2026dosedependentcipn @stache2023taxanelongitudinal — making CIPN a candidate accelerated model for studying the time course of small fiber pathology relevant to post-infectious SFN. In POTS, CCM identified reduced corneal nerve density and microneuromas in a Johns Hopkins cohort (n=43) @karakus2025potseye and SFN in a pilot study @cantrell2025potssfn. CCM has been deployed in primary care optometry settings for diabetic neuropathy screening (450 patients, 94.9% success rate, 12.9% sub-clinical neuropathy detected) @carmichael2022ccmprimarycare, demonstrating feasibility outside academic ophthalmology. Crucially, a direct head-to-head comparison of CCM versus skin biopsy (IENFD) in mixed-etiology polyneuropathy found only moderate correlation @ghadban2025mixed, indicating that CCM and skin biopsy measure partially distinct aspects of small fiber pathology — CCM is non-invasive and repeatable but does not replace the established IENFD gold standard.

*Certainty:* 0.45 (was 0.40 → 0.45: CCM cross-disease validation and normative data from 6+ disease populations strengthens the SFN assessment framework, though the CNS metabolic mechanism remains speculative; general-population + cross-disease evidence, not ME/CFS-specific)
] <spec:sfn-interface-failure>

#open-question(title: [Is ME/CFS Corneal Nerve Pathology Trigeminal-Specific or Systemic?])[
*(Origin: brainstorm — Phase 4, idea 1.3.)* The corneal subbasal nerve plexus is innervated exclusively by the ophthalmic division of the trigeminal nerve (CN V1). ME/CFS CCM findings — tortuosity-dominant, non-length-dependent pattern @Azcue2025sfn — could reflect trigeminal neuropathy rather than systemic small fiber neuropathy. Trigeminal nerve involvement is documented in migraine (CCM-detected corneal nerve alterations independent of subtype) @vanwelie2026migraine and in Sjögren's syndrome (CCM corneal nerve density reduction only in the SFN+ subset) @luzu2022cornealinnervation. The critical untested question: if CCM abnormalities in ME/CFS reflect CN V1-specific pathology that does *not* correlate with distal leg IENFD, the paper's framing of CCM as a "systemic SFN biomarker" would need revision toward a cranial neuropathy model. Only one ME/CFS CCM study exists, and it did not perform concurrent IENFD measurement — the assumption that corneal findings reflect systemic SFN is untested.

*Falsifiable prediction:* In ME/CFS patients with abnormal CCM (tortuosity >95th percentile of normative data @tavakoli2015normaliveccm), concurrent IENFD at distal leg will be normal in ≥50% of cases; trigeminal reflex testing (blink reflex, corneal sensitivity) will be abnormal in ≥60%. Falsified if CCM abnormalities correlate tightly with distal IENFD (supporting systemic SFN) and trigeminal reflexes are normal. Falsified in the opposite direction if CCM is abnormal but both IENFD and trigeminal reflexes are normal (indicating a corneal-specific, non-neural confound).

*Consequence:* Answering this question would determine whether an abnormal eye scan signals a body-wide nerve problem (current assumption) or a cranial nerve problem (alternative model) — two very different treatment targets and prognostic messages for patients.
] <oq:ccm-trigeminal-vs-systemic>

#speculation(title: [CCM Tortuosity Index as a Composite ME/CFS Small Fiber Severity Score])[
*(Origin: brainstorm — Phase 4, idea 9.1.)* *(Certainty: 0.30.)* Current CCM parameters (CNFD, CNBD, CNFL, tortuosity, dendritic cell density) are reported individually. A composite "Tortuosity Index" — e.g., (tortuosity percentile) × (DC density percentile) / (CNFD percentile × CNFL percentile) normalized against the Tavakoli multinational normative database @tavakoli2015normaliveccm — could provide a single disease-severity score. Azcue 2025 found tortuosity alone at AUC=0.720 versus CNFD at AUC≈0.63; a composite combining all four parameters plus DC density might push AUC ≥0.85. Such a score would enable: (a) single-number disease severity tracking, (b) cross-study comparison, and (c) treatment response quantification in trials. The concept is borrowed from composite scores in other specialties (e.g., radiographic progression scores, composite cognitive indices) but has no precedent in CCM. No validation cohort exists.

*Falsifiable prediction:* A pre-registered CCM composite score weighted by logistic regression on ME/CFS vs controls (n=100) will discriminate at AUC ≥0.85, significantly above tortuosity-alone AUC=0.720, and will correlate with SFN symptom severity (r ≥0.6) and IENFD (r ≥0.5). The composite will be reproducible across two independent scanners. Falsified if composite AUC does not significantly exceed tortuosity-alone AUC, or if the composite is not reproducible across scanners.

*Consequence:* A composite score could standardize CCM reporting across studies, but the formula proposed here is illustrative, not validated — its clinical analogs (HbA1c, composite radiographic scores) required decades of calibration against gold standards. No such calibration data exist for CCM in ME/CFS. This is a measurement standardization proposal awaiting its first validation cohort.
] <spec:ccm-tortuosity-composite-score>

#limitation(title: [CCM as SFN Biomarker in ME/CFS — Evidence Quality and Specificity Concerns])[
*(Origin: brainstorm — Phase 4, categories 10–12 critical self-critique.)* Multiple concerns about CCM as an ME/CFS biomarker have not been resolved and moderate the enthusiasm of the preceding discussion:

*Methodological heterogeneity.* Automated vs manual CCM image analysis produces significantly different parameter values, with no pre-processing and analysis pipeline standard (analogous to the ENIGMA consortium for neuroimaging) existing for CCM.

*Systemic vs cranial specificity.* The trigeminal-innervation concern (@oq:ccm-trigeminal-vs-systemic) means abnormal CCM may indicate CN V1 pathology, not systemic SFN. Concurrent IENFD measurement — the simplest control for this — was not performed in the singular ME/CFS CCM study @Azcue2025sfn.

*Dry eye confound.* ME/CFS patients may have higher rates of dry eye disease (DED) from autonomic lacrimal dysfunction, medication anticholinergic effects, or increased screen time. DED independently affects corneal epithelium and subbasal nerves, potentially mimicking or exacerbating the CCM tortuosity signal. This is the most parsimonious alternative explanation for the ME/CFS CCM findings: autonomic dysfunction → reduced tear production → corneal epithelial stress → secondary nerve remodeling → increased tortuosity — with zero involvement of systemic neuropathy. Neither tear osmolarity nor ocular surface staining was reported in Azcue 2025, so DED confound cannot be excluded.

*Single-study risk.* Only one ME/CFS CCM study has been published versus 85+ for diabetic neuropathy. Replication failure or regression to the mean (Azcue 2025 AUC=0.720 → lower in a larger replication) is possible. File-drawer risk: negative CCM studies in ME/CFS may exist but remain unpublished.

*Normative gap.* The Azcue 2025 ME/CFS cohort has never been compared against the Tavakoli multinational normative database @tavakoli2015normaliveccm, so the deviation of ME/CFS CCM parameters from age-adjusted expectations is unknown.

*Publication bias.* All 15 Phase 1 CCM papers report positive findings. The field may be selecting for studies that show CCM "works" across every disease — a pattern consistent with methodological enthusiasm rather than discriminatory value.

*Conclusion.* CCM is a promising research biomarker with cross-disease precedent. But at present, the evidence for CCM as a specific, validated SFN diagnostic in ME/CFS rests on a single study without concurrent IENFD, without normative comparison, and without replication. These concerns do not refute CCM's utility — they define what a validation study must resolve.

*Consequence:* Clinicians should not yet use CCM to diagnose SFN in ME/CFS outside research settings. Researchers can use CCM as a non-invasive exploratory endpoint but must include concurrent IENFD and DED assessment in their protocols to control for confounds.
] <lim:ccm-mecfs-evidence-quality>

#synthesis(title: [Corneal Confocal Microscopy — A Cross-Disease Measurement Framework for Small Fiber Neuropathy in ME/CFS])[
The cross-disease CCM evidence from 15 papers across seven disease groups establishes that CCM detects small fiber pathology consistently across conditions — but consistent cross-disease positivity is a double-edged finding: it demonstrates CCM's sensitivity while raising the question of specificity (if every disease shows abnormalities, what does abnormal CCM actually tell us about ME/CFS specifically?). The single ME/CFS CCM study (Azcue 2025, AUC=0.720, tortuosity as primary discriminator) gains credibility from cross-disease precedent but remains unreplicated, without concurrent IENFD measurement, without dry-eye control, and without normative comparison against the Tavakoli multinational database. The tortuosity-dominant, density-preserved pattern is *suggestive* of immune-mediated rather than metabolic neuropathy — consistent with the Sjögren's disease literature (@oq:ccm-sjogren-overlap-pattern) — but the most parsimonious alternative explanation (dry eye disease from autonomic lacrimal dysfunction, causing local corneal nerve remodeling) has not been excluded. The moderate CCM-vs-IENFD correlation (Ghadban 2025 @ghadban2025mixed) means CCM and skin biopsy provide complementary information rather than interchangeable measurements. The theoretical possibility that tortuosity and density represent distinct pathological processes — ECM degradation-driven kinking vs axonal die-back — has been proposed but cannot be calibrated from existing data. The critical unresolved question that governs all downstream interpretation: do CCM findings reflect systemic SFN, trigeminal-specific neuropathy, or a corneal-confined artifact (@oq:ccm-trigeminal-vs-systemic)? Until answered, CCM serves as a promising but unvalidated research tool — not a clinical diagnostic, not a trial endpoint, and not a confirmed window into CNS pathology. The measurement framework has been assembled; the validation has not.

*Consequence:* For researchers: CCM offers a non-invasive SFN assessment tool with cross-disease precedent, but protocols must include concurrent IENFD, dry-eye assessment, and normative comparison to avoid over-interpreting a signal whose specificity and anatomical origin are unknown. For clinicians: CCM is not yet validated for ME/CFS SFN diagnosis and should not be used for clinical decision-making outside research.
] <syn:ccm-cross-disease-sfn-model>
