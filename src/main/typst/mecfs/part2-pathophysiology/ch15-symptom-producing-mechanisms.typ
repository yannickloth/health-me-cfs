...
*The recovery lag.* These three structural changes impose a temporal decoupling between sleep quality and clearance efficiency: pharmacological restoration of SWS architecture (via trazodone, DORA) can improve the *signal* (NE oscillations, delta power, SWS duration) within days, while the *machinery* that converts that signal into clearance (AQP4 polarity, endfoot contact area, perivascular compliance) requires weeks to months to remodel. A patient who achieves objectively normal sleep architecture after years of non-restorative sleep may still have impaired glymphatic function — the pump is running at full speed but the pipes are narrowed, the seals are leaky, and the osmotic gradient is blunted. This explains the clinical observation that sleep quality can be pharmacologically normalized without parallel improvement in the restorative quality of sleep: the patient sleeps better but does not wake refreshed, because clearance efficiency lags behind sleep architecture improvement by a factor of 10--100x in time.
...
*Progressive improvement under sustained restorative sleep.* The structural degradation model is not a claim that damage is permanent. It is a claim that the recovery timescale is orders of magnitude longer than the sleep-restoration timescale. If consistent restorative sleep is maintained for months — via nightly DORA + trazodone + glymphatic position optimization — the same structural processes that degrade over years would begin to reverse: AQP4 repolarizes toward perivascular endfeet as perivascular NE tone normalizes, reactive astrogliosis resolves and endfeet re-extend as neuroinflammation subsides, and perivascular fibrosis remodels as the reduced waste burden permits extracellular matrix turnover. The trajectory is nonlinear: initial clearance improvement is subclinical because accumulated waste must be cleared before net benefit accrues (the brain must first pay down the waste debt before the balance sheet shows positive clearance). The patient may feel no different for weeks despite objectively improving glymphatic function. Then, at a threshold that depends on the magnitude and duration of prior impairment, cumulative clearance gains cross into symptomatic territory — less brain fog, reduced PEM trigger sensitivity, more consistent cognitive stamina. This threshold phenomenon explains why patients who report meaningful improvement from sleep interventions consistently describe it as something that "finally started working" after sustained use, not an acute effect. The model predicts that treatment discontinuation after 2--4 weeks of normal sleep architecture will show no lasting benefit, while 3--6 months of continuous normalized sleep with preserved glymphatic function will produce progressive and partially retained improvement.
