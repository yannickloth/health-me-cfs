...
The competing gain-model (consolidation = hypermethylation) and loss-model (consolidation = hypomethylation at ProB repeats) are not ultimately contradictory. The unified vector model developed in Chapter @ch:causal-hierarchy-formal (@oq:consolidation-directionality) shows that DNMT3B redistribution --- a single mechanism --- produces gain at some loci and loss at others simultaneously. The gain and loss models are complementary manifestations of one underlying process, unified under the per-locus vector $bold(cal(M)) in [0, 1]^n$. The biological exposition below focuses on the loss-dominant subset of this unified model; for the full mathematical treatment including the DNMT3B zero-sum constraint ($sum_i Delta m_i = 0$), the irreversibility threshold $m_i^"crit"$, the derived $B_"strength"$ variable, tissue/cell-type indexing, and histone mark coupling, see @oq:consolidation-directionality.
