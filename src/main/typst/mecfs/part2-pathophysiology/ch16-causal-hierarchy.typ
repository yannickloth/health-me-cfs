...
#speculation(title: [Consolidation as Loss of Methylation])[
// Geneviève Fourel (personal communication, May 2026) proposed the question: what if
// epigenetic consolidation in ME/CFS is fundamentally about loss of DNA methylation?
// The ProA/ProB repeat sequence framework is from Bonnet, Hulo, Fourel et al. (2026 bioRxiv).
// ***CORRECTION (May 2026): Geneviève has clarified that the global DNMT3B redistribution
// model (DNMT3B transfer from B to A compartment) is proposed by her for cancer and likely
// chronic stimulation, NOT for ME/CFS. For ME/CFS, her proposal is: specific loss-meCpG
// at HSAT2 (and possibly HSAT3) in certain brain cell types initially, with propagation
// to other cells. Loss of DNMT3B activity at HSAT2 is a CONSEQUENCE of HSAT2 activation,
...
3. *Propagation.* Once established in initial cell populations, the HSAT2-active state may propagate to other cells, potentially via exosomal transmission mechanisms (Chapter @ch:immune-dysfunction).
...
- *ProB repeat sequence erosion as 3D genome mechanism:* Bonnet, Hulo, Fourel et al.\ (2026 preprint) demonstrated that CpG methylation loss disproportionately impacts ProB RepSeqs---repeat sequences that promote the B/heterochromatin compartment, including pericentromeric satellites, young LINE-1s, and selected ERVs---selectively weakening heterochromatin and causing genome-wide unfolding @BonnetFourel2026ProAB. Supplementary Data Figure 17 of that preprint shows direct experimental validation: GSATII (HSAT2) satellite repeat methylation is lost in liver cancer, confirming that satellite hypomethylation is a measurable, specific event in disease. In cancer, DNMT3B redistribution away from the B compartment drives targeted methylation erosion at these repeats, with compensatory Polycomb repression at some loci producing the characteristic "global hypo + focal hyper" bidirectional methylation pattern. For ME/CFS, Geneviève Fourel (pers. comm. May 2026) proposes a more targeted mechanism: specific loss-meCpG at HSAT2 (and possibly HSAT3) pericentromeric repeats in certain brain cell types initially, with propagation to other cells — rather than the global DNMT3B redistribution modeled for cancer. The key clarification: in this ME/CFS model, loss of DNMT3B activity at HSAT2 is a *consequence* of HSAT2 transcriptional activation, not the initial cause. What triggers HSAT2 activation? Recent work by Lahaye, Lomonte, Manel et al. (Cell 2025) demonstrates that herpesvirus proteins disrupt centromeres, triggering centromeric DNA amplification — a pathway termed VICAR (viral-induced centromeric DNA amplification and recognition) @Lahaye2025centromere. This centromere rearrangement is the proximate trigger for pericentromeric satellite transcription. The 2019 finding that HCMV IE1 and IE2 proteins are required for HSAT2 transcription @Nogalski2019HSATII is now interpretable as: IE1/IE2 are required for the centromere rearrangement, which in turn triggers HSAT2 transcription, rather than directly transactivating HSAT2.

- *ICF syndrome as Mendelian proof:* ICF syndrome (Immunodeficiency, Centromeric instability, Facial anomalies) is caused by DNMT3B mutations → pericentromeric hypomethylation → satellite repeat derepression → immune deficiency @Pappalardo2021repeats. This is a Mendelian confirmation that methylation loss at pericentromeric loci alone is sufficient to cause immune pathology. ME/CFS may represent a milder, acquired version: partial DNMT3B dysfunction (not mutation) → partial pericentromeric hypomethylation → subclinical immune exhaustion rather than overt immunodeficiency @deVega2021dna_methylation. Geneviève Fourel (pers. comm. May 2026) notes that in ME/CFS, the analogy to ICF is valid with one crucial distinction: in ICF the DNMT3B defect is the initial cause; in ME/CFS, perte d'activité DNMT3B at HSAT2 would be a *consequence* of HSAT2 activation, not the initial cause. The causal sequence in ME/CFS is: viral infection → centromere rearrangement / stress response → HSAT2 transcription → loss-meCpG at HSAT2 → propagation to other cells. The parallel to ICF lies in the shared endpoint (pericentromeric hypomethylation → immune pathology), not in the initiating mechanism.

*Certainty:* 0.45 for the bidirectional-with-loss-signal claim (multiple independent EWAS cohorts document both hyper- and hypomethylation; the dominant balance may depend on tissue and method; PTPRN2 hypomethylation survives correction @ChalderMoreau2026ptprn2; RRBS of PBMCs found 67.8% hypermethylated vs 32.2% hypomethylated in n=5 ME/CFS @Peppercorn2025methylation). 0.30 for the consolidation-as-loss *causal* claim (cross-sectional methylation data cannot establish direction; longitudinal methylation trajectories during disease progression are absent; the evidence is bidirectional, not loss-only). 0.20 for the treatment inversion (remethylation as therapeutic strategy is entirely untested in ME/CFS).
...
*Certainty:* 0.65 for the ICF mechanism (Mendelian genetics; satellite hypomethylation confirmed by multiple independent labs @Pappalardo2021repeats). 0.35 for the ME/CFS analogy (pericentromeric methylation status untested in ME/CFS; DNMT3B function not directly measured; ME/CFS immune phenotype is exhaustion, not deficiency).
