...
- *ProB repeat sequence erosion as 3D genome mechanism:* Bonnet, Hulo, Fourel et al.\ (2026 preprint) demonstrated that CpG methylation loss disproportionately impacts ProB RepSeqs---repeat sequences that promote the B/heterochromatin compartment, including pericentromeric satellites, young LINE-1s, and selected ERVs---selectively weakening heterochromatin and causing genome-wide unfolding @BonnetFourel2026ProAB. In cancer, DNMT3B redistribution away from the B compartment drives targeted methylation erosion at these repeats, with compensatory Polycomb repression at some loci producing the characteristic "global hypo + focal hyper" bidirectional methylation pattern. In ME/CFS, viral infection-induced DNMT3B dysfunction or redistribution could similarly produce selective loss of methylation at ProB repeats (pericentromeric satellites → HSAT2 derepression), while compensatory hypermethylation occurs at gene promoters in the A compartment. This framework unifies the bidirectional methylation pattern observed in ME/CFS under a single mechanism: *DNMT3B redistribution away from heterochromatin (ProB repeats) toward euchromatin (ProA elements), producing hypomethylation where silencing is needed and hypermethylation where activity is needed.*

- *ICF syndrome as Mendelian proof:* ICF syndrome (Immunodeficiency, Centromeric instability, Facial anomalies) is caused by DNMT3B mutations → pericentromeric hypomethylation → satellite repeat derepression → immune deficiency @Pappalardo2021repeats. This is a Mendelian confirmation that methylation loss at pericentromeric loci alone is sufficient to cause immune pathology. ME/CFS may represent a milder, acquired version: partial DNMT3B dysfunction (not mutation) → partial pericentromeric hypomethylation → subclinical immune exhaustion rather than overt immunodeficiency @deVega2021dna_methylation.
...
*Which patients would benefit.* Under the model, consolidation-as-loss is not universal but subgroup-specific. Three factors determine whether a patient carries this lock: (1) *Duration.* Consolidation deepens over the first 18--24 months of disease; recent-onset patients inside the intervention window retain epigenetic plasticity and do not yet have a lock to reverse. (2) *Entry pathway.* Patients who entered ME/CFS through inflammatory/metabolic doors (post-viral trigger, metabolic safe mode) sustain more DNMT3B-redistributing signals and are predicted to develop deeper ProB repeat erosion. Patients who entered through autoantibody-dominant pathways (GPCR autoantibodies as primary trigger) may have less inflammatory drive and therefore less epigenetic erosion. (3) *Severity.* The longest-duration, most severely affected patients---those who have already failed Tier 1 interventions and immunotherapy---are most likely to have deep consolidation. These patients have the worst prognosis and the most to gain.

*Treatments under the loss-model.* Methyl-donor support provides the safest default strategy pending resolution of the directionality question, since methyl donors support both methylation maintenance (preventing further ProB erosion) and de novo remethylation regardless of whether the pathology is gain- or loss-dominant. Already-listed treatments with relevant mechanisms: SAMe (methyl donor, 1600--3200 mg/day), methyl-folate + methyl-B12 (MTHFR bypass, cofactor substrates), riboflavin (MTHFR cofactor for C677T variant carriers), zinc (DNMT structural cofactor), and LDN (reduces DNMT3B-redistributing inflammatory signals). Treatments not yet listed but mechanistically consistent: betaine (alternative methyl donor via BHMT pathway, bypasses folate cycle). Treatments that are explicitly *contraindicated* if the loss-model is correct: HDAC inhibitors, TET pathway activators, 5-azacitidine, and any demethylating agent---all would worsen existing hypomethylation at ProB repeats. DNMT3A/B activators (RG-108 derivatives) and TET inhibitors (Bobcat339) are preclinical and carry oncogenic risk. Efficacy of any methylation-targeted therapy for ME/CFS symptoms remains unproven.
...
Existing ME/CFS methylation studies used mixed cell populations (PBMCs, saliva) and cross-sectional designs; cell-type-specific and longitudinal data are absent. The Peppercorn 2025 RRBS finding that hypermethylation dominates in PBMCs (67.8%) directly challenges the loss story and cannot be dismissed. The intervention window prediction () places 0.25--0.30 probability on the DNMT3B-redistribution mechanism being substantially correct for ME/CFS.

*Unresolved tensions with the paper's current model.* The paper's ODE formulation (Chapter @ch:causal-hierarchy-formal, Section @sec:lock-sequence) defines $cal(M)$ as driven *upward* by DNMT3A/B activity, models consolidation as methylation gain, and predicts that energy restoration must precede epigenetic intervention---with 5-azacitidine and HDAC inhibitors named as the epigenetic modifiers of choice. If consolidation is loss, all three commitments invert: $cal(M)$ would be driven *downward* by disease-state signals, the therapeutic agents recommended (demethylators) would worsen the very hypomethylation they should reverse, and remethylation might need to *precede* energy restoration (since restoration of heterochromatin silencing at ProB repeats may itself reduce the inflammatory signals driving energy dysfunction). Until this tension is resolved by longitudinal methylation data, both models coexist in the paper and should be read as competing hypotheses, not reconciled predictions.
