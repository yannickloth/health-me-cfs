...
*Falsifiable predictions.* (a) Post-thermal heart rate recovery (T90) in ME/CFS patients exceeds healthy controls by ≥50% at equivalent thermal dose. (b) T90 correlates with daily activity tolerance (accelerometer-measured step count the following day). (c) T90 shortens after treatments that improve autonomic function (e.g., pyridostigmine, low-dose propranolol, Waon therapy). (d) T90 does NOT correlate in patients whose PEM is primarily metabolic (Subtype A per Section @spec:ch14h-subtype-protocol), distinguishing vascular-autonomic from metabolic PEM.
...
*Falsifiable predictions.* Re-analysis of existing cold pressor datasets (e.g., De Becker 1998, Wyller 2007 — the latter measured thermoregulatory responses to hand cooling in 15 adolescent CFS patients @Wyller2007thermoregCFS) should reveal prolonged HR/baseline return time in CFS vs controls even when acute response magnitudes do not differ.
