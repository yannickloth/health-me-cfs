...
#open-question(title: [Stuck Doors Explain Simultaneous Over- and Under-Activity])[
ME/CFS presents a puzzling immunological paradox: the immune system appears simultaneously _overactive_ (chronic inflammation, elevated cytokines, persistent immune activation markers) and _underactive_ (impaired NK cell cytotoxicity, poor pathogen clearance, T cell exhaustion). How can both be true?
...
=== The TRPM3-GPCR Signaling Convergence Hypothesis

#open-question(title: [Autoantibodies and Ion Channels: Connected Dysfunction])[
...
*Possible connections:*
    ...
    - GPCR autoantibodies $arrow.r$ aberrant second messenger signaling $arrow.r$ altered TRPM3 phosphorylation $arrow.r$ channel dysfunction
    - Chronic receptor stimulation $arrow.r$ depletion of PIP#sub[2] (required for TRP channel function) $arrow.r$ reduced TRPM3 activity
    - Autoantibody-induced receptor internalization $arrow.r$ loss of TRPM3-regulating GPCR pathways $arrow.r$ unregulated channel states
    - Alternatively: shared autoimmune targeting of GPCRs and ion channels

If GPCR dysfunction and TRPM3 dysfunction are linked, therapies targeting autoantibodies (immunoadsorption, BC007, daratumumab) might restore both GPCR signaling _and_ TRPM3 function.
]
...
*Testable predictions.*
    ...
    - _In vitro_, adding ME/CFS patient IgG to healthy cells should impair TRPM3 responses
    - TRPM3 agonists might partially rescue function even in presence of autoantibodies

=== The Systemic Channelopathy Hypothesis

#open-question(title: [TRPM3 Dysfunction Beyond Immune Cells])[
The Sasso et al.\ study demonstrated TRPM3 dysfunction specifically in _immune cells_. However, TRPM3 is not limited to immune cells—it is expressed in:
...
What if TRPM3 dysfunction in ME/CFS is _systemic_—affecting all tissues where the channel is expressed?
...
*Proposed mechanism:*
    ...
    - The Sasso et al.\ study found TRPM3 dysfunction characterized as channels that fail to allow adequate calcium entry (“stuck doors”). However, ion channel dysfunction can theoretically manifest in multiple ways:
        ...
        - Stuck closed $arrow.r$ inability to respond to physiological stimuli (consistent with the study findings)
        - Stuck partially open $arrow.r$ chronic low-level calcium leak (speculative alternative)
        - Altered gating kinetics $arrow.r$ inappropriate timing of responses
    
    - In sensory neurons, a partially open channel would cause:
    
        - Baseline hyperexcitability
        - Lowered activation thresholds
        - Spontaneous firing $arrow.r$ restlessness, hypersensitivity
...
What if TRPM3 dysfunction _causes_ mitochondrial dysfunction?
...
*Proposed mechanism:*
    ...
    - Scenario B (stuck partially open): Chronic calcium leak
        ...
        - Mitochondrial calcium overload $arrow.r$ oxidative stress
...
=== The Post-Infectious TRPM3 Acquisition Hypothesis

#open-question(title: [How Does Infection Lead to Channel Dysfunction?])[
...
*Viral interference with ion channels.*
...
*Cofactor depletion.*
TRPM3 function may require specific cofactors or post-translational modifications. If infection depletes these (e.g., zinc, selenium, PIP#sub[2]), and they are not fully restored during recovery, channel function remains impaired.
]
...
=== TRPM3-Targeted Therapeutic Speculation
...
*Restoring channel environment:*
    ...
    - PIP#sub[2] repletion strategies (inositol supplementation?)
...
#warning-env(title: [Highly Speculative Therapeutics])[
...
The TRPM3 findings may help define ME/CFS subgroups:
    ...
    - *TRPM3 + autoantibody positive*: Combined channelopathy and autoimmune; may need immunomodulation _plus_ channel restoration
...
=== TRPV1-Thermal-Autonomic Axis: Thermal Challenge as Controlled Autonomic Stress Test
<sec:trpv1-thermal-autonomic-stress-test>

#open-question(title: [Post-Thermal Autonomic Recovery Time as At-Home Autonomic Stress Test for ME/CFS])[
An exogenous thermal challenge — sauna exposure, cold-water hand immersion, or a brief shower at controlled temperature — produces a predictable autonomic response in healthy individuals: skin temperature change activates TRPV1-expressing sensory neurons, which drive sympathetic outflow and measurable changes in heart rate, blood pressure, and skin blood flow @Larson2023TRPV1bezoldjarisch. When the thermal stimulus is removed, autonomic parameters return to baseline within minutes. This return-to-baseline kinetics reflects the recovery capacity of the autonomic nervous system.

In ME/CFS, where autonomic dysfunction is well-documented (Chapter @ch:cardiovascular) and post-exercise parasympathetic reactivation is impaired @Nelson2021HRVrecoveryMECFS @Oosterwijck2017autonomicPEM, the recovery trajectory after a standardized thermal challenge may be prolonged — and this prolongation may correlate with the patient's PEM threshold. The transition from a thermal challenge to a controlled autonomic stress test requires only three measurements obtainable at home: (1) baseline heart rate and skin temperature, (2) a standardized thermal challenge (e.g., a 60°C sauna room × 10 minutes, or a 15°C cold-water hand immersion × 60 seconds), and (3) time-series heart rate and skin temperature recording during recovery (30--60 minutes). The primary endpoint is *time to 90% baseline recovery* (T90) in heart rate. The secondary endpoint is skin temperature recovery kinetics.

*Biological rationale layered on existing paper content.* The paper has already established that TRPV1-expressing sensory neurons drive sympathetic outflow (@hyp:arteriolar-trpv1-pem), that post-exercise heart rate recovery (HRR) is impaired in ME/CFS (@Nelson2021HRVrecoveryMECFS — HRR ≤ 34.5 bpm discriminatory, ROC AUC 74.8%), that cold pressor testing reveals sympathetic overactivity @DeBecker1998coldpressorCFS, and that wearable HRV suppression post-exercise predicts PEM threshold in Long COVID @Ruijgt2026wearableHRVLongCOVID. A thermal challenge functions as a *calibrated* autonomic stressor — unlike exercise, which introduces confounding variables (muscle damage, metabolite accumulation, central fatigue), a thermal stimulus is a single-modality provocation with a known molecular target (TRPV1), predictable dose-response (temperature × time), and minimal tissue damage at moderate intensities. If post-thermal autonomic recovery time is prolonged in ME/CFS, and if this prolongation varies by disease severity and predicts daily PEM threshold, the thermal recovery index becomes a low-cost, repeatable at-home autonomic stress test.

*Precedent.* De Becker et al. (1998) applied cold pressor testing (hand immersion in 4°C water) to 21 CFS patients and 13 healthy controls and found sympathetic overactivity at rest and a trend toward higher heart rate during the cold challenge (non-significant in n=21), but did not measure *recovery* kinetics @DeBecker1998coldpressorCFS. Soejima et al. (2015) demonstrated that Waon therapy (60°C sauna × 15 minutes, daily for 4 weeks) was safe in 10 ME/CFS patients and improved fatigue and mood, but measured only subjective outcomes — no autonomic recovery endpoints @Soejima2015WaonTherapyCFS. Van Campen et al. (2021) showed that cerebral blood flow recovery post-tilt is delayed in ME/CFS in a severity-graded manner (severe patients: CBF still −25% after 5 minutes supine recovery), establishing that *autonomic recovery time specifically* is a severity-correlated physiological parameter @VanCampenEtAl2021. The thermal challenge extends this logic to a simpler, drug-free provocation modality.

*Motivation from CPET null.* Mancini et al. (2026) conducted the largest 2-day CPET replication in ME/CFS (n=58) and found no significant Day 1→Day 2 decline in peak VO#sub[2] or VO#sub[2] at VT @Mancini2026CPETrecoveryNull. This null result — from a well-powered, rigorously controlled study — does not refute the existence of PEM but challenges 2-day CPET as a reliable measurement tool. A thermal autonomic stress test avoids the methodological problems of 2-day CPET: it requires no maximal effort, introduces no muscle damage confounding, and can be repeated frequently without PEM risk at moderate intensities.

*What the test does NOT claim.* Thermal challenge is not proposed as a diagnostic test for ME/CFS (thermal intolerance is non-specific — 81.3% of Long COVID patients report it without meeting ME/CFS criteria @Kouyoumdjian2025LCvsMECFSthermoreg). It is proposed as a *within-patient monitoring tool*: does my recovery time change after treatment X? Has my PEM threshold improved or worsened? Longitudinal tracking could detect functional changes before subjective reports do.

*Falsifiable predictions.* (a) Post-thermal heart rate recovery (T90) in ME/CFS patients exceeds healthy controls by ≥50% at equivalent thermal dose. *Falsified if:* T90 ratio ≤ 1.3 (patients recover no more than 30% slower) or if the group difference disappears after adjusting for baseline HR (ANCOVA: group effect p > 0.10). (b) T90 correlates with daily activity tolerance (accelerometer-measured step count the following day; Spearman ρ ≥ 0.4, n ≥ 40). *Falsified if:* ρ < 0.2 with 95% CI excluding ρ ≥ 0.3. (c) T90 shortens after treatments that improve autonomic function (e.g., pyridostigmine, low-dose propranolol, Waon therapy), with treatment effect detectable as within-subject reduction ≥ 15% from baseline at ≥ 4 weeks. (d) T90 does NOT correlate in patients whose PEM is primarily metabolic (Subtype A per Section @spec:ch14h-subtype-protocol), distinguishing vascular-autonomic from metabolic PEM. *Falsified if:* T90-step count correlation is equivalent in Subtype A and non-A patients (difference in Fisher z-transformed r ≤ 0.1, equivalence margin).

(Certainty: 0.25. No direct study of post-thermal autonomic recovery time exists in ME/CFS. The mechanistic components are individually supported: TRPV1→sympathetic outflow established in animal models @Larson2023TRPV1bezoldjarisch; HRR impaired in ME/CFS @Nelson2021HRVrecoveryMECFS; wearable HRV predicts PEM threshold in Long COVID @Ruijgt2026wearableHRVLongCOVID; post-tilt recovery time is delayed in ME/CFS @VanCampenEtAl2021; cold pressor reveals sympathetic overactivity @DeBecker1998coldpressorCFS. The combined protocol is constructed from converging but separate lines of evidence — zero direct human data on thermal recovery time in ME/CFS. Origin: brainstorm — cross-domain synthesis, not primary literature. Testable in principle with consumer equipment; no validated protocol exists.)
...
#speculation(title: [Cold Pressor → Autonomic Recovery Convergence: A Low-Cost Alternative to 2-Day CPET])[
...
De Becker et al. (1998) measured blood pressure and heart rate during 90 seconds of foot immersion in 4°C water in 21 CFS patients and 13 controls @DeBecker1998coldpressorCFS. The acute responses showed only a trend (CFS patients had higher resting HR and marginally higher HR during the challenge — non-significant, possibly underpowered). If the same data were re-analyzed for *recovery kinetics* (time from challenge end to 90% baseline HR, slope of HR deceleration over the first 2 minutes post-challenge), a discriminating parameter might emerge from the same raw data. The mechanism predicts that patients with impaired parasympathetic reactivation (as documented post-exercise @Nelson2021HRVrecoveryMECFS) would show prolonged cold pressor recovery irrespective of the acute response magnitude.

A second line of convergent evidence comes from Long COVID: Ruijgt et al. (2026, n=121 Long COVID) demonstrated that wearable-measured HRV remains suppressed for 24 hours after exercise above the ventilatory threshold, and lower recovery HRV predicts PEM severity @Ruijgt2026wearableHRVLongCOVID. Cold pressor recovery measured over 30 minutes would capture the same autonomic recovery parameter on a shorter, safer timescale without exercise confounds.

*Falsifiable predictions.* (a) Retrospective re-analysis of existing cold pressor datasets (e.g., De Becker 1998, Wyller 2007 — the latter measured thermoregulatory responses to hand cooling in 15 adolescent CFS patients @Wyller2007thermoregCFS) should reveal a significant group×time interaction for HR and BP decay slope (ANCOVA: group×time interaction p < 0.05), with CFS patients showing prolonged return to baseline. *Falsified if:* group×time interaction p > 0.10 or if recovery time-series data were not recorded in the original studies (preventing re-analysis). (b) Prospective de novo cold pressor study (n ≥ 40 CFS, n ≥ 40 HC, 60-second hand immersion in 4°C water, 30-minute HR/HRV recovery monitoring) should yield a T90 ratio ≥ 2.0 in CFS vs controls. *Falsified if:* T90 ratio ≤ 1.4 or ANCOVA p > 0.10. If the acute response magnitude (ΔHR during cold exposure) is normal but T90 is prolonged, the recovery deficit is independent of afferent TRPV1 sensitivity and reflects efferent parasympathetic infrastructure damage.
...
*Structural subtype* --- Reduced VMAT2/vesicle number due to PTPRN2 dysregulation, normal ATP per vesicle. Requires transcriptional/epigenetic intervention (PTPRN2 targeting, possibly lithium, HDAC inhibitors). May respond better to droxidopa (bypasses DBH regardless of vesicle number) than to NRI agents (which require vesicles to fill and release). New VMAT2 PET evidence from long COVID (Observation @cf:vmat2-longcovid) provides the first direct empirical validation that VMAT2 loss can occur in a postviral illness, lending cross-condition support to the structural-subtype concept — though whether the PTPRN2-hypomethylated ME/CFS structural subtype maps onto the post-COVID VMAT2 PET reduction remains untested.
