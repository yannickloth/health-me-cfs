...
*PANS/PANDAS.*
    ...
    - Acute-onset OCD, tics, behavioural regression temporally linked to infection (most commonly streptococcal) @Swedo1998PANDAS50 @Chang2015PANSevaluation
    - Anti-neuronal antibodies cross-reacting with basal ganglia targets via molecular mimicry: GAS N-acetyl-beta-D-glucosamine antibodies recognise neuronal lysoganglioside, activating CaMKII in dopaminergic neurons @Kirvan2003MimicryChorea @Kirvan2006CaMKII @Cesaroni2026PANDASconundrum
...
For the paper's purposes, PANS/PANDAS provides Architecture B's missing piece: a parallel disease domain in which the infection-to-autoantibody-to-neuropsychiatric-phenotype pipeline has been biochemically characterised at the molecular level, and in which immunotherapy response — partial, stratified, contested, but real — has been documented. The circuit is different (basal ganglia in PANDAS vs\ diffuse neuroinflammatory/thalamocortical/metabolic in ME/CFS), but the disease logic is identical: infection triggers an immune response that produces neuropsychiatric symptoms that syndromic classification attributes to a "different" disorder.

*Consequence:* PANS/PANDAS lowers the plausibility threshold for Architecture B in ME/CFS: if streptococcal molecular mimicry can produce acquired OCD indistinguishable from developmental OCD, the claim that ME/CFS neuroinflammation can produce acquired inattention indistinguishable from developmental ADHD is not unprecedented — it has a well-characterised parallel one synapse over.

#speculation(title: [PANS/PANDAS as Architecture B Existence Proof for Acquired Neuropsychiatric Phenotypes])[
*Certainty: 0.35.* The PANS/PANDAS literature — molecular mimicry at the receptor level @Kirvan2003MimicryChorea, immunotherapy response in a subset @Latimer2015TPA @Kovacevic2015IVIG, and clinical recognition of immune-mediated acquired behavioural regression @Masterson2025PANSclassification @LaBella2023PANDASreview — constitutes a worked example of Architecture B in a closely related disease domain. The mechanism differs from the ME/CFS-proposed cascade (streptococcal→basal ganglia in PANDAS vs\ viral/immune→diffuse neuroinflammatory→thalamocortical/metabolic in ME/CFS), but the disease logic — infection→immune response→neuropsychiatric phenotype→potentially reversible with immunomodulation — is identical. The PANDAS controversy's resolution through stratification (some patients are immune-mediated, some are not) prefigures the same resolution the present paper advocates for ME/CFS-associated ADHD/ASD-like features. *Falsification condition:* If systematic anti-neuronal antibody screening in acute-paediatric-onset ME/CFS finds zero patients with elevated anti-basal ganglia, anti-dopamine receptor, or anti-CaMKII antibodies, the PANDAS analogy for ME/CFS-specific Architecture B is weakened (though the disease-logic parallel survives). *Limitations:* PANDAS is paediatric, streptococcal-triggered, and basal-ganglia-specific — all three diverge from the proposed ME/CFS Architecture B mechanism. The analogy strengthens Architecture B's plausibility without directly testing it. The PANDAS immunotherapy RCT was underpowered and its GAS+ subgroup analysis is hypothesis-generating, not confirmatory @Williams2016IVIGRCT. No study has examined whether PANDAS/PANS resolves into an ME/CFS-like state.

*Consequence:* A clinician observing new-onset inattention or sensory rigidity in an ME/CFS patient should consider that the same disease logic that produces acquired OCD in PANDAS may produce acquired ADHD-like or ASD-like features in ME/CFS — the question is not "does this patient have ADHD?" but "is this patient's inattention immune-mediated?" The treatment implication (speculative, not yet tested) is that immunotherapy may help some patients with ME/CFS-associated neuropsychiatric features, just as it helps some PANDAS patients — but only the immune-mediated subset, and the PANDAS lesson is that selecting that subset is the hard part.
