...
Taken together, the dementia and mental-illness meta-analyses @Kuring2026InflammatoryDementia @Kuring2023InflammatoryMood and the CFS inflammatory-protein meta-analysis @Strawbridge2019CFSInflammatory establish three convergent cross-disease points relevant to ME/CFS. First, low-grade inflammation can be dissociated from psychiatric comorbidity — and disease-inherent markers survive that exclusion — supporting the inflammatory component of ME/CFS as real rather than purely conferred by coexisting mood or inflammatory disorders (@spec:bidirectional-mood-inflammation-template; @lim:dementia-comorbidity-masking). Second, the causal direction between mental state and inflammation is not resolved and may run in both directions; the dementia template (mental-illness inflammation → CNS pathology) and the neuroinflammation-cascade model (@spec:neuroinflammation-cascade-generator) are two poles of one unresolved bidirectional loop rather than competing explanations. Third, a specific measurable Th17/myeloid axis (G-CSF, GM-CSF, IL-17 family) appears shared across chronic inflammatory-CNS conditions, including an overlap with severity-correlated ME/CFS markers, and is worth testing as a distinct biomarker axis rather than as nonspecific "inflammation" (@spec:th17-myeloid-shared-signal). None of these claims is yet confirmed in comorbidity-screened ME/CFS cohorts, and the specific dementia markers do not transfer directly to ME/CFS; the value is a template, a methodological warning, and a specific testable axis, not a proven ME/CFS mechanism.

*Consequence:* together these cross-disease parallels suggest that a shared, measurable immune axis — not vague inflammation — may be a productive ME/CFS biomarker and stratification target, and that mental and inflammatory states in ME/CFS should be considered coupled rather than one always causing the other. ] <syn:dementia-inflammation-template>
