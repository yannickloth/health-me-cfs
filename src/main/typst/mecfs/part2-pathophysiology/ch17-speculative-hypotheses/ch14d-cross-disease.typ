...
*PANS/PANDAS.*
    ...
    - Anti-neuronal antibodies cross-reacting with basal ganglia targets via molecular mimicry: GAS N-acetyl-beta-D-glucosamine antibodies recognize neuronal lysoganglioside, activating CaMKII in dopaminergic neurons @Kirvan2003MimicryChorea @Kirvan2006CaMKII @Cesaroni2026PANDASconundrum
    - Some patients respond dramatically to IVIG or therapeutic plasma exchange (open-label 83--89%, RCT equivocal overall but GAS+ subgroup better) @Latimer2015TPA @Kovacevic2015IVIG @Williams2016IVIGRCT
    ...
    - The maternal immune activation literature provides a prenatal variant of this spectrum: ~23% of ASD cases are attributable to maternal anti-fetal brain autoantibodies targeting specific neuronal proteins, defining a distinct immune-mechanism ASD subgroup @Braunschweig2013MATAbs @Croen2008MaternalAbASD @Meltzer2017ImmuneASD. The postnatal equivalent — infection-triggered anti-neuronal antibodies producing autism-like symptoms in previously neurotypical children — is structurally identical to PANDAS and to the proposed ME/CFS Architecture B, differing only in which developmental window the immune insult occurs and which circuits it targets
...
*PANS/PANDAS.* A second, stronger line comes from a different disease domain whose mechanism is characterized at the molecular level. Pediatric Acute-onset Neuropsychiatric Syndrome (PANS) and its streptococcal-triggered subtype (PANDAS) are defined by abrupt-onset OCD, tics, and behavioral regression temporally linked to infection — most commonly group A streptococcal (GAS) pharyngitis @Swedo1998PANDAS50. The diagnostic criteria require acute, dramatic symptom onset with a documented infectious or inflammatory trigger, producing a clinical picture that can closely resemble severe autism exacerbation or late-presenting neurodevelopmental regression @Chang2015PANSevaluation @Masterson2025PANSclassification. Crucially, the molecular mechanism has been traced: GAS-directed antibodies cross-react with neuronal lysoganglioside in the basal ganglia via molecular mimicry, activating calcium/calmodulin-dependent protein kinase II (CaMKII) in dopaminergic neurons and producing circuit-specific dysfunction in the same basal ganglia-thalamocortical loops that subserve behavioral control @Kirvan2003MimicryChorea @Kirvan2006CaMKII @Cesaroni2026PANDASconundrum.

PANS/PANDAS provides an existence proof for Architecture B that ME/CFS alone cannot yet offer: (1) a characterized infection-to-autoantibody pipeline (GAS → anti-neuronal IgG → CaMKII activation), (2) an acquired neuropsychiatric phenotype (OCD, tics, regression) that is indistinguishable from developmental neuropsychiatric conditions at the symptomatic level, and (3) documented immunotherapy responsiveness — a subset of PANS/PANDAS patients improve dramatically with intravenous immunoglobulin (IVIG) or therapeutic plasma exchange @Latimer2015TPA @Frankovich2017PANSimmunotherapy, establishing that the symptoms were immune-mediated, not neurodevelopmental. The treatment response pattern is informative: open-label series report 83--89% response rates @Latimer2015TPA @Kovacevic2015IVIG, but the single double-blind RCT ($n = 35$) found IVIG not significantly superior to placebo overall, with a post-hoc GAS-positive subgroup showing greater response @Williams2016IVIGRCT. A systematic review rated overall evidence quality as low, citing study heterogeneity @Sigra2018SystematicReview. The pattern — dramatic responders alongside non-responders, positive unblinded series but equivocal RCT — is structurally identical to the immunotherapy data in ME/CFS and reflects the same underlying problem: syndromic inclusion criteria capture a biologically heterogeneous population, and immunotherapy only works in the immune-mediated subset @LaBella2023PANDASreview.

The PANDAS controversy itself reinforces the Architecture B argument. Whether PANDAS "really exists" as a distinct entity has been debated for two decades @Chiarello2017Controversy @Cesaroni2026PANDASconundrum. The debate is structurally identical to the historical polarization over ME/CFS: one side argues the syndrome is biologically real and immune-mediated (the Swedo/Frankovich line), the other argues that temporally coincident infections and OCD are common enough that overlap proves nothing (the Gilbert/Singer line). The resolution — emerging from the Cunningham Panel era and the Stanford PANS clinic's 10-year data @Masterson2025PANSclassification — is that the debate is mis-framed. The question is not whether PANDAS is a "real disease" but what proportion of acute-onset paediatric neuropsychiatric presentations are immune-mediated and would respond to immunomodulation. This is precisely the question the present paper asks about ME/CFS and acquired ADHD/ASD-like features.

For the paper's purposes, PANS/PANDAS provides Architecture B's missing piece: a parallel disease domain in which the infection-to-autoantibody-to-neuropsychiatric-phenotype pipeline has been biochemically characterized at the molecular level, and in which immunotherapy response — partial, stratified, contested, but real — has been documented. The circuit is different (basal ganglia in PANDAS vs\ diffuse neuroinflammatory/thalamocortical/metabolic in ME/CFS), but the disease logic is identical: infection triggers an immune response that produces neuropsychiatric symptoms that syndromic classification attributes to a "different" disorder.

*Consequence:* PANS/PANDAS lowers the plausibility threshold for Architecture B in ME/CFS: if streptococcal molecular mimicry can produce acquired OCD indistinguishable from developmental OCD, the claim that ME/CFS neuroinflammation can produce acquired inattention indistinguishable from developmental ADHD is not unprecedented — it has a well-characterized parallel one synapse over.

#speculation(title: [PANS/PANDAS as Architecture B Existence Proof for Acquired Neuropsychiatric Phenotypes])[
*Certainty: 0.35.* The PANS/PANDAS literature — molecular mimicry at the receptor level @Kirvan2003MimicryChorea, immunotherapy response in a subset @Latimer2015TPA @Kovacevic2015IVIG, and clinical recognition of immune-mediated acquired behavioral regression @Masterson2025PANSclassification @LaBella2023PANDASreview — constitutes a worked example of Architecture B in a closely related disease domain. The mechanism differs from the ME/CFS-proposed cascade (streptococcal→basal ganglia in PANDAS vs\ viral/immune→diffuse neuroinflammatory→thalamocortical/metabolic in ME/CFS), but the disease logic — infection→immune response→neuropsychiatric phenotype→potentially reversible with immunomodulation — is identical. The PANDAS controversy's resolution through stratification (some patients are immune-mediated, some are not) prefigures the same resolution the present paper advocates for ME/CFS-associated ADHD/ASD-like features.

*Falsification condition:* In a cohort of n ≥ 80 ME/CFS patients with documented post-infectious onset (≤ 3 months from infection to ME/CFS symptom onset) and new-onset inattention or executive dysfunction post-dating ME/CFS onset, systematic anti-neuronal antibody screening (anti-basal ganglia, anti-D1/D2 receptor, anti-CaMKII, anti-lysoganglioside IgG) versus matched healthy controls (n ≥ 80) will show enrichment of ≥ 1 specificity at ≥ 2× control prevalence (p < 0.05 after FDR correction for the panel size). *Falsified if* the panel shows no significant enrichment (all q > 0.30) and the antibody burden does not correlate with ADHD-symptom-severity change-from-premorbid-baseline (Spearman ρ < 0.15). A null result falsifies the claim that the PANDAS mechanism — infection-triggered anti-neuronal antibodies producing acquired neuropsychiatric symptoms — generalizes to ME/CFS-associated inattention. (The PANDAS literature remains an existence proof that immune-mediated acquired neuropsychiatric phenotypes are possible in humans regardless of ME/CFS outcome; the speculation tested here is the claim that this same mechanism operates in the ME/CFS-ADHD overlap population.)
