...
*Long COVID.* The most obvious parallel:
    ...
    - _Epidemiological grounding:_ a large electronic health record cohort ($n = 147{,}377$) found elevated new-onset ME/CFS risk persisting up to four years after SARS-CoV-2 infection (adjusted hazard ratio 1.46 hospitalized, 1.56 non-hospitalized) @Hadidchi2025CFSrisk, and the RECOVER study found 51% of long COVID patients meet ME/CFS criteria @RECOVER2025 — together indicating that the SARS-CoV-2 → ME/CFS link is not a transient phenomenon but a sustained post-infectious outcome
...
*Treatment-response convergence: a strong shared-mechanism signal.*
Despite possible PEM-divergence, patient-reported *treatment responses* strongly converge across the two conditions. In the largest treatment-outcome survey to date (Eckey et al.\ 2025, $n = 3{,}925$ ME/CFS and long COVID patients, 150+ treatments), the Net Assessment Scores of treatments were highly correlated between ME/CFS and long COVID patients ($R^2 = 0.68$) @Eckey2025PatientReported. The same four symptom/comorbidity clusters emerged largely independent of diagnosis label, and disease severity (capacity) — not the ME/CFS-versus-long-COVID label — was the strongest predictor of treatment response. Shared cluster membership predicted shared treatment benefit (e.g., a POTS-dominant patient benefited from autonomic modulators regardless of originating diagnosis). Only two treatments (midodrine; benfotiamine/thiamine) showed significantly different responses between the conditions @Eckey2025PatientReported. This treatment-response convergence is functionally parallel to the symptom convergence documented above and argues that shared pathophysiology outweighs condition-specific differences for the majority of patients, even where a specific physiological signature (such as CPET-2) diverges.
...
#speculation(title: [Peripheral Blood Methylome as a Long COVID vs ME/CFS Distinguishing Signature])[
*Certainty: 0.52.* The peripheral blood mononuclear cell (PBMC) methylome separates ME/CFS, Long COVID (LC), and healthy controls into three distinct clusters by principal component analysis, while also showing a largely shared epigenetic makeup between the two conditions @Peppercorn2025methylation. In a direct age/sex-matched RRBS comparison (n=5 each), ME/CFS had 214 differentially methylated fragments (DMFs) vs healthy controls and Long COVID had 429 (both a methylation difference of more than 10%, p < 0.05); 118 DMFs were shared, with a Pearson correlation of 0.88 between the two disease cohorts, but 26 of the shared fragments differed by more than 10% between the two conditions and 6 showed opposite-direction methylation (five hypermethylated in LC and hypomethylated in ME/CFS, one the reverse) @Peppercorn2025methylation. This bidirectional-overlap-plus-specific-signature pattern is consistent with the paper's per-locus vector model of directional methylation change (@oq:consolidation-directionality). The shared makeup favors treating ME/CFS and LC as mechanistically overlapping; the six opposite-direction loci (e.g., the FGD2 promoter hypermethylated in LC but hypomethylated in ME/CFS) are candidate condition-specific epigenetic signatures that could aid differential diagnosis and stratification, though unvalidated.

*Alternative interpretations / competing mechanisms* (Standing Epistemic #4). The apparent LC-vs-ME/CFS divergence may reflect disease *stage* rather than condition identity: the LC cohort was ~1 year from onset while the ME/CFS cohort averaged ~12 years, so LC's larger DMF count could represent an earlier, more dynamic methylome that later settles into the more stable ME/CFS profile (or vice versa) @Peppercorn2025methylation. SARS-CoV-2-specific effects (rather than any post-infectious mechanism) could also drive the LC-specific loci. The six opposite-direction DMFs rest on n=5 per cohort and are not validated for diagnosis, so they are hypothesis-generating, not diagnostic.
...
=== Skeletal Asymmetry as a Distinct Mechanical Driver
...
- *Sympathetic chain vulnerability.* Schulte et al. (2010, @Schulte2010SympathChainScoliosis) documented sympathetic chain dysfunction (confirmed by sudomotor testing) in 12/31 patients following anterior scoliosis surgery — establishing that the thoracic sympathetic chain is anatomically vulnerable to vertebral pathology. Whether idiopathic spinal asymmetry produces similar autonomic deficits without surgical trauma is entirely untested.
- *Scoliosis-autonomic co-occurrence.* In Rett syndrome (n=913), severe scoliosis (>40°) occurs in 27% of patients alongside autonomic dysfunction driven by MECP2 mutations @Killian2017ScoliosisRett. This provides a disease model where scoliosis and dysautonomia co-occur, but does not establish scoliosis as the cause.
- *Compensatory-overuse precedent.* Post-polio syndrome fatigue is driven by metabolic exhaustion of surviving motor neurons compensating for lost ones @Jubelt2004PostPolio — establishing a precedent for compensatory overuse as a fatigue mechanism, though the lesion (motor neuron loss) differs fundamentally from skeletal asymmetry.
- *Bidirectional ANS-bone regulation.* The SNS regulates bone remodeling via $beta$#sub[2]-adrenergic receptors on osteoblasts @He2014AutonomicBone, establishing that ANS and skeletal structure interact — but the direction tested is ANS→bone, not bone→ANS.
- *LLD treatment gap.* A systematic review of shoe lifts for leg length discrepancy found they reduce low back pain (1 RCT only, low-quality evidence) but no study assessed fatigue as an outcome @Campbell2019ShoeLiftLLD.
- *Null signal.* Leg strength asymmetry was NOT associated with fatigue or fatigability in multiple sclerosis (n=15) @Proessl2018LegAsymmetryMS — the only study to test an asymmetry→fatigue pathway in a neurological disease found no association, suggesting additional factors are required.

*Four-pathway mechanistic model (provisional, untested):*

*Pathway 1 — Compensatory muscle overuse.* A pelvic obliquity of 10–15 mm (common in mild LLD) requires the ipsilateral hip abductors, contralateral lumbar extensors, and cervical stabilizers to work continuously to maintain head and eye level. In healthy individuals, this imposes a ~5–10% increase in basal energy expenditure during standing and walking (biomechanical modeling, no ME/CFS data). In a patient whose cellular energy metabolism is already compromised — whether by WASF3-mediated supercomplex disruption @wang2023wasf3, PDH inhibition, or AMPK suppression — this constant additional demand may be sufficient to breach the critical ATP threshold (@sec:sodium-calcium-cascade), triggering PEM from trivial activity. This provides a mechanical explanation for why some patients crash from mere standing or slow walking: their baseline energy consumption is elevated by postural compensation before any voluntary activity begins.

*Pathway 2 — Sympathetic chain irritation.* The paravertebral sympathetic chain runs along the anterolateral aspect of vertebral bodies from T1 to L2. In rotoscoliosis, vertebral rotation displaces the chain asymmetrically; osteophytic encroachment from degenerative changes may directly compress ganglia. Schulte et al. @Schulte2010SympathChainScoliosis demonstrated that surgical manipulation of this region causes measurable sudomotor dysfunction — a proof of principle that mechanical disruption of the sympathetic chain alters autonomic output. Spontaneous mechanical irritation (rather than surgical transection) would produce a more complex picture: intermittent compression → irritative firing (sympathetic hyperactivity, hyperadrenergic POTS-like state); chronic compression → conduction block → compensatory denervation hypersensitivity. The multi-organ distribution follows sympathetic anatomy: T5–T9 → splanchnic nerves → digestive, hepatic; T10–L1 → renal; T1–T4 → cardiac, pulmonary.

*Pathway 3 — Mechanical nerve compression and sensitization.* Asymmetric pelvic tilt produces asymmetric fascial tension throughout the myofascial chains. Fascial thickening and loss of glide planes (documented in chronic pain via ultrasound elastography) can entrap peripheral nerves against bony structures. The lateral femoral cutaneous nerve (meralgia paresthetica), sciatic nerve (piriformis syndrome), and pudendal nerve (Alcock canal syndrome) are mechanically vulnerable to pelvic asymmetry. Chronic low-grade compression produces a distinct clinical picture from acute entrapment: fluctuating paresthesia, positional allodynia, and a sensation of "wrongness" rather than lancinating pain. Central sensitization (@sec:ch15-central-sensitization) amplifies these peripheral signals, producing the diffuse hypersensitivity pattern characteristic of ME/CFS.

*Pathway 4 — Cervical aging spiral.* Skeletal asymmetry below the cervical spine alters head position: a pelvic obliquity is compensated by lumbar scoliosis → thoracic countercurve → cervical tilt to maintain horizontal gaze. This imposes asymmetric loading on cervical facet joints and uncovertebral joints, accelerating osteoarthritic degeneration. With age, osteophyte formation narrows the intervertebral foramina and the craniocervical space, progressively worsening any pre-existing CCI or creating it de novo. Unlike the acute CCI from ligamentous laxity (@spec:ch08-sympathetic-ganglion-compression), this is a slow mechanical progression — a "wear and tear" CCI that worsens with age, explaining the clinical observation that some ME/CFS patients deteriorate in their 40s–60s despite stable disease activity.

*How does the asymmetry originate? Competing hypotheses.*
The four pathways describe the cascade once skeletal asymmetry is established, but its origin is an open question with multiple plausible explanations:
...
*Missing mechanistic link: mechanotransduction.* The four-pathway model posits that compensatory muscle overuse drives anaerobic metabolism, but does not specify how sustained mechanical load becomes mitochondrial failure. Muscle cells express mechanosensitive ion channels (TRPV4, Piezo1) that open under sustained stretch, admitting Ca#super[2+]. In ME/CFS muscle where baseline Ca#super[2+] handling is already dysregulated, sustained stretch-induced Ca#super[2+] influx may convert a normally adaptive signal (CaMKII→PGC-1α→mitochondrial biogenesis) into the pathological Na#super[+]/Ca#super[2+] overload cascade. This hypothesis would be falsified if TRPV4/Piezo1 expression and stretch-induced Ca#super[2+] responses are normal in ME/CFS muscle biopsies. (Origin: brainstorm.)
] <spec:skeletal-asymmetry-cascade>

#limitation(title: [Skeletal Asymmetry Hypothesis: Evidence Base is Near-Zero])[
The causal chain proposed in @spec:skeletal-asymmetry-cascade has no direct ME/CFS evidence at any link. The literature base consists of: one surgical complication study (n=31, thoracic approach, not idiopathic scoliosis) @Schulte2010SympathChainScoliosis; one genetic disease model (Rett syndrome, MECP2-driven, n=913) @Killian2017ScoliosisRett; one review establishing the concept of compensatory-overuse fatigue (post-polio, motor neuron lesion, not skeletal asymmetry) @Jubelt2004PostPolio; one systematic review of shoe lifts for LLD (low-quality, fatigue not assessed) @Campbell2019ShoeLiftLLD; and one null study (asymmetry not associated with fatigue in MS, n=15) @Proessl2018LegAsymmetryMS. None of these directly support the hypothesis. Furthermore, the clinical entities DAMI and ILMI yield zero PubMed results — they are observational concepts from French clinical practice, not validated measurement constructs. This hypothesis should be considered a structured clinical intuition meriting systematic investigation, not a mechanism with evidentiary support. The 0.10 certainty reflects the near-total absence of direct evidence, not a negative assessment of the hypothesis's plausibility.
...
*Directionality reversal.* The entire four-pathway model assumes skeletal asymmetry precedes ME/CFS. But deconditioning from severe ME/CFS — prolonged bedrest, muscle wasting, loss of postural tone — predictably produces *acquired* functional asymmetry (pelvic tilt from muscle imbalance, antalgic scoliosis from asymmetric pain). The Proessl 2018 null result in MS @Proessl2018LegAsymmetryMS further suggests that asymmetry alone, even in a neurologically compromised patient, does not cause fatigue. Distinguishing congenital/developmental asymmetry from acquired post-morbid asymmetry requires pre-illness radiographs or longitudinal post-infectious cohorts — neither exists.
...
*Rett syndrome model is a common-cause not a causal-chain model.* The co-occurrence of scoliosis and autonomic dysfunction in Rett syndrome @Killian2017ScoliosisRett is driven by a shared genetic cause (MECP2), not by scoliosis causing dysautonomia. The Rett model supports that skeletal and autonomic abnormalities can co-occur in a single disease, but does not support the directional claim that skeletal asymmetry drives autonomic dysfunction.

*Schulte2010 is a surgical lesion, not a spontaneous asymmetry model.* The documented sympathetic chain dysfunction @Schulte2010SympathChainScoliosis occurred after anterior surgical approach to the thoracic spine — a controlled mechanical disruption qualitatively different from the low-grade, chronic, intermittent compression hypothesised for idiopathic skeletal asymmetry. Surgical transection is to chronic compression what amputation is to neuropathy: an extreme case that demonstrates vulnerability but does not model the natural disease.
] <lim:skeletal-asymmetry-evidence-gap>

#speculation(title: [Skeletal Asymmetry as a Convergent Mechanical Framework: Synthesis])[
*Certainty: 0.10.* The four-pathway model (@spec:skeletal-asymmetry-cascade), the thoracolumbar sympathetic extension (@spec:ch08-thoracolumbar-sympathetic-irritation), the mechanical/postural subgroup definition (@spec:mechanical-postural-subgroup), and the mechanical nerve compression mechanism (Chapter @ch:symptom-mechanisms) collectively propose that fixed skeletal asymmetry — regardless of whether its origin is developmental (Gerlier neonatal subluxation hypothesis), congenital, acquired, or idiopathic — may act as a distinct upstream driver of ME/CFS pathophysiology that operates through *mechanical* rather than biochemical or immunological routes. This framework is structurally independent of the connective tissue/hEDS/CCI model: it requires normal collagen but abnormal bone geometry. The two are not mutually exclusive — a patient may have both hypermobile joints and asymmetric bones, compounding the mechanical burden. The framework's primary weakness is the near-total absence of direct ME/CFS evidence (@lim:skeletal-asymmetry-evidence-gap); its primary strength is that it generates specific, low-cost, falsifiable predictions — EOS full-spine radiography + COMPASS-31 in n=200 ME/CFS patients, n-of-1 shoe lift autonomic trials, and prospective post-infectious cohorts with pre-existing asymmetry measurement — using technology that already exists. If any of these predictions is confirmed, a structural, non-pharmacological treatment pathway (postural correction, targeted physiotherapy, orthotics) would become testable for a subgroup currently unrecognized by any existing ME/CFS model. (Origin: synthesis — Gerlier 2026-07-21 core hypothesis, refined by brainstorm.)
...
- Basal ganglia-directed autoantibodies (anti-D1/D2, anti-lysoganglioside, anti-tubulin) produce OCD, tics, and behavioral regression — the PANDAS/PANS end of the spectrum @Kirvan2003MimicryChorea @Kirvan2006CaMKII
- Brainstem/thalamic-directed autoantibodies (anti-adrenergic, anti-muscarinic) produce fatigue, autonomic instability, and orthostatic intolerance — the ME/CFS-POTS end @LaBella2023PANDASreview
...
*Severity applicability:* Unknown — the NES framework is a disease-class proposal, not a clinical staging model. Individual conditions within NES have their own severity spectra (e.g. mild PANS vs progressive PANS @Masterson2025PANSclassification; mild ME/CFS vs very severe).

*Limitations:* Entirely speculative. No multi-disease anti-neuronal antibody profiling study exists. The specific circuit-target → phenotype mapping is inferred from existing single-disease antibody studies and has not been tested prospectively. The existence of ME/CFS cases without detectable autoantibodies, and PANS cases without immunotherapy response, confirms that NES covers a subset within each diagnostic category, not the entire condition. The PANS/PANDAS evidence base consists predominantly of narrative reviews and case series ($n = 12$–$35$ for treatment studies) from specialist referral centers; the single RCT was underpowered and equivocal @Williams2016IVIGRCT. The PANDAS literature has a population-weight discount (paediatric, tertiary referral, rare condition) that should temper generalization to adult ME/CFS.
] <spec:neuroimmune-encephalopathy-spectrum>

#synthesis(title: [Post-Infectious Immune-Mediated Neuropsychiatric Syndromes as a Trans-Disease Biological Category])[
...
- *PANS/PANDAS:* The best-characterized post-infectious autoimmune neuropsychiatric pipeline — GAS → anti-neuronal IgG → CaMKII activation → basal ganglia dysfunction → OCD/tics — with documented immunotherapy responsiveness in a subset @spec:pans-pandas-architecture-b
- *Long COVID neuropsychiatric manifestations:* ADHD-like executive dysfunction, depressive symptoms, and cognitive impairment arising post-SARS-CoV-2 in a phenotypically consistent pattern @Azcue2022cognitiveProfileCFSLongCOVID
- *Anti-NMDAR encephalitis:* The most severe end of the spectrum — florid psychosis, catatonia, cognitive collapse — established as treatable autoimmune disease @Dalmau2008NMDAREncephalitis
- *Maternal anti-fetal brain autoantibodies in ASD:* ~23% of ASD cases are attributable to maternal antibodies targeting specific fetal neuronal proteins, defining an immune-mechanism ASD subgroup that is structurally identical to the postnatal post-infectious pattern, differing only in the developmental window of the immune insult @Braunschweig2013MATAbs @Croen2008MaternalAbASD
- *ME/CFS neuropsychiatric overlap:* Elevated ADHD, ASD, and depression in ME/CFS, with a plausible immune-mediated subset identified by circuit-target autoantibody profiles @spec:neuroimmune-encephalopathy-spectrum
...
#speculation(title: [The Inflammatory-Mediated Neurodegeneration Template: Bidirectional Mental-Illness–Inflammation Link])[
*(Certainty: 0.40; raw 0.64–0.60, dementia/mental-illness populations → cross-disease discount; reduced from 0.45 on adversarial review because the two supporting meta-analyses share a single research group testing the same hypothesis, and the inference is a double-hop from dementia to ME/CFS. Origin: literature.)* A meta-analysis of 41 studies testing the “inflammatory-mediated neurodegeneration” hypothesis in dementia @Kuring2026InflammatoryDementia found that Alzheimer's disease is associated with elevated IL-17A, IL-1$alpha$, IL-10, G-CSF, GM-CSF, and IL-3 independent of comorbid psychiatric and physical inflammatory conditions. A companion meta-analysis found depression, anxiety, and PTSD are each associated with inflammation independent of such comorbidities @Kuring2023InflammatoryMood. IL-17A is elevated in both Alzheimer's disease and depression, and G-CSF in Alzheimer's, depression, and PTSD — a shared inflammatory pathway candidate linking mental illness to CNS pathology.

This template is the *reverse direction* of the “neuroinflammation as common upstream driver” model above @spec:neuroinflammation-cascade-generator. That model treats depression as a *secondary* manifestation of ME/CFS neuroinflammation; the inflammatory-mediated-neurodegeneration template posits that pre-existing mental-illness-associated inflammation can itself drive CNS degeneration. The two are not contradictory — they imply a bidirectional inflammation–mental-state loop, with the direction of causality unresolved in cross-sectional data. Critically for ME/CFS, the dementia experience shows that excluding comorbid inflammatory conditions *removes* the classically reported IL-6 and TNF-$alpha$ elevations @Kuring2026InflammatoryDementia — a methodological caution that comorbidity masking may inflate or distort inflammatory-marker claims in any chronic condition, including ME/CFS (Chapter @ch:immune-dysfunction).
...
#speculation(title: [Shared Th17/Myeloid-Axis Signal Between ME/CFS and Inflammatory CNS Conditions])[
*(Certainty: 0.30. Origin: brainstorm.)* Against the overall divergence between the ME/CFS inflammatory profile (TNF-$alpha$, IL-2, IL-4, TGF-$beta$, CRP elevated @Strawbridge2019CFSInflammatory) and the Alzheimer profile (IL-17A, IL-1$alpha$, IL-10, G-CSF, GM-CSF, IL-3 elevated @Kuring2026InflammatoryDementia), one specific axis converges: *myeloid/Th17 hematopoiesis*. G-CSF and GM-CSF are severity-correlated inflammatory markers in ME/CFS (Montoya cohort @Montoya2017) and are among the markers elevated in Alzheimer's disease independent of comorbidity @Kuring2026InflammatoryDementia; IL-17F (a Th17 cytokine) is part of early-disease ME/CFS signatures (Hornig 2015) while IL-17A is elevated in Alzheimer's and depression @Kuring2023InflammatoryMood. This raises the possibility of a shared set of G-CSF/GM-CSF/IL-17 markers across chronic inflammatory-CNS conditions — grouped here by their observed co-elevation, not by a demonstrated shared regulatory node (G-CSF and GM-CSF are myeloid/hematopoietic growth factors, while IL-17A/IL-17F are T-cell-derived, so any single underlying axis would require direct evidence that does not yet exist).
...
#synthesis(title: [Inflammation as Cross-Disease Driver: Template, Direction, and a Methodological Caution])[
Taken together, the dementia and mental-illness meta-analyses @Kuring2026InflammatoryDementia @Kuring2023InflammatoryMood and the CFS inflammatory-protein meta-analysis @Strawbridge2019CFSInflammatory yield three distinct take-aways relevant to ME/CFS on separate evidentiary grounds. First, low-grade inflammation can be dissociated from psychiatric comorbidity — and disease-inherent markers survive that exclusion — supporting the inflammatory component of ME/CFS as real rather than purely conferred by coexisting mood or inflammatory disorders (@spec:bidirectional-mood-inflammation-template; @lim:dementia-comorbidity-masking). Second, the causal direction between mental state and inflammation is not resolved and may run in both directions; the dementia template (mental-illness inflammation → CNS pathology) and the neuroinflammation-cascade model (@spec:neuroinflammation-cascade-generator) are two poles of one unresolved bidirectional loop rather than competing explanations. Third, a specific measurable set of G-CSF/GM-CSF/IL-17 markers appears co-elevated across chronic inflammatory-CNS conditions, including an overlap with severity-correlated ME/CFS markers, and is worth testing as a distinct biomarker set rather than as nonspecific "inflammation" (@spec:th17-myeloid-shared-signal). None of these claims is yet confirmed in comorbidity-screened ME/CFS cohorts, and the specific dementia markers do not transfer directly to ME/CFS; the value is a template, a methodological warning, and a specific testable marker set, not a proven ME/CFS mechanism.

*Consequence:* together these cross-disease parallels suggest that a shared, measurable marker set — not vague inflammation — may be a productive ME/CFS biomarker and stratification target, and that mental and inflammatory states in ME/CFS should be considered coupled rather than one always causing the other. ] <syn:dementia-inflammation-template>
...
Shared mechanisms supporting a unified inflammatory etiology include: (a) elevated pro-inflammatory cytokines (IL-6, TNF-alpha) documented independently in both ADHD and ME/CFS @Quadt2024neurodivergentfatigue @Dunn2019neuroinflammationadhd; (b) co-localised dopaminergic deficit and microglial activation in ADHD @Yokokura2021D1Rmicroglia, paralleling neuroinflammation patterns in ME/CFS; (c) inflammatory diversion of BH4 from catecholamine synthesis toward neopterin production — the same BH4 bottleneck in both conditions @Fanet2021BH4neuro @Williams2025GCH1BH4; (d) shared prefrontal cerebral hypoperfusion @Berthier2025cbfadhd; (e) convergent stimulant pharmacology — both conditions respond to dopamine/norepinephrine reuptake inhibition @Blockmans2006methylphenidate @Eckey2025PatientReported; (f) overlapping mtDNA haplogroup effects: haplogroup U is protective in both ADHD @Chang2020haploADHD and symptom-modifying in ME/CFS; (g) mitochondrial OXPHOS impairment documented independently @Verma2016ADHDcybrid @Almutairi2024mitoadhd. The ALSPAC finding that ADHD traits predict 2x chronic fatigue risk with IL-6 mediation @Quadt2024neurodivergentfatigue directly supports inflammation as the shared driver preceding both phenotypes. The same inflammation-mitochondria-energy cascade is implicated in Alzheimer's and Parkinson's, suggesting a broad spectrum of inflammation-driven brain energy failure where the specific diagnosis depends on which neural circuits are most affected. Key prediction: PBMC spare respiratory capacity will form a gradient across controls > ADHD-only > ME/CFS-only > ADHD+ME/CFS, correlating with serum IL-6 and TNF-alpha levels. Falsified if ADHD-only patients show normal inflammatory markers AND normal mitochondrial function. Not yet replicated.
...
*PANS/PANDAS.* A second line comes from a different disease domain with partial mechanistic characterization (contested — see @Chiarello2017Controversy @Sigra2018SystematicReview). Pediatric Acute-onset Neuropsychiatric Syndrome (PANS) and its streptococcal-triggered subtype (PANDAS) are defined by abrupt-onset OCD, tics, and behavioral regression temporally linked to infection — most commonly group A streptococcal (GAS) pharyngitis @Swedo1998PANDAS50. The diagnostic criteria require acute, dramatic symptom onset with a documented infectious or inflammatory trigger, producing a clinical picture that can closely resemble severe autism exacerbation or late-presenting neurodevelopmental regression @Chang2015PANSevaluation @Masterson2025PANSclassification. The leading molecular model — from the Cunningham laboratory — holds that GAS-directed antibodies cross-react with neuronal lysoganglioside in the basal ganglia via molecular mimicry, activating CaMKII in dopaminergic neurons and producing circuit-specific dysfunction in the same basal ganglia-thalamocortical loops that subserve behavioral control @Kirvan2003MimicryChorea @Kirvan2006CaMKII @Cesaroni2026PANDASconundrum. Independent replication has been inconsistent, and the Cunningham Panel (based on this pathway) is research-use only and not FDA-approved @Chiarello2017Controversy @Sigra2018SystematicReview.

PANS/PANDAS provides a plausibility precedent for Architecture B that ME/CFS alone cannot yet offer — though the analogy is imperfect given the biological divergences: (1) a partially characterized infection-to-autoantibody pipeline (GAS → anti-neuronal IgG → CaMKII activation — proposed, contested), (2) an acquired neuropsychiatric phenotype (OCD, tics, regression) that is clinically indistinguishable from developmental neuropsychiatric conditions, and (3) treatment response — a subset of PANS/PANDAS patients improve dramatically with IVIG or therapeutic plasma exchange in unblinded series @Latimer2015TPA @Kovacevic2015IVIG, suggesting the symptoms are immune-mediated in at least some cases, though the single double-blind RCT (n=35) failed to demonstrate a significant advantage over placebo overall @Williams2016IVIGRCT. A systematic review rated overall evidence quality as low, citing study heterogeneity @Sigra2018SystematicReview. The pattern — dramatic responders alongside non-responders, positive unblinded series but equivocal RCT — is structurally similar to the immunotherapy data in ME/CFS and reflects the same underlying problem: syndromic inclusion criteria capture a biologically heterogeneous population, and immunotherapy only works in the immune-mediated subset @LaBella2023PANDASreview. However, this pattern is equally consistent with substantial open-label expectation effects in a condition where symptoms are subjective and treatment cannot be truly blinded.

The PANDAS controversy itself is instructive. Whether PANDAS "really exists" as a distinct entity has been debated for two decades @Chiarello2017Controversy @Cesaroni2026PANDASconundrum. The debate has legitimate methodological concerns on both sides: proponents argue the syndrome is immune-mediated (the Swedo/Frankovich line), while skeptics note that GAS infection and OCD are common in the pediatric population, diagnostic criteria lack specificity, and the one RCT was null (the Gilbert/Singer line) @Chiarello2017Controversy. Unlike the ME/CFS biological-psychosomatic polarization, the PANDAS debate involves methodologically substantive disagreements and is not settled. The emerging view from the Stanford PANS clinic's 10-year data @Masterson2025PANSclassification — which itself originates from the pro-PANDAS side of the debate — is that the question may be mis-framed: rather than asking whether PANDAS "really exists," the clinically tractable question is what proportion of acute-onset pediatric neuropsychiatric presentations are immune-mediated. This is precisely the question the present paper asks about ME/CFS and acquired ADHD/ASD-like features, though with the critical difference that in ME/CFS this hypothesis has no direct evidence at all.
...
#synthesis(title: [Syndromic Labels Obscure Mechanism-Based Subgroups: The PANDAS-Architecture B Convergence])[
...
- *The PANDAS controversy resolution:* Two decades of debate over whether PANDAS "really exists" resolved not with a verdict but with a reframing — the question is not whether the syndrome is real but what proportion of acute-onset paediatric neuropsychiatric presentations are immune-mediated @Chiarello2017Controversy @Masterson2025PANSclassification. The answer is "some, not all." The residual is that a diagnostic label functions as a heterogeneous bin, and the clinically consequential distinction — immune-mediated vs not — is orthogonal to the label itself.
- *Architecture B in ME/CFS:* The same structural problem recurs in the ME/CFS-ADHD/ASD overlap. Whether acquired inattention and sensory rigidity are immune-mediated (Architecture B), shared-vulnerability (Architecture A), or bidirectional amplification cannot be determined from diagnostic labels alone — because the labels collapse these biologically distinct trajectories into a single "comorbidity" category @spec:pans-pandas-architecture-b
- *IVIG response in ASD:* The Connery 2018 vs Plioplys 1998 contrast — dramatic immunotherapy response in autoantibody-stratified ASD, null in unselected ASD — is the clearest proof that the therapeutic consequence of this binning problem is real, not theoretical. When recruitment uses syndromic criteria, the trial is underpowered by the inclusion of non-immune-mediated cases; when recruitment uses antibody profiling, the same intervention shows efficacy in the subset it targets @Connery2018IVIGautism @Plioplys1998IVIGautism
...
*Limitations:* The PANDAS RCT was underpowered and equivocal @Williams2016IVIGRCT. The Connery 2018 IVIG-in-ASD study is a single specialist-center series, not a multi-site trial. No prospective study has simultaneously applied the PANDAS antibody panel, ASD maternal-antibody panel, and ME/CFS functional autoantibody panel to the same cohort. The claim that syndromic labels *systematically* obscure immune-mediated subsets is supported by multiple independent examples but has not been proven as a general principle.
...
Hidradenitis suppurativa (HS; maladie de Verneuil) is a chronic autoinflammatory skin disease of the intertriginous areas, characterised by painful nodules, abscesses, and sinus tracts. Unlike psoriasis (which has established Th17/IL-17-centred treatment paradigms), HS is driven by a broader inflammatory architecture — TNF-alpha, IL-1beta, IL-17, and IL-23 — with the NLRP3 inflammasome as a central organising node. HS is increasingly classified as a systemic autoinflammatory rather than purely dermatological disease, and it carries a significant comorbidity burden including metabolic syndrome, inflammatory arthritis, and fibromyalgia. A single large population study (Prens 2022, Lifelines Cohort, n = 56,084) found HS associated with ME/CFS at adjusted OR 1.72 (95% CI 1.06–2.78), alongside fibromyalgia (OR 2.26, 95% CI 1.64–3.11), after adjustment for age, sex, BMI, smoking, and socioeconomic status @Prens2022HSComorbidities. This association should be treated as provisional: the lower CI (1.06) is barely above the null; both HS and ME/CFS were self-reported (not clinically validated); depression, sleep quality, and physical activity level were not adjusted for (all shared confounds); and the finding has not been replicated in an independent cohort four years after publication. Fatigue prevalence in HS is 30–61% across studies — comparable to SLE and RA — and is independently associated with pain, depression, and kinesiophobia (fear of movement) @McGrath2026HSFatigue @Almahdi2026HSPalliative @MeralKetenci2026HSKinesiophobia. This prevalence range comes predominantly from dermatology clinic populations (Hurley II–III disease disproportionately represented; tertiary referral bias) and may overestimate fatigue in the community HS population, most of whom have mild (Hurley I) disease and are undiagnosed @Prens2022HSComorbidities. (Origin: brainstorm — evidence quality concern.)
...
#limitation(title: [Conflating HS Type 2 Inflammatory Fatigue with ME/CFS Type 1 PEM Harms Both Patient Populations])[
...
#limitation(title: [Vaccination-Era Data Are Heavily Confounded and Must Not Be Over-Read])[
...
#limitation(title: [Single-System Cohort Limits Generalisability of Effect Magnitudes])[
The four-year risk estimate rests on a single integrated health system in the Bronx @Hadidchi2025CFSrisk, a population with distinct socioeconomic and comorbidity profiles (high NAFLD, poverty, and chronic-disease burden). The qualitative finding (elevated risk, direction consistent with RECOVER and INSPIRE) is robust, but the specific hazard ratios (1.46--1.56) should not be treated as universal constants. (Certainty: n/a -- external-validity caveat.)
...
=== Iron Redox Polarity: ME/CFS vs Long COVID Diagnostic Bifurcation

#hypothesis(title: [Functional Iron Deficiency as a Diagnostic Differentiator Between ME/CFS and Long COVID])[
*Certainty: 0.55.* ME/CFS and Long COVID show divergent iron handling profiles with direct clinical consequences for treatment selection. In ME/CFS, the pattern is functional iron deficiency: elevated ferritin with low serum iron and low transferrin saturation (TSAT) @Swiatczak2022, @Baklund2021RoutineBlood. Serum ferritin is elevated by an average of +28.13 µg/L in ME/CFS compared to controls @Baklund2021RoutineBlood. In Long COVID, a broader multi-compartment iron dysregulation is present: Hanson et al. demonstrated iron maldistribution with monocyte iron loading concurrent with lymphocyte iron starvation at single-cell resolution, plus stress erythropoiesis at 2 weeks post-onset that predicted PASC at 3–5 months with 72% accuracy @Hanson2024iron. Sonnweber et al. documented that 35% of LC patients have hyperferritinaemia at 60 days, while 24% have genuine iron deficiency and 9% have anaemia @Sonnweber2022CovILD. A direct comparative study by the Scheibenbogen group found that post-COVID ME/CFS patients showed elevated ferritin associated with hand grip strength, while post-COVID patients without ME/CFS did not @Kedor2022PostCOVIDfatigue. Yamamoto et al. found ferritin levels of 193.0 µg/L in post-COVID patients meeting ME/CFS criteria versus 98.2 µg/L in those who did not @Yamamoto2023Ferritin. (Evidence source: serum/blood — Inference target: systemic iron handling. Link is direct for serum iron parameters; indirect for tissue-level iron maldistribution — single-cell data @Hanson2024iron provides the only direct evidence of monocyte-lymphocyte iron redistribution.)

*The hepcidin paradox.* A critical finding distinguishes ME/CFS from classic inflammatory anemia. Kavyani et al. measured hepcidin in ME/CFS — the only published hepcidin measurement in this population — and found it decreased by nearly half @Kavyani2023Hepcidin. This is the opposite of what the IL-6→STAT3→hepcidin pathway (documented in @spec:hepcidin-endocrine-bridge) would predict for a condition with elevated IL-6. Three competing explanations exist: (1) chronic hypoxia-driven HIF-mediated hepcidin suppression overriding IL-6 signalling, (2) a compensatory attempt to mobilize sequestered iron from macrophages, or (3) a distinct ME/CFS-specific hepcidin biology where chronic upregulation exhausts the HAMP transcription machinery. In Long COVID, Gietl et al. found that patients who recovered at 12 months had higher hepcidin than those with persistent symptoms @Gietl2024PostCOVIDlab — suggesting adequate hepcidin response may be protective. The polarity is stark: low hepcidin in ME/CFS (Kavyani2023) vs variable but predictively associated with recovery in LC (Gietl2024).

*Therapeutic bifurcation.* This creates a dangerous therapeutic fork where treating one condition with the other's approach may cause harm. In ME/CFS functional iron deficiency, the body already has iron — it is trapped in macrophages and unavailable to tissues. Oral iron supplementation carries risk: if ferroportin is blocked (hepcidin paradox notwithstanding), oral iron further elevates hepcidin and worsens the iron sequestration trap @vanRensburg2001CFSmetals. In Long COVID, 24% of patients have genuine iron deficiency @Sonnweber2022CovILD and may benefit from iron repletion, but should be phenotyped first: measure full iron panel (serum Fe, ferritin, TSAT, sTfR) plus CRP before deciding between iron supplementation versus anti-inflammatory versus iron chelation. The most dangerous clinical error: administering iron to an ME/CFS patient with high ferritin (>150 µg/L) and low TSAT (\<20%) because "iron is low" — this is functional iron deficiency. Iron supplementation here may fuel ferroptosis risk, as iron provides the catalytic substrate for lipid peroxidation @Sousa2023Ferroptosis.
...
#speculation(title: [Ferroptosis Susceptibility as a Shared Mechanism with Divergent Triggers])[
*Certainty: 0.35.* Ferroptosis — iron-dependent lipid peroxidation cell death — is a candidate shared mechanism in both ME/CFS and Long COVID, but triggered by different upstream pathology. In Long COVID, the trigger is inflammatory iron overload: COVID-induced oxidative stress combined with monocyte iron loading @Hanson2024iron creates an iron-rich, high-ROS environment that depletes GPX4 and triggers lipid peroxidation @Sousa2023Ferroptosis. In ME/CFS, the trigger may instead be chronic low-grade oxidative stress plus impaired antioxidant capacity (including glutathione depletion, documented in Chapter @ch:energy-metabolism) in the context of functional iron deficiency — the iron is trapped in macrophages but ferroptosis-competent iron may be redistributed to susceptible tissues via non-transferrin-bound iron (NTBI) during oxidative bursts. The net result (ferroptotic cell death) is shared; the upstream supply of catalytic iron differs. (Evidence source: serum iron parameters + ferroptosis biomarkers in LC — Inference target: tissue-level ferroptosis. Link is indirect — no tissue-level ferroptosis measurement in either condition.)
...
*Limitations.* The ferroptosis-ME/CFS connection is completely inferential — no ME/CFS study has measured ferroptosis-specific biomarkers. The LC ferroptosis literature is review-level @Sousa2023Ferroptosis, not primary interventional data. Mantle 2025 @Mantle2025FerroptosisOverlap reviews ferroptosis across multiple conditions but provides no ME/CFS-specific evidence. The connection between functional iron deficiency and ferroptosis is mechanistically paradoxical — trapped iron should reduce ferroptosis risk, not increase it — and requires the NTBI redistribution hypothesis for coherence. (Origin: literature synthesis + brainstorm.)

]<spec:iron-ferroptosis-divergent-triggers>

#open-question(title: [Iron Chelation vs Iron Supplementation: When to Use Which and in Whom])[
...
(1) Does oral iron supplementation increase or decrease fatigue in ME/CFS patients stratified by baseline ferritin and TSAT? (2) Does iron chelation (deferiprone, deferasirox) improve fatigue and physical function in ME/CFS patients with functional iron deficiency? (3) Do the ~24% of Long COVID patients with genuine iron deficiency @Sonnweber2022CovILD benefit from iron repletion, and does the ~35% with hyperferritinaemia worsen? (4) Does the hepcidin level predict response to iron supplementation in either condition — specifically, do low-hepcidin patients (ME/CFS, @Kavyani2023Hepcidin) respond differently than normal-high hepcidin patients (recovering LC, @Gietl2024PostCOVIDlab)?
...
=== Additional Iron Redox Speculations (Brainstorm Origin)

#speculation(title: [Hepcidin-Independent Ferroportin Blockade: Four Candidate Mechanisms Resolving the Hepcidin Paradox])[
*Certainty: 0.40.* The hepcidin paradox — low hepcidin @Kavyani2023Hepcidin yet persistent functional iron deficiency (high ferritin, low serum iron, low TSAT) @Swiatczak2022, @Baklund2021RoutineBlood — requires a hepcidin-independent explanation for why iron remains trapped in macrophages. Four candidate mechanisms, none mutually exclusive and all untested in ME/CFS, could resolve the paradox:

(a) *Ferroportin post-translational trafficking defect.* Even with low hepcidin, ferroportin may fail to traffic to the cell membrane. IRP1/IRP2 (iron regulatory proteins) control ferroportin translation via the iron-responsive element (IRE) in ferroportin's 5' untranslated region. If IRP1 is aconitase-inactive — as the ACO1/IRP1-telomere correlation in fibrotic Long COVID suggests @Mulet2026mitochondrial — ferroportin translation could be suppressed independently of hepcidin. In this model, the IRP/IRE iron-sensing system that sits upstream of hepcidin is the primary defect; hepcidin reduction is a compensatory response to intracellular iron deficiency.
...
(d) *Lipocalin-2 (LCN2/NGAL) iron scavenging.* LCN2 binds siderophore-iron complexes and delivers them to cells via the 24p3R receptor, bypassing the transferrin system entirely. Dufrusine et al. found LCN2 altered in Long COVID proteomics @Dufrusine2022Iron5LOX. If LCN2 is elevated in ME/CFS, it could scavenge any iron that does get exported, delivering it right back to macrophages — a futile iron cycle maintaining functional deficiency.
...
*Consequence.* The body's iron-regulating thermostat is correctly calling for iron release — but the release machinery is broken. If validated, this shifts treatment from "lower hepcidin to release iron" (danazol, per @spec:danazol-hepcidin) to "fix the export machinery downstream of hepcidin." Clinically, this makes deferiprone (iron chelator, see @spec:deferiprone-functional-iron-deficiency) a more rational target than hepcidin antagonism — release iron that is trapped, rather than trying to change the setpoint of a thermostat that is already calling for release.
...
#speculation(title: [Temporal Phase-Shift Hypothesis: ME/CFS as Late-Stage Long COVID Iron Phenotype])[
...
#prediction(title: [Ferritin:TSAT Ratio as a Diagnostic Differentiator and Treatment-Stratification Biomarker])[
...
#speculation(title: [Deferiprone for Functional Iron Deficiency in ME/CFS: Rationale and Caution])[
*Certainty: 0.30.* Deferiprone (3-hydroxy-1,2-dimethylpyridin-4-one) is a membrane-permeable oral iron chelator approved for transfusional iron overload in thalassemia. Its properties make it a candidate for ME/CFS functional iron deficiency where hepcidin antagonism may be misdirected (see @spec:hepcidin-independent-ferroportin-blockade): deferiprone chelates intracellular labile iron (the iron pool available for ferroptosis catalysis) and is small and lipophilic, penetrating cell membranes without requiring ferroportin-mediated export. Unlike hepcidin antagonists (danazol, @spec:danazol-hepcidin), deferiprone does not depend on functional hepcidin or ferroportin machinery — it removes iron directly from the cytosol, bypassing the entire hepcidin-ferroportin axis.
...
#limitation(title: [Null Hypotheses Constraining the Iron Redox Polarity Model])[
...
#synthesis(title: [The Iron Redox Polarity Model — A Convergent Diagnostic Bifurcation Across Four Independent Levels])[
Iron handling in ME/CFS and Long COVID is not a shared mechanism — it is a diagnostic bifurcation with potentially opposite therapeutic implications, supported by four independent levels of evidence. At the clinical epidemiology level, serum ferritin distinguishes post-COVID ME/CFS from non-ME/CFS Long COVID in two independent cohorts (Kedor2022: ferritin 193 vs 98 µg/L, @Kedor2022PostCOVIDfatigue; Yamamoto2023: ferritin correlated with fatigue scores, @Yamamoto2023Ferritin), though the Okayama group's second paper (@Morita2024ME-CFSphase) shares the same patient stream and is not an independent replication. At the molecular regulatory level, the Kavyani2023 hepcidin measurement — the only published in ME/CFS — challenges the classic hepcidin model: hepcidin is low (~50% reduction @Kavyani2023Hepcidin), not high, yet functional iron deficiency persists (@spec:hepcidin-independent-ferroportin-blockade, cert 0.40). This paradox drives the search for hepcidin-independent trapping mechanisms (ferroportin trafficking, ferritinophagy blockade, ceruloplasmin deficiency, LCN2 scavenging). At the single-cell compartment level, Hanson2024 demonstrates monocyte iron loading concurrent with lymphocyte iron starvation in Long COVID @Hanson2024iron — a maldistribution pattern with no equivalent measurement in ME/CFS. At the erythrocyte functional level, Kronstein-Wiedemann2024 documents impaired RBC oxygen binding in LC @KronsteinWiedemann2024RBC — a defect not described in ME/CFS (@hyp:erythrocyte-dysfunction-lc-specific, cert 0.45). The convergent clinical implication is a simple, testable rule: measure ferritin + TSAT before prescribing iron for post-viral fatigue. Functional iron deficiency (ferritin $>$150 µg/L, TSAT $<$ 20%) = do NOT supplement iron — the body has iron, it's just trapped (@pred:ferritin-tsat-ratio-diagnostic, cert 0.40). Genuine iron deficiency (ferritin $<$ 30 µg/L or TSAT $<$ 16%) = supplement iron regardless of diagnosis. The four hepcidin-independent trapping mechanisms and the iron chelation hypothesis (@spec:deferiprone-functional-iron-deficiency, cert 0.30) remain purely mechanistic and untested. The single most tractable uncertainty is N1 — whether the Kavyani2023 low-hepcidin finding replicates in an independent ME/CFS cohort (@lim:iron-redox-null-hypotheses). A second measurement using mass spectrometry (not ELISA) in an independent cohort with matched controls would cost \<\$50,000 and take $<$6 months — and would settle whether the hepcidin paradox is biology or artifact. Until then, the clinical bifurcation model functions at the blood-test level regardless of mechanism: ferritin + TSAT tells you what to do, even if we don't yet know why the thermostat is broken.
...
Peripheral inflammatory cytokines (IL-1beta, TNF-alpha) actively suppress orexin neuron firing in the lateral hypothalamus @Grossberg2011orexinLethargy.
...
#synthesis(title: [Post-Infectious Trigger Convergence: Sustained Post-COVID Risk as a Large-Cohort Anchor for the Post-Viral ME/CFS Model])[
A large-cohort electronic health record study ($n = 147{,}377$) finds that new-onset ME/CFS risk after SARS-CoV-2 infection remains elevated up to four years (adjusted HR 1.46 hospitalized, 1.56 non-hospitalized) @Hadidchi2025CFSrisk, directionally consistent across RECOVER ($n = 11{,}785$) and the CDC INSPIRE cohort @RECOVER2025 @Unger2024MECFSinspire. This converges with the historical post-infectious literature to anchor the paper's post-viral onset model in three ways. First, it replicates the established post-infectious trigger template (Epstein-Barr virus, SARS-CoV-1, Q fever) at pandemic scale @Hickie2006postinfectious @Moldofsky2011PostSARS @Katz2009IMadolescentCFS, confirming that acute infection in a susceptible subset resolves into chronic ME/CFS rather than representing a transient fatigue phenomenon. Second, the four-year persistence extends the documented post-infectious risk window well beyond the 6--24 month follow-up of prior post-infectious cohorts @Hickie2006postinfectious @Unger2024MECFSinspire, widening the monitoring horizon. Third, the acute-biomarker null (ferritin, CRP, D-dimer, LDH do not predict converters) feeds the iron-redox temporal model (@hyp:iron-redox-polarity-diagnostic-bifurcation, cert 0.55): acute inflammatory iron changes are universal and non-discriminatory, so the converter-discriminating signal is expected post-acute (@hyp:postcovid-postacute-iron-prediction, cert 0.40).

*Caveats.* The four-year horizon is single-study and ICD-code-based (diagnostic-criteria unconfirmed; pre-COVID baseline not excluded); the direction is robust, the exact magnitudes are not universal (@lim:postcovid-4yr-diagnostic-delay, @lim:postcovid-single-system-generalizability).
