...
#open-question(title: [Stuck Sickness Behavior Program])[
...
#open-question(title: [Impaired Brain Waste Clearance])[
...
*Applications.*
    ...
    - *Dose titration guide:* For CA inhibitors, titrate until morning ONSD stabilizes in target range (4.0–5.0 mm for ME/CFS—higher than IIH target of below 3.5 mm to account for pressure hypersensitivity described by Midtlien et al. 2024 @Midtlien2024CVDphenotype) while monitoring cognitive function.
...
The hypotheses in this chapter—metabolic "safe mode," glymphatic impairment, synergistic CSF drainage, sequential therapy, overnight ONSD monitoring, and micro-NPH—share a common epistemic limitation: each is internally consistent and mechanistically plausible, but none has been directly tested in ME/CFS patients. Key boundaries:
    ...
    - No prospective study has demonstrated that any of these mechanisms _precedes_ or _causes_ ME/CFS; all are inferred from cross-sectional data, analogy to other diseases, or theoretical modeling.
...
The re-silencing failure could be conditioned by: pre-existing pericentromeric hypomethylation (due to MTHFR variants, folate-cycle impairment causing H3K9me3 loss, prior epigenetic hits), severity of the initial infection (larger initial HSAT2 burst exceeding re-silencing capacity), or specific HERVs/HSAT2 loci affected. At the biophysical level, pericentromeric heterochromatin assembly requires HP1-driven liquid-liquid phase separation above a critical nucleation threshold @Strom2017HP1phaseseparation @Larson2017HP1droplet. If infection-induced H3K9me3 loss drops HP1 concentration below this threshold, the heterochromatin condensate dissolves and cannot re-form until the methylation/H3K9me3/HP1 density is restored. This threshold mechanism explains why re-silencing may be a nonlinear, one-way transition for some loci: once the condensate dissolves at a given pericentromeric repeat, the HP1 concentration needed for re-nucleation exceeds the concentration that maintained the existing droplet, creating hysteresis at the chromatin domain level — a biophysical analogue of the epigenetic hysteresis already captured in the ODE model (@sec:epigenetic-dynamics, @oq:consolidation-directionality).
...
*Mechanistic Rationale.* Traditional approaches target VEGF-mediated angiogenesis in ME/CFS, but VEGF deletion studies failed to rescue ECM abnormalities. HIF-1alpha directly drives pathological ECM remodeling independently of VEGF: it promotes basement membrane thickening in capillaries, upregulates MMP-3 for collagen degradation, and alters collagen matrix organization. An additional concern is that HIF-1alpha also promotes Th17 differentiation and B cell activation, meaning systemic HIF-1alpha inhibition may carry autoimmune-modulating effects @Little2023HIF1lupus. DMF inhibits HIF-1alpha through NRF2 activation and is known to cross the blood-brain barrier, potentially addressing both peripheral and central manifestations of connective tissue pathology.
...
#open-question(title: [Monocarboxylate Transporter Dysfunction])[
...
=== Compartmentalization Pathophysiology
...
=== Why MCT Function Might Be Impaired
...
=== Testable Predictions
    ...
    - Lactate imaging (using #super[13]C-MRS or hyperpolarized #super[13]C-lactate) should reveal abnormal compartmentalization
...
#open-question(title: [Persistent Interoceptive Threat Signaling])[
...
The vagus nerve is predominantly afferent ($tilde$80% of fibers carry signals TO the brain). These afferents:
...
#open-question(title: [Neurotoxic Kynurenine Dominance])[
...
*Serotonin Pathway ($tilde$5% of tryptophan).*
#align(center)[
Tryptophan $arrow.r^("TPH")$ 5-HTP $arrow.r^("AADC")$ Serotonin $arrow.r$ Melatonin
]
This pathway produces neurotransmitters essential for mood, sleep, cognition, and gut function.

*Kynurenine Pathway ($tilde$95% of tryptophan).*
#align(center)[

$
"Tryptophan" arrow.r^("IDO/TDO") "Kynurenine" arrow.r cases( "Kynurenic acid (KYNA)"  
 "3-HK" arrow.r "Quinolinic acid (QUIN)" arrow.r "NAD+" )
$
...
#open-question(title: [Corrupted Intercellular Communication])[
...
#open-question(title: [ATP as Pathological Danger Signal])[
...
*P2X Receptors (ion channels).*
    ...
    - Activation causes cation influx, including Ca#super[2+]
...
=== Consequences of Purinergic Dysregulation
...
#open-question(title: [Loss of Redox Boundaries])[
...
=== Consequences of Boundary Collapse
...
#open-question(title: [Stable Epigenetic Reprogramming])[
...
#open-question(title: [Peripheral Clock Misalignment])[
...
*Master Clock (SCN).* The suprachiasmatic nucleus:
    ...
    - Contains $tilde$20,000 neurons with autonomous rhythms
...
=== How Desynchronization Might Occur

    - *Infection:* Inflammatory cytokines disrupt clock gene expression
    - *Autonomic dysfunction:* Impairs SCN $arrow.r$ peripheral signaling
