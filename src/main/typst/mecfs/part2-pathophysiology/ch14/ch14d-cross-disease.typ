...
*Caveat (2026-05-24):* The pDC literature reveals a more nuanced picture than simple "elevation." Blood pDC depletion is documented across 19/23 COVID-19 studies @VanderSluis2022pDCcovid, and the blood↓/tissue↑ pattern is the norm across autoimmune diseases (SLE, RA, psoriasis) @Li2017pDCdisease. The pDC "elevation" reported in ME/CFS and pSS may reflect tissue-predominant pDC expansion with concurrent blood depletion — a chemokine-driven migration pattern (CXCR3/CXCL9-11) rather than true numerical increase. Functional pDC assays (DNase1L3 production, IFN-α secretion capacity) have not been performed in ME/CFS. The pDC biology is consistent across conditions; the apparent contradiction is likely a measurement compartment artefact (blood vs. tissue) rather than a biological difference.
