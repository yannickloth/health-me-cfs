...
#speculation(title: [Alpha-Delta Sleep Impairs Synaptic Downscaling — Mechanistic Link to Unrefreshing Sleep])[
*Certainty: 0.30.* (General-population human evidence for synaptic homeostasis from Elmenhorst 2026 @Elmenhorst2026SV2Asleep; no ME/CFS-specific SV2A or synaptic density data exists.)
...
If this framework applies to ME/CFS, the alpha-delta sleep anomaly — where alpha (wake-like) EEG activity intrudes into NREM delta sleep — may prevent the synaptic downscaling that normal slow-wave sleep provides. Each night, the ME/CFS brain may partially fail to reset its synaptic landscape, accumulating the day's synaptic burden into the next. Over days and weeks, this produces a net excess of excitatory connections, increasing the brain's resting energy demand even during sleep — a "synaptic debt" that explains why ME/CFS patients can sleep 10+ hours and wake unrefreshed.

*Energy implications.* Higher-frequency oscillations (alpha at 8--12 Hz) consume substantially more ATP than delta oscillations (0.5--4 Hz) due to the ion-pumping costs of sustaining faster action potential rates. Alpha intrusion during NREM sleep thus places the brain in a metabolically wake-like state when it should be in a low-energy deep-sleep state. Combined with the failure to downscale synapses (each synapse that persists overnight continues consuming maintenance ATP), the overnight energy expenditure of the ME/CFS brain may far exceed that of a healthy sleeper — deepening the metabolic deficit rather than recovering from it.

*Consistency with established findings.* This mechanism is complementary to, not competing with, the glymphatic clearance hypothesis (Section @sec:ch15-glymphatic) and the nocturnal autophagy failure hypothesis (@spec:nocturnal-autophagy-sleep). Glymphatic clearance addresses extracellular waste; autophagy addresses intracellular clearance; synaptic downscaling addresses structural synaptic reset. All three processes peak during deep sleep and all three would be impaired by alpha-delta sleep — representing parallel failures of overnight brain maintenance.
...
*T-type calcium channel convergence.* Thalamic T-type calcium channels (CaV3.1) — the proposed gatekeepers of alpha-delta sleep (Section @sec:thalamic-calcium-sleep) — may also gate the synaptic downscaling cascade itself. The low-threshold Ca#super[2+] spikes that CaV3.1 generates during delta oscillations activate CaMKII-dependent signaling that triggers Homer1a transcription, which in turn drives AMPA receptor internalization during sleep @Diering2017Homer1aSHY. If CaV3.1 burst firing is disrupted (by neuroinflammation, metabolic failure, or channelopathy), the chain Ca#super[2+]#sub[V3.1] → CaMKII → Homer1a → AMPA removal may break at the first link — explaining why alpha-delta intrusion (loss of delta-frequency bursts) impairs synaptic reset. This would make T-type calcium channels a dual-integrator: their dysfunction impairs both the *generation* of deep sleep rhythms and the *downstream* structural reset that deep sleep normally triggers. (Origin: brainstorm.)
...
*Alternative possibilities.* The SV2A PET signal reflects synaptic vesicle glycoprotein 2A density, which varies with vesicle trafficking state and recycling pool size in addition to synapse count @Mikkelsen2023SV2Apostmortem. Elmenhorst's observed SV2A increase may partially reflect activity-dependent vesicle loading rather than new synapse formation — the finding still supports synaptic *potentiation* during wakefulness but the magnitude of structural change may be smaller than headline percentages suggest. Separately, ME/CFS involves sustained inactivity and reduced cognitive engagement (use-dependent drivers of synaptic pruning); chronic adaptation over months to years could produce homeostatic *downregulation* rather than accumulation, producing the opposite of the predicted PET finding. Both alternatives are empirically testable. (Origin: brainstorm + Durkin 2016 and Mikkelsen 2023 evidence.)
