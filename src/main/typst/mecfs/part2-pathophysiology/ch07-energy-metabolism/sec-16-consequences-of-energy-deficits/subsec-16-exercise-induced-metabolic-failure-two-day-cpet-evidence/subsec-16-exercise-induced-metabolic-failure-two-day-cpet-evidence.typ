...
==== Effort-Independent Thresholds and Activity Matching Refute Deconditioning in Long COVID

Beyond the bed-rest comparator (Section @sec:deconditioning-refutation), two additional lines of rebuttal distinguish Long COVID PEM from deconditioning @Appelman2025ReplyMuscle. First, the lower gas exchange threshold and respiratory compensation point in Long COVID patients with PEM are _effort-independent_: they emerge from the cardiopulmonary exercise test regardless of whether the patient is maximally exerted, so they cannot be dismissed as a product of low motivation or effort avoidance. Second, when patients were re-matched to controls by objective physical activity (step count, 5181 vs 4727 steps/day), the group difference in exercise capacity persisted (V̇O#sub[2]max −24%, _p_=0.004; peak power output −31%, _p_=0.043). Matching by activity level therefore does not eliminate the deficit, directly rebutting the claim that the exercise limitation is a consequence of physical inactivity. Consistent with the bed-rest comparator in Chapter @ch:universal-mechanisms, Long COVID patients also showed no reduction in capillarization or fibre cross-sectional area and a qualitative reduction in intrinsic mitochondrial respiration (not a loss of mitochondrial content), a phenotype distinct from disuse @Appelman2025ReplyMuscle. Together with the ME/CFS two-day CPET evidence (Achievement @ach:two-day-cpet), these effort-independent and activity-matched findings in Long COVID provide convergent objective support, across these two post-viral populations, that the exercise intolerance is disease-specific rather than deconditioning-driven.
