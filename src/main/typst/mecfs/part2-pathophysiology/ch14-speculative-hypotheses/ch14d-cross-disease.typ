...
*Non-generalizability caveat:* SARS-CoV-2 directly infects pulmonary vascular endothelium via ACE2 — a property not shared by EBV, enteroviruses, or influenza (the classic ME/CFS triggers). If HPV impairment is ACE2-specific, shunt may be a COVID-restricted mechanism with zero relevance to the ~75% of ME/CFS cases triggered by non-COVID pathogens. The hypothesis that HPV impairment generalizes across viral triggers (via shared endothelial inflammation pathways) is mechanistically plausible but entirely untested.
...
This paper proposes that post-viral ME/CFS involves a HIF isoform imbalance: sustained HIF-2α activation in endothelial cells (@hyp:hif2a-sustained-postviral, cert 0.50) paired with blunted HIF-1α inducibility in other tissues (@spec:hif-pathway-inertia, cert 0.50). The paper further proposes a β2AR-autoantibody-negative subtype where HIF-2α-mediated endothelial dysfunction drives tissue hypoxia independently of vasoconstriction (@hyp:hif2a-beta2ar-negative, cert 0.55). Intrapulmonary shunt offers a unifying upstream mechanism for this HIF imbalance: if shunt produces chronic low-grade arterial hypoxemia (PaO#sub[2] persistently ~75--85 mmHg rather than 95--100 mmHg), this signal profile would differentially activate the endothelial HIF-2α system (which responds to chronic, mild hypoxia) over the ubiquitous HIF-1α system (which responds to acute, severe hypoxia). The shunt would thus bias the HIF isoform ratio toward HIF-2α dominance in endothelium while chronic mild hypoxia desensitizes HIF-1α signaling in other tissues — exactly the pattern proposed. Bidirectional causality is equally plausible: HIF-2α-driven endothelial dysfunction could impair pulmonary endothelial HPV signaling → shunt worsens → more chronic hypoxia → further HIF-2α activation — a positive feedback loop.
...
*Retinal tissue --- a CNS window.* Miller's Yale study (n=15) used three methods (autopsies, live-patient retinal imaging + electroretinography, and organoid models) to demonstrate increased microglial activity (neuroinflammation), astrogliosis, and protein aggregation pathways in the retinas of Long COVID patients @Miller2025RetinalAmyloid. Because the retina is part of the central nervous system and accessible non-invasively, it provides a window into CNS pathology. Retinal Aβ pathology was reversible with an NRP1 inhibitor (not clinically available). Retinal microcirculation measured by OCT-A correlates with chronic fatigue severity in post-COVID syndrome @Schlick2022RetinalMicrocirc, and corneal confocal microscopy shows neuroinflammation two years post-infection @Cañadas2023CornealConfocal.
...
*Consequence:* If validated, a 10-minute non-invasive eye scan could replace lumbar puncture or PET-MRI for CNS biomarker monitoring in ME/CFS --- dramatically lowering the barrier to repeated measurements in clinical trials.
