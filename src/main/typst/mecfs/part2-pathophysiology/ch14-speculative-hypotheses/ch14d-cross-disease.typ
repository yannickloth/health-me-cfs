...
Connective tissue pathology in ME/CFS is addressed by three major integration cycles spanning 110+ hypotheses: the full connective tissue disorders plan (64 ideas across pathophysiology, treatment, and biomarkers), the HIF-2α endothelial postviral integration, and the mast cell IgE→CT degradation analysis @syn:mast-cell-ige-ct-model. These converge on two independent degradation mechanisms — mast cell mediator-mediated MMP/histaminylation (via tryptase, chymase, IgE pathway, mechanical stress @spec:acquired-progressive-hypermobility-mc) and HIF-1α-driven ECM remodelling (via MMP-3 upregulation, collagen synthesis suppression, and basement membrane thickening @spec:cardiac-ecm-mc-prototype) — plus one structural vulnerability: the permissive matrix hypothesis @hyp:permissive-matrix-mcas posits that the same ECM defect causing hypermobility also enables pathological mast cell tissue infiltration. HIF-2α endothelial dysfunction may compound both mechanisms by impairing vascular delivery of collagen precursors and antioxidant defences to CT repair sites. The two degradation routes are potentially addressable with existing drugs (MC stabilisers, subantimicrobial doxycycline, belzutifan for HIF-2α), but no study has tested whether intervening in either mechanism alters CT integrity or hypermobility progression. The central unresolved question is whether CT degradation in ME/CFS is primarily MC-mediated, HIF-mediated, both synergistically, or neither — competing non-MC/non-HIF mechanisms (metabolic insufficiency, HPA catabolism, mechanical unloading @lim:ct-degradation-non-mc) cannot be excluded without partitioning studies.
