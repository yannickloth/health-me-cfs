...
*Treatment implications (if confirmed):* Postural correction (shoe lifts, custom orthotics, targeted physiotherapy for pelvic realignment) would address the mechanical root cause rather than downstream metabolic consequences. Osteopathic manipulative treatment targeting the thoracic sympathetic chain could be trialed as a diagnostic probe (improvement confirms mechanical-autonomic coupling). Cervical decompression surgery would have a stronger rationale in patients with documented progressive foraminal stenosis. However, with current evidence (certainty 0.10), no treatment recommendation can be made.

*Consequence:* If skeletal asymmetry contributes to ME/CFS in even 5–10% of patients, a simple, low-cost intervention — shoe lift or postural physiotherapy — could be disease-modifying for that subgroup. But the hypothesis is currently untested at every link in the chain, and the one study testing an asymmetry→fatigue pathway in a related disease (Proessl 2018, MS) found no association. This is a high-impact/low-probability hypothesis that requires systematic biomechanical and autonomic phenotyping before clinical translation.

*Alternative formulation: the two-hit model.* A more defensible framing recasts skeletal asymmetry not as a primary driver but as a permissive substrate that lowers the threshold for developing ME/CFS when a second hit occurs (infection, trauma, hormonal change). In this model, asymmetry alone rarely causes disease — explaining why most people with scoliosis (2–3% of the general population) never develop ME/CFS — but it reduces reserve capacity across autonomic, metabolic, and pain-processing systems such that a trigger that would be subclinical in a symmetric person becomes disease-initiating in an asymmetric person. This reframes the hypothesis from "asymmetry causes ME/CFS" to "asymmetry is a risk factor of magnitude X for developing ME/CFS after trigger Y," a testable claim: in a prospective post-infectious cohort, pre-existing LLD >10 mm would increase the odds of developing ME/CFS at 12 months by ≥2× compared to symmetric individuals. (Origin: brainstorm — two-hit framing.)
