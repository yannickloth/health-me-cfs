...
The hypoxia environments across this chapter and the energy-metabolism and core-mechanistic chapters assemble a mechanism distinct from the post-viral HIF-2$alpha$ endothelial story (@sec:hif2a-endothelial) and from the VEGF-convergence synthesis (@syn:vegf-pathway-convergence): a *failure of the HIF-1$alpha$ adaptive programme itself*. The organising claim is HIF pathway inertia (@spec:hif-pathway-inertia) — that ME/CFS tissues experience chronic hypoxia yet fail to mount, or fail to switch off, the normal HIF-1$alpha$ transcriptional response, producing the paradox of hypoxia without adequate glycolytic/angiogenic adaptation. Three observations cohere around this. First, the *altitude paradox* (@spec:altitude-paradox): a predicted biphasic dose-response in which mild hypoxia might retrain a sluggish HIF response while higher altitude harms via failed cerebral autoregulation — a directly testable, if unstudied, clinical prediction. Second, *PEM as maladaptive HIF persistence* (@spec:pem-hif-maladaptation): the mirror-image failure in which exertion-induced HIF activation fails to *deactivate* on reoxygenation, locking cells in a hibernation-mimetic hypometabolic state — inertia in the "won't switch off" direction rather than "won't switch on." Third, a downstream *HIF-1$alpha$–mitochondria–ECM triad* (@hyp:hif1alpha-mito-ecm-triad, @hyp:hif1alpha-connective-tissue) in which sustained HIF-1$alpha$ suppresses oxidative phosphorylation and drives MMP-mediated connective-tissue remodelling, with tendinopathy offered as a parallel model. A gut-derived modulator, butyrate-mediated HIF-1$alpha$ stabilisation via HDAC inhibition (@spec:butyrate-hif1a), links microbiome state to this axis. Because the framing is adaptive *failure*, the therapeutic corollaries are correspondingly about *retraining or restoring dynamic range* — intermittent hypoxic training, breathing retraining, and (with explicit caution) HIF-PH inhibitors — rather than simple inhibition; these are developed as investigational proposals in the treatment chapters.

The honest status is that HIF-1$alpha$ pathway inertia is a *unifying hypothesis in search of its confirming measurement*: no study has measured the post-exertional HIF-1$alpha$ target-gene time course in ME/CFS, no altitude-chamber study exists, and the ECM-triad rests on tendinopathy and connective-tissue analogy rather than primary ME/CFS tissue. The model's value is that its two failure modes make *opposite, discriminable* predictions (blunted induction versus failed de-activation) that a single post-CPET transcriptional time-course study could separate — and that it explicitly competes with, rather than assumes, the alternative that tissue hypoxia in ME/CFS is adequately compensated and the pathology lies elsewhere. Where the HIF-1$alpha$ and HIF-2$alpha$ arms meet — whether early disease is HIF-1$alpha$-driven and late disease HIF-2$alpha$-driven — is left as the open question already posed in @syn:vegf-pathway-convergence, not re-adjudicated here.
