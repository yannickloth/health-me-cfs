...
*For researchers*: read the systems-biology, AI multi-omics, and unifying-mechanisms sections. These frame the formal modeling of Part V and cross-link to Chapter @ch:comparative-nosology.
]
...
Certainty: 0.45 — the discrepancy phenomenon is well-documented (0.60, @ach:subjective-measurable-discrepancy-domains), but the subtyping prediction and treatment-response hypotheses are untested. No prospective study has used a discrepancy index to stratify patients or predict outcomes. Severity applicability: unknown.
...
The following medications in this paper exhibit non-monotonic dose-response patterns; eighteen have a documented dose-range window and constitute the hormesis framework referenced throughout this chapter and Chapter @ch:medication-response-reference (sec-13), while taVNS is a hypothesis-stage candidate whose non-monotonicity is not yet confirmed. This candidate list was generated by pattern-review of this paper's own content and has not been validated in any external dataset — whether these patterns share a common mechanism or are drug-specific artifacts is the core question the hormetic reserve hypothesis poses:
...
*Consequence:* Time-dependent dosing is not a dosing convenience — it is a mechanistic variable at the same level of importance as dose selection. For every drug in the 18-drug hormesis framework, the clinical question is now "what dose AND at what interval?" rather than "what dose?" alone. A drug holiday restore-test costs nothing, requires no lab, and could prevent years of taking a medication that stopped working months ago simply because the resensitization interval was never provided.
