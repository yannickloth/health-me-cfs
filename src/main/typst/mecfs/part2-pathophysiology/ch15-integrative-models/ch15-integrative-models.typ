...
*Hormetic dose-response as a systems principle (triggered by Kevin Lee, personal communication, July 2026):* A recurring pattern across ME/CFS pharmacotherapy is the non-monotonic (inverted-U) dose-response: LDN (optimal dose varies from 0.5--4.5 mg), LDA (therapeutic at 0.2--2 mg, harmful at higher antipsychotic doses), lithium (potential benefit at 2--5 mg/day, null at 10--15 mg, re-emerging at 40--45 mg in different endpoints), melatonin (sleep benefit at 0.5 mg, worsening at 3+ mg), and sulforaphane (theoretical Nrf2 hormetic window). The hormesis framework — in which low-dose stress triggers compensatory adaptive upregulation while higher doses suppress the same pathway — provides a unifying mechanistic explanation that the receptor-level pharmacology alone cannot capture. In ME/CFS specifically, the broken hormetic loop documented in energy metabolism (Step 9, NRF2–PGC-1α bidirectional circuit — where patients cannot exercise to activate beneficial Nrf2/AMPK/PGC-1α signalling, @Ristow2009oxidativehormesis) — may mean the responsivity of hormetic pathways to pharmacological triggers is either exaggerated (hypercompensation at low doses) or blunted (failure to mount compensatory response). Falsifiable: if the hormesis framework is correct, patients with a narrow LDN hormetic window (benefit lost above 1.5 mg) should also show narrow windows for at least one other hormetic drug (LDA, lithium, melatonin) in a within-patient crossover — the inversion-point positions should correlate across drugs (r ≥ 0.4). Falsified if inversion-point correlation across drugs is r < 0.2 — then dose-response curves are drug-specific, not a general systems property. Which direction a patient falls depends on baseline activation state and individual genetic variation in Nrf2, TLR4, and opioid receptor genes. @Calabrese2021Nrf2 @Calabrese2021UltraLow @Calabrese2023NO @Dara2023 @Toljan2018

*Research questions:* Distinguishing which mechanisms are primary drivers vs. secondary consequences remains challenging. Longitudinal studies tracking biomarker trajectories from acute onset are needed. The hormetic dose-response hypothesis is entirely theoretical in ME/CFS — no prospective dose-response trial has tested the inverted-U prediction for any drug in this population. A within-range LDN dose-response trial (0.5, 1.5, 3.0, 4.5 mg) would simultaneously test the hormesis framework and establish the single most clinically relevant data point for the most widely prescribed off-label medication in ME/CFS.
