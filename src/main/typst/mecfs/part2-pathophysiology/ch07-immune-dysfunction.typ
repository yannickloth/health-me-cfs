...
*Certainty: 0.30 (ME/CFS extrapolation).* Microclots have been reported in ME/CFS (Chapter @ch:cardiovascular) but their structural composition (including NET content) has not been directly examined. If NETs stabilize microclots, DNase I therapy could degrade the NET scaffold and destabilize microclots—a mechanism distinct from anticoagulation that would not increase bleeding risk.
...
Study: (systematic review, certainty: 0.70, synthesizes findings across multiple groups).
...
#speculation(title: [pDC Migration Hijack: Blood-to-Tissue Sequestration Model])[
...
#open-question(title: [Can pDC Functional Assays Distinguish Migration from Depletion in Post-Viral ME/CFS?])[
...
Most recently, Glass et al.\ (2025) found upregulation of complement system proteins in extracellular vesicles post-exercise in males with ME/CFS, with ER stress response changes correlating with PEM severity @Glass2025ev. This finding is consistent with the earlier soluble C4a observations and extends them to the vesicular compartment, though the male-only sample limits generalizability.
...
#speculation(title: [Complement Dynamics During PEM: Reconciling Sorensen 2003 vs Nunes 2024])[
*Certainty: 0.45.* Sorensen et al.\ measured C4a elevation at 6 h post-exercise in CFS patients @Sorensen2003complement, establishing complement activation as an acute PEM-phase event. Nunes et al.\ found reduced complement component C4a in ME/CFS plasma at rest (not post-exercise) @Nunes2024S100A9MECFS. These findings can be reconciled by a biphasic complement model: acute activation (Sorensen: elevated 6 h post-exercise) followed by chronic consumption with supranormal fragments but reduced intact component levels in the resting state (Nunes: low C4a at rest). This pattern — activation peaks during exertion, consumption depletes the reservoir between bouts — is characteristic of a system under chronic intermittent demand.

*Time-resolved model.* Phase 1 (0–6 h post-exertion): Complement cascade activated via classical (immune complex-driven) and lectin (endothelial glycocalyx damage) pathways → C4a, C3a, C5a spike. Phase 2 (6–48 h): Anaphylatoxins drive mast cell degranulation and vascular permeability; C5b-9 (MAC) deposits on endothelium → microvascular injury. Phase 3 (48 h+ sustained): Complement component consumption (reduced C4, C3, factor B) persists if activation continues, as seen in post-COVID ME/CFS @Klein2024longcovid. If the patient rests fully, complement returns to baseline by approximately 72 h. If repeat exertion occurs before full recovery, activation sums — explaining why repeated activity without recovery produces escalating PEM severity.
...
#open-question(title: [Does NCS-1/InsP3R1 Amplify Mast Cell Degranulation in ME/CFS?])[
Mast cells express NCS-1 (neuronal calcium sensor protein 1), which amplifies InsP3R1 channel open probability approximately five-fold. In rat basophilic leukemia mast cells, NCS-1 regulates IgE-triggered exocytosis via PI4Kβ, a kinase that generates PIP#sub[2] substrate from PI @Kappel2003NCS1MastExocytosis. NCS-1 also stimulates ERK signaling through endocytic recycling in mast cells @Kappel2006NCS1ERK and co-regulates mast cell function alongside synaptotagmins @Kappel2007NCS1SynaptotagminMast. Lithium disrupts the NCS-1/InsP3R1 interaction at IC#sub[50] ~350 µM @schlecker2006ncs1, raising the possibility that even ultralow concentrations of Li#super[+] could dampen mast cell reactivity by reducing IP3-mediated Ca#super[2+] release. A critical gap must be acknowledged: the estimated tissue Li#super[+] concentration after 2 mg/day oral dosing is likely ~0.2–2 µM, which falls 100–1,000× below the in vitro IC#sub[50] for NCS-1/InsP3R1 disruption. Lithium is not known to concentrate preferentially in mast cells. NCS-1/InsP3R1 disruption at ultralow dose is biochemically implausible unless one of the following applies: (a) Li#super[+] has a higher-affinity binding mode at a different site on NCS-1 or InsP3R1 that is undetected in the existing binding assay; (b) chronic low-dose exposure produces cumulative effects (e.g., altered NCS-1 expression or post-translational modification) not captured by acute IC#sub[50] measurements; or (c) the relevant target is not NCS-1/InsP3R1 but another Li#super[+]-sensitive enzyme (e.g., GSK-3β, inositol monophosphatase) with different dose-response characteristics. Without evidence for any of these conditions, the concentration gap undermines the hypothesis.

The mechanistic chain linking lithium to mast cell stabilization is assembled from separate literatures and has no direct experimental support: (a) Li#super[+] disrupts NCS-1/InsP3R1 — proven in neurons @schlecker2006ncs1; (b) NCS-1 is expressed in mast cells and regulates degranulation — proven by Kappel 2003; (c) MRGPRX2, C3aR, and C5aR signal through Gαq → PLC → IP3 → Ca#super[2+] → degranulation — well-established (reviewed in Roy 2021 @Roy2021MRGPRX2Review); (d) Orai/STIM1-dependent SOCE is required for sustained MRGPRX2-mediated Ca#super[2+] responses @Chaki2022OraiMRGPRX2 — damping IP3R would reduce both the initial Ca#super[2+] spike and the sustained SOCE. The connection between (a) and (b) is entirely unattested — no study has tested lithium's effect on mast cell degranulation at any concentration. One 1978 study found LiCl did NOT cause mast cell degranulation in rat mesentery @Nahmod1978LithiumMastCellNull, consistent with a stabilizing rather than activating effect, but this is a weak negative control (n small, 1978, indirect).

If the NCS-1/InsP3R1 → mast cell degranulation link operates in ME/CFS, ultra-low-dose lithium (~2 mg/day elemental Li#super[+]) could act as a functional mast cell stabilizer via a mechanism distinct from existing agents: cromolyn (mechanism incompletely understood; calcium influx inhibition), ketotifen (H1 antagonist + mast cell stabilization; mechanism unclear), and omalizumab (IgE sequestration). Lithium would target the GPCR→PLC→IP3→Ca#super[2+] arm of mast cell activation — particularly relevant if GPCR autoantibodies (anti-M3, anti-β2AR) chronically engage this pathway in mast cells, lowering the degranulation threshold. Notably, both FcεRI (ITAM→Syk→PLCγ) and GPCR (Gαq→PLCβ) pathways converge on IP#sub[3]→InsP#sub[3]R#sub[1]→Ca#super[2+] mobilization, so NCS-1 amplification via PI4Kβ — which supplies the PIP#sub[2] substrate for both PLC isoforms — could theoretically influence either route. However, the relative dependence on NCS-1 may differ: the Gαq-PLCβ pathway may rely more heavily on PI4Kβ-mediated PIP#sub[2] resynthesis because PLCβ is more sensitive to PIP#sub[2] availability than PLCγ, which can be recruited to membrane phosphotyrosine clusters. Consequently, lithium should reduce IgE-mediated degranulation less than MRGPRX2-mediated degranulation, rather than leaving it unaffected. This partial-effect prediction is more defensible than a binary affected/unaffected claim and can be resolved experimentally by comparing the dose-response curves for both stimuli. Whether lithium ion concentrations reach mast cells in tissue at sufficient levels after 2 mg/day oral dosing is completely unknown.
...
#synthesis(title: [Mast Cells Converge on ME/CFS from Four Independent Routes])[
The paper integrates six distinct mast cell integration cycles (Domain 6 @sec:domain6-mast-cell, MCAS-ISR crosstalk, Theoharides 2026 neuroimmune axis, GPCR-PIP2-lithium axis, IgE→CT degradation @spec:acquired-progressive-hypermobility-mc, and MCAS-as-primary-driver analyses). These collectively identify at least four independent mechanisms by which mast cells may contribute to ME/CFS pathophysiology: (1) IgE-FcεRI-mediated connective tissue degradation via MMP activation and histaminylation (chapters @ch:immune-dysfunction, @ch:speculative-hypotheses); (2) GPCR autoantibody-mediated sensitisation via Gαq→PLC→PIP₂→IP3→Ca²⁺ lowering of degranulation threshold, with lithium as a candidate functional mast cell stabiliser via NCS-1/InsP3R1 disruption @oq:lithium-mast-cell-ncs1; (3) mechanical stress-sensitised degranulation via CADM1 adhesion structures @hyp:cadm1-mechanical-mast and matrix stiffness feedback @hyp:matrix-stiffness-mast, creating a self-reinforcing cycle in hypermobile tissue; and (4) pathogen-associated activation via EBV protein-driven MMP-9 release @spec:ebv-mast-cell-mmp9 and MRGPRX2 imprinting @spec:mrgprx2-long-covid-mast. No single drug blocks all four routes, and the relative contribution of each is unknown — the omalizumab therapeutic probe @oq:omalizumab-ct-therapeutic-probe tests only route (1). The permissive matrix hypothesis @hyp:permissive-matrix-mcas proposes a unifying structural vulnerability (ECM abnormality enabling mast cell tissue infiltration) but lacks tissue-level validation. Three cross-cutting open questions remain: what fraction of ME/CFS symptom burden in MCAS-comorbid patients is attributable to each activation route; whether combination therapy targeting multiple routes (anti-IgE + MC stabiliser + tVNS) produces synergistic benefit; and whether mast cell activation is a primary driver or a secondary amplifier of pre-existing pathophysiology.
...
Acute viral infection — including SARS-CoV-2, EBV, and HHV-6, all documented triggers of ME/CFS — induces endothelial cell senescence at the blood-brain barrier, cerebral arteries, gastrointestinal microvasculature, and skeletal muscle capillaries. Senescent endothelial cells are not simply dormant; they develop a *senescence-associated secretory phenotype (SASP)* that renders them actively harmful:
    ...
    - *Proinflammatory*: SASP includes IL-6, IL-8, TNF-$alpha$, and other cytokines that sustain chronic inflammation. The central molecular trigger for the SASP is mTORC1-mediated translational upregulation of IL-1$alpha$, which activates NF-$kappa$B and thereby drives expression of the entire SASP cytokine program @Laberge2015SASP; rapamycin selectively suppresses this inflammatory arm of the SASP without affecting cell cycle arrest @Laberge2015SASP @Rolt2019IL6
...
*Cross-reference.* This mechanism complements the HIF-1alpha-Mitochondria-ECM Triad (Chapter @ch:energy-metabolism, Hypothesis @hyp:hif1alpha-mito-ecm-triad) and ECM1-Axis Dysfunction (Speculation @spec:ecm1-axis-mecfs). Together, these three hypotheses describe overlapping but distinct pathways by which immune dysfunction and metabolic stress converge on connective tissue pathology.
...
#speculation(title: [ECM Fragment Autoantigen Generation in ME/CFS])[
*Certainty: 0.35.* Extracellular matrix degradation releases matrikines—bioactive ECM fragments—into circulation, where they act as damage-associated molecular patterns (DAMPs) and autoantigens that trigger or sustain autoimmunity in ME/CFS. This "ECM leak" hypothesis connects connective tissue degradation to the GPCR autoantibody phenomenon (Section @sec:gpcr-autoantibodies). @Wirth2026CTDMECFS

*Mechanism.* When collagen, elastin, laminin, or fibronectin are degraded by MMPs or other proteases, bioactive fragments are released. Some of these fragments (e.g., collagen type II peptides, elastin fragments) are established neoantigens that trigger autoantibody production in other diseases (rheumatoid arthritis, systemic sclerosis). In ME/CFS, chronic ECM degradation from mast cell MMP release (Hypothesis @hyp:matrix-stiffness-mast), inflammatory priming (Hypothesis @hyp:inflammatory-ecm-priming), or HIF-1alpha-driven MMP upregulation (Chapter @ch:energy-metabolism) would continuously release ECM fragments into circulation. These fragments could: (1) activate pattern recognition receptors (TLRs, NLRs) on immune cells, sustaining low-grade inflammation; (2) serve as neoantigens for autoantibody generation, explaining the GPCR autoantibodies documented in ME/CFS; (3) form immune complexes that deposit in vascular tissue, contributing to microvascular pathology.
...
#speculation(title: [Tendinopathy-ME/CFS Shared HIF-1alpha-VEGF-MMP-3 Cascade])[
*Certainty: 0.35.* Moschini et al. established that HIF-1alpha drives tendinopathy through a VEGF-MMP-3 cascade that operates independently of classical VEGF-mediated angiogenesis @Moschini2026HIFTendinopathy. ME/CFS may represent a systemic tendinopathy with the same HIF-1alpha-VEGF-MMP-3 cascade operating diffusely across tissues, explaining the overlap between ME/CFS symptoms and connective tissue pathology. @Sahin2012HIFTendon

*Mechanism.* In the Moschini model, HIF-1alpha stabilizes in hypoxic or inflamed tendon tissue, upregulating both VEGF and MMP-3. VEGF does not primarily induce angiogenesis in this context but acts as a survival factor for tenocytes and increases vascular permeability, allowing inflammatory cell infiltration. MMP-3 (stromelysin) degrades tendon ECM, directly weakening tissue structure. Critically, HIF-1alpha-driven MMP-3 upregulation is VEGF-dependent—blocking VEGF signaling reduces MMP-3, indicating a linear cascade. If ME/CFS represents systemic HIF-1alpha elevation (Chapter @ch:energy-metabolism documents HIF-1alpha in ME/CFS), then the same cascade could operate diffusely in muscle, connective tissue, and vascular adventitia, producing systemic ECM weakening.
...
#speculation(title: [Gut Microbiome-ECM Degradation Axis])[
*Certainty: 0.30.* Gut dysbiosis produces bacterial proteases that degrade connective tissue locally (intestinal wall) and systemically, contributing to connective tissue pathology and autoantibody generation against collagen/elastin in ME/CFS. This microbiome-ECM axis bridges the gut-immune interface (Chapter @ch:gut-microbiome) to systemic connective tissue pathology. @Wirth2026CTDMECFS

*Mechanism.* The gut microbiome produces proteases as part of normal metabolism, but dysbiotic states are characterized by altered protease profiles—increased elastase, collagenase, and gelatinase-producing bacteria. These proteases can: (1) degrade intestinal wall collagen and elastin, increasing gut permeability ("leaky gut"); (2) enter systemic circulation via the compromised barrier; (3) degrade extracellular matrix throughout the body, particularly in vascular walls where they access connective tissue; (4) release ECM fragments that act as neoantigens, triggering autoantibody production. This creates a self-reinforcing cycle: dysbiosis $arrow.r$ protease production $arrow.r$ gut barrier breakdown $arrow.r$ systemic protease spread $arrow.r$ connective tissue degradation $arrow.r$ immune activation $arrow.r$ further dysbiosis.

*Evidence from other conditions.* In inflammatory bowel disease, bacterial proteases contribute to tissue damage and fistula formation. In systemic sclerosis, altered gut microbiome and increased bacterial protease activity correlate with fibrosis. In ME/CFS, gut dysbiosis is well-documented (Chapter @ch:gut-microbiome), and connective tissue pathology is clinically apparent (hypermobility, skin hyperextensibility). The missing link—whether dysbiosis-derived proteases mediate this connection—has not been tested.
...
#speculation(title: [ATG13 Neutralization via Monoclonal Antibodies])[
