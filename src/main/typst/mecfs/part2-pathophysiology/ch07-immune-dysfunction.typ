...
The largest comparative immunophenotyping study to date (n=207 total; 103 ME/CFS, 63 long COVID, 41 healthy controls — though ''largest'' is relative to a field with few comparative studies) provides a refined picture of monocyte dysfunction in ME/CFS @Petrov2026monocyteDC. Using 12-parameter flow cytometry, Petrov et al.\ demonstrated that ME/CFS monocytes exhibit reduced expression of the costimulatory molecule CD80 on M1-like (CD206#super[low]) monocytes — a finding that is paradoxical given M1-polarised cells typically *upregulate* CD80, suggesting a functional defect within the M1-like subset beyond polarisation state. ME/CFS monocytes also showed impaired C-C chemokine receptor type 7 (CCR7) expression, a receptor essential for lymph node homing and immune cell trafficking (though reduced CCR7 could also reflect tissue redistribution after homing, rather than a primary trafficking defect). At the same time, Petrov et al.\ found T cell activation features in ME/CFS — increased CD8+ effector memory/terminal differentiation (CD45RA+CD62L#super[-]) and elevated CD4+CD95#super[+] — which complicate a uniform ''immune suppression'' label. The monocyte/DC compartment therefore appears predominantly suppressively skewed, with concurrent T cell activation, in contrast to the persistent immune activation with exhaustion observed in long COVID (M2-like monocyte polarisation, elevated CD80, dendritic cell expansion). The study achieved moderate discrimination between ME/CFS and long COVID using composite immune markers (VIP scores: CD80 on monocytes, CCR7 on dendritic cells, M2-like polarisation); AUC was not reported, and the magnitude of ''moderate'' discrimination is therefore unquantified. Certainty: 0.50. #text(style: "italic")[Limitations: single-center, cross-sectional, unreplicated; healthy controls significantly younger than patient groups (age-adjusted ANCOVA applied, but residual confounding possible given age-dependent CCR7 expression); excluded common ME/CFS medications (LDN, antihistamines, beta-blockers) — which may bias the cohort toward milder or less-treated patients; vaccination status not analysed; significant within-group heterogeneity with partial inter-group overlap; post-hoc power (≥ 0.80) is reported in the source paper but conveys no information beyond the observed p-values (Hoenig & Heisey 2001).] Certainty reduced from initial 0.70 because the study is unreplicated, medication exclusions limit generalisability, and functional costimulatory capacity was not measured (surface expression ≠ function).

*Macrophage Polarization*
<obs:macrophage-polarization>
Tissue macrophages can adopt pro-inflammatory (M1) or anti-inflammatory (M2) phenotypes. Evidence suggests M1 polarization in ME/CFS, with impaired transition to the resolving M2 phenotype, resulting in chronic inflammatory macrophage activation @heng2025mecfs. #text(style: "italic")[Note: the Petrov finding of reduced CD80 on M1-like monocytes (above) creates an unresolved tension with M1-polarisation claims — M1-polarised cells normally upregulate CD80, so reduced CD80 on M1-like cells may indicate a dysfunctional M1 state (polarised but functionally impaired), an M1 surface marker profile that does not reflect true M1 functional polarisation, or divergence between tissue macrophages and circulating monocytes.]
=== Complement System
<sec:complement>
...
The dissociation between pain transfer (consistent across all four studies) and cognitive transfer (absent in Mignolet 2026 and Chen 2026; present for balance/coordination and fatigue-like behaviour in Santos Guedes 2026 @SantosGuedes2024autoantibodiesLC) suggests that different symptom domains may be mediated by distinct IgG populations with different antigen targets: DRG-targeting IgG for pain and peripheral sensitisation versus CNS-protein-targeting IgG for balance, coordination, and potentially fatigue.
...
Study: (mechanistic inference from Santos Guedes 2026 BBB crossing data + FcRn biology; certainty: 0.30, downgraded from 0.45 after adversarial review because: the quantitative 3--10× amplification factor is extrapolated from blood-placenta and gut barriers, not measured at the BBB; FcRn expression and kinetics at the human BBB are poorly characterised; CNS FcRn may serve protective functions making blockade unpredictable; speculative. Falsification condition: hypothesis refuted if FcRn blockade does NOT produce disproportionate CNS IgG reduction (>70% beyond serum IgG reduction) in passive-transfer mice. If CNS:serum IgG ratio remains proportional under FcRn blockade, the amplification amplifier does not exist at the BBB and the hypothesis is wrong. The "FcRn kinetics at human BBB poorly characterised" caveat expires when CNS FcRn expression and kinetics are directly measured.).
...
*Certainty: 0.35.* (Single unreplicated study; age mismatch confound; mouse data for inflammaging model. 0.30→0.35: convergence with HMGB1 DAMP — both feed into NLRP3 inflammasome endpoint via distinct pathways @HMGB1DAMPRegistry.)
