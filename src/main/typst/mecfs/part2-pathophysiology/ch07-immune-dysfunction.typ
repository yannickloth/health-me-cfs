...
Infectious mononucleosis (IM), caused by primary EBV infection, may trigger a quantitatively more severe form of ME/CFS involving three simultaneous immune compartment failures. First, massive polyclonal B cell activation during acute IM establishes a larger latent EBV reservoir in CD27+ memory B cells than subclinical EBV seroconversion. Second, NK cells, which expand dramatically during acute IM to control the infection, may become functionally exhausted as the massive acute response resolves, failing to maintain long-term EBV surveillance. Third, CD8+ T cells targeting EBV epitopes develop exhausted phenotypes (PD-1+, TIM-3+, LAG-3+) from chronic antigen exposure, mirroring the T cell exhaustion already documented in ME/CFS. The consequence is an EBV-specific ME/CFS subtype with a distinct immunological fingerprint: larger viral reservoir, impaired innate and adaptive antiviral surveillance, and enrichment of autoantibodies cross-reactive with EBV antigens. Whether this subtype differs from non-IM post-infectious ME/CFS in treatment response---particularly to antivirals such as valacyclovir or to immunomodulatory therapies---remains an open question. The IM association is disproportionately concentrated in early-onset ME/CFS (OR 2.32) @McGrath2026bimodalOnset, consistent with the thymic-EBV synchrony model (Section @sec:bimodal-cross-disease) where adolescent thymic involution coincides with peak EBV exposure.
...
*Falsifiable prediction:* Head-to-head immunophenotyping of IM-triggered vs non-IM ME/CFS will show: (a) EBV reservoir in CD27+ memory B cells > 2 SD higher in IM-triggered group (effect size d > 0.8); (b) NK cytotoxicity (K562 assay) > 1.5 SD lower (d > 0.6); (c) exhausted CD8+ T cell frequency (PD-1+TIM-3+LAG-3+) > 2 SD higher (d > 0.8). If any of these show no significant difference (p > 0.2, d < 0.3) or the pattern reverses direction, the three-compartment distinct subtype hypothesis is not supported.
