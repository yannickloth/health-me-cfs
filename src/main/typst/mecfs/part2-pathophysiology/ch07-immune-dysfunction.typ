...
If the NCS-1/InsP3R1 → mast cell degranulation link operates in ME/CFS, ultra-low-dose lithium (~2 mg/day elemental Li#super[+]) could act as a functional mast cell stabilizer via a mechanism distinct from existing agents: cromolyn (mechanism incompletely understood; calcium influx inhibition), ketotifen (H1 antagonist + mast cell stabilization; mechanism unclear), and omalizumab (IgE sequestration). Lithium would target the GPCR→PLC→IP3→Ca#super[2+] arm of mast cell activation — particularly relevant if GPCR autoantibodies (anti-M3, anti-β2AR) chronically engage this pathway in mast cells, lowering the degranulation threshold. This mechanism would be inoperative for IgE-mediated (FcεRI) activation, which uses a different Ca#super[2+] mobilization pathway (ITAM→Syk→PLCγ, not Gαq). Whether lithium ion concentrations reach mast cells in tissue at sufficient levels after 2 mg/day oral dosing is completely unknown.

*Testable predictions.* (1) NCS-1 expression in ME/CFS-derived mast cells (cultured from peripheral blood CD34#super[+] progenitors) should be quantified — predicted to be elevated as a compensatory response to chronic GPCR stimulation. (2) Lithium at 1-10 µM pre-incubation should reduce β-hexosaminidase release from patient-derived mast cells stimulated with MRGPRX2 agonists (compound 48/80, substance P) by ≥30%. Falsified if lithium has no effect on β-hexosaminidase release or increases it. (3) A lithium effect should be specific to GPCR-mediated activation: IgE-mediated degranulation (anti-DNP-IgE + DNP-BSA) should be unaffected, confirming pathway specificity. (4) Plasma tryptase should decrease after 4 weeks of lithium 2 mg/day in MCAS-positive ME/CFS patients — though baseline tryptase is frequently normal in MCAS, limiting the utility of this biomarker.
