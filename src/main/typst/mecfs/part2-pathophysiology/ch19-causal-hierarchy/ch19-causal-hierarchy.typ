...
Before examining each candidate, what is _not_ on this list is equally important. Mitochondrial dysfunction, arguably the most studied mechanism in ME/CFS, is not classified as trigger-capable. Neither is gut dysbiosis, immune dysregulation considered generically, or hormonal disruption. This may seem counterintuitive given the evidence for each of these processes. The reason is Criterion 1: none of these mechanisms arises directly from known ME/CFS precipitants without an intermediary. Mitochondrial dysfunction does not spontaneously develop in response to viral infection; it develops because viral infection triggers neuroinflammation, metabolic suppression, or autoimmunity, which then damages mitochondria. The mitochondrial damage is real and consequential, but it is an amplifier of the upstream process, not the initiating event. The same applies to the reproductive axis: it is a modulatory/amplifier layer that changes how symptoms are expressed across the menstrual cycle, pregnancy, and the menopause transition, but it is not a trigger-capable root cause (see the reproductive-lifespan synthesis @syn:fra-reproductive-lifespan-model). This classification does not diminish its clinical importance — for many women, cycle-phase or menopausal amplification is a principal determinant of daily symptom burden.
...
From CNS hypometabolism, the cascade propagates through every system the brain coordinates:
    ...
    - *Sleep architecture disruption* (Chapter @ch:neurological): Sleep-wake cycling depends on the coordinated activity of hypothalamic orexin neurons, ventrolateral preoptic area GABAergic neurons, and brainstem monoaminergic nuclei. In rodent models, inflammatory cytokines actively suppress orexin neuron firing @Grossberg2011orexinLethargy, and partial orexin deficiency produces the sleep fragmentation and REM-gating failures characteristic of narcolepsy Type 2 @Ito2023orexinREM — a pattern that may overlap with ME/CFS sleep dysfunction (Section @spec:mecfs-functional-narcolepsy, certainty 0.30). Energy-depleted sleep-regulating circuits cannot maintain normal slow-wave sleep architecture, producing the unrefreshing sleep universal in ME/CFS.
...
#include "sec-01-misc/subsec-02-metabolic-safe-mode-lock/hypotheses/hyp-metabolic-safe-mode-as-trigger-capable-r.typ"
...
#include "sec-01-misc/subsec-03-safe-mode-threat-signal-miscalibration/speculations/spec-oxidative-stress-sensing-polymorphisms-a.typ"
...
The autoimmune hypothesis faces a structural tension with the genetic evidence that must be acknowledged directly. The DecodeME GWAS and Maccallini 2026 meta-analysis find zero immune cell-type enrichment — no signal in T cells, B cells, NK cells, or macrophages (Section @syn:genetic-neuronal-convergence). This does not refute the autoimmune hypothesis — the GPCR autoantibody cascade can be acquired (infection-triggered molecular mimicry or bystander activation) rather than genetically encoded — but it does constrain the interpretation. The autoantibodies cannot be the downstream expression of a constitutional immune predisposition toward autoimmunity, because no such predisposition is detectable. They must instead represent an acquired immunological event — a post-infectious break in tolerance — occurring in a host whose genetic vulnerability lies elsewhere (in neuronal circuits). The two-hit model resolves this tension: the neuronal genetic vulnerability (hit 1) determines _who_ is susceptible, and the acquired autoimmune event (hit 2) determines _how_ the disease initiates in a given patient. The strongest independent evidence supporting this framing is the 2026 Germain et al. REAP study (n=172), which found no autoantibody reactivity in chronic, pre-COVID ME/CFS using an unbiased proteome-wide screen — consistent with GPCR autoantibodies being an acquired, trigger-specific phenomenon rather than a genetically encoded autoimmune diathesis.

The evidence base is substantial though internally conflicting, a tension that demands honest assessment. Elevated anti-$beta_2$-adrenergic and anti-muscarinic autoantibodies are reported in 29–91% of ME/CFS patients across studies @Loebel2016 @Bynke2020, with the wide range reflecting differences in assay methodology, patient selection, and definition of “elevated.” Symptom severity correlates with autoantibody titers in some studies @Sotzny2021 @Azcue2026gpcr, though the correlation is modest and not replicated in all cohorts.
...
#include "sec-01-misc/subsec-04-gpcr-autoantibody-cascade/hypotheses/hyp-gpcr-autoantibody-cascade-as-trigger-cap.typ"
...
#include "sec-01-misc/subsec-05-trpm3-channelopathy/hypotheses/hyp-trpm3-channelopathy-as-trigger-capable-r.typ"
...
#include "sec-01-misc/subsec-06-trpm3-sensitization-via-pregnenolone-sul/speculations/spec-pregnenolone-sulfate-as-trpm3-sensitizer.typ"
...
#include "sec-01-misc/subsec-05-gut-dysbiosis-amplifier-or-fifth-root-ca/open-questions/oq-gut-dysbiosis-trigger-capable-in-a-subgr.typ"
...
#include "sec-01-misc/subsec-07-epigenetic-consolidation/speculations/spec-consolidation-as-loss-of-methylation.typ"

#include "sec-01-misc/subsec-08-scope-clarification-cancer-vs-me-cfs-mec/speculations/spec-icf-syndrome-as-proof-that-pericentromer.typ"
...
#include "sec-01-misc/subsec-01-the-subthreshold-reservoir-population/speculations/spec-subthreshold-reservoir-population.typ"
...
#include "sec-06-load-bearing-versus-secondary-locks/hypotheses/hyp-load-bearing-lock-identification.typ"
...
#include "sec-07-treatment-implications-of-the-causal-hie/warnings/warn-treatment-priority-eq-not-causal-priorit.typ"
...
#include "sec-01-misc/subsec-01-the-patient-accessible-epigenetic-revers/warnings/warn-not-a-treatment-recommendation.typ"

#include "sec-01-misc/subsec-01-the-patient-accessible-epigenetic-revers/speculations/spec-timed-epigenetic-reversal-strategy.typ"
...
#include "sec-08-research-implications/open-questions/oq-experimental-validation-of-causal-hierar.typ"
