...
Global methylation analyses reveal both hypomethylation and hypermethylation in ME/CFS, with different genes showing methylation changes in opposite directions. This bidirectional pattern — global hypomethylation at repetitive elements with focal hypermethylation at gene promoters — may reflect DNMT3B redistribution away from heterochromatin (ProB repeats) toward euchromatin (ProA elements), a mechanism established in cancer @BonnetFourel2026ProAB and hypothesized to operate in ME/CFS through viral-infection-induced DNMT3B dysfunction rather than cancer-associated redistribution (see Chapters @ch:immune-dysfunction and @ch:causal-hierarchy, Speculation @spec:methylation-loss-consolidation). The per-locus vector model in Chapter @ch:causal-hierarchy-formal (@oq:consolidation-directionality) unifies both directions under the single mechanism: DNMT3B redistribution produces coupled gain and loss simultaneously, with the balance between loci determining whether a patient is gain-dominant, loss-dominant, or mixed. The ME/CFS methylation signature suggests dysregulated methylation machinery rather than unidirectional change, consistent with altered activity of DNA methyltransferases (DNMTs) and ten-eleven translocation (TET) demethylases.
