...
ATP yields in this chapter are reported on a _glucose basis_: ATP molecules produced per mole of glucose fully oxidized under aerobic conditions. The textbook total is approximately 30--32 ATP per glucose, using consensus P/O ratios of 2.5 for NADH and 1.5 for FADH#sub[2] (Hinkle 2005). Each step is reported with three numbers: _direct yield_ (ATP or ATP-equivalent reducing equivalents produced at this step), _gated yield_ (downstream ATP that depends on this step completing), and _percent of total_ (gated yield / ~30 ATP).
...
#open-question(title: [Does Intrapulmonary Shunt Contribute to Tissue Hypoxia in ME/CFS?])[
Intrapulmonary right-left shunt — blood traversing the pulmonary circulation without being properly oxygenated — is a recognized cause of exertional hypoxemia. In healthy individuals, the shunt fraction (Qs/Qt) is approximately 3--5%. In post-COVID patients, elevated shunt (~8%) persists at 6 months despite normal spirometry, normal DLCO, and normal lung volumes @Farrow2023IntrapulmonaryShunt @Sandhu2026AlveolarDeadspace. This "normal PFTs with desaturation" paradox — which mirrors the pattern of exertional dyspnea in some ME/CFS patients — is mechanistically explained by intrapulmonary arteriovenous anastomoses (IPAVAs) that persist in ~30% of adults at rest and open during exertion @Lyne2024IntrapulmonaryShunt. SARS-CoV-2 impairs hypoxic pulmonary vasoconstriction (HPV), the protective reflex that normally diverts blood flow away from poorly ventilated lung regions @Li2024RepercussionsShunt — a mechanism that, if it generalizes to other viral triggers, could increase shunt vulnerability in post-infectious ME/CFS. Hyperpolarized #super[129]Xe MRI studies in breathless Long COVID patients show impaired gas exchange (reduced RBC:tissue-plasma ratio) despite normal CT and PFTs @Grist2022LungAbnormalities — a pattern consistent with shunt rather than ventilation-perfusion mismatch.
...
*Directional ambiguity — a two-phase model reconciles both arrows.* This speculation frames shunt as *causing* prolonged hypoxia that drives PEM. However, a reversed causal arrow is equally mechanistically plausible and supported by different chemistry: PEM involves a well-documented post-exertional immune cascade (mast cell degranulation at ~30 min--2h, cytokine elevation at ~2--6h, complement activation) in which histamine, tryptase, C3a/C5a, bradykinin, and iNOS-derived NO are all pulmonary vasodilators — each capable of overriding HPV and worsening shunt. Under this reversed model, the initial HPV impairment produces only mild post-exercise shunt (Phase 1, 0--2h, insufficient alone to cause PEM), and the subsequent PEM immune cascade amplifies shunt via vasodilation (Phase 2, 2--48h, producing the deep hypoxia that locks in the crash). The two-phase model makes a specific, testable prediction that neither single-direction model makes: serial Qs/Qt measurements after exercise should show a *bimodal pattern* — an early elevation at 0--1h (HPV failure) followed by a second, larger elevation at 2--6h coinciding with the cytokine peak (immune vasodilation). A single early peak without the late peak favors the paper's model; a single late peak without the early peak favors the immune-amplification model; both peaks confirm the two-phase model. Until serial post-exercise Qs/Qt is measured, the direction of the shunt--PEM relationship is unresolved, and the *consequence* framing above — targeting the early window for PEM prevention — may be premature if the late immune-driven amplification is the dominant driver.
] <spec:post-exertional-shunt-window>

#synthesis(title: [Intrapulmonary Shunt: A Testable Upstream Candidate for Pre-Capillary Oxygen Deficit])[
The intrapulmonary shunt environments (@oq:intrapulmonary-shunt-mecfs, @spec:rbc-ipava-synergy, @spec:post-exertional-shunt-window) and the cross-disease bridge in Chapter @ch14d (@spec:intrapulmonary-shunt-postviral, cert 0.35; @spec:shunt-hif-imbalance, cert 0.25) collectively propose a candidate mechanism that is conceptually distinct from the microcirculatory and mitochondrial dysfunction already established in this chapter: *reduced arterial oxygen content before blood reaches the capillaries*. The core evidence — persistent Qs/Qt ~8% at 6 months post-COVID with normal spirometry, normal DLCO, and normal lung volumes @Farrow2023IntrapulmonaryShunt — establishes that intrapulmonary shunt can explain exertional hypoxemia with normal pulmonary function, a pattern that overlaps with the exertional dyspnea reported in a subset of ME/CFS patients. The anatomical substrate (IPAVAs present in ~30% of adults @Lyne2024IntrapulmonaryShunt) and the documented HPV impairment after SARS-CoV-2 @Li2024RepercussionsShunt provide a mechanistic basis for shunt persistence. The critical open question is whether this mechanism operates in ME/CFS at all: zero direct measurements exist, and the SARS-CoV-2 ACE2 tropism that may drive HPV impairment is not shared by classical ME/CFS triggers. Until Qs/Qt is measured in ME/CFS patients — feasible with existing gas exchange equipment and potentially answerable from archived CPET A-a gradients — shunt remains a plausible but entirely unconfirmed contributor. If confirmed, the shunt mechanism would add a *pulmonary-level* source of oxygen deficit to the paper's existing model of capillary-level (endothelial, RBC) and tissue-level (mitochondrial) impairment. If refuted — i.e., Qs/Qt is normal in ME/CFS — the paper's emphasis on microcirculatory and mitochondrial primary dysfunction is strengthened. The downstream speculations (RBC-IPAVA synergy, post-exertional shunt window, HIF isoform imbalance) are explicitly contingent on the base hypothesis and collapse if shunt is absent. The most important open question is not whether shunt *could* explain tissue hypoxia — the physiology supports that — but whether it *does* in the specific context of ME/CFS pathophysiology.

*Synthesis caveat — directional ambiguity:* The post-exertional shunt window speculation (@spec:post-exertional-shunt-window) frames shunt as *causing* PEM via prolonged hypoxia. A reversed model — PEM immune cascade → pulmonary vasodilation → shunt worsening → amplified crash — is equally mechanistically plausible given the well-documented pulmonary vasodilatory effects of mast cell mediators, complement anaphylatoxins, bradykinin, and iNOS-derived NO released during PEM. The two models are not mutually exclusive: a two-phase model (early HPV-driven shunt at 0--2h, late immune-amplified shunt at 2--48h) reconciles both arrows and makes a specific testable prediction — serial Qs/Qt should show a bimodal post-exercise pattern — but this has never been measured. Until serial post-exercise shunt kinetics are characterized, the direction of the shunt--PEM relationship is an open empirical question, and therapeutic implications (targeting the early vs. late window) depend on which phase dominates.

*Consequence:* A single measurement (Qs/Qt or A-a gradient in existing CPET archives) could either open a new therapeutic axis — pulmonary-targeted interventions for oxygen delivery — or eliminate shunt as a candidate mechanism and redirect attention entirely to microcirculatory and mitochondrial targets. Until that measurement is made, shunt is a physiologically grounded but empirically empty placeholder in the energy metabolism cascade.
] <syn:intrapulmonary-shunt-model>
...
#achievement(title: [High-Quality Evidence for CoQ10 Deficiency and Treatment])[
The Maes 2009 case-control study (n=58 ME/CFS, 22 controls) demonstrated that 44.8% of ME/CFS patients have plasma CoQ10 below the lowest control value (490 micro g/L), with lower levels correlating with fatigue, autonomic, and neurocognitive symptoms @Maes2009CoQ10. A 2022 meta-analysis of 13 RCTs (n=1,126) found CoQ10 significantly reduces fatigue (Hedges' g = -0.398, p = 0.001) with dose-dependent (coefficient = -0.0017 per mg, p < 0.001) and duration-dependent (coefficient = -0.0042 per day, p = 0.007) effects @Tsai2022CoQ10fatigue. Novel cocrystal ubiquinol formulations show 2.01-3.43x higher bioavailability vs standard ubiquinone @Mei2026UbiquinolBioavailability.
]
...
#figure(
  kind: table,
  caption: [Step 7 (ANT) research gap prioritization.],
  table(
    columns: (auto, auto, auto, auto, auto, auto, auto),
    align: (left, left, left, left, left, left, center),
    table.header([*Gap*], [*Essentiality*], [*Worst-case impact*], [*Tractability*], [*Therapeutic leverage*], [*Measurement*], [*Priority*]),
...
#figure(
  kind: table,
  caption: [Step 9 (ROS scavenging) research gap prioritization. Step 9 is net-negative for ATP; failure is amplificatory, propagating damage to Steps 3, 5, 6, 7.],
  table(
    columns: (auto, auto, auto, auto, auto, auto, auto),
    align: (left, left, left, left, left, left, center),
    table.header([*Gap*], [*Essentiality*], [*Worst-case impact*], [*Tractability*], [*Therapeutic leverage*], [*Measurement*], [*Priority*]),
...
*Certainty: 0.45. (0.40→0.45: two independent lines—SNS/PKA→AMPK suppression via Brown2018AMPK and NAD+→SIRT1→PGC-1α deacetylation via the sirtuin hub @sec:nad-sirtuin-acetylation-hub—converge on PGC-1α functional suppression through different mechanisms [autonomic signaling and metabolic co-substrate depletion] from different labs using different methods.)*
...
The Brown et al.\ finding that pharmacological AMPK activators (metformin, compound 991) rescued the deficit @Brown2018AMPK suggests that the AMPK machinery itself is intact---the problem is upstream signaling. If sympathetic PKA is the upstream suppressor, then interventions reducing sympathetic tone ($beta$-blockers, vagal nerve stimulation) could theoretically restore AMPK activity without requiring direct pharmacological activation. This is speculative: no study has tested whether autonomic modulation improves AMPK phosphorylation in ME/CFS tissue.

==== mTOR/AMPK Balance and Autophagy Regulation
<sec:mTOR-AMPK-autophagy-balance>

The AMPK/mTOR axis forms a bistable homeostatic switch: when cellular energy is abundant (low AMP/ATP ratio), mTORC1 is active, promoting anabolic processes (protein synthesis, ribosome biogenesis) while phosphorylating ULK1 at Ser757 to inhibit autophagy initiation @Rachakatla2022AMPK. When energy is scarce (high AMP/ATP), AMPK activates and simultaneously inhibits mTORC1 (via phosphorylation of RAPTOR at Ser792 and TSC2) and directly activates ULK1 at Ser317/Ser777 to trigger autophagosome formation @Rachakatla2022AMPK. The two pathways converge on ATG13: mTORC1 phosphorylates ATG13 at Ser258 to prevent ULK1/ATG13/FIP200 complex assembly --- the rate-limiting step for autophagosome nucleation @Mannick2023mTOR.
...
The Drosen et al.\ (2025) murine model provides direct causal evidence for this cascade in a PEM-like context: genetic inactivation of ATG13 (mimicking the mTOR-driven phosphorylation block) triggers M1 macrophage infiltration into muscle tissue via STAT3 activation, producing IL-6 and RANTES (CCL5), followed by progressive demyelination of muscle-serving nerves @Drosen2025ATG13. The mice develop reproducible exercise intolerance with symptom worsening 24--48 hours after exertion --- mirroring the characteristic PEM time course. An extension study by the same group (Toriola et al.\ 2026) confirmed that ATG13 depletion also impairs mitochondrial energy metabolism, increases oxidative stress, and polarizes macrophages to the M1 inflammatory mode @Toriola2026ATG13.

The mTOR/AMPK imbalance also directly impacts mitochondrial quality control through PGC-1$alpha$ regulation: mTORC1 hyperactivation suppresses PGC-1$alpha$ expression, while AMPK and SIRT1 activate it --- creating a coordinated block on both mitophagy (removal of damaged mitochondria) and biogenesis (production of new mitochondria) @HuangY2025AMPK. This dual blockade would explain the paradoxical observation in ME/CFS of both impaired AMPK activation and upregulated PGC-1$alpha$ mRNA @wang2023wasf3: the cell senses mitochondrial insufficiency and transcribes compensatory PGC-1$alpha$, but mTOR-driven translational preference and SIRT1 deacetylase insufficiency render the PGC-1$alpha$ protein functionally inactive.

@hyp:atg13-circulating-damp documents the downstream consequence of this block: ATG13 accumulates extracellularly and acts as a DAMP, while impaired mitophagy sustains oxidative stress. The convergence of the mTOR/AMPK/ATG13 pathway with the SNS$arrow.r$PKA$arrow.r$AMPK suppression described above and the NAD#super[+]-sirtuin-acetylation hub (Section @sec:nad-sirtuin-acetylation-hub) suggests a hierarchically organized failure: autonomic imbalance suppresses AMPK $arrow.r$ mTORC1 is disinhibited $arrow.r$ ATG13 is phosphorylated $arrow.r$ autophagy and mitophagy are blocked $arrow.r$ damaged mitochondria accumulate $arrow.r$ ROS and mtDNA release activate innate immunity $arrow.r$ inflammation further suppresses AMPK --- closing a self-reinforcing loop.

*Clinical translation.* The mTOR pathway is pharmacologically tractable: rapamycin (sirolimus) binds FKBP12 to form a complex that allosterically inhibits mTORC1 @Mannick2023mTOR. Ruan et al.\ (2025) reported improved fatigue and PEM with low-dose weekly rapamycin in an uncontrolled pilot study of 86 ME/CFS patients, with exploratory biomarker changes (reduced pSer258-ATG13, increased BECLIN-1) consistent with restored autophagy @Ruan2025rapamycin. However, this is an uncontrolled observation --- the ~34% responder rate documented in the treatment map is provisional, and no randomized controlled trial has been completed. Bar-Tana (2025) has proposed mTORC1 syndrome (TorS) as a unifying paradigm for post-acute infection syndromes including ME/CFS @BarTana2025mTOR, though this remains a hypothesis paper without primary data.

#warning-env(title: [Rapamycin Is an Immunosuppressant — Research-Stage Only])[
Rapamycin is an FDA-approved immunosuppressant with known risks including opportunistic infections, hyperlipidaemia, impaired wound healing, and interstitial pneumonitis. It is a CYP3A4 and P-gp substrate with major drug-drug interactions. The Ruan et al.\ (2025) study @Ruan2025rapamycin lacked a control group, exposing findings to placebo effect, regression to the mean, and natural PEM fluctuation. No clinical recommendation can be made pending a randomized controlled trial. Off-label use without medical supervision is dangerous.
]
#warning-env(title: [Falsifiability of the mTOR/AMPK/Autophagy Cascade])[
The causal chain described above (SNS $arrow.r$ PKA $arrow.r$ AMPK inhibition $arrow.r$ mTORC1 disinhibition $arrow.r$ ATG13-Ser258 phosphorylation $arrow.r$ autophagy block $arrow.r$ mitophagy failure $arrow.r$ ROS/mtDNA release) is presented as a mechanistic framework synthesizing evidence from multiple systems (autonomic, metabolic, immune). The following predictions would individually falsify specific links; a coordinated study measuring all nodes in the same ME/CFS samples would provide a decisive test of the full cascade:
...
The mechanisms of mitochondrial damage described above---oxidative stress, calcium overload, impaired mitophagy---do not operate in isolation. Thyroid hormones, particularly triiodothyronine (T3), regulate mitochondrial biogenesis, respiratory chain assembly, and quality control through well-characterized pathways in general endocrinology @Singh2022bioenergetic @Liao2024thyroidMito. Whether disruption of thyroid hormone signaling contributes to mitochondrial dysfunction in ME/CFS specifically has not been directly tested. Comparable mechanistic arguments could be constructed for other hormones with broad mitochondrial regulatory roles (cortisol, insulin, IGF-1), so the existence of a mechanistic overlap is necessary but not sufficient to prioritise this axis. The case for considering thyroid hormones here rests on the documented Low T3 Syndrome in ~16% of ME/CFS patients @ruiznunez2018thyroid and the selenoprotein P autoantibody finding @Sun2023selenop (Chapter @ch:endocrine, Section @sec:thyroid), which provide ME/CFS-specific evidence---albeit from single unreplicated studies---for disrupted thyroid hormone availability.
...
A persistent alternative explanation for exercise intolerance in ME/CFS is that patients are merely deconditioned from inactivity. Note that deconditioning may be a _contributing_ factor (as discussed above in the mitophagy section, which describes deconditioning as both a consequence and a cause of mitochondrial quality control failure through reduced AMPK signaling). The claim examined here is the stronger one: whether deconditioning is a _sufficient_ explanation. NASA bed rest studies provide a direct test of this stronger claim @MECFSScience2024deconditioning. In controlled bed rest experiments, prolonged immobilization produces measurable deconditioning---but the resulting phenotype differs qualitatively from ME/CFS:
...
These findings support the interpretation that PEM is a biological phenomenon better managed through pacing-based approaches (Chapter @ch:symptom-management) than through behavioral modification via CBT or GET.
...
=== Emerging Metabolic Mechanisms: 2026 Update

#hypothesis(title: [ATG13 as Circulating Inflammatory Signal and Autophagy/Mitophagy Dysregulation in ME/CFS])[
*Certainty: 0.42.* Gottschalk et al. reported ATG13 — an autophagy protein essential for autophagosome initiation — markedly elevated in ME/CFS serum across independent cohorts, with exogenous ATG13 triggering inflammatory signaling in vitro. This is significant because ATG13 is not normally circulating at high levels; its presence extracellularly may function as a DAMP (damage-associated molecular pattern), activating pattern recognition receptors (TLRs, RAGE) and amplifying the Cell Danger Response independently of ongoing viral protein production. @Watton2026unified

The upstream mechanism controlling ATG13 availability is now well-characterized: mTORC1 phosphorylates ATG13 at Ser258 (pSer258-ATG13), preventing assembly of the ULK1/ATG13/FIP200 complex required for autophagosome initiation (Section @sec:mTOR-AMPK-autophagy-balance). Thus, the elevated circulating ATG13 documented in ME/CFS likely reflects mTORC1-driven disassembly of the autophagy initiation complex, with non-complexed ATG13 accumulating intracellularly before extracellular release. This interpretation is supported by the Drosen et al.\ (2025) murine model, in which mTOR-driven ATG13 inactivation triggered M1 macrophage infiltration, IL-6/RANTES production via STAT3, and PEM-like exercise intolerance @Drosen2025ATG13. Ruan et al.\ reported reduced pSer258-ATG13 following rapamycin treatment in their pilot study, consistent with mTORC1 de-repression of autophagy initiation @Ruan2025rapamycin.

Impaired mitophagy (mitochondrial autophagy) provides a biologically plausible pathway linking infection-triggered immune activation to sustained redox stress, reduced bioenergetic capacity, and amplified inflammatory signaling — without requiring major tissue damage or ongoing productive viraemia. When mitophagy is inefficient, damaged mitochondria accumulate, producing excess ROS and releasing mtDNA that activates innate immune pathways (cGAS-STING). This creates a self-reinforcing loop: immune activation impairs mitophagy → mitochondrial damage accumulates → mtDNA release amplifies immune activation → further mitophagy impairment. @Watton2026unified (Certainty: 0.42)

*Therapeutic implication.* Autophagy/mitophagy restoration may be a tractable therapeutic target. Hochecker et al. demonstrated that whole-body hyperthermia (WBH, 39°C) in ME/CFS patients (n=9) reduced autophagy marker LC3-II by 17.84% (p=0.0065) and improved mitochondrial respiratory capacity across all measures — basal respiration (+66.60%), ATP production (+61.41%), maximal respiration (+97.88%), and spare respiratory capacity (+112.35%) — in PBMCs @Hochecker2025HyperthermiaMECFS. HSPA5 (HSP70) mRNA increased 48.33%, suggesting that WBH acts through HSP70-mediated chaperone buffering that shifts resources from chronic autophagic quality control toward oxidative phosphorylation. Pharmacological mTORC1 inhibition (rapamycin/sirolimus) represents a direct approach: Ruan et al.\ (2025) reported improved fatigue and PEM in an uncontrolled pilot study (n=86, no control group) with exploratory biomarker changes (↓pSer258-ATG13, ↑BECLIN-1) @Ruan2025rapamycin. AMPK activators (metformin, berberine) offer an indirect route via mTORC1 inhibition; Brown et al.\ demonstrated AMPK rescue by metformin in ME/CFS myotubes @Brown2018AMPK, and Fineberg et al.\ (2025) propose metformin for ME/CFS and Long COVID via AMPK→mTOR→autophagy restoration @Fineberg2025Metformin. None of these approaches has been tested in a randomized controlled trial in ME/CFS. @Watton2026unified
...
#speculation(title: [mTOR Tone Index: pSer258-ATG13/Total ATG13 Ratio as mTORC1 Activity Biomarker])[
...
This ratio has several properties making it suitable as a clinical biomarker: (a) it is mechanistically proximal (one step from functional autophagy), (b) it normalises inter-individual variation in ATG13 expression, (c) it is dynamic (changes within hours of mTORC1 modulation), and (d) it has preliminary clinical signal (Ruan et al.\ reported reduced pSer258-ATG13 after rapamycin @Ruan2025rapamycin). In practice, measurement requires paired PBMC immunoblotting for phospho- and total ATG13 --- a moderate-cost assay (USD 200--300 per sample) feasible in research labs but not yet standardized for clinical use.
...
#speculation(title: [SMPDL3B: Lipid Raft--Mitochondrial Quality Control Bridge])[
...
*Certainty: 0.30. Probability of clinically meaningful efficacy in ME/CFS: 0.04.* Spermidine, a well-established dietary autophagy inducer, could pharmacologically recapitulate the hyperthermia effect demonstrated by Hochecker et al. (autophagy markers decreased 17.84%, mitochondrial respiration increased 61-112% @Hochecker2025HyperthermiaMECFS), potentially shifting cells from stalled CDR to adaptive metabolic state. Spermidine-induced autophagy is well-established in aging biology; no ME/CFS data exists. Falsifiable: spermidine supplementation in ME/CFS PBMCs will increase autophagic flux (LC3-II ratio, p62 degradation) and improve mitochondrial respiration.
] <spec:spermidine-autophagy>

#speculation(title: [Butyrate for Mitochondrial-Nuclear Crosstalk Restoration])[
*Certainty: 0.35. Probability of clinically meaningful efficacy in ME/CFS: 0.06.* Butyrate is an HDAC inhibitor that can epigenetically reprogramme mitochondrial gene expression. SCFA deficits (Chapter @ch:gut-microbiome) plus CDR-induced epigenetic silencing may be counteracted by butyrate supplementation, restoring PGC-1$alpha$ and NRF1-dependent mitochondrial biogenesis. Butyrate HDAC inhibition and mitochondrial biogenesis upregulation are established in vitro; no ME/CFS-specific data. Falsifiable: butyrate treatment of ME/CFS immune cells will increase histone acetylation at PGC-1$alpha$ promoters and upregulate TFAM/mtDNA copy number.
...
Does ATG13 utilize distinct receptors from HMGB1/mtDNA, or does it synergise with conventional DAMPs to amplify inflammatory signaling @Ibrahim2026HMGB1Neuroinflammation @Chen2026HMGB1pCTSLAxis? Characterising ATG13 receptor usage (TLRs, RAGE, STING) and downstream pathway convergence (NF-$kappa$B, cGAS-STING) will clarify its unique contribution to CDR amplification and whether ATG13 neutralization would produce effects additive to or redundant with standard anti-inflammatory approaches. Falsifiable: ATG13 will utilize distinct receptors from HMGB1/mtDNA and synergise with conventional DAMPs to amplify cytokine production via converging on NF-$kappa$B and STING pathways. Probability of novel therapeutic target: 0.10.
] <oq:atg13-damp-interactions>

#hypothesis(title: [HIF-1alpha-Mitochondria-ECM Pathogenic Triad])[
*Certainty: 0.60.* Moschini et al. demonstrated that HIF-1alpha drives tendinopathy independently of VEGF in a murine model, establishing HIF-1alpha as a direct mechanistic driver of connective tissue pathology @Moschini2026HIFTendinopathy. @Sahin2012HIFTendon This finding, when integrated with established ME/CFS findings on mitochondrial dysfunction and HIF-1alpha elevation, suggests a self-reinforcing pathogenic triad.

*Mechanism.* HIF-1alpha suppresses mitochondrial oxidative phosphorylation and shifts metabolism toward glycolysis—the "Warburg-like" metabolic phenotype documented in ME/CFS immune cells (Chapter @ch:immune-dysfunction) and muscle. miRNA-mediated silencing of HIF target genes may further impair the adaptive transcriptional response to tissue hypoxia, blunting EPO and VEGF induction and locking the system in a maladapted state @Kaczmarek2023miRNAhypoxia. Mitochondrial dysfunction increases reactive oxygen species (ROS) production, which stabilizes HIF-1alpha through prolyl hydroxylase inhibition, creating a vicious cycle. Both HIF-1alpha-driven metabolic shift and mitochondrial ROS impair extracellular matrix homeostasis: HIF-1alpha directly upregulates matrix metalloproteinases (MMPs) and downregulates collagen synthesis, while ROS damages existing ECM components and fibroblasts. The triad becomes self-reinforcing: HIF-1alpha drives ECM pathology; mitochondrial dysfunction impairs ECM repair; ROS from mitochondria stabilizes HIF-1alpha.

*ME/CFS context.* ME/CFS tissue samples should show correlated abnormalities in HIF-1alpha, mitochondrial function, and ECM markers. The high prevalence of hypermobility syndromes (hEDS, HSD) in ME/CFS—up to 70% in some cohorts—may represent the clinical manifestation of this triad operating in connective tissue. @Ganesh2024LongCOVIDHSD @Ramirez-Paesano2023CCI

*Cross-references.* Chapter @ch:integrative-models synthesizes cross-system pathophysiological loops from HIF-1alpha elevation documented in ME/CFS stress response studies.
...
#hypothesis(title: [ARB-Mediated Capillary Basement Membrane Restoration])[
*Certainty: 0.45.* Angiotensin II receptor blockers (ARBs; losartan, telmisartan) may restore capillary basement membrane structure and improve microcirculation in ME/CFS by reducing TGF-$beta$ signaling, which drives excessive ECM production at capillary basement membranes. @Wust2024capillary @Wirth2026CTDMECFS

*Mechanism.* Wust et al. demonstrated massive collagen IV deposition and basement membrane thickening in ME/CFS muscle capillaries via electron microscopy—a structural abnormality that creates a mechanical bottleneck independent of mitochondrial function @Wust2024capillary. Basement membrane thickening impairs oxygen and nutrient diffusion, creating pre-mitochondrial hypoxia. ARBs reduce TGF-$beta$ signaling and ECM production, with demonstrated efficacy in diabetic nephropathy where they normalize glomerular basement membrane thickness and preserve filtration function. In ME/CFS, ARBs could thin basement membranes, restore diffusion gradients, and improve microcirculation without directly targeting mitochondria.

*Clinical rationale.* ARBs are well-tolerated, widely available, and have favorable safety profiles. The TGF-$beta$ elevation documented in ME/CFS (Chapter @ch:immune-dysfunction) provides mechanistic support. The bidirectional connective tissue disorder-ME/CFS relationship established by Wirth et al. suggests that connective tissue pathology is not merely a comorbidity but may contribute to disease maintenance @Wirth2026CTDMECFS.
...
#speculation(title: [Fibroblast Glycogen Metabolism and ECM Production])[
*Certainty: 0.30.* ME/CFS metabolic dysfunction may impair fibroblast glycogen storage and utilization, reducing ECM quality and contributing to connective tissue pathology. This novel connection links the metabolic trap hypothesis (Chapter @ch:energy-metabolism) to connective tissue pathophysiology.
...
#speculation(title: [cGAS-STING Chronicity Loop: NAD Depletion-Driven mtDNA Release])[
*Certainty: 0.40.* NAD depletion is documented in ME/CFS (Chapter @ch:energy-metabolism, Section @sec:step6-cofactors). Reduced NAD impairs mitophagy via SIRT1-PINK1/Parkin deacetylation failure, leading to mitochondrial depolarization and mtDNA release into the cytosol. Cytosolic mtDNA activates cGAS-STING, producing type I interferons (IFN-$beta$). IFN-$beta$ induces indoleamine 2,3-dioxygenase (IDO), which consumes tryptophan along the kynurenine pathway, further depleting NAD via nicotinamide phosphoribosyltransferase (NAMPT) upregulation — a futile compensatory loop that accelerates NAD consumption.
