...
*Mechanistic scope limitation:* AIMM is a DOWNSTREAM mechanism, not a root cause. The calcium toxicity cascade (hypoperfusion $arrow.r$ Na#super[+]/K#super[+]-ATPase failure $arrow.r$ NCX reversal $arrow.r$ Ca#super[2+] overload) occurs as a consequence of upstream processes that reduce muscular blood flow or impair endothelial function. These upstream drivers include GPCR autoantibodies (Section @sec:gpcr-autoantibodies), beta-2 adrenergic receptor dysfunction, mast cell activation, and other mechanisms classified as Tier 1 (trigger-capable) in the causal hierarchy (Chapter @ch:causal-hierarchy). MDC002 addresses the proximate PEM trigger but does not correct the upstream cause that initiated the disease state. Consequently, MDC002 monotherapy is expected to produce *partial or temporary benefit* in many patients, and *no benefit* in patients whose ME/CFS is driven primarily by non-AIMM mechanisms (e.g., CNS energy crisis, metabolic safe mode lock, TRPM3 channelopathy without secondary AIMM). The most clinically realistic expectation, consistent with Wirth's own position that "severely ill patients will need multiple medications," is that MDC002 will work best as part of combination therapy: downstream ion homeostasis restoration (MDC002) plus upstream mechanism-targeted interventions (autoantibody removal, beta-2 AR resensitization, blood volume restoration). Expected responder phenotype: sodium MRI-confirmed overload, exercise-triggered PEM with delayed onset, muscle-ischemic symptoms dominant over CNS symptoms. Population-level response rate: 30--40% based on analogous subtype-targeted interventions (daratumumab 60% in autoantibody-positive subgroup) @Rucker2026WirthScheibenbogen.
] <hyp:calcium-pem>
