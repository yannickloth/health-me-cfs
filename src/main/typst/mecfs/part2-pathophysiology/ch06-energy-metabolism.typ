...
A parallel cytosolic mechanism amplifies the effect. Cytosolic aconitase (ACO1) moonlights as Iron Regulatory Protein 1 (IRP1): when its [4Fe-4S] cluster is destroyed by oxidative stress, ACO1 gains RNA-binding activity, upregulating transferrin receptor (increasing iron import) while suppressing ferritin (reducing iron storage) and ferroportin (reducing iron export). The net result is intracellular labile iron accumulation despite normal or low serum ferritin — a pattern that would produce intracellular labile iron accumulation despite normal or low serum ferritin.
