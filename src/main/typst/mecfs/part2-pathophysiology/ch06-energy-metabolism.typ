...
*Certainty: 0.45.* Hochecker et al. demonstrated that WBH increases spare respiratory capacity by 112.35% in ME/CFS PBMCs @Hochecker2025HyperthermiaMECFS. However, whether spare respiratory capacity limits thermoregulation in vivo is entirely inferred — the direct link (PBMC respirometry → whole-body heat/cold tolerance) has never been tested. Thermoregulation is energetically expensive: sweating requires sustained ATP for eccrine gland ion transport, shivering requires burst mitochondrial output, and vasomotion requires constant smooth muscle ATP turnover. Spare respiratory capacity — the difference between maximal and basal respiration — theoretically represents the mitochondrial reserve available for these thermoregulatory demands. In ME/CFS, where basal respiration may be adequate but reserve is exhausted, thermoregulation may fail not because of a fixed defect in temperature sensing but because thermoregulatory work exceeds available mitochondrial capacity. This predicts that (a) baseline spare respiratory capacity correlates with heat and cold tolerance, (b) interventions that increase reserve (WBH, gradual exercise) improve thermoregulation, and (c) the dual heat+cold intolerance paradox reflects a single mechanism: insufficient mitochondrial reserve for any thermal challenge. Falsifiable: spare respiratory capacity in ME/CFS PBMCs will correlate with heat tolerance time (r > 0.6) and cold tolerance time (r > 0.5); improving reserve via WBH will increase both tolerances.
