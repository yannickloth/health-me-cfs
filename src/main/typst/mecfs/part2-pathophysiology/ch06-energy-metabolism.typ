...
A parallel cytosolic mechanism amplifies the effect. Cytosolic aconitase (ACO1) moonlights as Iron Regulatory Protein 1 (IRP1): when its [4Fe-4S] cluster is destroyed by oxidative stress, ACO1 gains RNA-binding activity, upregulating transferrin receptor (increasing iron import) while suppressing ferritin (reducing iron storage) and ferroportin (reducing iron export). The net result is intracellular labile iron accumulation despite normal or low serum ferritin — a pattern consistent with the clinical observation that many ME/CFS patients have unremarkable serum iron panels yet show signs of oxidative stress consistent with iron-mediated damage.
...
#figure(
  ...
  table(
    ...
    [G27: Magnesium status (serum + RBC)], [High — required by every kinase and ATPase; but Cox 1991 positive finding not replicated (3 negative: Clague 1992, Hinds 1994, Swanink 1995)], [Indirect; gates ATP _utilisation_ more than production], [Easy — RBC magnesium is available], [Immediate — magnesium supplementation is OTC], [2],
  )
...
A critical pharmacological distinction: GAA is converted to creatine by guanidinoacetate N-methyltransferase (GAMT), which consumes S-adenosylmethionine (SAM) as the methyl donor. Creatine synthesis is the single largest consumer of labile methyl groups in human metabolism — accounting for approximately 40% of all SAM-derived methylation (revised downward from the earlier ~70% estimate after accounting for methyl-group recycling) @Brosnan2011creatineSAM. In ME/CFS patients with documented methylation pathway impairment (MTHFR variants are over-represented; homocysteine may be elevated; B12/folate cycling is disrupted), GAA supplementation may _worsen_ the methylation bottleneck by diverting SAM toward creatine synthesis at the expense of DNA methylation, neurotransmitter synthesis (catecholamines, melatonin), phospholipid methylation, and other SAM-dependent processes. *Direct creatine monohydrate supplementation bypasses GAMT entirely*, eliminating the methylation cost. The Ostojic 2016 null result for fatigue should therefore not be interpreted as "creatine loading does not help CFS" — the study used the wrong compound for a population with potential methylation vulnerability. Future creatine trials in ME/CFS should use creatine monohydrate, not GAA.
]
...
*Pattern:* Muscle creatine loading alone does not resolve fatigue in either CFS (GAA trial; caveat: GAA is a creatine precursor, not creatine monohydrate, so pharmacological differences cannot be excluded) or fibromyalgia (creatine monohydrate RCT). The Godlewska signal is brain-specific — brain creatine correlated with cognitive improvement, not muscle creatine with fatigue resolution. A future ME/CFS creatine RCT should therefore be designed with brain MRS as the primary mechanistic outcome and should measure brain-specific endpoints (cognition, brain fog) alongside whole-body fatigue.
...
The brain-specific creatine signal deserves particular emphasis because the PCr shuttle plays a uniquely critical role in CNS energy metabolism. Unlike skeletal muscle, where glycolytic ATP generation can partially substitute during demand spikes, synaptic transmission and action potential propagation require ATP delivery on a millisecond timescale that only the PCr shuttle can provide. Mitochondrial CK at the synapse pre-phosphorylates creatine using locally generated ATP; when a burst of synaptic vesicle release or ion-pump activity occurs, cytosolic CK instantly regenerates ATP from the local PCr pool without waiting for mitochondrial oxidative phosphorylation to ramp up. This temporal buffering function is non-redundant in brain tissue: there is no alternative pathway that can deliver ATP at the required speed.
...
*Limitation:* This speculation rests on a single open-label feasibility study ($n = 11$) for brain creatine and a small RCT ($n = 21$) for muscle creatine loading. The brain-vs-muscle comparison has never been performed within the same ME/CFS cohort. The cause of brain-selective depletion (impaired SLC6A8 transport, astrocyte consumption, or reduced local synthesis) is unknown.
...
#figure(
  ...
  table(
    ...
    [G37: Muscle F2-isoprostanes], [Plasma F2-isoprostanes _established_ as elevated in CFS: Kennedy 2005 ($n = 47$ vs 34, correlated with PEM $p = 0.027$) @Kennedy2005isoprostanes and Robinson 2010 ($n = 33$ vs 33, elevated at rest and persisting 24h post-exercise) @Robinson2010isoprostanes — muscle-specific measurement remains genuinely open], [Objective quantification of tissue-level ROS-damage state], [Moderate — LC-MS assay available; muscle biopsy required for tissue-specific data], [Immediate — baseline and response-to-intervention marker], [1],
...
The preceding sections treat NAD#super[+] depletion (G22, Step 6), NRF2 pathway dysfunction (G34, Step 9), and PGC-1$alpha$ hyperacetylation (G39, Step 10) as separate research gaps. They are mechanistically linked through a single upstream regulator: NAD#super[+]-dependent sirtuin activity. This convergence is not merely conceptual — each connection has specific enzymatic evidence.

SIRT1 deacetylates PGC-1$alpha$ at multiple lysine residues, converting it from an inactive hyperacetylated form to the active transcription factor that drives mitochondrial biogenesis @MorrisMaes2014mitochondria. SIRT1 also directly deacetylates NRF2 at K588 and K591, promoting its nuclear retention and transcriptional activity on antioxidant response elements. SIRT3, a mitochondrial matrix sirtuin, deacetylates and activates isocitrate dehydrogenase 2 (IDH2), MnSOD, and components of Complex I — the last of which directly affects electron transport chain flux. All three sirtuins require NAD#super[+] as a co-substrate (not merely a cofactor — NAD#super[+] is consumed stoichiometrically, one molecule per deacetylation event, producing nicotinamide and O-acetyl-ADP-ribose).

In ME/CFS, three independent NAD#super[+]-consuming mechanisms converge:

+ *PARP hyperactivation:* The Shankar 2025 MnSOD deficit @Shankar2025oxidativestress predicts increased mitochondrial superoxide $arrow.r$ oxidative DNA damage $arrow.r$ PARP1 activation $arrow.r$ NAD#super[+] consumption. A single DNA double-strand break can consume hundreds of NAD#super[+] molecules through PARP poly-ADP-ribosylation.
+ *CD38 upregulation:* CD38 is the dominant NAD#super[+]-consuming enzyme in most tissues and is strongly upregulated by inflammatory cytokines (TNF-$alpha$, IFN-$gamma$) — both chronically elevated in ME/CFS. CD38 also degrades the NAD#super[+] precursor NMN, creating a double block on the salvage pathway.
+ *Kynurenine pathway diversion:* Tryptophan is the sole substrate for de novo NAD#super[+] synthesis via the kynurenine pathway. IDO1 upregulation by IFN-$gamma$ diverts tryptophan toward quinolinic acid (neurotoxic) rather than completing the synthesis to NAD#super[+] — a mechanism already documented in ME/CFS metabolomics @Naviaux2016metabolomics.

#hypothesis(title: [NAD#super[+]--Sirtuin--Acetylation Hub: Unified Upstream Regulator of G22, G34, and G39])[
*Certainty: 0.35.* NAD#super[+] depletion in energy-intensive tissues (skeletal muscle, neurons — not PBMCs, where Heng 2025 found NAD#super[+] _elevated_, consistent with substrate backup rather than depletion) simultaneously inactivates SIRT1, SIRT3, and shifts the SIRT1/SIRT4 balance toward the inhibitory SIRT4 arm @Fluge2016. The downstream consequences propagate through four documented pathways:
...
+ *PDC regulation via SIRT4:* When SIRT1 activity falls but SIRT4 remains active (as documented in ME/CFS PBMCs @Fluge2016), SIRT4 ADP-ribosylates glutamate dehydrogenase and opposes PDC activity $arrow.r$ reduced pyruvate entry into the TCA cycle (Step 3).
...
*Testable predictions:*
+ NAD#super[+] in ME/CFS skeletal muscle will be _reduced_ (opposite to the PBMC elevation found by Heng 2025), predicting a tissue-compartment dissociation.
...
+ A factorial RCT (NAD#super[+] precursor $times$ AMPK activator) should show synergistic rather than additive effects on biogenesis markers, because AMPK and SIRT1 mutually activate each other (AMPK phosphorylates SIRT1; SIRT1 deacetylates LKB1 which activates AMPK).
...
*Limitation:* The PBMC NAD#super[+] elevation (Heng 2025) and the predicted muscle NAD#super[+] depletion have never been measured in the same cohort. The tissue-compartment dissociation is a prediction, not an observation. SIRT1 has numerous substrates beyond PGC-1$alpha$ and NRF2 (p53, FOXO3, NF-$kappa$B), and the acetylation changes may reflect a broader regulatory shift rather than a specific biogenesis/antioxidant mechanism. The hub model risks unfalsifiable circularity if every downstream effect is attributed to NAD#super[+] without specifying which effects would disprove the connection.
] <hyp:nad-sirtuin-hub>
