...
*Clinical translation.* The mTOR pathway is pharmacologically tractable: rapamycin (sirolimus) binds FKBP12 to form a complex that allosterically inhibits mTORC1 @Mannick2023mTOR. Ruan et al.\ (2025) reported improved fatigue and PEM with low-dose weekly rapamycin in an uncontrolled pilot study of 86 ME/CFS patients, with exploratory biomarker changes (reduced pSer258-ATG13, increased BECLIN-1) consistent with restored autophagy @Ruan2025rapamycin. However, this is an uncontrolled observation --- the ~34% responder rate documented in the treatment map is provisional, and no randomised controlled trial has been completed. Bar-Tana (2025) has proposed mTORC1 syndrome (TorS) as a unifying paradigm for post-acute infection syndromes including ME/CFS @BarTana2025mTOR, though this remains a hypothesis paper without primary data.

#warning-env(title: [Rapamycin Is an Immunosuppressant — Research-Stage Only])[
Rapamycin is an FDA-approved immunosuppressant with known risks including opportunistic infections, hyperlipidaemia, impaired wound healing, and interstitial pneumonitis. It is a CYP3A4 and P-gp substrate with major drug-drug interactions. The Ruan et al.\ (2025) study @Ruan2025rapamycin lacked a control group, exposing findings to placebo effect, regression to the mean, and natural PEM fluctuation. No clinical recommendation can be made pending a randomised controlled trial. Off-label use without medical supervision is dangerous.
]
#warning-env(title: [Falsifiability of the mTOR/AMPK/Autophagy Cascade])[
The causal chain described above (SNS $arrow.r$ PKA $arrow.r$ AMPK inhibition $arrow.r$ mTORC1 disinhibition $arrow.r$ ATG13-Ser258 phosphorylation $arrow.r$ autophagy block $arrow.r$ mitophagy failure $arrow.r$ ROS/mtDNA release) is presented as a mechanistic framework synthesising evidence from multiple systems (autonomic, metabolic, immune). The following predictions would individually falsify specific links; a coordinated study measuring all nodes in the same ME/CFS samples would provide a decisive test of the full cascade:
