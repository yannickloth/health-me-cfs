...
The monolayer-myoblast layer of this paradigm has not survived replication: a pre-registered study exposing healthy myoblasts to ME/CFS versus control serum across more than 1,700 mitochondrial stress tests found no difference in maximal respiratory capacity (see Limitation @lim:ryback-serum-transfer-null) @Ryback2026myoblastNull. This null undercuts the specific Fluge monolayer-myoblast observation cited above, but does not extend to the structurally intact 3D biofabricated muscle or to in-tissue/activity-dependent models, which differ in architecture and are not refuted by the monolayer assay.

*Certainty: 0.26* (discounted). The in-vitro design (biofabricated tissue, population weight 0.40) caps confidence; the underlying serum-transfer phenomenon rests on the 3D biofabricated model and Schreiner's cell-transfer findings (Schreiner 0.41, Appelman 0.68). The Fluge monolayer-myoblast result (previously cited at 0.52) is no longer counted as independent support after its large-scale null replication @Ryback2026myoblastNull.
