...
[Auto\-anti\-body-monocyte re\-pro\-gram\-ming (@hyp:autoantibody-monocyte)], [Pre\-lim\-i\-nary], [GPCR autoantibodies re\-pro\-gram monocyte cytokine pro\-duc\-tion @Hackel2025monocyte; MIP-1$delta$, PDGF-BB, TGF-$beta$3 el\-e\-va\-tion], [Sys\-tem\-ic in\-flam\-ma\-tion; why ef\-fects per\-sist be\-yond re\-cep\-tor bind\-ing; tis\-sue re\-mod\-el\-ing], [Auto\-anti\-body re\-mov\-al + monocyte mod\-u\-la\-tion (JAK in\-hib\-i\-tors)], [Moderate–High],
	...
	[Connective tissue disorders — bidirectional relationship with ME/CFS (@hyp:ct-mecfs-bidirectional)], [Preliminary], [Hypermobility 30–57% in ME/CFS (vs 10–15% general population) @Ganesh2024LongCOVIDHSD; CCI produces ME/CFS-like symptoms @Ramirez-Paesano2023CCI; bidirectional: CTD may predispose, ME/CFS may worsen CTD via ROS, HIF-1$alpha$, mast cell activation @Wirth2026CTDMECFS], [POTS, orthostatic intolerance, fatigue, pain; hypermobility overlap], [Screen for CTD; treat POTS; consider CCI in hypermobile; MCAS stabilization], [Moderate–High (in CTD subset)],
...
#speculation(title: [Mitochondrial Reserve Threshold as ME/CFS Predisposition])[
*(Certainty: 0.25 — consistent with heritable component data and two-hit disease models; direct onset-predisposition evidence consistently null across two independent sequencing studies and one review.)*

ME/CFS has a documented heritable component: first-degree relatives carry a relative risk of 2.70 (p = 0.001) compared to the general population, declining to 2.34 for second-degree and 1.93 for third-degree relatives in the largest family study @Albright2011heritability. This RR captures all heritable contributions — nuclear DNA, mtDNA, shared early-life environment, and assortative mating — and does not preferentially support mitochondrial inheritance over nuclear pathways. The molecular substrate of this predisposition is unknown. Mitochondrial DNA haplogroup variants are one candidate — not because pathogenic mtDNA mutations cause ME/CFS (two independent sequencing studies and one genetics review find no such association @BillingRoss2016mtDNA @Venter2019mtDNA @Dibble2020genetics), but because haplogroup-determined differences in mitochondrial efficiency and respiratory reserve capacity might lower the threshold at which a major stressor (viral infection, oxidative stress, immune activation) triggers a transition to a persistent hypometabolic state.

The proposed mechanism follows the two-hit threshold model: *Hit 1* — inherited mtDNA haplogroup reduces mitochondrial reserve capacity below a population average, without producing overt disease; *Hit 2* — an acute stressor (e.g. EBV, SARS-CoV-2, surgical insult) activates the Cell Danger Response @Naviaux2014cdr (note: Naviaux's CDR framework does not itself postulate inherited mitochondrial predisposition; the two-hit extension here is an inference applied to the framework, not a claim Naviaux makes). In individuals with adequate reserve, CDR activation resolves. In individuals below the reserve threshold, CDR activation cannot be resolved and a chronic hypometabolic state (Section @sec:synthesis) is established. This framing accommodates the observation that only a fraction of people exposed to the same trigger develop ME/CFS, and the anecdotal observation of maternal clustering in some ME/CFS families (mtDNA is maternally inherited almost exclusively, with heteroplasmy producing sibling variation).

An important counter-finding complicates the model: ME/CFS patients carry *fewer* mildly deleterious mtDNA variants than healthy controls — a pattern replicated independently in two geographically distinct cohorts (UK and South Africa; @Venter2019mtDNA). This is the opposite of what a simple "accumulated mitochondrial damage" predisposition model would predict. It may indicate: (a) different selection pressures operating in ME/CFS populations; (b) threshold effects where even low-level mitochondrial stress is disproportionately consequential; or (c) nuclear-mitochondrial interaction dynamics not captured by mtDNA sequencing alone. The mechanism behind this anomalous distribution is unexplained.

*Falsifiable predictions:* (a) Prospective cohort studies of individuals with high haplogroup risk should show higher ME/CFS conversion rates after a viral trigger. (b) At-risk individuals should show lower mitochondrial reserve capacity (assessed by cellular bioenergetics) at baseline, before any illness. (c) If reserve enhancement is causally relevant and any intervention can be shown to demonstrably raise spare respiratory capacity (no such intervention has been validated in ME/CFS), initiating it during acute infection should reduce conversion rates in at-risk individuals; the specific intervention class is secondary to demonstrating the causal mechanism first.

*Limitations:* The onset-predisposition claim is consistently null in existing data; two independent sequencing studies and one genetics review found no association between mtDNA variants and ME/CFS *onset* @BillingRoss2016mtDNA @Venter2019mtDNA @Dibble2020genetics. Prospective measurement of reserve capacity in at-risk populations has not been done. Maternal vs. paternal inheritance clustering in ME/CFS families has never been formally assessed — the critical data point that would confirm or rule out mitochondrial inheritance as the predisposition pathway. The CDR reserve-capacity inference from @Naviaux2014cdr is not explicitly stated by Naviaux; it is an inference from the framework. Not yet replicated.
] <spec:mtdna-reserve-threshold>

#speculation(title: [Haplogroup H Tight Coupling as the PEM Substrate])[
*(Certainty: 0.20 — rests on a single unreplicated symptom-association study, n=193; no CPET haplogroup comparison has been done in ME/CFS.)*

Mitochondrial haplogroup H, found in ~50% of European-descent individuals, is associated in ME/CFS with increased "feeling dead after exercise" @BillingRoss2016mtDNA. This may reflect a genuine bioenergetic phenotype: haplogroup H couples OXPHOS more tightly than haplogroups J or U, with reduced proton leak via uncoupling proteins. Tighter coupling is metabolically efficient at rest but narrows the spare respiratory capacity window when ATP demand spikes acutely. In ME/CFS, where Complex I/III and PDH function are already compromised, a haplogroup H background may leave essentially no reserve against exertional challenges — translating mechanistically to greater severity of post-exertional crashes.

*Falsifiable prediction:* In two-day CPET protocols, haplogroup H ME/CFS patients will show a greater day-2 VO#sub[2]max decrement (≥15% relative worsening) compared to haplogroup U/J patients matched for baseline severity, disease duration, and sex. This would directly implicate haplogroup-determined coupling efficiency as a modulator of PEM severity. *Limitations:* No CPET study has stratified ME/CFS patients by haplogroup. The coupling-efficiency advantage of haplogroup H is established in mitochondrial physiology literature but has not been directly measured in ME/CFS PBMCs or muscle. Not yet replicated.
] <spec:haplogroup-h-pem>

#speculation(title: [Survivorship Bias in the Fewer-Deleterious-Variants Anomaly])[
*(Certainty: 0.20 — post-hoc rescue for a directly contradictory finding; risks unfalsifiability until pediatric test is done. The reserve-threshold model should be treated as weakened, not preserved, by Venter 2019.)*

Venter et al. @Venter2019mtDNA found ME/CFS patients carry *fewer* mildly deleterious mtDNA variants than controls — the reverse of a simple "damaged mtDNA = predisposition" model. One parsimonious explanation is survivorship bias: individuals carrying *both* high mtDNA variant load AND exposure to ME/CFS-triggering stressors may not survive to ME/CFS diagnosis — they instead develop overt mitochondrial disease (MELAS, MERRF phenocopies) or die. The ME/CFS-diagnosed population is then enriched for an "intermediate vulnerability" stratum: low enough mtDNA damage to survive and maintain basic function, but insufficient reserve to recover from a major stressor.

This framing predicts a specific age effect: *pediatric* ME/CFS patients — who have not yet experienced the survivorship filtering that operates over decades — should NOT show the "fewer deleterious variants" pattern of adult cohorts. If pediatric ME/CFS sequencing reveals a normal or elevated mtDNA variant burden, the survivorship interpretation is supported. If pediatric patients also carry fewer deleterious variants, a different mechanism must explain the anomaly. *Limitations:* This explanation is post-hoc and risks unfalsifiability: any inversion of the predicted direction can be accommodated by invoking survivorship, and the threshold model can absorb either direction of the empirical evidence. The reserve-threshold model is *weakened* by Venter 2019, not explained away. No pediatric mtDNA sequencing study in ME/CFS has been conducted. The survivorship mechanism requires that high-variant-load individuals fail to present to ME/CFS clinics — a pathway that is plausible but entirely unverified and could accommodate any outcome. Not yet replicated.
] <spec:survivorship-bias-variants>

#speculation(title: [Dopaminergic Demand Crossover: ADHD and ME/CFS as Reserve-Ceiling Expressions])[
*(Certainty: 0.20 — cross-disease leap from n=3 cybrid study (Verma 2016) and a cross-population haplogroup finding; direct shared-substrate testing has not been done.)*

ADHD reflects prefrontal cortex (PFC) dopaminergic circuits failing under tonic demand; ME/CFS post-exertional malaise reflects whole-body energy circuits failing under phasic demand. Both failure patterns are consistent with a shared low-mitochondrial-reserve substrate, with the specific phenotypic outcome determined by which circuit reaches its energy ceiling first. PFC is metabolically among the most expensive brain structures; dopamine signaling is tightly coupled to ATP availability @Verma2016ADHDcybrid @Giannoulis2024sysrevmtADHD. (Note: the Verma 2016 cybrid study was conducted with n=3 ADHD patients; results require independent replication before they can be considered load-bearing evidence.)

The hypothesis predicts a developmental sequence: low-reserve individuals may express ADHD-like symptoms in childhood — when PFC maturation places maximal oxidative demands — and then develop ME/CFS susceptibility in adulthood after an immune or oxidative Hit 2 depletes the remaining reserve below the systemic threshold. Haplogroup U is protective against ADHD @Chang2020haploADHD and symptom-moderating in ME/CFS @BillingRoss2016mtDNA, consistent with the shared-reserve framing.

*Falsifiable prediction:* Adult ME/CFS cohorts will show significantly higher rates of childhood ADHD diagnosis (or subclinical inattention symptoms by WURS-25 score >36) than healthy matched controls, with the elevation concentrated in haplogroups *other than* U/K — the haplogroups that are ADHD-protective and ME/CFS-symptom-moderating. If ADHD overrepresentation is present but haplogroup-independent, the shared-reserve substrate model is weakened. *Limitations:* No study has co-measured ADHD history, haplogroup, and ME/CFS diagnosis. Causal direction between ADHD dopamine dysregulation and mitochondrial dysfunction is unresolved. Not yet replicated.
] <spec:dopaminergic-crossover>

#speculation(title: [Reserve-Threshold Variable in the CDR Ordinary Differential Equation Model])[
*(Certainty: 0.15 — formal modeling exercise; empirical support for parameter values is unavailable; Naviaux CDR does not postulate inherited predisposition. Certainty reflects underlying evidence, not internal model coherence.)*
...
*Falsifiable prediction:* A fitted ODE using population-level haplogroup frequencies, published mitochondrial aging trajectories, and epidemiological stressor distributions will reproduce the bimodal onset age distribution of ME/CFS within ±3 years of the observed peaks. Failure to reproduce the bimodal pattern would require revision of the reserve-threshold model or additional state variables. *Limitations:* Parameters $k_"bio"$, $k_"dam"$, $R_"crit"$ are not empirically established for ME/CFS; the model would require calibration to clinical data. No ODE modeling of CDR predisposition has been published. Not yet replicated.
] <spec:reserve-threshold-ode>

#speculation(title: [Two-Hit DAG Extension: Haplogroup as Upstream Modifier Node])[
*(Certainty: 0.20 — DAG extension formalism is internally coherent but rests on the same small evidence base as the parent speculations; no dataset contains all required variables. Mathematical structure does not increase empirical support.)*

The existing ME/CFS causal DAG treats predisposition as a monolithic background factor. An extension would add *haplogroup* as an explicit upstream node with directed edges to: (a) baseline reserve $R(0)$ (haplogroup determines coupling efficiency and initial reserve); (b) symptom severity *given* onset (haplogroup H → greater PEM; haplogroup U/J → milder bloating/joint symptoms per @BillingRoss2016mtDNA); and (c) ADHD comorbidity probability (haplogroups K/U protective per @Chang2020haploADHD). This structure allows formal mediation analysis: what fraction of the haplogroup → ME/CFS-severity association is mediated by ADHD comorbidity vs. direct energetic effects?

*Falsifiable prediction:* In a cohort with haplogroup + ADHD comorbidity + ME/CFS phenotype data (n > 500), path analysis will demonstrate a non-zero indirect path: haplogroup → ADHD probability → ME/CFS cognitive-symptom severity, independent of the direct haplogroup → severity path. If the indirect path coefficient is not statistically distinguishable from zero, the DAG extension collapses back to the simpler two-node model. *Limitations:* No published dataset contains all four variables (haplogroup, ADHD comorbidity, ME/CFS diagnosis, symptom subscale). The edges weights are currently estimable only from separate studies, not co-measured. Not yet replicated.
] <spec:two-hit-dag-haplogroup>

#speculation(title: [Migraine as a Third Expression of the Reserve-Ceiling Substrate])[
*(Certainty: 0.15 — speculative extension of the reserve-ceiling model to a third condition with no new mtDNA evidence; risks becoming unfalsifiable if every comorbidity can be accommodated.)*

Migraine demonstrates mitochondrial dysfunction, responds to riboflavin, CoQ10, and magnesium — all mitochondrial support agents — and shows elevated comorbidity with both ADHD and ME/CFS in clinical populations. Cortical spreading depression (CSD), the neurophysiological substrate of migraine aura, requires massive rapid ATP consumption followed by a slow recovery phase. Under the reserve-ceiling model, migraine may represent a *third* phenotypic expression of the same low-mitochondrial-reserve substrate: PFC dopaminergic demand failure → ADHD; systemic energy circuit failure after Hit 2 → ME/CFS; cortical ATP-demand spike exceeding ceiling → migraine.

*Falsifiable prediction:* Migraine prevalence in ME/CFS will be significantly elevated specifically in haplogroup H (tight coupling, exercise-crash phenotype) and reduced in haplogroup U, controlling for sex and disease duration. If haplogroup distribution among ME/CFS patients with comorbid migraine does not differ from ME/CFS patients without migraine, the common-reserve model for this comorbidity cluster is not supported. *Limitations:* No published study has tested haplogroup × migraine comorbidity in ME/CFS. The reserve-ceiling model for migraine is an analogy not yet formalized. Not yet replicated.
] <spec:migraine-reserve-bridge>

#speculation(title: [Long COVID ME/CFS Subset Haplogroup Prediction])[
*(Certainty: 0.20 — rests entirely on a single unreplicated haplogroup H symptom-association study (Billing-Ross 2016, n=193) extended to Long COVID without independent evidence.)*

If mitochondrial reserve capacity gates ME/CFS-style outcomes after a viral trigger (Section @spec:mtdna-reserve-threshold), this predicts a specific and testable pattern in Long COVID cohorts: patients who meet ICC ME/CFS criteria post-SARS-CoV-2 should show haplogroup distributions skewed toward symptom-severity-predicting haplogroups (haplogroup H enrichment per @BillingRoss2016mtDNA), while Long COVID patients who recovered or who do not meet ME/CFS criteria should not show the same haplogroup skew.

This is among the most tractable predictions from the reserve-threshold model: large Long COVID biobanks (RECOVER, PHOSP-COVID) already hold stored DNA and phenotypic data. Haplogroup determination from existing samples requires only targeted mtDNA sequencing — no additional patient contact. A null result (equivalent haplogroup distribution across ME/CFS-criteria vs non-ME/CFS Long COVID) would constrain the predisposition model to non-mtDNA mechanisms. *Limitations:* Long COVID biobanks may not have applied strict ME/CFS diagnostic criteria at enrollment; retrospective phenotyping from biobank data is imperfect. Haplogroup H's predicted enrichment is based on a single unreplicated study (Billing-Ross 2016, n=193). Not yet replicated.
] <spec:long-covid-haplogroup>
...
#hypothesis(title: [Bidirectional Connective Tissue-ME/CFS Relationship])[
*Certainty: 0.45.* Connective tissue disorders (CTDs), particularly hypermobile Ehlers-Danlos syndrome (hEDS) and hypermobility spectrum disorders (HSD), show a bidirectional relationship with ME/CFS @Ganesh2024LongCOVIDHSD, Logarbo2024LongCOVIDEDS, Eccles2021hypermobility. Wirth @Wirth2026CTDMECFS proposes that CTD may predispose to ME/CFS, while ME/CFS pathophysiology (ROS-mediated inhibition of prolyl hydroxylases, chronic HIF-1$alpha$ elevation, mast cell activation) may exacerbate underlying CTD pathology. Study: (n=~2000 survey @Berg2026HSDSurvey, n=58 wearable @Wilson2026hEDSWearable, certainty: Medium for most findings).

*CTD → ME/CFS predisposition:* Pre-existing connective tissue disorders (hEDS/HSD) may predispose to ME/CFS through: (1) Ligament laxity and craniocervical instability → mechanical strain on nervous system @Ramirez-Paesano2023CCI; (2) Autonomic dysfunction → orthostatic intolerance and fatigue @Almutairi2026DysautonomiaMSD, Kulin2026POTSGI, Wilson2026hEDSWearable; (3) Mast cell hyperactivity → inflammatory milieu @Kulin2026POTSGI; (4) Hypermobility prevalence: 30-57% in ME/CFS vs 10-15% general population @Ganesh2024LongCOVIDHSD, Eccles2021hypermobility.

*ME/CFS → CTD exacerbation:* ME/CFS pathophysiology may worsen connective tissue through: (1) ROS-mediated inhibition of prolyl hydroxylases → impaired collagen crosslinking @Wirth2026CTDMECFS; (2) Chronic HIF-1$alpha$ elevation → altered extracellular matrix composition @Moschini2026HIFTendinopathy, Sahin2012HIFTendon; (3) Mast cell activation → MMP-mediated collagen degradation @Wirth2026CTDMECFS, Sahin2012HIFTendon; (4) Basement membrane thickening in capillaries → impaired perfusion @Wirth2026CTDMECFS.

*Clinical implications:* Screening all ME/CFS patients for hypermobility using Beighton score, screening Long COVID patients for connective tissue disorders, assessing autonomic function in hypermobile patients (COMPASS-31, HRV monitoring), and considering craniocervical instability in hypermobile patients with ME/CFS-like symptoms. Treatment approaches include paced physical therapy adapted for hypermobility, autonomic management (salt/fluids, compression, midodrine, ivabradine), and mast cell stabilization (antihistamines, cromolyn, ketotifen).

*Limitation:* Bidirectional relationship is biologically plausible but not directly tested longitudinally. No studies have tested whether treating CTD improves ME/CFS outcomes. Much evidence extrapolated from tendinopathy, EDS, and POTS literature rather than direct ME/CFS-CTD measurements. (Not yet replicated.)
] <hyp:ct-mecfs-bidirectional>
...
The inflammatory consequences of repeat de-silencing in aging ("repeat-driven inflammaging") provide a direct mechanistic framework for ME/CFS: viral infection could trigger an acute, high-velocity version of the same slow process that underlies age-associated immune dysfunction. Supporting convergences:
    ...
    - Senescent cells accumulate LINE-1 and HERV-K RNA via DNMT3B downregulation; ME/CFS shows features of accelerated biological aging (epigenetic clock studies, shortened telomeres in some cohorts)
...
Two independent routes to HSAT2 derepression have been established: (1) HSF1 nuclear stress body formation at pericentromeric satellite sequences under heat or proteotoxic stress @Decottignies2012Sat2heatshock; (2) CTCF chromatin insulator loss, as demonstrated by Miyata et al., where HSAT2 ncRNA accumulates in senescent cells and displaces CTCF from insulator sites flanking HSAT2 loci, permitting satellite read-through and SASP gene activation. Oxidative stress reduces CTCF expression, opening this second route independently of HSF1.
...
*Limitation:* No cross-condition muscle biopsy study exists. Fibromyalgia and Gulf War illness ISR data are absent; long COVID ISR data exist only in cell models. The Omdal peripheral blood null result reminds that tissue compartmentalisation complicates any blood-based cross-condition comparison. (Not yet replicated.)
