...
=== Deliberate Intermittent Dosing as Interoceptive Dose-Finding <sec:interoceptive-dose-finding>

A clinically observed behaviour in ME/CFS — patients deliberately skipping doses, taking drug holidays, or pulsing medications on alternate days — is typically categorised as non-adherence. But the same behaviour, viewed through the lens of the interoceptive prediction error framework (Section @sec:ch15-interoceptive-dissociation) and the allostatic-interoceptive network (Section @sec:ch15-inflammation-interoception), admits a different interpretation: the patient is running an N-of-1 dose-response trial using interoceptive feedback as the endpoint.

#speculation(claim: [Deliberate intermittent dosing is interoceptive N-of-1 experimentation.], evidence: [No direct study of intermittent dosing as interoceptive experimentation exists. The mechanistic components are individually established: (a) the allostatic-interoceptive network — insula, ACC, brainstem autonomic nuclei — computes allostatic significance from interoceptive afferent signals and updates predictive models of bodily state @ZhangWager2025Allostatic; (b) in ME/CFS, interoceptive accuracy and sensibility dissociate — patients attend intensely to bodily signals (hypervigilance) but cannot resolve the prediction errors they generate @hyp:ch15-interoceptive-dissociation; (c) the hormetic dose-response framework establishes that the therapeutic window is a 3D volume (dose × time × response), with time-dependent features documented in the Calabrese corpus @Sun2018SeesawHormesis @Mushak2016TemporalHormesis, and that individual inversion-point positions vary with baseline activation state and genetic variation (Section @sec:ch-integrative-models); (d) the GPCR resensitization clock requires a drug-free interval whose length is receptor-specific — continuous occupancy fills this interval and produces tachyphylaxis (Section @sec:ch-integrative-models). The hypothesis that the patient *deliberately* exploits these temporal features through interoceptive feedback is a conceptual synthesis of established components, not a separately evidenced claim.], certainty: [0.20 — no study has ever asked ME/CFS patients about their dosing strategy in terms that would detect deliberate interoceptive experimentation. The individual mechanistic components range from cert 0.30 (time-dependent hormesis) to cert 0.70 (allostatic-interoceptive network mapping in healthy volunteers, @ZhangWager2025Allostatic). The conceptual link between them is untested.], falsifiability: [If qualitative interview data from ME/CFS patients who pulse medications reveal no pattern of deliberate interoceptive experimentation — i.e., patients report skipping doses due to side effects, forgetfulness, or cost, not because they are testing the dose-response window — the interoceptive hypothesis is not supported. Falsified if < 10% of pulsing patients report interoceptive dose-finding as a reason.], dntu: [whether deliberate pulsing produces better outcomes than continuous dosing in ME/CFS specifically. The pulsed therapy framework (Section @sec:ch-integrative-models) provides the pharmacological rationale; the interoceptive framing adds a *patient-experience* interpretation that predicts patients will be more likely to pulse when they have accurate interoceptive awareness, but this prediction is untested.])[

*Mechanism:* The patient takes a dose → the allostatic-interoceptive network registers the resulting change in bodily state (symptom intensity, alertness, pain, fatigue) as a prediction error against the expected state → the network updates its predictive model → the patient adjusts the next dose based on the updated model. Repeated perturbation-and-sensing cycles — which require drug-free intervals for the system to return to baseline — allow the network to approximate the dose-response curve without any biomarker or external measurement. The patient is not non-adherent; they are running a Bayesian dose-finding algorithm through their own interoceptive system. The fact that ME/CFS patients are often interoceptively hypervigilant (Section @sec:ch15-interoceptive-dissociation) — attending intensely to bodily signals — makes them *more* likely, not less, to detect subtle dose-response patterns that a clinician relying on questionnaire scores would miss.

*Relationship to the paradoxical reactor phenotype:* The paradoxical reactor framework (Section @sec:paradoxical-reactor) treats micro-dosing as a safety measure for fragile patients. The interoceptive dose-finding framework treats intermittent dosing as a rational strategy for *all* patients — the patient who pulses LDN at 0.5 mg every other day, who skips a duloxetine dose before a cognitively demanding day, or who takes a drug holiday after three weeks of continuous dosing is not necessarily fragile. They are probing the temporal dimension of their own hormetic window.

*Relationship to the pulsed therapy framework:* The time-dependent hormesis framework (Section @sec:ch-integrative-models) provides the pharmacological justification for intermittent dosing — GPCR resensitization, differential off-rate kinetics, Nrf2 signal recovery. The interoceptive framing adds a clinical observation: patients were doing this before the pharmacology was understood. The paper provides the mechanism; patients provided the pattern.
