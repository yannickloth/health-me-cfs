...
#speculation(title: [Deliberate Intermittent Dosing as Interoceptive N-of-1 Experimentation])[
*Evidence:* No direct study of intermittent dosing as interoceptive experimentation exists. The mechanistic components are individually established: (a) the allostatic-interoceptive network — insula, ACC, brainstem autonomic nuclei — computes allostatic significance from interoceptive afferent signals and updates predictive models of bodily state @ZhangWager2025Allostatic; (b) in ME/CFS, interoceptive accuracy and sensibility dissociate — patients attend intensely to bodily signals (hypervigilance) but cannot resolve the prediction errors they generate @hyp:ch15-interoceptive-dissociation; (c) the hormetic dose-response framework establishes that the therapeutic window is a 3D volume (dose × time × response), with time-dependent features documented in the Calabrese corpus @Sun2018SeesawHormesis @Mushak2016TemporalHormesis, and that individual inversion-point positions vary with baseline activation state and genetic variation (Chapter @ch:integrative-models); (d) the GPCR resensitization clock requires a drug-free interval whose length is receptor-specific — continuous occupancy fills this interval and produces tachyphylaxis (Chapter @ch:integrative-models). The hypothesis that the patient *deliberately* exploits these temporal features through interoceptive feedback is a conceptual synthesis of established components, not a separately evidenced claim.

*Certainty: 0.20* — no study has ever asked ME/CFS patients about their dosing strategy in terms that would detect deliberate interoceptive experimentation. The individual mechanistic components range from cert 0.30 (time-dependent hormesis) to cert 0.70 (allostatic-interoceptive network mapping in healthy volunteers, @ZhangWager2025Allostatic). The conceptual link between them is untested.
