...
*Certainty: 0.40.* (0.50→0.40: downgraded per adversarial review — Kleshnev kinetics derive from competitive rowers at 18--36 spm, not ultra-slow 5--10 spm; day-to-day PEM threshold variability undermines the precision-calibration premise. The concept extends generic sub-threshold pacing using a rowing-specific parameter, not a novel treatment principle.) Rowing power-stroke rate kinetics are characterized in healthy athletes @Kleshnev2004; the therapeutic-window application to ME/CFS and the relationship at ultra-low stroke rates are unvalidated. Not yet replicated.

Rowing allows incremental intensity adjustment via stroke rate — approximately 15--20 percent power change per stroke/min at standard training speeds. At the ultra-low stroke rates (5--10 spm) used in the ME/CFS protocol, this relationship has not been validated. The concept extends generic sub-threshold pacing (start low, increase gradually, monitor PEM, roll back) using a rowing-specific metric rather than constituting an independent treatment principle. Importantly, PEM threshold varies day-to-day with sleep quality, hormonal cycle, illness status, and prior activity — a calibration performed every 2--3 weeks may be invalid on days when threshold shifts by more than the 15--20 percent step size between stroke rates. On such days, the "calibrated" rate may become an overexertion rate.
