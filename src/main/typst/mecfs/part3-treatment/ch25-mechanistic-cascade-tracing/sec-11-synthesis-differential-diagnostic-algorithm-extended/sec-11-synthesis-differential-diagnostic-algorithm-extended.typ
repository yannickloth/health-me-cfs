...
The binary-probe version of this algorithm used sequential probes. This extended version incorporates the full dimensionality: effect magnitude, onset latency, response duration, dose-response slope, side-effect patterns, drug-drug interactions, endogenous probe patterns, and the null matrix constraint. It also incorporates the PEM budget — probes are sequenced by (diagnostic yield) / (PEM risk). The *which-probe-first* ordering that this sequencing presupposes — restorative before threshold-modulatory before symptomatic, PEM-cost-first, zero-energy mechanical probes first, null-elimination, and prerequisite/contraindication gating — is formalized as an explicit five-rule principle in Section @sec:sequential-ordering-principle and its stopping counterpart, the do-not-disturb rule (Section @sec:do-not-disturb-rule); this synthesis applies those rules at every step below rather than restating them. (On the "zero-energy mechanical probes first" rule: it applies as a *priority jump* — mechanical probes such as shoe-lift and cervical collar precede any drug probe when postural/positional modulation is present; absent that trigger they sit at Step 5 below, because Steps 0–1 extract maximal information from existing data and near-zero-marginal-cost probes before any spent probe.) The ordering framework is a clinical-research scaffold that has not been prospectively validated (certainty 0.30; PARTIAL cap; not a drug or dose recommendation).
