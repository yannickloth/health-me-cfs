...
*(Certainty: 0.40 — mechanistic rationale for mast cell-ECM pathway modulation; tVNS clinical efficacy in post-infectious fatigue is null in controlled trials @Balan2026taVNS).*
*WARNING — RESEARCH-STAGE ONLY.* tVNS for mast cell-ECM modulation has never been tested in a controlled trial for any ME/CFS endpoint. The dosing protocol below is for research consideration only and does not constitute a clinical recommendation. Standard tVNS parameters can provoke PEM/crashes in severe ME/CFS (Lugg 2024 @Lugg2024MECFS). Do not apply to severe or very-severe patients outside a clinical trial.
tVNS via left ear concha (5 min, 1-2x daily). Addresses autonomic dysfunction AND mast cell-mediated ECM degradation. Early ME/CFS data (Yu2022, Natelson2022).
...
*Evidence Base.* tVNS reduces inflammatory cytokines and mast cell activation in controlled trials. Small ME/CFS open-label studies show symptom improvement (Yu2022, Natelson2022). Mast cell-ECM degradation pathway is established in other conditions. No ME/CFS-specific connective tissue endpoint data. *Caution:* All ME/CFS tVNS evidence is open-label; sham-controlled RCTs in the closely analogous post-COVID-19 condition are consistently null for clinical efficacy despite confirmed HRV engagement @Balan2026taVNS. The open-label ME/CFS signal should be interpreted in light of this evidence.
...
#speculation(title: [taVNS for Post-COVID-19 Condition — Systematic Review Places Controlled Trial Evidence in Context])[
*(Certainty: 0.25 — certainty of the clinical inference that tVNS is not an effective treatment for post-infectious fatigue. The underlying systematic review (Balan 2026) has certainty 0.65 — high-quality methodology, but the clinical inference is constrained by small total n (154), heterogeneous populations, and the cross-condition extrapolation to ME/CFS. Origin: literature-derived.)* A 2026 systematic review of transcutaneous auricular VNS for post-COVID-19 condition (PCC) (Balan et al., n=154 across 5 studies: three RCTs, two single-arm) rated all efficacy outcomes as "very low" certainty (GRADE) @Balan2026taVNS. The pattern of evidence is notable: (a) every adequately controlled study found no superiority over sham; (b) the best-powered RCT (Percin et al., ~n=50) produced a paradoxical sham > active result on fatigue @Percin2025taVNS; (c) the COVIVA sham-controlled RCT (n=45) found no significant between-group differences @Gierthmuehlen2026COVIVA. Positive efficacy signals come exclusively from uncontrolled single-arm studies (Azabou 2026, n=17 @Azabou2026taVNS; Zheng 2024, n=24 @Zheng2024tVNS). Safety was rated "low" certainty: mild adverse events only (skin irritation, vertigo, headache) with no serious events across all PCC studies.
...
#speculation(title: [Candidate Explanations for the tVNS Sham-Superior Paradox and Implication for Trial Design])[
*(Certainty: 0.20--0.55 across candidate mechanisms. Origin: brainstorm.)* The systematic review establishes that HRV-modulation dissociates from clinical benefit in controlled tVNS trials for post-infectious fatigue. Five candidate explanations — not mutually exclusive — compete to explain this dissociation @Balan2026taVNS @Percin2025taVNS.
...
*Candidate 1: GPCR autoantibody receptor-level blockade (certainty 0.35).* The cholinergic anti-inflammatory pathway (CAP) requires taVNS → vagal afferents → NTS → vagal efferents → splenic nerve → norepinephrine release → β2-AR on splenic T cells → ACh release → α7-nAChR on macrophages → TNF-α suppression. GPCR autoantibodies — particularly β2-AR AAb, elevated in both ME/CFS and PCC @Azcue2026gpcr @Blitshteyn2026Neuroimmune — may block the splenic T-cell β2-AR step, rendering increased vagal efferent firing inert downstream of the cardiac level. If confirmed, this predicts a treatment-by-biomarker interaction: AAb-negative patients should show clinical benefit from taVNS; AAb-positive patients may show HRV improvement without clinical translation. *Falsified if* AAb-high patients respond to taVNS equivalently to AAb-low patients, or if the CAP is confirmed to be intact in AAb-positive patients via SPM or TNF-α suppression biomarkers.
...
#synthesis(title: [tVNS in Post-Infectious Fatigue — Mechanistic Plausibility Without Clinical Validation])[
The convergent evidence from five PCC studies — three sham-controlled RCTs and two uncontrolled pilots, systematically reviewed by Balan et al. (2026) @Balan2026taVNS — establishes a clear pattern: transcutaneous vagus nerve stimulation consistently engages cardiac autonomic targets (increased HRV, RMSSD, HF power) but consistently fails to produce between-group clinical benefit over sham in controlled trials @spec:tvns-pcc-systematic-review. This dissociation has been replicated across independent labs and populations (Percin 2025: sham > active @Percin2025taVNS; COVIVA: no between-group differences @Gierthmuehlen2026COVIVA) and represents the strongest controlled evidence base in any post-infectious fatigue population, even if it is negative evidence. The mechanistic framework — cholinergic anti-inflammatory pathway, vagal-SPM coupling, parasympathetic augmentation, mast cell modulation — remains biologically sound, but the therapeutic inference from that framework has failed its most direct test to date @spec:tvns-dissociation-mechanisms.
...
*Open questions.* The central unresolved question is the mechanism of the dissociation @oq:tvns-sham-superior-paradox: four competing hypotheses (AAb receptor-level CAP blockade, inadequate target engagement readout — HRV misses the CAP, non-monotonic dose-response, sham non-specific superiority) are distinguishable by a single adequately designed trial @spec:tvns-dissociation-mechanisms. Until that trial is done, the evidence quality concerns — small sample sizes, selection bias toward milder patients, concomitant medication confounding, and HRV as an incomplete engagement biomarker — further constrain interpretation @spec:tvns-evidence-quality-concerns. The most actionable implication is that research funding priorities should shift from additional open-label tVNS studies to mechanistic trials that resolve the dissociation, ideally with biomarker verification of central vagal engagement (NTS fMRI) and CAP activation (plasma SPMs, TNF-α suppression).
...
LDN (1--4.5 mg nightly) modulates microglial activation via toll-like receptor 4 antagonism and has documented effects on TRPM3 calcium channel function in ME/CFS NK cells @Cabanas2021. LDN has four mechanisms with non-overlapping dose optima — TLR4/Nrf2 hormetic priming (0.5--1.5 mg), opioid compensatory upregulation (1.5--3.0 mg), TRPM3 restoration (3.0--4.5 mg), and orexin disinhibition — the thermoregulatory benefit may itself show inverted-U dose-dependence consistent with the broader hormetic framework (@sec:hormetic-dose-response). If central thermoregulatory centers (hypothalamus, brainstem) are under chronic microglial-mediated inflammatory tone, LDN could improve thermoregulation indirectly by reducing neuroinflammation, independently of its TRPM3 effects. The dual mechanism — peripheral TRPM3 normalization + central glial modulation — would predict thermoregulatory benefit as a secondary outcome. Falsifiable: LDN over 12 weeks improves TRPM3 calcium influx by 25% and reduces heat intolerance symptoms by 30%; improvement correlates with baseline inflammatory markers. No ME/CFS LDN thermoregulation data; LDN evidence in ME/CFS is from small open-label studies.
] <oq:ldn-thermoreg>

#limitation(title: [Low-Dose Naltrexone: Fibromyalgia Secondary-Outcome Responder Evidence is Null])[
The FINAL trial (n=99 women with fibromyalgia, LDN 6 mg vs placebo for 12 weeks) found no significant primary pain benefit @DueBruun2024LDNFibromyalgia. A responder re-analysis of the six secondary non-pain outcomes (tenderness, fatigue, sleep disturbances, depression, memory problems, stiffness) found *no significant difference* in 30% responder rates on any outcome @Nielsen2026LDNFMResponder. The highest risk ratio was memory (RR 1.67, 95% CI 0.82–2.95), which does *not* reach significance — the "may improve memory problems" signal from the primary paper does not survive responder-category analysis. An independent 12-month RCT (INNOVA) also found no pain benefit over placebo @RodriguezFreire2026LDNFMINNOVA, and a meta-analysis found no between-group superiority vs placebo @Ologunowa2025LDNFMMeta. The re-analysis group itself concluded LDN efficacy in fibromyalgia is lower than previously reported @DueBruun2026LDNFMReanalysis.

This does not question LDN's favorable safety profile — a meta-analysis found no excess serious adverse events vs placebo (RR 0.84) @Bolton2019NaltrexoneSAE — but the clinical-benefit case for fibromyalgia secondary symptoms is not supported by the responder evidence. The mechanistic rationales (TLR4 antagonism, opioid-growth-factor rebound) are biologically plausible but did not translate into measurable symptom benefit at clinical doses in fibromyalgia. The responder analysis itself carries methodological caveats: n=99 is underpowered for six secondary outcomes, the 30% responder threshold is arbitrary, and multiple testing inflates false-positive risk; the memory CI (RR 1.67, 0.82–2.95) spans null-to-large, so it neither establishes nor excludes a real cognitive effect. Severity applicability: fibromyalgia population, not stratified by severity; ME/CFS applicability unknown — no direct ME/CFS responder data in this analysis.
...
#speculation(title: [Low-Dose Naltrexone: The TLR4 Mechanism May Never Be Engaged at Clinical Dose])[
*Certainty: 0.30.* The null fibromyalgia responder results may be mechanistically *expected* rather than contradictory. (+)-naltrexone is a weak TLR4 antagonist; the optimized derivative CIAC101 required ~6200× potency gain to reach nanomolar TLR4 antagonism @Gao2025CIAC101TLR4. Clinical LDN is racemic (both enantiomers) at 4.5–6 mg. If the TLR4-blocking species needs concentrations orders of magnitude above what LDN achieves at these doses, then the FM null reflects a dose-to-target translation failure — the TLR4 rationale was never actually tested by LDN at clinical dose. This is consistent with the alternative OGFr/enkephalin-rebound mechanism @Zagon2018OGFrBlockade, which may require longer duration or a fatigue-specific subtype not captured by the FM responder categories @Nielsen2026LDNFMResponder. Note this occupancy question stands in tension with the hormetic-window framing in @sec:hormetic-dose-response, which assumes LDN engages TLR4 at low doses to trigger Nrf2 priming — if the TLR4 target is never engaged at clinical dose, that specific arm of the hormetic model is conditional on a dose-occupancy measurement. Severity applicability: fibromyalgia, not severity-stratified; ME/CFS applicability unknown.
...
#open-question(title: [Does Low-Dose Naltrexone Improve Pain in Fibromyalgia? Divergent Meta-Analyses])[
Two meta-analyses of LDN in fibromyalgia reach opposite conclusions on pain. A 2025 meta-analysis found no between-group superiority of LDN over placebo @Ologunowa2025LDNFMMeta, while a 2024 meta-analysis reported significant pain reduction (MD −0.86) and higher pressure pain threshold @Vatvani2024LDNFMMetaTSA. The discrepancy reflects differing trial sets and analytic methods, and the positive analysis carries an erratum. Because the certainty difference between the two (0.56 vs 0.52 discounted) is within measurement uncertainty, the paper presents both without weighting toward either. Fibromyalgia responder data (null) leans toward the null meta-analysis @Nielsen2026LDNFMResponder, but the pain-specific question remains open. Severity applicability: fibromyalgia, not severity-stratified.
...
#speculation(title: [Intermittent Hypoxia-Hyperoxia Treatment (IHHT) — REenergizeME Trial])[
*Certainty: 0.30.* IHHT — repeated cycles of hypoxic (FiO#sub[2] 9--13%) and hyperoxic (FiO#sub[2] ~36%) exposure — is distinct from the hypoxia-only IHT above: the alternating hyperoxic restitution phase is hypothesised to enhance mitochondrial antioxidant/repair pathway activation via hormetic redox signalling @Zhang2023HypoxiaConditioning. The registered, blinded, placebo-controlled REenergizeME RCT (Nochi et al.\ 2026) is testing this in 104 female ME/CFS patients (International Consensus Criteria, mild--moderate NICE) over 6 sessions in 8 weeks, primary endpoint SF-36 vitality @Nochi2026REenergizeME.

*Mechanistic rationale (hypothesis, not established).* Recurrent mild hypoxic injury to brain and muscle — from microvascular dysfunction and functional GPCR autoantibodies disrupting oxygen delivery — is hypothesised to contribute to the mitochondrial dysfunction, oxidative stress and inflammation associated with PEM; each step is a candidate, not a confirmed chain. Mild hormetic hypoxia stabilises HIF-1$alpha$, activating mitochondrial antioxidant/repair, angiogenesis and vascular regulation pathways. This is the controlled ambulatory instantiation of the altitude/HIF-1$alpha$ preconditioning hypothesis in this paper (@spec:hif-pathway-inertia, hypoxia-altitude corpus) @Nochi2026REenergizeME @Zhang2023HypoxiaConditioning.

*Distinction from pathological chronic IH (dose-dependent, not categorical).* The mild, brief, low-frequency hormetic IHHT profile is proposed to differ from the pathological sustained intermittent hypoxia of OSA, which produces sustained HIF-1$alpha$ elevation, oxidative stress, sympathetic activation and cognitive harm @Erdem2026HIF1aOSA. Dose/cycle structure determines the outcome @She2026IHParadigms — the hormetic window is the safety-critical boundary @Zhang2023HypoxiaConditioning. However, this is a dose-response bet, not a demonstrated category boundary: no data place IHHT's specific protocol inside the hormetic window in ME/CFS, and concurrent undiagnosed OSA in the cohort is not excluded (the protocol screens for obstructive lung disease but not sleep-disordered breathing). The protocol's hypoxic phase uses FiO#sub[2] 9--13%, but it does not specify the target arterial SpO#sub[2] excursion during hypoxia; how far each patient descends toward the pathological pole is therefore unmeasured.

*Evidence status — protocol only.* Zero direct ME/CFS efficacy data exist. Prior long-COVID IHHT evidence is non-randomized or non-controlled (Doehner2024 open pilot; Kapel2025 non-controlled cohort), and one cohort (Kapel2025) overlaps the REenergizeME team and device with a commercial interest — its effect size underpins the trial's power calculation @Doehner2024IHHTLongCovid @Kapel2025IHHC. The blinded placebo-controlled design is the standard corrective to that conflict, but the COI-derived effect assumption also risks underpowering the trial, which could yield a non-informative (rather than merely uncertain) 2029 result. No blinded sham-controlled IHHT RCT has published; results expected 2029.

*Safety.* IHHT as administered is reported safe in human studies (transient headache, short-lived fatigue; no serious AEs). No ME/CFS-specific harm data exist; PEM exacerbation is the documented protocol-defined AE. Contraindicated in obstructive lung disease, pneumothorax history, pregnancy, claustrophobia, interfering devices. Bedbound/very-severe excluded from the trial; dose unvalidated in ME/CFS. Research-stage — not a clinical recommendation.

*Falsifiable prediction.* In the REenergizeME RCT, IHHT will produce a greater SF-36 vitality improvement than sham together with the specific hormetic signature — improved NIRS muscle oxygenation recovery and reduced oxidative-stress markers (not merely subjective vitality gain). A between-group vitality difference WITHOUT these mechanistic changes (consistent with placebo or generic autonomic placebo) does NOT support the hormetic-adaptation mechanism; absence of any between-group difference falsifies it outright. This differs from the hypoxia-only IHT prediction by the added requirement of the hyperoxic-phase-specific redox/antioxidant biomarker response.

*Consequence:* None yet for patients — IHHT is an unproven registered trial; its value is that a rigorous placebo-controlled result (due 2029) will test whether the hypoxia-hormesis mechanism can be harnessed to treat ME/CFS, and its objective mechanistic endpoints (muscle oxygenation, autonomic, biomarkers) may reveal which patients carry the hypoxia-related pathology.

*Severity applicability:* mild-moderate only (trial inclusion) and female-only (all participants female, 20–59 y); generalizability to male patients untested. Unknown for severe/very-severe, who are excluded and must not attempt IHHT unsupervised (seek a trial site or await results rather than self-treat).

*Evidence type:* RCT protocol (Nochi2026); non-randomized/non-controlled long-COVID cohorts (Doehner, Kapel); mechanism review (Zhang2023). Not a completed RCT result in ME/CFS.
] <spec:ihht-reenergizeme>
...
#include "brain-clearance-drugs/brain-clearance-drugs.typ"
