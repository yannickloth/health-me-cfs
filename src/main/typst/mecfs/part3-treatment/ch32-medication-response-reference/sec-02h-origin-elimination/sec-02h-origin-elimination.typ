...
*Certainty: 0.70.* The unfalsifiability claim is logical, not empirical. A pharmacodiagnostic null tests "is mechanism X currently rate-limiting?" — not "does mechanism X have a genetic basis?" Genetic contribution is a property of the system's architecture, not an interrogable bottleneck.
...
#table(
  ...
  [Valacyclovir/valganciclovir], [Active herpesvirus replication as ongoing driver (0.45--0.58)], [Viral replication as dominant ongoing driver], [Trigger-only viral origin; intermittent amplifier with sanctuary-site latency], [0.45],
  ...
  [Any drug], [Its specific mechanism target], [Genetic contribution — cannot be eliminated by any null result], [Genetic vulnerability as system set-point: determines susceptibility to all other origin hits without being addressable as a bottleneck], [0.70],
)

*Consequence:* Every null response is asymmetric — it eliminates its target mechanism at higher certainty than it constrains the origin category that mechanism belongs to. The rituximab null (0.80 mechanism cert) reduces autoimmune-origin posterior but does not falsify it (0.60 origin cert). The valacyclovir null (0.45 mechanism cert) reduces viral-origin posterior but barely constrains it (0.45 origin cert). The asymmetry is structural: a drug targets a specific molecular node; an origin hypothesis is a category of causation that can manifest through multiple nodes, some of which the drug does not probe. Origin inference requires combining pharmacodiagnostic data with temporal trajectory — a three-axis inference that the null ladder alone cannot perform @Maziarz2024CausalPluralism.
