...
== Null Subtyping — Absent vs. Blocked vs. Overwhelmed <sec:null-subtyping>

*Certainty: 0.20.* The tripartite null framework extends the binary null logic of @sec:null-ladder-clinical and @sec:null-matrix with a pharmacological distinction that the existing framework acknowledges implicitly but never formalizes. The distinction between absent, blocked, and overwhelmed receptor states is grounded in century-old receptor occupancy theory @Finlay2020receptorOccupancy and the well-established pharmacology of autoantibodies as endogenous GPCR modulators @Skiba2021AAbGPCRmodulation, but the specific diagnostic probes proposed here (allosteric bypass, concentration-competition, bypass agonist) have never been tested in ME/CFS. The framework is logically coherent and mechanistically specific — but zero empirical validation exists in this disease population.

The null ladder tells us a drug can eliminate multiple mechanisms in one trial. The null matrix formalizes the pattern of nulls as Bayesian evidence. But both treat null as binary: mechanism eliminated or not. Receptor pharmacology reveals this is a lossy compression. A drug failure can mean any of three mutually exclusive states, and the *kind* of null — not just its existence — determines what it actually eliminates.

=== The Three Null Subtypes
...
- Beta-3 adrenergic agonist (mirabegron) when β2-AR agonist (salbutamol) produces null — if β3 works, β2 is specifically absent while other β-adrenergic subtypes are present.
...
*Examples:*
- β2-AR autoantibody (AAb) occupying the orthosteric site → isoproterenol fails (orthosteric agonist). But salbutamol or salmeterol (which bind partially distinct pockets) might produce a partial or full response. If so → AAb present at β2-AR orthosteric site confirmed. If both fail → absent or allosteric site also disrupted.
- M2 muscarinic AAb as negative allosteric modulator (documented in POTS @Sunami2025GPCRPOTS) → standard muscarinic agonist (pilocarpine) fails. But an allosteric modulator that binds away from the AAb site may recover function. If so → BLOCKED, not ABSENT — the receptor is present but AAb-modulated.

*Caveat — β-arrestin-biased blockade.* Some GPCR autoantibodies produce sustained biased signaling (β-arrestin over G-protein) and receptor internalization, making the receptor functionally absent for Gαs pathways while β-arrestin pathways remain active @DodgeKafka2026betaArrestinAAb. This is a hybrid state: the receptor is structural present but functionally absent for certain signaling modes. The probe in this case is a β-arrestin-biased ligand — if it works where a Gαs-biased ligand fails, chronic AAb-mediated desensitization is the mechanism.

*Evidence basis:* Autoantibodies acting as orthosteric antagonists and allosteric modulators at GPCRs is well-established pharmacology @Skiba2021AAbGPCRmodulation. GPCR autoantibodies are documented in POTS, Long COVID, and ME/CFS — though their existence in ME/CFS is contested (Germain2025 null by REAP/Luminex; positive by CellTrend ELISA). No published study has tested an allosteric bypass probe (salbutamol vs isoproterenol) in AAb-positive ME/CFS patients.
...
*Example — KYNA (kynurenic acid) at NMDA receptors.* KYNA is an endogenous NMDA antagonist that binds the glycine co-agonist site, not the glutamate site @Hazrati2024kynureninePain. If NMDA function is tested with ketamine and fails at standard dose but works at high dose → KYNA dominance confirmed. The low-dose failure looked like NMDA absence but was competitive blockade by elevated KYNA. The kynurenine pathway is dysregulated in ME/CFS @Kavyani2024kynurenineMEcfs — though KYNA direction is inconsistent (elevated in some studies, depleted in others), meaning the overwhelmed null subtype only applies to patients with confirmed KYNA elevation.
...
A null response under the existing binary framework means: "mechanism eliminated — this system is not rate-limiting." Under the tripartite framework, the same null means one of three things depending on which probe resolves it:
...
*Consequence:* For researchers designing pharmacodiagnostic protocols, null subtyping suggests that "drug failed" is underspecified — the protocol should distinguish whether the failure was due to target absence (genetic/developmental), target inaccessibility (AAb/competitive block), or target saturation (endogenous antagonist). Each null type has different treatment implications and research priorities. For patients, the framework reframes treatment failures as potentially informative about *how* a mechanism is broken, not just *whether* it is broken.
...
The tripartite framework assumes a single disease-steady-state — that a receptor is either absent, blocked, or overwhelmed — and that this state is stable across time. Real pharmacology is messier:
...
The most common null type — ABSENT — can sometimes be predicted before a drug trial begins. Pharmacogenetic variants that render a receptor non-functional (knockout), reduce expression (promoter polymorphisms), or alter ligand binding affinity (missense variants) pre-identify patients for whom a drug targeting that receptor will produce null — regardless of disease state.

Extended pharmacogenetic panels that screen for loss-of-function variants in ME/CFS-relevant receptors could pre-assign the ABSENT null subtype for a subset of targets without requiring a drug trial:
- β2-AR polymorphisms (Gly16Arg, Gln27Glu) → altered desensitization kinetics and expression
- D2 receptor variants (Taq1A, DRD2 C957T) → altered expression and D2/D3 ratio
- CYP2D6 poor metabolizer status → functional null for prodrugs requiring CYP2D6 activation
- OPRM1 A118G → altered μ-opioid receptor signalling and LDN response variability
