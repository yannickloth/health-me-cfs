...
*(Certainty: 0.40 — mechanistic rationale for mast cell-ECM pathway modulation; tVNS clinical efficacy in post-infectious fatigue is null in controlled trials @Balan2026taVNS).* tVNS via left ear concha (5 min, 1-2x daily). Addresses autonomic dysfunction AND mast cell-mediated ECM degradation. Early ME/CFS data (Yu2022, Natelson2022).
...
*(Certainty: 0.20–0.35 across candidate mechanisms. Origin: brainstorm.)* The systematic review establishes that HRV-modulation dissociates from clinical benefit in controlled tVNS trials for post-infectious fatigue. Five candidate explanations — not mutually exclusive — compete to explain this dissociation @Balan2026taVNS @Percin2025taVNS.
...
*What this evidence constrains, and what it does not.* The PCC trials constrain clinical efficacy expectations: until a sham-controlled trial demonstrates specific benefit, tVNS cannot be presented as an effective intervention for post-infectious fatigue syndromes. This applies to the POTS tVNS protocol (@ch:action-mild-moderate, Section @sec:tavns-lifestyle), the corticosteroid-alternative speculation, the SPM restoration protocol, the mast cell-ECM modulation claim, and the autonomic-epigenetic modifier hypothesis — all of which carry mechanistic plausibility but none of which have controlled clinical validation. The evidence does NOT constrain: (a) the use of tVNS as a research probe of vagal function — HRV engagement is confirmed and the device remains a useful experimental tool, (b) the validity of vagal dysfunction as a pathophysiological mechanism — the null clinical evidence says nothing about whether vagal tone is abnormal in ME/CFS, only that modulating it does not improve symptoms in the available trials, (c) the possibility that a correctly stratified subgroup (GPCR AAb-negative, mild-moderate, POTS-predominant) could respond — this subgroup has never been tested in a controlled trial.
