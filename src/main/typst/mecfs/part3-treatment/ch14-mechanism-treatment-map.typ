...
The gain-vs-loss tension that previously divided the paper's model has been resolved by the unified per-locus vector model in Chapter @ch:causal-hierarchy-formal (@oq:consolidation-directionality). ME/CFS shows bidirectional methylation alterations: some loci hypermethylated, others hypomethylated. The vector model captures both via $bold(cal(M)) in [0, 1]^n$, with consolidation depth $||bold(cal(M)) - bold(cal(M))^"baseline"||$. DNMT3B redistribution (ProB → ProA/gene-region) is the single mechanism producing both patterns simultaneously, making gain and loss complementary manifestations rather than competing models. The therapeutic implication is locus-dependent:
