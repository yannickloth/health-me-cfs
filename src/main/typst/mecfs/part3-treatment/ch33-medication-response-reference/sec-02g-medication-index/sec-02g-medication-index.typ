== Medication Index — Alphabetical Quick-Reference

Select any medication to jump directly to its clinical interpretation section — response/non-response, dose-dependent differential diagnosis, side-effects-per-dose, combination inference, and compendium cross-reference.

#set list(marker: none)

/ @sec:low-dose-aripiprazole-lda: Low-Dose Aripiprazole (LDA) — Immune/Neuroimmune
/ @sec:beta-blockers: Beta-Blockers (Propranolol, Nebivolol) — Autonomic/Cardiovascular
/ @sec:b-vitamins: B Vitamin Supplementation — Emerging/Investigational
/ @sec:cimetidine: Cimetidine — Immune/Neuroimmune
/ @sec:cns-stimulants: CNS Stimulants (modafinil, methylphenidate, etc.) — Stimulants
/ @sec:mitochondrial-cofactors: Coenzyme Q10, NADH, D-Ribose, L-Carnitine, Alpha-Lipoic Acid — Mitochondrial
/ @sec:creatine: Creatine — Mitochondrial
/ @sec:dao: DAO (Diamine Oxidase) — Mast Cell/Histamine
/ @sec:dornase-alfa: Dornase Alfa (Pulmozyme) — Emerging/Investigational
/ @sec:droxidopa: Droxidopa — Autonomic/Cardiovascular
/ @sec:fludrocortisone: Fludrocortisone — Autonomic/Cardiovascular
/ @sec:fluvoxamine: Fluvoxamine — Emerging/Investigational
/ @sec:ginkgo: Ginkgo biloba (EGb 761) — Oxidative/Inflammatory
/ @sec:glycyrrhizin: Glycyrrhizin (Licorice Root) — Emerging/Investigational
/ @sec:h1-h2-antihistamines: H1 and H2 Antihistamines — Mast Cell/Histamine
/ @sec:ivig-rituximab: Immunoglobulins (IVIG) and Rituximab — Immune/Neuroimmune
/ @sec:ivabradine: Ivabradine — Autonomic/Cardiovascular
/ @sec:low-dose-il2-ch33: Low-Dose IL-2 — Emerging/Investigational
/ @sec:low-dose-naltrexone-ldn: Low-Dose Naltrexone (LDN) — Immune/Neuroimmune
/ @sec:magnesium: Magnesium — Electrolytes
/ @sec:methyl-donor: Methyl-Donor Support (SAMe, Methylfolate, etc.) — Emerging/Investigational
/ @sec:midodrine: Midodrine — Autonomic/Cardiovascular
/ @sec:nac: NAC (N-Acetylcysteine) — Oxidative/Inflammatory
/ @sec:omega3: Omega-3 Fatty Acids (EPA/DHA) — Oxidative/Inflammatory
/ @sec:pea: PEA (Palmitoylethanolamide) — Oxidative/Inflammatory
/ @sec:pyridostigmine-ch33: Pyridostigmine (Mestinon) — Immune/Neuroimmune
/ @sec:quercetin-luteolin: Quercetin and Luteolin — Mast Cell/Histamine
/ @sec:sleep-medications: Sleep Medications — Sleep/Pain
/ @sec:pain-medications: Pain Medications — Sleep/Pain
/ @sec:sodium: Sodium — Electrolytes
/ @sec:tudca: TUDCA / 4-Phenylbutyrate — Emerging/Investigational
/ @sec:tvns: tVNS (transcutaneous Vagal Nerve Stimulation) — Emerging/Investigational
/ @sec:valacyclovir-valganciclovir: Valacyclovir and Valganciclovir — Antiviral

Deeper pharmacodiagnostic entries (mechanism-exclusion logic, dose-specific side-effect patterns, worsening risk profiles) for all drugs are in @ch:mechanistic-cascade-tracing (sec-12). A consolidated per-drug dose-range interpretive reference — what benefit, lost benefit, and side effects at each dose reveal — is at @sec:hormetic-dose-range-reference.
