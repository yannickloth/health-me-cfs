...
*Clinical implication:* The damage is done. The goal shifts from "recovery" to "adaptation" — managing the patient at their new, lower baseline. The medication that caused this should be listed as an allergy or severe intolerance in the patient's record. Any medication sharing the same pharmacodynamic target should be approached with extreme caution — if the mechanism was VMAT2 depletion, all catecholamine-releasing agents are contraindicated; if the mechanism was microglial triggering, all neuroactive medications that activate TLR4 should be avoided.
]

==== The Energy Ratchet as the Unifying Mechanism for Medication-Induced Worsening

The energy-ratchet hysteresis model (@sec:ratchet-cascade) describes progressive, irreversible loss of functional capacity across repeated PEM episodes. The same model applies to medication-induced worsening, with one critical difference: the drug is the PEM trigger, not exertion. And unlike voluntary exertion — which the patient can stop — medication-induced PEM can continue for the full duration of drug exposure, producing a relentless, weeks-to-months-long metabolic collapse that far exceeds any PEM episode the patient has experienced from physical exertion alone.
...
The drug produces a paradoxical reaction (Pattern 2, @sec:side-effects-diagnostic — Pattern 2) — instead of the expected therapeutic or side effect, the target system responds in the opposite direction. This paradoxical response can trigger a cascade that outlasts receptor occupancy.
...
*Mechanism:* Mechanisms B (metabolic PEM — +7% REE, sustained tachycardia) + E (withdrawal-PEM convergence). Unlike amphetamines, methylphenidate does not cause direct VMAT2 depletion (Mechanism A) — this is the critical distinction. The worsening from methylphenidate is primarily metabolic: the drug worked (the patient felt better) but consumed more energy than the system could sustain. The crash-on-cessation is withdrawal superimposed on PEM — the dopamine drop triggers an acute crash in a system that was already metabolically depleted.
...
*Mechanism:* Over-sedation at doses below therapeutic threshold for MCAS — identifies histaminergic arousal dependency (Pattern 4, @sec:side-effects-diagnostic — Pattern 4). Does NOT cause protracted or permanent worsening — sedation resolves with drug clearance (hours to days), and there is no known mechanism for H1 blockade to cause structural damage, immune sensitization, or HPA axis suppression.
...
*Mechanism:* Paradoxical worsening (Pattern 5 — energy worsening, @sec:side-effects-diagnostic — Pattern 5) identifies the mitochondrial bottleneck node. This is NOT protracted or permanent worsening — it is acute energy depletion from pushing electrons into a blocked ETC. The worsening resolves within hours to days of discontinuation — the supplements do not accumulate, and the metabolic stress is self-limited. The diagnostic information (which node is blocked) is obtained at minimal cost.
...
5. *Document as a severe intolerance.* The medication that caused permanent worsening should be listed as an allergy or severe intolerance in the patient's record, with the specific mechanism noted (e.g., "amphetamine — VMAT2 depletion with permanent functional decline"). This is not an allergy in the IgE-mediated sense, but calling it an allergy ensures it will never be prescribed accidentally.
...
7. *Screen for iatrogenic comorbidities.* The medication may have caused a new disease entity on top of ME/CFS: iatrogenic adrenal insufficiency (corticosteroids), iatrogenic peripheral neuropathy (DCA), iatrogenic bone marrow failure (valganciclovir), PTSD from a traumatic adverse reaction (ketamine, LDN psychiatric). Treat these as separate conditions, with ME/CFS as a modifier of treatment tolerance.
]
