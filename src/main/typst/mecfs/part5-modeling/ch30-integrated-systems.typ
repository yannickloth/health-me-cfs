...
*Scalar approximation caveats.* The $d cal(M)/d t$ equation treats $cal(M)$ as a scalar driven upward by DNMT3A/B activity (gain-model), with passive demethylation as reversal. This approximation omits three structural features captured in the full per-locus vector model (Chapter @ch:causal-hierarchy-formal, @oq:consolidation-directionality): (1) *DNMT3B redistribution* — loss at ProB repeats and gain at ProA/gene-region are coupled through a shared enzyme pool ($sum_i Delta m_i = 0$ at the allocation level), not independent; (2) *irreversibility threshold* — loci falling below $m_i^"crit"$ lose the MeCP2/MBD-DNMT1 maintenance loop and require active, SAM-dependent de novo remethylation, which passive demethylation kinetics cannot represent; (3) *direction-dependent therapeutic response* — the scalar model implies demethylation is always therapeutic and methyl donors are always neutral, while the vector model shows that ProB loss-dominant patients require remethylation (methyl donors) and are harmed by demethylating agents (5-azacitidine, HDAC inhibitors, TET activators). The scalar model's hysteresis and intervention-window predictions remain qualitatively valid for gain-dominant patterns but may invert for loss-dominant patterns.
...
- *Epigenetic* (n+1 variables): $bold(cal(M))$ (n per-locus methylation entries) and $cal(A)$ (histone acetylation). For the integrated ODE, the scalar approximation $cal(M)$ is used as a compressed summary; see @oq:consolidation-directionality for the full vector specification.
...
#proposal(title: [Multi-Compartment ECM Turnover Model with Tissue-Specific Parameters])[
...
$
"MMP_i" = "MMP_0" dot.op (1 + alpha_"inf" dot.op ["IL-6"] + alpha_i_"strain" dot.op sigma_i + alpha_i_"MC" dot.op "MC_d")
$

The tissue-specific strain coefficients $alpha_i_"strain"$ reflect differing mechanotransduction sensitivity: cervical ligaments are highly strain-sensitive (high $alpha_"cerv"^"strain"$) and accumulate MMP under minor orthostatic stress, explaining POTS exacerbation in hEDS patients; tendons respond to activity-related strain; vascular ECM responds to shear stress and pulsatile pressure. Mast cell coupling $alpha_i^"MC"$ varies by tissue density: gut mucosa has high mast cell density and high coupling, driving SIBO-mediated ECM degradation. The model predicts that symptom clusters correlate with dominant compartments: patients with dominant vascular ECM instability present with orthostatic intolerance and syncope; cervical ligament dominance presents with neck pain, headaches, and dysautonomia; tendon dominance presents with joint instability and dislocations; dermal dominance presents with skin extensibility and wound healing deficits. Treatment strategies can be compartment-targeted: strengthening cervical support (physical therapy for deep neck flexors) reduces $sigma_"cerv"$ and MMP activation in that compartment; beta-blockers reduce pulsatile pressure on vascular ECM; mast cell stabilizers reduce $alpha_i^"MC"$ across compartments.
