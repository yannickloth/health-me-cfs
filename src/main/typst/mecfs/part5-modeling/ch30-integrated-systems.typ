...
*Competing formulation (loss-model).* The $d cal(M)/d t$ equation above represents the gain-model: $cal(M)$ rises toward 1 as disease signals drive DNMT3A/B activity, and recovery requires demethylation. A competing loss-model, motivated by the ProB repeat erosion framework (Bonnet, Hulo, Fourel et al.\ @BonnetFourel2026ProAB; Speculation @spec:methylation-loss-consolidation), models consolidation as methylation *loss*:

$ frac(d cal(L), d t) = -k_upright("erosion") dot.op bold(C)_upright("pro") dot.op cal(L) + k_upright("remeth") dot.op (1 - cal(L)) $

where $cal(L) = 1$ is healthy (fully methylated at ProB repeats), $cal(L) = 0$ is fully consolidated, and disease signals drive $cal(L)$ *downward* through DNMT3B redistribution away from heterochromatin. This makes three opposite commitments: (1) recovery requires remethylation, (2) the therapeutic agent class is methyl donors (SAMe, methyl-folate, betaine) and DNMT3B activators, (3) remethylation may need to *precede* energy restoration. If the loss-model is correct, demethylating agents (5-azacitidine, HDAC inhibitors, TET activators) are contraindicated.

*DNMT3B substrate specificity.* The current model treats methylation as CpG-only for simplicity. However, DNMT3B also methylates non-CpG sites (CAG trinucleotides) in early development and in neurons, and this activity may be relevant to tissue-specific epigenetic dynamics. A complete model would distinguish CpG from CAG methylation at DNMT3B target sites.

*Gain-model basis.* In the gain-model formulation above, pro-inflammatory cytokines promote DNA methylation (DNMT upregulation), acetyl-CoA availability limits histone acetylation (HAT activity), and SIRT1/SIRT3 (NAD⁺-dependent deacetylases) modulate histone state. *This model produces hysteresis---a phenomenon that can only be identified through dynamical systems analysis*: the disease state promotes epigenetic changes that stabilize the disease parameters, which in turn maintain the disease state. Even if the original trigger (e.g., viral infection) is fully resolved, the epigenetic state $bold(cal(E))$ may have shifted to values that keep the system in the disease attractor. Recovery then requires not just removing the trigger but reversing the epigenetic changes---a process that operates on the slow timescale $tau_upright("epi") ~ 1 \/ k_upright("demeth") ~$ months to years.
