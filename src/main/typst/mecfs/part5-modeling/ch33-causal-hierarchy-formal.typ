...
where $f$ is a monotonically increasing function of per-locus methylation at ProB RepSeqs. DNMT3B redistribution → $bold(cal(M))_"ProB"$ falls → $B_"strength"$ falls → A/B compartment identity weakens → genome unfolds → genes normally silenced by heterochromatin proximity become derepressed. This derived variable connects per-locus methylation to the structural genome effect that Fourel et al.\ identify as the mechanism of methylation-loss pathology @BonnetFourel2026ProAB. $B_"strength"$ is a candidate parameter for linking the epigemetic subsystem to downstream gene expression models, though the functional form of $f$ is currently unknown.
...
*Certainty.* 0.55 for bidirectional pattern (documented across multiple studies). 0.40 for DNMT3B redistribution as the coupling mechanism (ProA/ProB framework @BonnetFourel2026ProAB is a preprint, unvalidated for ME/CFS; the zero-sum constraint is mechanistically plausible but unmeasured). 0.35 for the irreversibility threshold $m_i^"crit"$ (MeCP2/DNMT1 maintenance loop is well-established; the existence and value of a critical threshold at specific loci is not. The clinical import — distinguishing loci that self-correct from those requiring active intervention — makes this a high-value measurement target). 0.25 for the vector model providing better clinical guidance than the scalar model (direction-correct per-locus targeting is principled but pharmacologically unsupported). 0.15 for the tensor product $bold(cal(M)) ⊗ bold(cal(H))$ being operationally necessary (methylation-only models already capture the dominant signal; histone coupling may be a refinement rather than a structural requirement).
