...
The numbers make this concrete. The occupancy column uses the standard Hill equation $f = c^2/(1+c^2)$ (established pharmacology); the benefit column uses the simple model $B \propto f(1-f)^2$ proposed in Part B of this article (a hypothesis); $K_d = 1$ mg is illustrative and unmeasured:

| Dose | $c = D/K_d$ | $f$ — receptors blocked | $1-f$ — residual signal | benefit, relative to peak (proposed model) |
|------|------------|--------------------------|--------------------------|---------------------------------------------|
...
Read the table from the bottom up. A null at 4 mg establishes that the bottom row is ≈ 0. It says nothing about the first two rows, because those rows describe a state — partial blockade — that a 4 mg dose never produces. And 3 mg is yet another experiment: the low-dose window has closed there too, but other mechanisms (the endorphin plateau, the opening TRPM3 window) are at *their* optimum — different targets again. Each dose is a different experiment, and the failure of one convicts none of the others.
...
A failure at 4 mg yields only the second: the target combination engaged in the 4 mg band — TRPM3 restoration, orexin disinhibition, and TLR4 in its *over-blocked* state — is not the pathway limiting symptoms in this patient. It says nothing about the TLR4/Nrf2 hormetic state or the endorphin rebound, which exist only at 1–3 mg — because those states *never occurred* during a 4 mg trial.
