...
**Plugin:** `infolead-claude-subscription-router` (installed)

**ABSOLUTE RULE: Every user request gets routed. No shortcuts, no exceptions, no "just answering directly."**

### How Routing Works

The `infolead-claude-subscription-router` plugin provides a complete routing system with:

- **Pre-router visibility**: See routing recommendations before execution
- **Cost optimization**: Haiku pre-routing saves 60-70% quota
- **Metrics collection**: Track all routing decisions to `~/.claude/infolead-router/metrics/`
- **Risk assessment**: Automatic evaluation of destructive operations
- **Work coordination**: Parallel task tracking with completion guarantees

For EVERY user request:

1. **Pre-router analyzes** (via hook): Displays recommendation with confidence
2. **Main Claude routes**: Makes final decision using pre-router's advisory input
3. **Check project agents first**: Does `.claude/agents/` have a specialized agent that exactly matches?
4. **Fall back to general agents**: If no project agent matches, delegate to appropriate model tier
5. **Spawn via Task**: Delegate to the selected agent
6. **Metrics logged**: All decisions tracked for analysis
...
User request
    ↓
Pre-router (hook) → [ROUTER] Recommendation displayed
    ↓
Main Claude (sees recommendation) → Routing decision
    ↓
Task tool → Agent executes
    ↓
Metrics logged
```
...
**Priority 2: General agents** (from `infolead-claude-subscription-router`)

Use **short names** (namespace prefix optional):

- `haiku-general`: Mechanical tasks, no judgment needed, explicit paths
- `sonnet-general`: Default for reasoning, analysis, judgment calls
- `opus-general`: Complex reasoning, proofs, high-stakes decisions

**If routing decision is uncertain**: Spawn `router-escalation` (Opus) for deeper analysis.

### Router Visibility

You will see routing recommendations before main Claude processes requests:
...
Main Claude sees this recommendation and makes the final decision. The recommendation is **advisory, not mandatory**—Claude can follow, override, or escalate as needed.
...
The router plugin provides several cost-saving patterns:
...
- **Draft-then-evaluate**: Haiku batch-drafts, Sonnet batch-evaluates (40-60% savings at batch 10+)
...
cd ~/.claude/plugins/marketplaces/yannickloth-claude-router-system/plugins/infolead-claude-subscription-router/implementation/
...
### Router Integration with Project Agents

The routing system checks project-specific agents **first** before falling back to general agents:
...
Router analyzes intent
    ↓
Does .claude/agents/ have exact match?
    ↓
YES: Use project-specific agent (e.g., literature-integrator)
NO:  Use general agent based on complexity (haiku/sonnet/opus)
```

**Examples:**

- "Fix LaTeX syntax errors" → `syntax-fixer` (project agent)
- "Integrate research papers" → `literature-integrator` (project agent)
- "Fix typo in line 42" → `haiku-general` (no specialized agent needed)
- "Analyze this proof" → `opus-general` (requires deep reasoning)
