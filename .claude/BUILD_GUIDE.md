...
- Non-Nix unsupported (typst packages, fonts, env vars are Nix-managed)
- First build downloads Nix dependencies (~10–30 min); subsequent builds fast

## Build Commands

Run from project root. `result/` is a symlink into the Nix store — `cp result/<file> .` to persist.
...
└── figures/                  # Compiled .svg — generated, do not edit
```
...
| `build-pdf.yml` | push or PR → main, manual | PDF + GitHub Release `latest` (no `nix flake check`) |
