...
Cortisol (morning) & 6.3 $\mu$g/dL & 7--25 & \textbf{Low (below reference range)} \\
\midrule
...
Morning cortisol of 6.3 $\mu$g/dL is below the reference range (7--25 for morning). In ME/CFS, blunted cortisol awakening response and low cortisol are common findings reflecting HPA axis dysfunction. This may contribute to:
\begin{itemize}
...
\begin{description}
    ...
    \begin{itemize}
        \item \textbf{Childhood pharmaceutical exposure}: Regular Zyma Fluor (sodium fluoride) supplementation

        \begin{warning}[Speculative Retrospective Factor]
        This fluoride exposure is documented for \textbf{historical completeness of the patient record only}. Individual causation cannot be determined retrospectively from childhood exposure patterns. No clinical action is warranted based on this speculation alone. Many individuals received similar fluoride supplementation without developing sleep disorders or ME/CFS, indicating that if fluoride played any role, it would be as one of multiple contributory factors in a susceptible individual, not a sole cause.
        \end{warning}

        \begin{itemize}
            \item Product: Zyma Fluor tablets ($\sim$0.25\,mg fluoride per tablet), dental caries prevention
            \item Administration: Required by mother, ``rather regularly'' during childhood
            \item \textbf{Potential mechanistic relevance (HIGHLY SPECULATIVE)}:
            \begin{itemize}
                \item Pineal gland accumulates fluoride at higher concentrations than any other soft tissue \cite{Luke_2001}
                \item Children retain 80--90\% of absorbed fluoride (vs.\ 60\% in adults)
                \item U.S.\ National Research Council (2006): ``Fluoride is likely to cause decreased melatonin production'' \cite{US_NRC_2006}
                \item Mechanism: Inhibition of pineal enzymes (AANAT, HIOMT) involved in melatonin synthesis
                \item \emph{Speculative hypothesis}: Childhood fluoride accumulation $\rightarrow$ pineal dysfunction $\rightarrow$ chronic melatonin reduction $\rightarrow$ sleep/autonomic vulnerability
            \end{itemize}
            \item \textbf{Evidence quality}: Pineal accumulation HIGH (human autopsy studies), melatonin reduction MODERATE (animal models, mechanistically sound), relevance to patient SPECULATIVE (individual variation, no direct measurement)
        \end{itemize}

        \item May provide one possible mechanistic context among many for long-standing sleep dysfunction (idiopathic hypersomnia) and reduced autonomic reserve
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item \textbf{Mechanism}: Vagal syncope while sitting on high bar stool (``tabouret de comptoir'') after drinking Coke at noon $\rightarrow$ fall $\rightarrow$ head trauma
            \item \textbf{Post-traumatic amnesia}: 5 hours (significant, indicates moderate-severity concussion)
            \item \textbf{Clinical note}: ``Syncopes répétées'' (recurrent syncopes) --- not an isolated event
            \item \textbf{Pattern of vagal hypersensitivity --- Pre-existing vulnerability}:
            \begin{itemize}
                \item Patient reports heightened sensitivity to vagus nerve stimulation
                \item At least one prior vasovagal episode (less severe, required sitting but no loss of consciousness)
                \item June 2018 episode: Complete vasovagal syncope with full loss of consciousness
                \item Pattern indicates baseline autonomic instability/dysautonomia \emph{predating} the concussion
                \item \textbf{Critical interaction}: Pre-existing vagal hypersensitivity likely reduced capacity to compensate for TBI-induced autonomic injury
            \end{itemize}
            \item \textbf{Autonomic dysfunction mechanism (Concussion $\rightarrow$ Dysautonomia)}:
            \begin{itemize}
                \item \textbf{Diffuse axonal injury (DAI)}: Rotational forces during impact cause axonal shearing in autonomic control centers
                \item \textbf{Affected regions}: Brainstem autonomic nuclei (dorsal motor nucleus of vagus, nucleus tractus solitarius), hypothalamus, reticular activating system
                \item \textbf{Persistent effects}: DAI can produce autonomic dysfunction lasting years post-injury (documented in 40--90\% of TBI patients)
                \item \textbf{Sympathetic dominance}: Post-TBI autonomic dysfunction often manifests as sympathetic overdrive (elevated HR, impaired HRV, orthostatic tachycardia)
                \item \textbf{ME/CFS association}: ME/CFS patients have 4.89$\times$ higher odds of prior concussion \cite{Bateman_2024_Concussion_ME_CFS}
                \item \textbf{Compound injury model}: Pre-existing vagal hypersensitivity + acute TBI to autonomic centers = severe, persistent dysautonomia
                \item TBI appears to have been inflection point from compensated autonomic vulnerability to full dysautonomia/ME/CFS phenotype
            \end{itemize}

            \item \textbf{Evidence level clarification}:
            \begin{itemize}
                \item \textbf{TBI-ME/CFS association (Bateman 2024):} HIGH certainty --- large retrospective study with 4.89$\times$ odds ratio
                \item \textbf{DAI mechanism and persistent autonomic dysfunction:} MEDIUM certainty --- well-documented in TBI literature but not ME/CFS-specific
                \item \textbf{Specific contribution to this patient's dysautonomia:} LOW certainty --- clinical inference from temporal association and phenotype match; cannot definitively establish causation from single case without pre-TBI baseline autonomic testing
            \end{itemize}
            ...
            \begin{itemize}
                \item Lactic acid: \textbf{3.18 mmol/L} (ref.\ 0.50--2.20) --- elevated at baseline~\cite{Ghali2019}
            ...
            \begin{itemize}
                ...
                \item Pattern of vagal hypersensitivity may represent pre-existing autonomic vulnerability
...
\paragraph{Autonomic Dysfunction Patterns.}
\begin{itemize}
    \item \textbf{Recurrent vasovagal episodes}: Pattern of vagal hypersensitivity documented
    \begin{itemize}
        \item At least two documented episodes (one prior to 2018, complete syncope June 2018)
        \item Patient reports heightened sensitivity to vagus nerve stimulation
        \item May represent pre-existing autonomic vulnerability
        \item Related to orthostatic intolerance symptoms observed in current ME/CFS presentation
        \item Clinical significance: Suggests longstanding dysautonomia predating full ME/CFS phenotype
    \end{itemize}
\end{itemize}

\paragraph{Multi-Hit Pathway to ME/CFS --- Synthesis of Vulnerability Factors.}

\begin{itemize}
    \item \textbf{Model}: Patient's ME/CFS appears to result from cumulative burden of multiple factors depleting autonomic reserve over time, culminating in decompensation
    \item \textbf{Temporal sequence}:
    \begin{enumerate}
        \item \textbf{Childhood (speculative)}: Fluoride exposure $\rightarrow$ pineal dysfunction $\rightarrow$ melatonin reduction $\rightarrow$ impaired sleep/autonomic regulation
        \item \textbf{Adolescence--young adulthood}: Established vagal hypersensitivity pattern; long-standing fatigue, idiopathic hypersomnia
        \item \textbf{Late 2017}: Severe burnout $\rightarrow$ HPA axis dysregulation, further autonomic stress
        \item \textbf{June 2018}: Concussion with diffuse axonal injury to autonomic centers, superimposed on already vulnerable system
        \item \textbf{2018--present}: Full dysautonomia and ME/CFS phenotype --- autonomic reserve exhausted
    \end{enumerate}
    \item \textbf{Synergistic interactions}:
    \begin{itemize}
        \item Pre-existing vagal hypersensitivity reduced capacity to compensate for TBI-induced autonomic injury
        \item Years of sleep dysfunction (possibly fluoride-mediated) depleted baseline autonomic reserve
        \item Burnout-induced HPA dysfunction further reduced resilience immediately pre-TBI
        \item TBI was likely the critical trigger that overwhelmed an already compromised system
    \end{itemize}
    \item \textbf{Evidence quality}: HIGH for TBI, vagal pattern, and burnout; MODERATE for TBI-ME/CFS association; SPECULATIVE for fluoride contribution
    \item \textbf{Clinical implications}:
    \begin{itemize}
        \item Multi-modal treatment approach warranted: address sleep/circadian (melatonin), autonomic support (orthostatic interventions), HPA axis (stress management), neurological recovery (post-TBI support)
        \item Long-standing vulnerabilities may require prolonged intervention
        \item Cumulative burden model suggests incremental improvements possible by addressing multiple pathway components
    \end{itemize}
    \item \textbf{Literature support}:
    \begin{itemize}
        \item ME/CFS often results from multiple cumulative stressors (multi-hit hypothesis)
        \item Patient exemplifies TBI $\rightarrow$ autonomic dysfunction $\rightarrow$ ME/CFS pathway documented by Bateman et al.\ (2024)
        \item Pre-existing dysautonomia as risk factor for worse post-TBI outcomes is mechanistically plausible
    \end{itemize}
\end{itemize}
