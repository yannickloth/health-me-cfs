...
\subsection{Case Profile: Dual Diagnosis Assessment}
...
\subsubsection{Clinical History Summary}
...
\begin{description}
    ...
    \begin{itemize}
        ...
        \end{itemize}
        \item \textbf{Phase 2 (Post-2018):} Severe burnout in late 2017
        \begin{itemize}
    ...
    \begin{itemize}
        \item Mean sleep latency 9 minutes on MSLT (pathologically fast; normal $>$10 min)
        \item First nap latency extremely rapid (0.5 minutes)
...
\subsubsection{Comorbidity Classification: Relationship to Primary Diagnoses}
...
\paragraph{Conditions Consequent to ME/CFS}
...
\midrule
\textbf{Bilateral Sensorineural Hearing Loss} & Cochlear hair cells are among the most energy-demanding cells in the body~\cite{WongGee2023}; mitochondrial dysfunction impairs the ATP production required for mechanotransduction; high-frequency loss pattern typical of metabolic injury \\
\addlinespace
...
\addlinespace
\textbf{Chronic Muscle Cramps} (25+ years) & ATP depletion prevents proper muscle relaxation; impaired carnitine shuttle blocks fat oxidation~\cite{Reuter2011}; excessive lactate accumulation from compensatory anaerobic glycolysis \\
\addlinespace
...
\subparagraph{Clinical Significance.}
...
\paragraph{Conditions with Shared Underlying Cause}
...
\midrule
\textbf{Idiopathic Hypersomnia} & Causal relationship uncertain; both conditions present and documented. IH could be: (1) pre-existing constitutional vulnerability, (2) caused by ME/CFS pathophysiology affecting CNS arousal pathways, or (3) both conditions sharing common dopaminergic/mitochondrial dysfunction. Clinical reality: lifelong fatigue pattern with formal IH diagnosis co-occurring with ME/CFS. \\
\addlinespace
...
\paragraph{Conditions Related to ADHD/Attention Dysfunction}
...
\begin{itemize}
    \item The 2024 NIH finding~\cite{walitt2024deep} of low catecholamines (including dopamine) in ME/CFS cerebrospinal fluid
...
\paragraph{Integrative Summary: The Comorbidity Map}
...
\subparagraph{The Allergies Exception.}
...
\paragraph{Strategic Treatment Prioritization}
...
\subparagraph{Tier 1: Quick Wins (Low Cost, Immediate Implementation).}
...
\subparagraph{Tier 2: Foundational Supplements (Moderate Cost, High Impact).}
...
\subparagraph{Tier 3: Targeted Therapeutics (Prescription or Higher Cost).}
...
\subparagraph{Implementation Strategy: The ``3 Root Causes'' Approach.}
...
\subparagraph{Cost-Effectiveness Ranking.}
...
\subparagraph{What Success Looks Like.}
...
\subsubsection{Diagnostic Reasoning}
\label{subsec:diagnostic-reasoning}

\paragraph{Why This Is Not ``Pure'' ME/CFS}
...
\paragraph{Why This Is Not ``Pure'' Idiopathic Hypersomnia}
...
\paragraph{The Dual Diagnosis Model}
...
\textbf{Hit 2: Severe Burnout (Late 2017)}
\begin{itemize}
...
\subsubsection{Pathophysiological Framework}
\label{subsec:patho-framework}

Based on the symptom pattern, the following mechanisms are likely involved:

\paragraph{Primary Mechanisms (Highest Probability)}

\subparagraph{1. Dopaminergic System Dysfunction.}
...
\begin{itemize}
    ...
    \item 2024 NIH study~\cite{walitt2024deep} found low catecholamines in ME/CFS cerebrospinal fluid
\end{itemize}

\subparagraph{2. Iron Metabolism/Storage.}
Evidence supporting this:
\begin{itemize}
...
\subparagraph{3. Sleep Architecture Dysfunction.}
Evidence supporting this:
\begin{itemize}
...
\subparagraph{4. Mitochondrial Dysfunction.}
Evidence supporting this:
\begin{itemize}
...
\subparagraph{Cross-Domain Knowledge Application: Sports Recovery Medicine.}
...
\subparagraph{Theoretical Framework: ME/CFS as ``Permanent Post-Exercise State.''}
...
\subparagraph{Treatment Effectiveness Assessment.}
...
\paragraph{Pattern Recognition: Progressive Multi-Sensory Mitochondrial Failure}
...
\subparagraph{Vision (Progressive Since $\sim$2021).}
\begin{itemize}
...
\subparagraph{Hearing (Documented 2024).}
\begin{itemize}
...
\subparagraph{Shared Mechanism: Mitochondrial Hypothesis.}
...
\subparagraph{Therapeutic Implications.}
...
\paragraph{Secondary/Contributing Mechanisms}

\subparagraph{5. Autonomic Dysfunction.}
...
\subparagraph{6. Neuroinflammation.}
...
\subsubsection{Proposed Investigation Protocol}
...
\paragraph{Essential Blood Work}
...
\paragraph{Functional Assessments (No Special Equipment)}
...
\subsection{Proposed Treatment Protocol}
...
\subsubsection{Phase 0: Baseline Assessment (Weeks 1--2)}
\label{subsec:phase0}
...
\subsubsection{Phase 1: Foundation Optimization (Weeks 3--6)}
\label{subsec:phase1}

Address the most likely deficiencies based on RLS diagnosis and ME/CFS overlap.

\paragraph{Iron Optimization (Highest Priority for RLS)}
...
\paragraph{Vitamin D Optimization}
...
\paragraph{Magnesium (For Cramps and Cellular Function)}
...
\paragraph{B-Vitamin Optimization}
...
\paragraph{Dopamine Receptor Sensitivity}
...
\subsubsection{Phase 3: Mitochondrial Support (Weeks 11--16)}
\label{subsec:phase3}
...
\subsubsection{Phase 4: Sleep and Circadian Optimization (Weeks 17--20)}
\label{subsec:phase4}
...
\paragraph{Sleep Hygiene Fundamentals}
...
\paragraph{Slow-Wave Sleep Enhancement}
...
\paragraph{Addressing Restless Legs}
...
\subsubsection{Phase 5: Vagal and Autonomic Support (Weeks 21--24)}
\label{subsec:phase5}

Implement the vagal rehabilitation concepts from Chapter~\ref{ch:emerging-therapies}:

\paragraph{Daily Vagal Toning Protocol}
...
\paragraph{Heart Rate Variability Training}
...
\subsubsection{Phase 6: Anti-Neuroinflammatory Support (If Not Already Taking LDN)}
\label{subsec:phase6}
...
\subsubsection{Monitoring and Adjustment Protocol}
\label{subsec:monitoring}

\paragraph{Weekly Assessment}
...
\subsubsection{What This Protocol Cannot Address}
...
\subsubsection{Realistic Prognosis and Treatment Expectations}
\label{subsec:realistic-prognosis}

\paragraph{Disease Course Analysis: Never Truly Functional}
...
\paragraph{The Two-Hit Disease Model}

Clinical evidence suggests overlapping pathologies:

\subparagraph{Primary Pathology: Lifelong Metabolic Dysfunction (30+ years).}
\begin{itemize}
...
\subparagraph{Secondary Pathology: Inflammatory/Autoimmune Overlay (Post-2017).}
\begin{itemize}
...
\subparagraph{Estimated Contribution to Current Severity.}

\textit{Note: The following proportions are clinical estimates based on symptom pattern and temporal progression, not measured values or validated biomarkers.}

\begin{itemize}
    \item Primary metabolic dysfunction: $\sim$30--40\% of current disability (estimated; lifelong baseline)
    \item Inflammatory amplification: $\sim$60--70\% of current disability (estimated; post-2017 overlay)
\end{itemize}

\paragraph{What Treatment Can and Cannot Achieve}

\begin{tcolorbox}[colback=yellow!5!white,colframe=yellow!75!black,title=Realistic Best-Case Outcome]

\textbf{If all interventions work optimally} (MCT oil, Acetyl-L-Carnitine, Low-Dose Naltrexone (LDN), D-Ribose, all metabolic support):
...
\paragraph{Intervention-Specific Expectations}

\subparagraph{Acetyl-L-Carnitine (1000\,mg daily).}
\begin{itemize}
...
\subparagraph{MCT Oil (1 tablespoon daily).}
\begin{itemize}
...
\subparagraph{D-Ribose (10\,g daily: 5\,g morning, 5\,g bedtime).}
\begin{itemize}
...
\subparagraph{LDN (3\,mg, plan to increase to 4--4.5\,mg).}
\begin{itemize}
...
\subparagraph{Riboflavin B2 (400\,mg daily).}
\begin{itemize}
...
\subparagraph{Magnesium Glycinate (300--400\,mg bedtime).}
\begin{itemize}
...
\subparagraph{Digestive Enzymes + Strategic Fat.}
\begin{itemize}
...
\paragraph{Overall Timeline}

\subparagraph{Weeks 1--4: Immediate Interventions.}
\begin{itemize}
...
\subparagraph{Weeks 4--8: Acetyl-L-Carnitine Initial Effect.}
\begin{itemize}
...
\subparagraph{Weeks 8--16: LDN Effect Emerges.}
\begin{itemize}
...
\subparagraph{Months 3--6: Accumulated Benefits.}
\begin{itemize}
    \item Acetyl-L-Carnitine reaching maximum effect
    \item Low-Dose Naltrexone (LDN) fully modulating immune system
...
\subparagraph{Months 6--12: Plateau and Assessment.}
\begin{itemize}
...
\paragraph{What This Protocol Cannot Achieve}
...
\paragraph{Why Pursue Treatment Despite Limited Expectations}
...
\subsection{Theoretical Integration: Why Two Conditions May Share Roots}
\label{sec:theoretical-integration}

\subsubsection{The Dopamine-Mitochondria-Sleep Axis}
...
\subsubsection{Implications for Treatment Prioritization}
...
\subsubsection{Why Stimulants Help But Don't Cure}
...
\subsection{Summary and Action Items}
